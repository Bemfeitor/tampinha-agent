...
Write path is the CLI (``tampinha cron notepad <job_id> set <key> <value>``),
...
NOTEPAD_FILE = get_tampinha_home().resolve() / "cron" / "notepad.db"
...
def render_notepad_section(job_id: str) -> str:
    ...
    return (
        ...
        f"`tampinha cron notepad {job_id} set <key> <value>` "
        f"(also: get/delete/list; `tampinha cron notepad {job_id} delete "
