...
renders natively:
  ...
  * Docs catalog          -> a copy-paste command + a ``tampinha://`` deep-link
...
def blueprint_deeplink(blueprint: AutomationBlueprint, values: Optional[Dict[str, Any]] = None) -> str:
    """Build the ``tampinha://blueprint/<key>?slot=val`` deep-link URL."""
    ...
    return f"tampinha://blueprint/{quote(blueprint.key)}{qs}"
