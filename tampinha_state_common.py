...
Extracted verbatim from tampinha_state.py so the SessionDB mixin modules
(tampinha_state_search / tampinha_state_schema / tampinha_state_portability) can
reference them without importing tampinha_state (which would be a cycle).
tampinha_state re-imports every name here for backward compatibility.
"""
...
# optimized via ``tampinha sessions optimize-storage``. A legacy DB sits at
...
# `tampinha sessions optimize-storage` rebuilds it on a capable host.
...
# triggers repairable UNTIL the user opts into `tampinha db optimize`. This is
...
#
# Several independent Tampinha processes routinely share one state.db (gateway
# service, the Desktop app's `tampinha serve` backend, interactive CLI sessions,
...
#
# Semantics mirror `tampinha_state._cross_process_repair_lock` (the schema-
...
# rebuilder cannot wedge future rebuilds. It lives here (not tampinha_state)
# because the search/schema mixins cannot import tampinha_state (cycle).
#
