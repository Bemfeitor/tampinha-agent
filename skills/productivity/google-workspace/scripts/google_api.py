#!/usr/bin/env python3
"""Google Workspace API CLI for Tampinha Agent.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing Tampinha-facing JSON contract and falls back to the Python client
...
from _tampinha_home import get_tampinha_home
...
def _gws_binary() -> str | None:
    override = os.getenv("TAMPINHA_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for Tampinha Agent")
