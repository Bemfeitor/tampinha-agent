...
Ledger path resolution (first wins):
  --ledger PATH
  $TAMPINHA_CITATION_LEDGER
  $TAMPINHA_HOME/cache/citations/ledger.json
"""
...
from _tampinha_home import get_tampinha_home  # noqa: E402
...
def resolve_ledger_path(explicit: str | None = None) -> Path:
    ...
    env = os.environ.get("TAMPINHA_CITATION_LEDGER", "").strip()
    if env:
        return Path(env).expanduser()
    return get_tampinha_home() / "cache" / "citations" / "ledger.json"
