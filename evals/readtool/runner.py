"""Run the read-tool eval through the REAL Tampinha AIAgent.

For each task: fresh temp TAMPINHA_HOME, fresh fixture workspace, real
...
def run_task(task, model: str, provider: str, timeout_mult: float,
    ...
    tampinha_home = Path(tempfile.mkdtemp(prefix="readtool-home-")) / ".tampinha"
    ...
    try:
        ...
        shutil.rmtree(tampinha_home.parent, ignore_errors=True)
    return result
...
def main() -> int:
    ...
    if not os.environ.get("OPENROUTER_API_KEY"):
        raise SystemExit(
            ...
            "source ~/.tampinha/.env; set +a  — then relaunch."
        )
