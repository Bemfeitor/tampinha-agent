"""Run a real Tampinha CLI turn and validate the Relay shared-metrics output."""
...
def _resolve_tampinha_executable(tampinha_repo: Path) -> Path:
    for relative_path in (
        Path(".venv") / "bin" / "tampinha",
        Path(".venv") / "Scripts" / "tampinha.exe",
    ):
        candidate = tampinha_repo / relative_path
        if candidate.is_file():
            return candidate
    discovered = shutil.which("tampinha")
    if discovered:
        return Path(discovered)
    raise SystemExit(
        "Tampinha executable not found in the repository virtual environment "
        "or on PATH"
    )
...
def _arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path.cwd(),
        help="Tampinha source checkout containing .venv/bin/tampinha",
    )
    parser.add_argument(
        "--relay-python",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Directory for the isolated TAMPINHA_HOME and captured output",
    )
...
def _validate_store(database_path: Path) -> list[dict[str, Any]]:
    ...
    if by_name["tampinha.client.active"] != [
        {
        ...
        raise AssertionError(
            f"Unexpected client-active counter: {by_name['tampinha.client.active']}"
        )
    [model] = by_name["tampinha.model_route.count"]
    expected_model = {
        "name": "tampinha.model_route.count",
        "dimensions": {
            "model": MODEL_CANARY,
            "provider": "custom",
        },
    ...
    if model != expected_model:
        raise AssertionError(
            f"Unexpected model counter: {by_name['tampinha.model_route.count']}"
        )
    expected_start = {
    ...
    if by_name["tampinha.task_run.started"] != [expected_start]:
        raise AssertionError(
            f"Unexpected task start: {by_name['tampinha.task_run.started']}"
        )
    [terminal] = by_name["tampinha.task_run.finished"]
    ...
    [tool] = by_name["tampinha.tool_call.count"]
    ...
    lifecycle = by_name["tampinha.skill.lifecycle.count"]
    expected_actions = {
        "archived",
        "created",
        "edited",
    ...
    loads = by_name["tampinha.skill.load.count"]
...
def _validate_packages(
    ...
    try:
        ...
        raise RuntimeError(
            "The Tampinha development environment requires jsonschema"
        ) from exc
    ...
    if metrics["tampinha.client.active"] != [
        {
            "name": "tampinha.client.active",
            "type": "counter",
            "dimensions": {},
        ...
        raise AssertionError(
            f"Unexpected client-active metric: {metrics['tampinha.client.active']}"
        )
    [model] = metrics["tampinha.model_route.count"]
    if model["dimensions"] != {
        "model": MODEL_CANARY,
        "provider": "custom",
    } or model["value"] != 2:
        raise AssertionError(
            f"Unexpected model metric: {metrics['tampinha.model_route.count']}"
        )
    [terminal] = metrics["tampinha.task_run.finished"]
    ...
    [tool] = metrics["tampinha.tool_call.count"]
    if (
        tool["dimensions"]
        != {
            "approval_outcome": "not_required",
    ...
    lifecycle = metrics["tampinha.skill.lifecycle.count"]
    ...
    loads = metrics["tampinha.skill.load.count"]
    if {
        (
...
def main() -> int:
    args = _arguments()
    tampinha_repo = args.tampinha_repo.resolve()
    relay_python = args.relay_python.resolve() if args.relay_python else None
    tampinha = _resolve_tampinha_executable(tampinha_repo)
    ...
    if args.output_dir:
        ...
        root = Path(tempfile.mkdtemp(prefix="tampinha-relay-shared-metrics-"))
    home = root / "tampinha-home"
    ...
    try:
        ...
        python_paths = [str(tampinha_repo)]
    ...
    (root / "tampinha.stdout.txt").write_text(result.stdout, encoding="utf-8")
    (root / "tampinha.stderr.txt").write_text(result.stderr, encoding="utf-8")
    if result.returncode != 0:
        raise AssertionError(
            f"Tampinha exited with {result.returncode}\n"
            f"stdout:\n{result.stdout}\nstderr:\n{result.stderr}"
        )
    ...
    if PROMPT_CANARY not in json.dumps(request.get("messages", [])):
        raise AssertionError("Tampinha model request did not contain the prompt canary")
    ...
    if TOOL_CALL_CANARY not in follow_up or TOOL_RESULT_CANARY not in follow_up:
        raise AssertionError("Tampinha did not return the tool result to the model")
    if RESPONSE_CANARY not in result.stdout:
        raise AssertionError("Tampinha did not print the mock model response")
    ...
    skill_result = subprocess.run(
        [
            ...
            "-c",
            "\n".join([
                "from tampinha_cli.observability import relay_shared_metrics",
    ...
    package_paths, packages = _validate_packages(
        ...
        / "observability"
        / "schemas"
        / "tampinha.shared_metrics.v2.schema.json",
    )

    print("Tampinha -> NeMo Relay shared-metrics smoke test passed")
