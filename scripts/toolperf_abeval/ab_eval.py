...
measured in the production session DB) through `tampinha chat` twice — once per
...
Environment:
  ...
  ABEVAL_HOME    TAMPINHA_HOME for runs   (default: $ABEVAL_ROOT/home)
                 Must be a configured Tampinha home with credentials for the
...
""".strip(),
            ...
            env.update({
                ...
                "TAMPINHA_NEMO_RELAY_PLUGINS_TOML": str(relay_config),
            })
            q = TASKS[name].replace("{WORK}", str(work))
            t0 = time.time()
            try:
                p = subprocess.run(
                    [sys.executable, "-m", "tampinha_cli.main", "chat", "--query", q,
