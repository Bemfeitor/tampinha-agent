...
# tree generates llms.txt — the index every LLM (Tampinha included, via the
# tampinha-agent skill) reads to learn what Tampinha can do — and
...
_MCP_CATALOG_FILES = {"tampinha_cli/mcp_catalog.py"}
...
# Rust crates — currently just the Tauri bootstrap installer (Tampinha-Setup).
...
def _py_test_only(p: str) -> bool:
    ...
    Product jobs (Desktop E2E's ``tampinha serve`` backend, the Docker image)
