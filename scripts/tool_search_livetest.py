#!/usr/bin/env python3
"""Live test harness for Tampinha Agent's Tool Search feature.
...
# Force-isolate the test environment BEFORE any tampinha imports.
ORIGINAL_HOME = os.environ.get("TAMPINHA_HOME")
ORIGINAL_AUTH = Path.home() / ".tampinha" / "auth.json"
...
def setup_isolated_home(enabled: bool, listing: str = "off",
    ...
    """Create a fresh ~/.tampinha/ for one test, copying minimal credentials.

    Also reads OPENROUTER_API_KEY from the user's real ``~/.tampinha/.env`` so
    the agent can authenticate against OpenRouter inside the isolated home.
    """
    home_dir = Path(tempfile.mkdtemp(prefix="tampinha_ts_live_"))
    tampinha_home = home_dir / ".tampinha"
    tampinha_home.mkdir(parents=True)

    if ORIGINAL_AUTH.exists():
        shutil.copy(ORIGINAL_AUTH, tampinha_home / "auth.json")

    # Copy .env so OPENROUTER_API_KEY (or others) are visible to the agent
    # running inside the isolated home.
    real_env_file = Path.home() / ".tampinha" / ".env"
    if real_env_file.exists():
        shutil.copy(real_env_file, tampinha_home / ".env")
        ...
        load_tampinha_dotenv(tampinha_home=str(Path.home() / ".tampinha"))
    ...
    (tampinha_home / "config.yaml").write_text(_yaml_dump(cfg), encoding="utf-8")
...
def reset_module_state():
    """Drop cached modules so the new TAMPINHA_HOME takes effect."""
    keys = [k for k in sys.modules.keys()
            if k.startswith(("tools.", "model_tools", "toolsets",
                             "tampinha_cli", "agent.", "run_agent"))]
    for k in keys:
        del sys.modules[k]
...
def run_one_scenario(scenario: Dict[str, Any], enabled: bool, out_dir: Path) -> Dict[str, Any]:
    ...
    os.environ["TAMPINHA_HOME"] = str(home)
...
def main():
    ...
    # Restore original TAMPINHA_HOME
    if ORIGINAL_HOME is not None:
        os.environ["TAMPINHA_HOME"] = ORIGINAL_HOME
    else:
