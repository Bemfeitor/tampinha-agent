...
When Tampinha runs in TUI mode, the gateway child process communicates with
...
# ``get_tampinha_home() / "plugins"`` (user) and ``./.tampinha/plugins/`` (project,
# gated behind ``TAMPINHA_ENABLE_PROJECT_PLUGINS``) — see
# ``tampinha_cli/plugins.py:10-12``.  The guard only checked the bundled
...
# this script runs as a standalone subprocess — ``tampinha_constants`` isn't
...
def main() -> int:
    ...
    # Add repo root to sys.path so we can import tampinha_constants (this script
    ...
    # plugins in get_tampinha_home()/plugins/ and project plugins in
    # ./.tampinha/plugins/, where code like ori/hooks.py can spawn
    # subprocesses with inherited stdin — #67639).
    plugin_roots: list[Path] = [get_tampinha_home() / "plugins"]
    if os.environ.get("TAMPINHA_ENABLE_PROJECT_PLUGINS"):
        plugin_roots.append(Path.cwd() / ".tampinha" / "plugins")
