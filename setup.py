...
Tampinha Agent (see website/docs/getting-started/platform-support.md). The
...
build sandbox. ``nix/python.nix`` sets ``TAMPINHA_NIX_BUILD=1`` on the
Tampinha package derivation, so only that build may create an artifact.
...
_IN_NIX_BUILD = os.environ.get("TAMPINHA_NIX_BUILD") == "1"

_BLOCK_MESSAGE = (
    "Building wheels or sdists for tampinha-agent is not supported.\n"
    "Tampinha is distributed via the shell installer, Docker image, or Nix.\n"
    ...
    "the Tampinha Nix derivation sets TAMPINHA_NIX_BUILD=1. If it does, file a bug."
)
