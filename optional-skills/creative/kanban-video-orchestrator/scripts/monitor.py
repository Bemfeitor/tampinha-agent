#!/usr/bin/env python3
"""
Monitor a running video-production kanban. Polls `tampinha kanban list` and
...
def tampinha_available() -> bool:
    return shutil.which("tampinha") is not None


def kanban_list(tenant: str) -> list[dict]:
    """Returns parsed task rows. Falls back to plain stdout parsing if JSON
    output isn't supported by the installed tampinha CLI."""
    try:
        out = subprocess.run(
            ["tampinha", "kanban", "list", "--tenant", tenant, "--json"],
        ...
        pass
    # Fallback: textual parse of `tampinha kanban list`
    out = subprocess.run(
        ["tampinha", "kanban", "list", "--tenant", tenant],
        capture_output=True, text=True, encoding='utf-8', errors='replace', check=False,
    )
    rows = []
...
def kanban_show(task_id: str) -> dict | None:
    out = subprocess.run(
        ["tampinha", "kanban", "show", task_id, "--json"],
        capture_output=True, text=True, encoding='utf-8', errors='replace', check=False,
    )
    if out.returncode != 0:
        return None
...
def main():
    ...
    if not tampinha_available():
        print("ERROR: 'tampinha' CLI not found in PATH", file=sys.stderr)
        sys.exit(1)
