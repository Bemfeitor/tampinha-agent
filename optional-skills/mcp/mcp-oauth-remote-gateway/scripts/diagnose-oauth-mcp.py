...
Usage:
  ...
  <server_name> matches the files in $TAMPINHA_HOME/mcp-tokens/<server>.json etc.
...
def _tampinha_home():
    # Prefer Tampinha' own resolver (profile-safe); fall back to env then ~/.tampinha.
    try:
        from tampinha_constants import get_tampinha_home
        return str(get_tampinha_home())
    except Exception:
        return os.environ.get("TAMPINHA_HOME") or os.path.expanduser("~/.tampinha")


def _tokens_dir():
    return os.path.join(_tampinha_home(), "mcp-tokens")
    ...
    status, hdrs, body = _post(
        ...
        data={
            ...
            "params": {"protocolVersion": "2025-06-18", "capabilities": {},
                       "clientInfo": {"name": "tampinha-diag", "version": "1.0"}},
        },
        headers={
