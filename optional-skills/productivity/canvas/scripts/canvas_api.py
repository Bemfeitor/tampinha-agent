#!/usr/bin/env python3
"""Canvas LMS API CLI for Tampinha Agent.
...
def _check_config():
    ...
    if missing:
        tampinha_env = os.path.join(
            os.environ.get("TAMPINHA_HOME", os.path.expanduser("~/.tampinha")), ".env"
        )
        print(
            f"Missing required environment variables: {', '.join(missing)}\n"
            f"Set them in {tampinha_env} or export them in your shell.\n"
...
def main():
    parser = argparse.ArgumentParser(
        description="Canvas LMS API CLI for Tampinha Agent"
    )
