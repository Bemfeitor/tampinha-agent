...
Usage (via cron with --no-agent):
    ...
    tampinha cron create tampinha-issues \\
      --schedule "*/5 * * * *" --no-agent \\
      --script "$TAMPINHA_HOME/skills/devops/watchers/scripts/watch_github.py" \\
      --script-args "--name tampinha-issues --repo NousResearch/hermes-agent --scope issues"

Set GITHUB_TOKEN (or GH_TOKEN) in the Tampinha .env file
(``${TAMPINHA_HOME:-~/.tampinha}/.env``) to avoid the 60 req/hr
...
def main() -> int:
    ...
    headers = {
        ...
        "User-Agent": "Tampinha-Watcher/1.0",
    }
