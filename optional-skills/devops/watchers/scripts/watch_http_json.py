...
Usage (via cron with --no-agent):
    ...
    tampinha cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$TAMPINHA_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "Tampinha-Watcher/1.0"})
