...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("TAMPINHA_HOME", os.path.expanduser("~/.tampinha")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("TAMPINHA_HOME", Path.home() / ".tampinha")) / "skills" / "red-teaming" / "godmode"
...
# ═══════════════════════════════════════════════════════════════════
# Tampinha config paths
# ═══════════════════════════════════════════════════════════════════

TAMPINHA_HOME = Path(os.getenv("TAMPINHA_HOME", Path.home() / ".tampinha"))
CONFIG_PATH = TAMPINHA_HOME / "config.yaml"
PREFILL_PATH = TAMPINHA_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/Tampinha models — already uncensored, just needs clean prompt
    "tampinha": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "tampinha" in model_lower or "nous" in model_lower:
        return "tampinha"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Tampinha config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.tampinha/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Tampinha for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Tampinha for changes to take effect.")
