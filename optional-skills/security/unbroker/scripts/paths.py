...
All per-subject data lives under PDD_DATA_DIR (default: $TAMPINHA_HOME/unbroker),
which is the same trust boundary Tampinha uses for .env and OAuth tokens.
"""
...
def tampinha_home() -> Path:
    return Path(os.environ.get("TAMPINHA_HOME") or (Path.home() / ".tampinha"))


def data_dir() -> Path:
    override = os.environ.get("PDD_DATA_DIR")
    return Path(override) if override else tampinha_home() / "unbroker"
