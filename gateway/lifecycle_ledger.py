...
``<TAMPINHA_HOME>/state/gateway.lifecycle.json``:
...
def _process_tampinha_home() -> Path:
    """TAMPINHA_HOME for process-level identity files (ignore task overrides)."""
...
def get_lifecycle_sentinel_path(home: Optional[Path] = None) -> Path:
    """Return ``<TAMPINHA_HOME>/state/gateway.lifecycle.json``."""
...
def read_prior_exit_label(profile_home: Path) -> str:
    ...
    ``tampinha_cli.container_boot`` to annotate ``container-boot.log``.
    """
    try:
