...
Defenses 1 and 2 (the ``_TAMPINHA_GATEWAY`` guard on ``tampinha gateway
...
raw ``terminal("launchctl kickstart -k gui/<uid>/ai.tampinha.gateway")``,
...
State lives in ``<TAMPINHA_HOME>/gateway/restart_loop.json`` so it is
...
def _state_path():
    return get_tampinha_home() / "gateway" / "restart_loop.json"
