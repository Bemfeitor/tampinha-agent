...
1. ``flush_pending_to_file()`` — called BEFORE ``_pending_messages.clear()``
   ...
   under ``<tampinha_home>/pending_messages/``.
...
def _get_flush_dir():
    """Return the pending-messages flush directory under the active TAMPINHA_HOME."""
    from tampinha_constants import get_tampinha_home

    flush_dir = get_tampinha_home() / "pending_messages"
...
def spool_dropped_transcript_message(
    ...
    ``<tampinha_home>/pending_messages/``), so a runtime cap rotation no
