...
# Audio file extensions Tampinha recognizes for native audio delivery.
# Keep Telegram's narrower attachment/voice sets below separate: formats such
# as MPEG-2 Layer II are audio to Tampinha but unsupported by sendAudio/sendVoice.
_AUDIO_MIME_TYPES = {
    ".ogg": "audio/ogg",
    ".opus": "audio/opus",
    ".mp3": "audio/mpeg",
...
def _thread_metadata_for_source(source, reply_to_message_id: str | None = None) -> dict | None:
    ...
    value. Telegram private-chat topics created through Tampinha' DM-topic helper
...
def _reply_anchor_for_event(event) -> str | None:
    ...
    replying to the triggering message. Tampinha-created Telegram private-chat
    ...
    if (
        ...
        and raw_message.get("_tampinha_no_thread_response")
    ):
...
def build_auto_tts_output_path(platform) -> str:
    ...
    TTS tool's ``TAMPINHA_SESSION_PLATFORM`` contextvar — that contextvar is
...
from tampinha_constants import get_default_tampinha_root, get_tampinha_dir, get_tampinha_home
...
GATEWAY_SECRET_CAPTURE_UNSUPPORTED_MESSAGE = (
    ...
    "Load this skill in the local CLI to be prompted, or add the key to ~/.tampinha/.env manually."
)
...
IMAGE_CACHE_DIR = get_tampinha_dir("cache/images", "image_cache")


def _resolve_cache_dir(constant_name: str, new_subpath: str, old_name: str) -> Path:
    """Resolve fresh via get_tampinha_dir (active profile), unless a test has
    monkeypatched the constant away from its import-time default."""
    fresh = get_tampinha_dir(new_subpath, old_name)
...
def get_inbound_media_max_bytes() -> int:
    ...
    """
    try:
        from tampinha_cli.config import load_config_readonly as _load_config
...
AUDIO_CACHE_DIR = get_tampinha_dir("cache/audio", "audio_cache")
...
VIDEO_CACHE_DIR = get_tampinha_dir("cache/videos", "video_cache")
...
DOCUMENT_CACHE_DIR = get_tampinha_dir("cache/documents", "document_cache")
SCREENSHOT_CACHE_DIR = get_tampinha_dir("cache/screenshots", "browser_screenshots")
...
_TAMPINHA_ROOT = get_default_tampinha_root()
MEDIA_DELIVERY_ALLOW_DIRS_ENV = "TAMPINHA_MEDIA_ALLOW_DIRS"
MEDIA_DELIVERY_TRUST_RECENT_ENV = "TAMPINHA_MEDIA_TRUST_RECENT_FILES"
MEDIA_DELIVERY_TRUST_RECENT_SECONDS_ENV = "TAMPINHA_MEDIA_TRUST_RECENT_SECONDS"
...
MEDIA_DELIVERY_STRICT_ENV = "TAMPINHA_MEDIA_DELIVERY_STRICT"
MEDIA_DELIVERY_SAFE_ROOTS = (
    ...
    _TAMPINHA_HOME / "image_cache",
    _TAMPINHA_HOME / "audio_cache",
    _TAMPINHA_HOME / "video_cache",
    _TAMPINHA_HOME / "document_cache",
    _TAMPINHA_HOME / "browser_screenshots",
    ...
    _TAMPINHA_HOME / "cache" / "images",
    _TAMPINHA_HOME / "cache" / "audio",
    _TAMPINHA_HOME / "cache" / "videos",
    _TAMPINHA_HOME / "cache" / "documents",
    _TAMPINHA_HOME / "cache" / "screenshots",
)
...
def _profile_cache_roots() -> List[Path]:
    """Return per-profile canonical cache roots under the shared Tampinha root.

    Profile gateways write generated artifacts to
    ``<root>/profiles/<name>/cache/{images,audio,...}``. The static safe-roots
    list only covers the *active* TAMPINHA_HOME's cache, so a gateway running at
    the root (e.g. ``TAMPINHA_HOME=/opt/data``) while the model emits a
    ...
    is consulted (which otherwise wins when TAMPINHA_HOME is symlinked under a
    denied prefix and $HOME is not that prefix). See issue #31733.
    """
    roots: List[Path] = []
    profiles_dir = _TAMPINHA_ROOT / "profiles"
    try:
...
def _kanban_attachment_roots() -> List[Path]:
    ...
    home_override = os.environ.get("TAMPINHA_KANBAN_HOME", "").strip()
    root = Path(home_override).expanduser() if home_override else _TAMPINHA_ROOT
...
def _media_delivery_denied_paths() -> List[Path]:
    ...
    # The active Tampinha profile and shared Tampinha root both contain control
    ...
    # TAMPINHA_HOME root. The set mirrors the canonical read guard in
    ...
    # tree, so skills/, logs/, and ad-hoc agent-written files under ~/.tampinha
    ...
    for tampinha_root in (_TAMPINHA_HOME, _TAMPINHA_ROOT):
        for rel in _ROOT_CREDENTIAL_FILES:
            denied.append(tampinha_root / rel)
...
def _path_under_denied_prefix(resolved: Path) -> bool:
    ...
    sub-directories inside home (``~/.ssh``, ``~/.aws``, ...) and Tampinha
    secrets (``~/.tampinha/.env``, ``auth.json``) are *separate, more-specific*
...
def _cache_dir_container_mounts() -> List[Tuple[Path, Path]]:
    """(host, container) pairs for the auto-mounted Tampinha cache dirs.

    The agent legitimately sees generated artifacts at ``/root/.tampinha/...``
...
def _translate_docker_container_media_path(candidate: Path, session_key: str = "") -> Optional[Path]:
    ...
    auto-mounted Tampinha cache dirs (``/root/.tampinha/...``), the session's
    ...
    # In-process gateways (Desktop backend, `tampinha serve`) may not have
    ...
    # are longer prefixes, so /root/.tampinha/... still translates to the host
    # cache — this only catches stray home writes like /root/out.png.
    if not any(c.as_posix() == "/root" for _, c in mounts):
        # /root/.tampinha/* that did NOT match a cache mount is the container's
        ...
        if not candidate.as_posix().startswith("/root/.tampinha"):
...
def validate_media_delivery_path(path: str, session_key: str = "") -> Optional[str]:
    ...
    ``TAMPINHA_MEDIA_DELIVERY_STRICT=1``): the file MUST live under a
    Tampinha-managed cache, under an operator-allowlisted root
    (``TAMPINHA_MEDIA_ALLOW_DIRS``), or be freshly produced inside the
    ...
    # credential/secret stores under the Tampinha root (~/.tampinha/.env,
    ...
    # ``MEDIA:~/.tampinha/google_token.json``) remain rejected.
...
_PLAINTEXT_GATEWAY_RESTART_PATTERNS: tuple[re.Pattern[str], ...] = (
    ...
    re.compile(r"^(?:please\s+)?restart\s+(?:the\s+)?tampinha\s+gateway[.!?\s]*$", re.IGNORECASE),
    re.compile(r"^(?:please\s+)?restart\s+tampinha[.!?\s]*$", re.IGNORECASE),
)
...
class BasePlatformAdapter(ABC):
    ...
    # into the ``TAMPINHA_SESSION_ASYNC_DELIVERY`` contextvar at session-bind
    ...
    # Tampinha commands.  Default "/" (most platforms deliver "/approve" etc.
    ...
    def __init__(self, config: PlatformConfig, platform: Platform):
        ...
        # Cross-TAMPINHA_HOME token takeover is armed by GatewayRunner only for
        ...
        self._busy_text_mode: str = (
            os.environ.get("TAMPINHA_GATEWAY_BUSY_TEXT_MODE", "interrupt").strip().lower()
            or "interrupt"
        )
        self._busy_text_debounce_seconds: float = _float_env(
            "TAMPINHA_GATEWAY_BUSY_TEXT_DEBOUNCE_SECONDS", 0.35
        )
        self._busy_text_hard_cap_seconds: float = _float_env(
            "TAMPINHA_GATEWAY_BUSY_TEXT_HARD_CAP_SECONDS", 1.0
        )
    ...
    def prefers_fresh_final_streaming(
        ...
        implementation supports. Telegram is the motivating case: Tampinha sends
    ...
    def _acquire_platform_lock(self, scope: str, identity: str, resource_desc: str) -> bool:
        ...
        A live cross-TAMPINHA_HOME holder may be replaced only when the runner
        ...
        if owner_profile:
            ...
            remedy = (
                ...
                f"(tampinha --profile {owner_profile} gateway stop)."
            )
        else:
            holder = f" (PID {owner_pid})" if owner_pid else ""
    ...
    def _wire_plugin_handlers(self, native: Any = None) -> None:
        ...
        try:
            from tampinha_cli.plugins import get_plugin_manager
    ...
    async def send_voice(
        ...
        **kwargs,
        ...
        host filesystem path that would leak the Tampinha home layout.
        """
    ...
    async def send_video(
        ...
        **kwargs,
        ...
        would leak the Tampinha home layout.
        """
    ...
    def _mask_json_string_media(content: str) -> str:
        ...
        Serialized tool results frequently embed a previous reply's text, e.g.::
            ...
            {"result": "MEDIA:/Users/x/.tampinha/media/generated/stale.png"}
    ...
    async def handle_message(self, event: MessageEvent) -> None:
        ...
        if session_key in self._active_sessions:
            ...
            from tampinha_cli.commands import (
    ...
    def _get_human_delay() -> float:
        ...
        Reads from env vars:
          TAMPINHA_HUMAN_DELAY_MODE: "off" (default) | "natural" | "custom"
          TAMPINHA_HUMAN_DELAY_MIN_MS: minimum delay in ms (default 800, custom mode)
          TAMPINHA_HUMAN_DELAY_MAX_MS: maximum delay in ms (default 2500, custom mode)
        """
        mode = os.getenv("TAMPINHA_HUMAN_DELAY_MODE", "off").lower()
        if mode == "off":
            return 0.0
        if mode == "natural":
        ...
        try:
            min_ms = int(os.getenv("TAMPINHA_HUMAN_DELAY_MIN_MS", "800"))
        except (TypeError, ValueError):
            min_ms = 800
        try:
            max_ms = int(os.getenv("TAMPINHA_HUMAN_DELAY_MAX_MS", "2500"))
        except (TypeError, ValueError):
            max_ms = 2500
        return random.uniform(min_ms / 1000.0, max_ms / 1000.0)
    ...
    async def _process_message_background(self, event: MessageEvent, session_key: str) -> None:
        ...
        try:
            ...
            if response:
                ...
                if (self._should_auto_tts_for_chat(event.source.chat_id)
                        ...
                        and not self._streaming_tts_turn_completed(
                            session_key,
                            getattr(interrupt_event, "_tampinha_run_generation", None),
                            event=event,
                        )):
                    try:
                        ...
                        if check_tts_requirements():
                            ...
                            # TAMPINHA_SESSION_PLATFORM contextvar the tool would
            ...
            # not at the top of this task.  _tampinha_run_generation is set on
    ...
    def build_source(
        ...
        per-profile TAMPINHA_HOME isolation.
        """
