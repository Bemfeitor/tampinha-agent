...
def _resolve_profile_path(path_value: Any) -> Optional[Path]:
    """Resolve a user path, mapping ~/.tampinha to the active profile home."""
    ...
    if raw.startswith("~/.tampinha/"):
        return tampinha_home / raw.removeprefix("~/.tampinha/")
    path = Path(raw).expanduser()
    if path.is_absolute():
        return path
    return tampinha_home / path


def _resolve_script_path(script_value: Any) -> tuple[Optional[Path], Optional[str]]:
    """Resolve a route script under TAMPINHA_HOME/scripts."""
    if not isinstance(script_value, str) or not script_value.strip():
        return None, "script path is empty"
    from tampinha_constants import get_tampinha_home

    scripts_root = (get_tampinha_home() / "scripts").resolve()
    raw_text = os.path.expandvars(script_value.strip())
    if raw_text == "~/.tampinha" or raw_text.startswith("~/.tampinha/"):
...
class WebhookRouteProcessor:
    ...
    def run_route_script(self, script_value: Any, payload: dict) -> tuple[bool, Optional[dict]]:
        ...
        if (
            ...
            or transformed.get("__tampinha_ignore__") is True
        ):
            return False, None
