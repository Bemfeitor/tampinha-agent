...
class WhatsAppBehaviorMixin:
    ...
    DEFAULT_REPLY_PREFIX: str = "⚕ *Tampinha Agent*\n────────────\n"
...
def resolve_whatsapp_bridge_dir() -> Path:
    """Resolve the WhatsApp bridge directory, mirroring to TAMPINHA_HOME if needed.

    When the install tree is read-only (e.g., Docker /opt/tampinha), this function
    mirrors the bridge source to a writable TAMPINHA_HOME location and returns that
    ...
    # Try TAMPINHA_HOME location first
    tampinha_home = get_tampinha_home()
    tampinha_home_bridge = tampinha_home / "scripts" / "whatsapp-bridge"
    ...
    # Install dir is read-only, mirror to TAMPINHA_HOME if needed
    if tampinha_home_bridge.exists():
        return tampinha_home_bridge

    # Mirror the bridge source to TAMPINHA_HOME
    try:
        tampinha_home_bridge.parent.mkdir(parents=True, exist_ok=True)
        shutil.copytree(
            install_bridge,
            tampinha_home_bridge,
            dirs_exist_ok=False,
        )
        return tampinha_home_bridge
    except Exception:
