...
class PlatformEntry:
    ...
    # status displays (``tampinha setup``, ``tampinha status``, the dashboard
    ...
    # Env vars this platform needs (for ``tampinha setup`` display).
    ...
    # built-ins).  Used by ``tampinha gateway setup`` to auto-enable the
...
class PlatformRegistry:
    ...
    def __init__(self) -> None:
        ...
        # Plugin adapters are isolated per resolved TAMPINHA_HOME and overlay the
        ...
        # plugin-discovery time added several seconds to *every* `tampinha`
        # invocation -- including plain `tampinha chat`, which never touches any
        ...
        # `tampinha setup`/`gateway status`, send_message).
    ...
    def _resolve_all(self) -> None:
        ...
        gateway startup, ``tampinha setup``/``gateway status``, channel
