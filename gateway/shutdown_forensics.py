    ...
    * Wall-clock and monotonic timestamps for cross-correlating later phases
    ...
    try:
        ...
        if tampinha_home_str:
            takeover_path = Path(tampinha_home_str) / ".gateway-takeover.json"
            if takeover_path.exists():
                try:
                    ...
                    pass
            planned_stop_path = Path(tampinha_home_str) / ".gateway-planned-stop.json"
...
def check_systemd_timing_alignment(
    ...
    upgraded tampinha-agent but never re-ran ``tampinha setup`` to regenerate
    ...
    try:
        # /proc/self/cgroup gives us "0::/user.slice/.../tampinha-gateway.service"
    ...
    # that's the common case for tampinha.
...
def parse_systemd_duration_to_us(raw: str) -> Optional[int]:
    ...
    Public: also consumed by tampinha_cli.gateway's restart-wait sizing.
    """
    if not raw:
        return None
    units = {
