...
# `export TAMPINHA_REDACT_SECRETS=false`) cannot disable redaction
# mid-session.  ON by default — secure default per issue #17691. Users who
# need raw credential values in tool output (e.g. working on the redactor
# itself) can opt out via `security.redact_secrets: false` in config.yaml
# (bridged to this env var in tampinha_cli/main.py, gateway/run.py, and
# cli.py) or `TAMPINHA_REDACT_SECRETS=false` in ~/.tampinha/.env. An opt-out
# warning is logged at gateway and CLI startup so operators see the
# downgrade — see `_log_redaction_status()` in gateway/run.py and cli.py.
_REDACT_ENABLED = os.getenv("TAMPINHA_REDACT_SECRETS", "true").lower() in {"1", "true", "yes", "on"}
...
def mask_secret(
    ...
    *,
    ...
    Canonical helper for display-time redaction across Tampinha — used by
    ``tampinha config``, ``tampinha status``, ``tampinha dump``, and anywhere
...
def redact_sensitive_text(
    ...
    *,
    ...
    for URLs, ``"eyJ" in text`` for JWTs). On a typical tampinha log line
...
# (``security.redact_secrets: false`` / TAMPINHA_REDACT_SECRETS) applies to
