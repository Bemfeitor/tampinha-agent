...
cache problem must never block Tampinha startup.
"""
...
def resolve_cache_home(home_path: Optional[Path] = None) -> Path:
    """Resolve the Tampinha home used for cache paths.

    ``home_path`` is whatever ``load_tampinha_dotenv()`` already resolved;
    falling back to ``$TAMPINHA_HOME`` / ``~/.tampinha`` keeps direct callers
    (and tests that don't thread a home through) working.
    """
    if home_path is None:
        from tampinha_constants import get_tampinha_home

        home_path = get_tampinha_home()
    return home_path
...
class DiskCache(Generic[K]):
    ...
    One JSON object per backend lives at ``<tampinha_home>/cache/<basename>``::
    ...
    def write(
        ...
        try:
            ...
            # metadata isn't exposed if TAMPINHA_HOME is ever made traversable.
            try:
