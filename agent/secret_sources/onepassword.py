...
* Authentication is whatever the user's ``op`` CLI already uses — a
  ...
  or a desktop/interactive session (``OP_SESSION_*``).  Tampinha never
...
* Failures NEVER block startup.  A missing ``op`` binary, expired auth, a
  ...
  Tampinha continues with whatever credentials ``.env`` already had.
...
are cached in-process and on disk under ``<tampinha_home>/cache/op_cache.json``
so back-to-back short-lived ``tampinha`` invocations don't re-shell ``op`` for
...
# In-process cache.  The key folds in str(home_path) so a TAMPINHA_HOME switch
...
def _op_child_env(token_value: str) -> Dict[str, str]:
    ...
    # configured Tampinha to source it from, so normalize to that name here.
    if token_value:
...
def apply_onepassword_secrets(
    *,
    ...
    Called by ``load_tampinha_dotenv()`` after the .env files have loaded.
    ...
    def remediation(self, kind, cfg: dict) -> str:
        if kind in (ErrorKind.AUTH_FAILED, ErrorKind.AUTH_EXPIRED):
            ...
            if isinstance(cfg, dict):
                token_env = str(cfg.get("service_account_token_env") or token_env)
            return (
                "Run `tampinha secrets onepassword token` to paste a fresh "
...
def clear_caches(home_path: Optional[Path] = None) -> None:
    ...
    Used after a token rotation (`tampinha secrets onepassword token`) so
