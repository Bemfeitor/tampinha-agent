...
Owns one Codex thread per Tampinha session. Drives `turn/start`, consumes
...
# Tampinha' tools.terminal.security_mode → Codex's permissions profile id.
# Defaults if config is missing → workspace-write (matches Codex's own default).
_TAMPINHA_TO_CODEX_PERMISSION_PROFILE = {
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse Tampinha/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per Tampinha session, lifetime owned by AIAgent.
    ...
    def __init__(
        ...
        *,
        ...
        self._permission_profile = (
            permission_profile or _TAMPINHA_TO_CODEX_PERMISSION_PROFILE.get(
                os.environ.get("TAMPINHA_TERMINAL_SECURITY_MODE", "auto"),
                "workspace-write",
            )
        )
    ...
    def ensure_started(self) -> str:
        ...
        self._client.initialize(
            client_name="tampinha",
            client_title="Tampinha Agent",
            client_version=_get_tampinha_version(),
        )
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but Tampinha' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into Tampinha' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # OAuth, form data). For our own tampinha-tools callback we
            # auto-accept — the user already approved Tampinha' tools
    ...
    def _decide_exec_approval(self, params: dict) -> str:
        ...
        This is protocol-level routing only — it carries NO Tampinha
        approval-mode/timeout logic. The Tampinha-side resolution happens
    ...
    def _decide_apply_patch_approval(self, params: dict) -> str:
        ...
        Protocol-level routing only; Tampinha approval-mode/timeout
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map Tampinha approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    Tampinha returns 'once', 'session', 'always', or 'deny'.
    ...
    NOT in tools/approval.py: the Tampinha approval mode/timeout resolution
    ...
    # "prompt expired"; the Tampinha-side messaging already distinguishes them.
    return "decline"
...
def _get_tampinha_version() -> str:
    """Best-effort Tampinha version string for codex's userAgent line."""
    try:
