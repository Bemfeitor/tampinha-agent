...
projection into Tampinha' display, approval bridging, transcript projection into
...
"codex_app_server"`. Tampinha' default tool dispatch is unchanged when this
...
class CodexAppServerClient:
    ...
    def __init__(
        ...
        # `os.environ.copy()` also handed it every Tier-1 Tampinha secret — gateway
        ...
        spawn_env = tampinha_subprocess_env(inherit_credentials=True)
        if env:
        ...
        if spawn_env.get("TAMPINHA_KANBAN_TASK"):
            kanban_db = spawn_env.get("TAMPINHA_KANBAN_DB")
            kanban_root = (
                os.path.dirname(kanban_db)
                if kanban_db
                else spawn_env.get(
                    "TAMPINHA_KANBAN_ROOT",
                    os.path.join(
                        spawn_env.get("TAMPINHA_HOME", os.path.expanduser("~/.tampinha")),
                        "kanban",
                    ),
                )
            )
    ...
    def initialize(
        self,
        client_name: str = "tampinha",
        client_title: str = "Tampinha Agent",
