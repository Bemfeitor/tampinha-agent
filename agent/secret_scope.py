...
- ``set_secret_scope(mapping)`` installs the active profile's secrets for the
  ...
  via ``copy_context()`` exactly like the TAMPINHA_HOME override).
...
_GLOBAL_ENV_EXACT = frozenset({
    # Tampinha runtime / deployment
    "TAMPINHA_HOME", "TAMPINHA_PROFILE", "TAMPINHA_GATEWAY_LOCK_DIR",
    "TAMPINHA_MAX_ITERATIONS", "TAMPINHA_MAX_TOKENS", "TAMPINHA_API_TIMEOUT",
    "TAMPINHA_REDACT_SECRETS", "TAMPINHA_NOUS_TIMEOUT_SECONDS",
    "_TAMPINHA_GATEWAY",
    # OS / interpreter
    "PATH", "HOME", "USER", "LANG", "LC_ALL", "TZ", "PWD", "SHELL", "TMPDIR",
    "VIRTUAL_ENV", "PYTHONPATH", "SSL_CERT_FILE",
    # Kanban paths (per-board, not per-profile-secret)
    "TAMPINHA_KANBAN_DB", "TAMPINHA_KANBAN_WORKSPACES_ROOT", "TAMPINHA_KANBAN_BOARD",
...
_GLOBAL_ENV_PREFIXES = (
    "TAMPINHA_KANBAN_",
    "TAMPINHA_TELEGRAM_",   # tuning knobs (batch delays, fallback toggles) — NOT the token
...
def load_env_file(env_path: Path) -> Dict[str, str]:
    ...
    ``set_secret_scope``. Parses the small KEY=VALUE subset Tampinha writes
    ...
    ``tampinha_cli.config._parse_env_value``) but never mutates the process
    ...
    # Parse values with the canonical Tampinha parser: save_env_value
    ...
    from tampinha_cli.config import _parse_env_value
...
def build_profile_secret_scope(tampinha_home: Path) -> Dict[str, str]:
    ...
    """
    home = Path(tampinha_home)
    secrets = load_env_file(home / ".env")

    try:
        from tampinha_cli.env_loader import get_secret_source_values
