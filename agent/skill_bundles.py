...
Storage
-------
Bundles live in ``~/.tampinha/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``TAMPINHA_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``tampinha bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under TAMPINHA_HOME.

    Honors ``TAMPINHA_BUNDLES_DIR`` for tests; falls back to
    ``<TAMPINHA_HOME>/skill-bundles``.
    """
    override = os.environ.get("TAMPINHA_BUNDLES_DIR")
    if override:
        return Path(override).expanduser()
    return get_tampinha_home() / "skill-bundles"
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `tampinha bundles` CLI subcommand.
# ---------------------------------------------------------------------------
