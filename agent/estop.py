...
``tampinha pause`` writes a sentinel file at ``$TAMPINHA_HOME/ESTOP``;
``tampinha resume`` removes it. While the sentinel exists:
...
* new gateway turns get a brief "Tampinha is paused" reply instead of an
...
hold even if the file was created by ``touch ~/.tampinha/ESTOP``.
...
def _tampinha_home() -> Path:
    """Resolve the active TAMPINHA_HOME (profile-aware) at call time."""
    try:
...
def sentinel_path() -> Path:
    """Path of the ESTOP sentinel under the active TAMPINHA_HOME."""
    return _tampinha_home() / SENTINEL_NAME
...
def is_engaged() -> bool:
    ...
    exists (permission error, transient I/O failure on TAMPINHA_HOME), report
...
def paused_reply() -> Optional[str]:
    ...
    if reason:
        return (
            f"⏸️ Tampinha is paused ({reason}). New work is on hold; "
            "run `tampinha resume` to pick things back up."
        )
    return (
        "⏸️ Tampinha is paused. New work is on hold; "
        "run `tampinha resume` to pick things back up."
    )
