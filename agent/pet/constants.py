...
row taxonomy is inferred from the atlas shape so Tampinha can render both legacy
...
# rows (1536x1872), while older Tampinha/petdex sheets used 9 columns x 8 rows
...
class PetState(str, Enum):
    ...
    These are Tampinha' activity state names. They are not always identical to the
...
# Legacy Tampinha/petdex row order (top -> bottom) used by the older 8-row,
...
# Canonical Tampinha activity names -> accepted row-name aliases in descending
