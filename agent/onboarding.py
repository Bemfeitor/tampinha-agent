...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Tampinha starts and finds ``~/.openclaw/``.

    Points users at ``tampinha claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``tampinha claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to Tampinha, run "
        "`tampinha claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `tampinha claw cleanup` (renames it to ~/.openclaw.pre-migration — "
