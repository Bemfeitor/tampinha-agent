...
as ``kind: backend``) or ``~/.tampinha/plugins/video_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class VideoGenProvider(abc.ABC):
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        Used by the tool layer for soft validation and by ``tampinha tools``
...
def _videos_cache_dir() -> Path:
    """Return ``$TAMPINHA_HOME/cache/videos/``, creating parents as needed."""
    from tampinha_constants import get_tampinha_home

    path = get_tampinha_home() / "cache" / "videos"
...
def save_b64_video(
    ...
    *,
    ...
    """Decode base64 video data and write under ``$TAMPINHA_HOME/cache/videos/``.
...
def save_url_video(
    ...
    *,
    ...
    """Download a video URL and write it under ``$TAMPINHA_HOME/cache/videos/``.
