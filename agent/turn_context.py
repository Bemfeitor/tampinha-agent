...
def build_turn_context(
    ...
    *,
    ...
    # Tag log records on this thread with the session ID for ``tampinha logs``.
    ...
    if (
        ...
        # Tampinha only initiates compaction in "tampinha" mode (#36801).
        _codex_native_auto = (
        ...
        if _preflight_deferred:
            ...
            logger.info(
                "Skipping Tampinha preflight compression for codex app-server "
                "(mode=%s); Tampinha will not start thread compaction here.",
    ...
    if agent._memory_manager:
        ...
        # to surface it. Rendered by Tampinha (via _emit_status), so it always
