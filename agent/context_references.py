...
from tampinha_cli._subprocess_compat import IS_WINDOWS, windows_hide_flags
...
_SENSITIVE_TAMPINHA_DIRS = (Path("skills") / ".hub",)
...
def _ensure_reference_path_allowed(path: Path) -> None:
    ...
    tampinha_home = get_tampinha_home().resolve()

    blocked_exact = {home / rel for rel in _SENSITIVE_HOME_FILES}
    blocked_exact.add(tampinha_home / ".env")
    blocked_dirs = [home / rel for rel in _SENSITIVE_HOME_DIRS]
    blocked_dirs.extend(tampinha_home / rel for rel in _SENSITIVE_TAMPINHA_DIRS)
    ...
    for blocked_dir in blocked_dirs:
        try:
            ...
            continue
        raise ValueError("path is a sensitive credential or internal Tampinha path and cannot be attached")
    ...
    # message text into reference expansion, so `@file:~/.tampinha/auth.json` from a
    ...
    try:
        ...
        if get_read_block_error(str(path)) is not None:
            raise ValueError(
                "path is a sensitive credential or internal Tampinha path and cannot be attached"
            )
    except ValueError:
        raise
    except Exception:
