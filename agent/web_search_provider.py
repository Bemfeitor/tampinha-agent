...
``kind: backend``) or ``~/.tampinha/plugins/web/<name>/`` (user, opt-in via
``plugins.enabled``).
...
def get_provider_env(name: str) -> str:
    ...
    Resolves *name* via :func:`tampinha_cli.config.get_env_value` (checks
    ``os.environ`` first, then ``~/.tampinha/.env``) so credentials set
    through Tampinha' config layer are visible even when they were never
...
class WebSearchProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``tampinha tools``. Defaults to ``name``."""
        return self.name
    ...
    def is_available(self) -> bool:
        ...
        runs at tool-registration time and on every ``tampinha tools`` paint.
        """
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        """Return provider metadata for the ``tampinha tools`` picker.

        Used by ``tampinha_cli/tools_config.py`` to inject this provider as a
