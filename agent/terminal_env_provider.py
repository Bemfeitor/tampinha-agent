...
Providers live in ``~/.tampinha/plugins/<name>/`` (user, opt-in via
...
* ``cache_path_base`` — where auto-synced ``~/.tampinha/cache`` files land
  inside the backend (e.g. ``"~/.tampinha"`` for home-synced backends,
  ``"/root/.tampinha"`` for root-homed containers), or ``None`` when host
...
The factory stamps ``_tampinha_backend_name`` on the returned object so
...
class TerminalEnvironmentProvider(abc.ABC):
    ...
    def cache_path_base(self) -> Optional[str]:
        """Base dir for synced Tampinha cache files inside the backend."""
        return None
    ...
    def setup_instructions(self) -> List[str]:
        """Lines printed by ``tampinha setup`` after this backend is selected.
