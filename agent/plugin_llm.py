...
Plugins built on Tampinha Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Tampinha
...
:class:`~tampinha_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors Tampinha' main config
...
handles every provider, fallback chain, and per-task override Tampinha
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`tampinha_cli.plugins.PluginContext`
