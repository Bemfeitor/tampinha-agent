...
token`` all expose exactly this contract. Tampinha already accepts a callable
...
def _mint(command: str, label: str) -> tuple[str, Optional[float]]:
    ...
    if stdout.lstrip().startswith("{"):
        ...
        if isinstance(payload, dict):
            ...
            # Imported lazily: tampinha_cli.auth imports from agent.* at module
            # level, so a top-level import here would risk a cycle.
            from tampinha_cli.auth import _parse_iso_timestamp
