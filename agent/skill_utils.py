...
from tampinha_constants import get_config_path, get_skills_dir, is_termux
...
# Org mirrors live under ~/.tampinha/skills/_org/<org_id>/. Resolution is
...
def _detect_environment(env: str) -> bool:
    ...
    TAMPINHA_KANBAN_* vars without owning them), so caching it process-wide would
    ...
    if env == "kanban":
        ...
        # dispatcher sets ``TAMPINHA_KANBAN_TASK`` / ``TAMPINHA_KANBAN_BOARD`` in the
        ...
        # tool availability.
        if os.getenv("TAMPINHA_KANBAN_TASK") or os.getenv("TAMPINHA_KANBAN_BOARD"):
        ...
        # The Tampinha Docker image runs s6-overlay as PID 1 (/init). s6 plants
...
def _load_raw_config() -> Dict[str, Any]:
    ...
    This module intentionally avoids importing ``tampinha_cli.config`` on the
...
# `tampinha-agent` skill is the agent's own operating manual — it drives
# configuring, extending, and troubleshooting Tampinha itself, and the system
# prompt unconditionally points at it. Disabling it leaves the agent unable
# to help with Tampinha, so disable requests for these names are ignored
# everywhere the disabled list is consulted.
ESSENTIAL_SKILLS: frozenset = frozenset({"tampinha-agent"})
...
def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    ...
    Args:
        platform: Explicit platform name (e.g. ``"telegram"``).  When
            *None*, resolves from ``TAMPINHA_PLATFORM`` or
            ``TAMPINHA_SESSION_PLATFORM`` env vars.  Returns the global
    ...
    resolved_platform = (
        platform
        or os.getenv("TAMPINHA_PLATFORM")
        or get_session_env("TAMPINHA_SESSION_PLATFORM")
    )
...
def parse_config_string_list(value) -> List[str]:
    ...
    ``tampinha config set`` and JSON-mode editor saves store lists as quoted
...
# which becomes the dominant cost of ``tampinha`` startup when ~120 skills
...
def get_external_skills_dirs() -> List[Path]:
    ...
    paths that resolve to the local ``~/.tampinha/skills/`` are silently skipped.

    Cached in-process, keyed on ``config.yaml`` mtime — the function is
    called once per skill during banner / tool-registry scans, and YAML
    parsing a non-trivial config dominates ``tampinha`` cold-start time
    ...
    for entry in raw_dirs:
        ...
        p = Path(expanded)
        # Resolve relative paths against TAMPINHA_HOME, not cwd
        if not p.is_absolute():
            p = (tampinha_home / p).resolve()
        else:
            p = p.resolve()
        if p == local_skills:
            continue
...
def get_all_skills_dirs() -> List[Path]:
    """Return all skill directories: local ``~/.tampinha/skills/`` first, then external.

    The local dir is always first (and always included even if it doesn't exist
    yet — callers handle that).  External dirs follow in config order.

    NOTE: trusted project-local dirs (``./.tampinha/skills`` at the git root) are
...
#   <root>/.tampinha/skills/   — Tampinha-native location
...
# a one-line "run `tampinha skills trust`" notice.
#
...
def _candidate_project_skills_dirs(root: Path) -> List[Path]:
    ...
    The exclusion matters when TAMPINHA_HOME itself lives inside a git checkout:
    ``<root>/.tampinha/skills`` would otherwise double as both the profile-local
...
def get_untrusted_project_skills_root() -> Optional[Tuple[Path, int]]:
    ...
    ``tampinha skills trust``. Returns None when there is nothing to notify
...
#
# Trust (`tampinha skills trust`) is a REPO-level decision made once; the repo's
...
#
# The scan cache lives under TAMPINHA_HOME, never inside the repo (we don't
...
def _project_scan_cache_dir() -> Path:
    ...
    return get_tampinha_home() / "cache" / "project_skill_scans"
...
def normalize_skill_lookup_name(identifier: str) -> str:
    ...
    under ``~/.tampinha/skills/`` (including via symlinks) or configured
    ...
    # ~/.tampinha/skills/<name> where <name> is a symlink to a checked-out
...
def is_external_skill_path(path) -> bool:
    ...
    ``skills.external_dirs`` are externally owned: Tampinha can discover and view
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    if not isinstance(metadata, dict):
        metadata = {}
    tampinha = metadata.get("tampinha") or {}
    if not isinstance(tampinha, dict):
        tampinha = {}
    return {
        "fallback_for_toolsets": tampinha.get("fallback_for_toolsets", []),
        "requires_toolsets": tampinha.get("requires_toolsets", []),
        "fallback_for_tools": tampinha.get("fallback_for_tools", []),
        "requires_tools": tampinha.get("requires_tools", []),
    }
...
def extract_skill_config_vars(frontmatter: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    if not isinstance(metadata, dict):
        return []
    tampinha = metadata.get("tampinha")
    if not isinstance(tampinha, dict):
        return []
    raw = tampinha.get("config")
    if not raw:
        return []
...
def iter_skill_index_files(skills_dir: Path, filename: str):
    ...
    Excludes Tampinha metadata, VCS, virtualenv/dependency, cache, and skill
