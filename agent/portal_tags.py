...
Every Tampinha request that hits the Nous Portal — main agent loop, auxiliary
client (compression / titles / vision / web_extract / session_search / etc.),
and any future code path — must carry the same product-attribution tags so
Nous can attribute usage to Tampinha Agent and bucket it by client release.

Tag shape (sent in OpenAI-compatible ``extra_body['tags']``):

    [
        "product=tampinha-agent",
        "client=tampinha-client-v<__version__>",
    ]

The version is sourced live from ``tampinha_cli.__version__`` so it auto-aligns
...
``tampinha_cli.__version__`` is the canonical source of truth.
"""
...
def _tampinha_version() -> str:
    """Return the current Tampinha release version, e.g. ``"0.13.0"``.

    Falls back to ``"unknown"`` if ``tampinha_cli`` cannot be imported (should
...
def tampinha_client_tag() -> str:
    """Return the ``client=...`` tag for Nous Portal requests.

    Format: ``client=tampinha-client-v<MAJOR>.<MINOR>.<PATCH>``.
    """
    return f"client=tampinha-client-v{_tampinha_version()}"


def conversation_tag(session_id: str) -> str:
    """Return the ``conversation=...`` tag for a Tampinha session/conversation.

    Format: ``conversation=<session_id>``. ``session_id`` is the canonical
    Tampinha conversation identifier (``AIAgent.session_id``) — the same value
    used for ``~/.tampinha/sessions/`` storage, session logs, and lineage.
...
def nous_portal_tags(session_id: str | None = None) -> List[str]:
    ...
    appended so Portal usage can be attributed to a specific Tampinha
    ...
    """
    tags = ["product=tampinha-agent", tampinha_client_tag()]
