...
Optional hooks (override to opt in):
  ...
  backup_paths() -> list[str]            — extra on-disk paths to include in `tampinha backup`
"""
...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - tampinha_home (str): The active TAMPINHA_HOME directory path. Use this
            for profile-scoped storage instead of hardcoding ``~/.tampinha``.
        ...
        kwargs may also include:
          ...
          - agent_workspace (str): Shared workspace name (e.g. "tampinha").
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        ...
        Used by 'tampinha memory setup' to walk the user through configuration.
    ...
    def save_config(self, values: Dict[str, Any], tampinha_home: str) -> None:
        ...
        Called by 'tampinha memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``tampinha_home`` is the active TAMPINHA_HOME directory path.
    ...
    def backup_paths(self) -> List[str]:
        """Return extra on-disk paths this provider stores OUTSIDE TAMPINHA_HOME.

        ``tampinha backup`` only walks TAMPINHA_HOME, so any provider state kept
        ...
        of the archive, and ``tampinha import`` restores them to their original
