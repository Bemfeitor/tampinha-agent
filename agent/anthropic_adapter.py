"""Anthropic Messages API adapter for Tampinha Agent.

Translates between Tampinha's internal OpenAI-style message format and
...
def _getenv(name: str, default: str = "") -> str:
    ...
    read) when on. Mirrors the same wrapper in tampinha_cli/runtime_provider.py.
    """
...
# Tampinha effort → Anthropic adaptive-thinking effort (output_config.effort).
...
def _is_kimi_family_endpoint(base_url: str | None, model: str | None = None) -> bool:
    ...
    See tampinha-agent#13848, #17057.
    """
...
def _build_anthropic_client_with_bearer_hook(
    ...
    *,
    ...
    kwargs = {
        ...
        # Delegate retry to tampinha's outer loop (honors Retry-After); the SDK
...
def build_anthropic_client(
    ...
    * a ``Callable[[], str]`` — an Entra ID bearer token provider from
    ...
    kwargs = {
        ...
        # Delegate all rate-limit / 5xx retry to tampinha's outer conversation
    ...
    if _is_opencode_endpoint(base_url):
        ...
        headers.setdefault("X-Title", "Tampinha Agent")
        headers.setdefault("User-Agent", f"HermesAgent/{_TAMPINHA_VERSION}")
        kwargs["default_headers"] = headers

    client = _anthropic_sdk.Anthropic(**kwargs)
    # Bearer-only construction leaves ``api_key`` unset, so the SDK fills it
    # from ``ANTHROPIC_API_KEY`` (Tampinha loads that into the process env from
    # ``~/.tampinha/.env``). The result is dual auth —
...
def _refresh_oauth_token(creds: Dict[str, Any]) -> Optional[str]:
    ...
    Tampinha notices an expired token, Claude Code may have already rotated it.
...
def _prefer_refreshable_claude_code_token(env_token: str, creds: Optional[Dict[str, Any]]) -> Optional[str]:
    ...
    Tampinha historically persisted setup tokens into ANTHROPIC_TOKEN. That makes
...
def _resolve_anthropic_pool_token() -> Optional[str]:
    ...
    ``account_usage`` and ``tampinha models``) never mutates ``~/.tampinha/auth.json``
...
def resolve_anthropic_token() -> Optional[str]:
    ...
    Priority:
      1. ANTHROPIC_TOKEN env var (OAuth/setup token saved by Tampinha)
      ...
      5. Anthropic credential_pool OAuth entry (~/.tampinha/auth.json)
    ...
    # 1. Tampinha-managed OAuth/setup token env var
    ...
    # 5. Tampinha credential_pool OAuth entry.
...
# ── Tampinha-native PKCE OAuth flow ────────────────────────────────────────
# Mirrors the flow used by Claude Code, pi-ai, and OpenCode.
# Stores credentials in ~/.tampinha/.anthropic_oauth.json (our own file).
...
def _get_tampinha_oauth_file() -> Path:
    return get_tampinha_home() / ".anthropic_oauth.json"
...
def run_tampinha_oauth_login_pure() -> Optional[Dict[str, Any]]:
    """Run Tampinha-native OAuth PKCE flow and return credential state."""
    import secrets
    import time
    import webbrowser
    ...
    print()
    print("Authorize Tampinha with your Claude Pro/Max subscription.")
    print()
    ...
    try:
        from tampinha_cli.auth import _can_open_graphical_browser as _can_open_gui
    except Exception:
...
def read_tampinha_oauth_credentials() -> Optional[Dict[str, Any]]:
    """Read Tampinha-managed OAuth credentials from ~/.tampinha/.anthropic_oauth.json."""
    ...
    if oauth_file.exists():
        try:
            ...
            logger.debug("Failed to read Tampinha OAuth credentials: %s", e)
    return None
...
def _convert_assistant_message(m: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # can validate the message history.  See tampinha-agent#13848.
    #
...
def _strip_orphaned_tool_blocks(result: List[Dict[str, Any]]) -> None:
    ...
    for i, m in enumerate(result):
        ...
        # replaying it verbatim.  See tampinha-agent: extended-thinking + parallel
...
def _manage_thinking_signatures(
    ...
    replayed assistant tool-call messages.  See tampinha-agent#13848 (Kimi) and
    tampinha-agent#16748 (DeepSeek).
...
def build_anthropic_kwargs(
    ...
    if is_oauth:
        ...
        for block in system:
            if isinstance(block, dict) and block.get("type") == "text":
                text = block.get("text", "")
                text = text.replace("Tampinha Agent", "Claude Code")
                text = text.replace("Tampinha agent", "Claude Code")
                text = text.replace("tampinha-agent", "claude-code")
        ...
        #      a) bare Tampinha-native tools (``read_file``)  -> ``mcp__read_file``
    ...
    # silently hides reasoning text that Tampinha surfaces in its CLI. We
