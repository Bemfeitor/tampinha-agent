...
The parent Tampinha process may itself be a Kanban dispatcher worker with
TAMPINHA_KANBAN_* variables in process env. delegate_task children run inside the
...
# though the worker's TAMPINHA_KANBAN_* vars are legitimately in os.environ (cron
...
_NON_DISPATCHER_OWNED_CONTEXT: ContextVar[bool] = ContextVar(
    "tampinha_non_dispatcher_owned_context",
    default=False,
)

DELEGATED_CHILD_ENV_MARKER = "TAMPINHA_DELEGATED_CHILD_CONTEXT"
...
def non_dispatcher_owned_context() -> Iterator[None]:
    ...
    own process, where ``TAMPINHA_KANBAN_TASK`` is legitimately set.  Without this
    ...
    ``kanban_complete`` defaults ``task_id`` to ``$TAMPINHA_KANBAN_TASK`` — letting
...
def is_dispatcher_owned_worker_context() -> bool:
    ...
    The single predicate every ``TAMPINHA_KANBAN_*`` identity gate should use
...
def delegated_child_subprocess_env(
    ...
    parent dispatcher ``TAMPINHA_KANBAN_*`` vars while losing the ContextVar in
