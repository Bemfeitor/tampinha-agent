...
2. **Command-type providers** declared under ``tts.providers.<name>:
   ...
   CLI into Tampinha with shell-template placeholders. **Wins over a
...
shipped today) or ``~/.tampinha/plugins/tts/<name>/`` (user-installed).
...
``{success: False, error: …}`` JSON envelope the rest of Tampinha
expects.
"""
...
class TTSProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``tampinha tools``.
    ...
    def is_available(self) -> bool:
        ...
        Must NOT raise — used by the picker and ``tampinha setup`` for
