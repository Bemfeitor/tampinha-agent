...
class MemoryManager:
    ...
    def add_provider(self, provider: MemoryProvider) -> None:
        ...
        _core_tool_names = set(_TAMPINHA_CORE_TOOLS)
    ...
    def _strip_skill_scaffolding(text: str) -> Optional[str]:
        ...
        When a user invokes a /skill or /bundle, Tampinha expands the turn into
    ...
    def _prefetch_provider(
        ...
        # Propagate the caller's contextvars (profile TAMPINHA_HOME override)
    ...
    def _submit_background(self, fn, *, kind: str = "write") -> None:
        ...
        dashboard, cron) is a ContextVar-scoped TAMPINHA_HOME override, and
    ...
    def initialize_all(self, session_id: str, **kwargs) -> None:
        ...
        Automatically injects ``tampinha_home`` into *kwargs* so that every
        provider can resolve profile-scoped storage paths without importing
        ``get_tampinha_home()`` themselves.
        """
        if "tampinha_home" not in kwargs:
            from tampinha_constants import get_tampinha_home
            kwargs["tampinha_home"] = str(get_tampinha_home())
        for provider in self._providers:
            try:
