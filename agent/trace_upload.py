"""Upload a Tampinha session transcript to Hugging Face as an agent trace.

Tampinha stores sessions in its own SQLite store (``tampinha_state.SessionDB``),
...
* **Zero LLM turn.** This is a deterministic export — it never spends a
  model call. The ``tampinha trace upload`` subcommand calls
...
* **Private by default.** Traces can contain prompts, tool output, local
  ...
  is passed through Tampinha' secret redactor (``force=True``) unless the
...
DEFAULT_DATASET_NAME = "tampinha-traces"
_TAMPINHA_VERSION = "tampinha-agent"
...
# ---------------------------------------------------------------------------
# Conversion: Tampinha OpenAI-format messages -> Claude Code JSONL
# ---------------------------------------------------------------------------
...
def _redact(text: Any, enabled: bool) -> Any:
    ...
    Non-strings pass through untouched. Uses Tampinha' shared redactor with
...
def build_trace_jsonl(
    ...
    *,
    ...
    """Render Tampinha conversation messages as Claude Code JSONL text.
...
_NO_TOKEN_MESSAGE = (
    ...
    "2. Add it to your environment as HF_TOKEN (e.g. in ~/.tampinha/.env):\n"
    "     HF_TOKEN=hf_xxxxxxxxxxxxxxxxxxxx\n"
    "3. Run /upload-trace again (or `tampinha trace upload`)."
)
...
def upload_session_trace(
    ...
    *,
    ...
    the user's private ``{user}/tampinha-traces`` dataset. Returns a
