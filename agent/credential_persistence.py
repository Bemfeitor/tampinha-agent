...
``auth.json``.  They intentionally have no dependency on ``tampinha_cli.auth`` so
...
# Sources Tampinha owns and can intentionally persist in auth.json.  Everything
...
def sanitize_borrowed_credential_payload(
    ...
    Owned sources (manual entries and Tampinha-owned OAuth/device-code state)
