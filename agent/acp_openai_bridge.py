"""OpenAI-shape bridge shared by Tampinha' ACP clients.

An ACP agent (``copilot --acp``, and the ACP CLIs that reach Tampinha as
...
the agent's *own* tool notifications. Tampinha' agentic surface — ``memory``,
...
* :func:`completion_to_stream_chunks` — re-shape a one-shot ACP response as
  ...
  is inherently one-shot from Tampinha' perspective).
...
of its own (Copilot) forwards everything Tampinha offers; a CLI that is an
autonomous agent with its own read/edit/execute tools must forward only Tampinha'
agent-level tools, because re-offering the overlapping ones makes Tampinha re-run
...
class StreamChunks(list):
    ...
    Tampinha reads provider-level extras off the object returned by
    ``chat.completions.create`` (e.g. ``tampinha_projected_messages``, consumed by
...
def tool_specs_from_openai_tools(
    ...
    *,
    ...
    name is in it survive — that is how a client forwards just Tampinha'
