"""AWS Bedrock Converse API adapter for Tampinha Agent.
...
def _require_boto3():
    ...
    try:
        ...
        raise ImportError(
            ...
            "Or install Tampinha with Bedrock support: pip install -e '.[bedrock]'"
        )
    # converse() / converse_stream() were added in boto3 1.34.59.
    # When Tampinha is installed editable into system Python, the system boto3
...
def resolve_bedrock_runtime_region(config: Optional[Dict[str, Any]] = None) -> str:
    ...
    ``tampinha_cli/runtime_provider.py``):
