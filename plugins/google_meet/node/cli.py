"""`tampinha meet node ...` subcommand tree.

Wired into the existing ``tampinha meet`` parser by the plugin's top-level
...
def register_cli(subparser: argparse.ArgumentParser) -> None:
    ...
    *subparser* is the ``tampinha meet node`` argparse object — typically
    ...
    run.add_argument("--display-name", default="tampinha-meet-node")
...
def node_command(args: argparse.Namespace) -> int:
    """Dispatch for ``tampinha meet node ...``.
    ...
    if cmd == "run":
        ...
        print(f"             tampinha meet node approve <name> ws://<host>:{args.port} {token}")
        try:
