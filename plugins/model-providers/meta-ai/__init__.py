"""Meta Model API (Muse Spark) provider plugin for Tampinha Agent.
...
Bundled from https://github.com/albertodepaola/hermes-meta-provider. Tampinha'
...
* **Zero core edits.** Everything rides on ``ProviderProfile`` hooks. No changes
  to tampinha' ``model_metadata.py`` / ``models.py`` / ``run_agent.py`` are needed:
...
def _resolve_effort(reasoning_config: dict | None) -> str:
    """Map Tampinha' reasoning_config to a Meta-safe ``reasoning_effort`` value.
...
meta_ai = MetaAIProfile(
    ...
    # in tampinha_cli/providers.py and the retention hint in
