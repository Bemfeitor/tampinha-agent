"""disk_cleanup — ephemeral file cleanup for Tampinha Agent.
...
Rules:
  ...
  - empty dirs    → always delete (under TAMPINHA_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly TAMPINHA_HOME and /tmp/tampinha-*
Never touches: ~/.tampinha/logs/ or any system directory.
"""
...
try:
    ...
    def get_tampinha_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("TAMPINHA_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".tampinha").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$TAMPINHA_HOME/logs/``."""
    return get_tampinha_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$TAMPINHA_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under TAMPINHA_HOME or ``/tmp/tampinha-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    tampinha_home = get_tampinha_home()
    try:
        path.resolve().relative_to(tampinha_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/tampinha-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("tampinha-"):
        return True
    return False
...
_EMPTY_DIR_PROTECTED_TOP_LEVEL = frozenset({
    ...
    "tampinha-agent", "backups", "profiles", ".worktrees",
...
# Paths under $TAMPINHA_HOME that must NEVER be deleted by quick(),
...
def _is_protected_cron_path(p: Path) -> bool:
    ...
    history at once.
    """
    # Lazily build the set once per process so TAMPINHA_HOME is resolved
    # exactly once.
    if not _PROTECTED_CRON_PATHS:
        tampinha_home = get_tampinha_home()
        for parent in ("cron", "cronjobs"):
            base = tampinha_home / parent
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside TAMPINHA_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under TAMPINHA_HOME, but never recurse into known
    # durable state trees.  Some installs place the Tampinha checkout, venv,
    # and desktop build under TAMPINHA_HOME; a full rglob over that tree can
    # stall the gateway event loop for minutes.
    tampinha_home = get_tampinha_home()
    empty_removed = 0
    sweep_stack: List[Tuple[Path, bool]] = []
    try:
        for top in tampinha_home.iterdir():
            if (
                top.is_dir()
...
def guess_category(path: Path) -> Optional[str]:
    ...
    tampinha_home = get_tampinha_home()
    try:
        rel = path.resolve().relative_to(tampinha_home)
        top = rel.parts[0] if rel.parts else ""
        if top in {
            "disk-cleanup", "logs", "memories", "sessions", "config.yaml",
            "skills", "plugins", ".env", "USER.md", "MEMORY.md", "SOUL.md",
            "auth.json", "tampinha-agent",
        ...
        # Path isn't under TAMPINHA_HOME (e.g. /tmp/tampinha-*) — fall through.
        pass
