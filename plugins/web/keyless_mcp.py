...
two endpoints so a fresh Tampinha install with **zero web credentials** still
...
def provider_tier(name: str) -> str:
    ...
    ``tampinha tools`` picker's Free/Paid rows). ``free`` forces the keyless
    ...
    try:
        ...
        return {
            ...
            "error": (
                ...
                "backend via `tampinha tools` for reliable service."
            ),
        }
    ...
    try:
        ...
        return {
            ...
            "error": (
                ...
                "via `tampinha tools` for reliable service."
            ),
        }
...
_KEENABLE_TITLE = "tampinha-agent"
