...
``{title, url, description, position}`` rows every other Tampinha web
provider produces.
...
prefers Tampinha-managed xAI Grok OAuth (via ``tampinha auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.tampinha/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``tampinha tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `tampinha auth` to sign in with "
