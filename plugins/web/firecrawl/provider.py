...
def check_firecrawl_api_key() -> bool:
    """Return True when the Firecrawl backend selected via `tampinha tools`
    ...
    existing tests and the ``tampinha tools`` setup flow.
    """
...
def _raise_web_backend_configuration_error() -> "NoReturn":
    ...
    if _wt.managed_nous_tools_enabled():
        message += (
            ...
            "run `tampinha tools` and select Nous Subscription as the web provider."
        )
    else:
