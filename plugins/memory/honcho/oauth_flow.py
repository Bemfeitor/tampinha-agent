...
``tampinha://`` handler. Endpoints are env-overridable with local-dev defaults
...
# The loopback redirect registered for the Tampinha OAuth client. IP-literal so
...
_DEFAULT_CLIENT_ID = "tampinha-agent"
...
def begin_authorization(
    ...
    *,
    ...
    (``tampinha-desktop`` / ``tampinha-cli``) so the consent side can attribute
...
_CALLBACK_HTML = (
    ...
    b"<div>Connected to Honcho. You can close this tab and return to Tampinha.</div>"
)
...
def start_loopback_flow_background(
    *,
    ...
    # the request, where a context-local TAMPINHA_HOME override can't reach.
