...
Configuration
-------------
Secret (lives in $TAMPINHA_HOME/.env or the environment):
...
Behavioral settings (live in $TAMPINHA_HOME/mem0.json, set via `tampinha memory
setup`):
  ...
  agent_id           — Agent identifier (default: tampinha)
...
_DEFAULT_USER_ID = "tampinha-user"
...
def _load_config() -> dict:
    """Load config from env vars, with $TAMPINHA_HOME/mem0.json overrides.
    ...
    config = {
        ...
        "agent_id": os.environ.get("MEM0_AGENT_ID", "tampinha"),
        "oss": {},
    }
    ...
    config_path = get_tampinha_home() / "mem0.json"
...
class Mem0MemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, tampinha_home):
        """Write config to $TAMPINHA_HOME/mem0.json."""
        import json
    ...
    def post_setup(self, tampinha_home: str, config: dict) -> None:
        ...
        post_setup(tampinha_home, config)
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        #   1. Operator-configured MEM0_USER_ID (env or $TAMPINHA_HOME/mem0.json) —
        ...
        self._agent_id = self._config.get("agent_id", "tampinha")
