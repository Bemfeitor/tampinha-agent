...
_DEFAULT_CONTAINER_TAG = "tampinha"
...
def _load_supermemory_config(tampinha_home: str) -> dict:
    config = _default_config()
    config_path = Path(tampinha_home) / "supermemory.json"
...
def _save_supermemory_config(values: dict, tampinha_home: str) -> None:
    config_path = Path(tampinha_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class _SupermemoryClient:
    def __init__(self, api_key: str, timeout: float, container_tag: str,
        ...
        self._client = Supermemory(
            ...
            timeout=timeout,
            max_retries=0,
            default_headers={"x-sm-source": "tampinha"},
        )

    def _merge_metadata(self, metadata: Optional[dict]) -> dict:
        # sm_source routes Tampinha writes into the "Tampinha" Space in the Supermemory
        ...
        merged = {"sm_source": "tampinha", **(metadata or {})}
...
def _resolve_container_tag_for_setup(tampinha_home: str, *, identity: str = "default") -> str:
    config = _load_supermemory_config(tampinha_home)
...
def _probe_supermemory_connection(api_key: str, tampinha_home: str, *, identity: str = "default") -> dict:
    config = _load_supermemory_config(tampinha_home)
    base_url = _resolve_base_url(config["base_url"])
    status = {
        "ok": False,
        "error": "",
        "container_tag": _resolve_container_tag_for_setup(tampinha_home, identity=identity),
        "profile_facts": 0,
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        # Only prompt for the API key during `tampinha memory setup`.
        # All other options are documented for $TAMPINHA_HOME/supermemory.json
    ...
    def save_config(self, values, tampinha_home):
        ...
        _save_supermemory_config(sanitized, tampinha_home)
    ...
    def get_status_config(self, provider_config: dict) -> dict:
        ...
        status = _probe_supermemory_connection(api_key, tampinha_home)
    ...
    def post_setup(self, tampinha_home: str, config: dict) -> None:
        ...
        from tampinha_cli.memory_setup import _prompt, _write_env_vars
        ...
        if env_writes:
            _write_env_vars(env_writes, tampinha_home=tampinha_home)
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        from tampinha_constants import get_tampinha_home
        self._tampinha_home = kwargs.get("tampinha_home") or str(get_tampinha_home())
        self._session_id = session_id
        self._turn_count = 0
        self._config = _load_supermemory_config(self._tampinha_home)
