...
Config via environment variables:
  ...
  HINDSIGHT_BANK_ID                — memory bank identifier (default: tampinha)
  ...
  HINDSIGHT_RETAIN_SOURCE          — metadata source value attached to retained memories (default: tampinha)
  HINDSIGHT_RETAIN_USER_PREFIX     — label used before user turns in retained transcripts
  HINDSIGHT_RETAIN_ASSISTANT_PREFIX — label used before assistant turns in retained transcripts

Or via $TAMPINHA_HOME/hindsight/config.json (profile-scoped), falling back to
...
# via the ``retain_source`` config key or HINDSIGHT_RETAIN_SOURCE (e.g. "tampinha").
...
def _check_local_runtime() -> tuple[bool, str | None]:
    ...
    so Tampinha can degrade gracefully instead of repeatedly trying to start
    ...
    healthy and ``tampinha memory status`` would stay green while the daemon
...
def _local_runtime_hint(reason: str | None) -> str:
    ...
    selected local_embedded without going through ``tampinha memory setup`` — a
    ...
    if "no module named" in text and ("hindsight'" in text or 'hindsight"' in text
        ...
        return (
            ...
            f"{sys.executable} hindsight-all — or run 'tampinha memory setup'. "
...
def _load_config() -> dict:
    ...
    Resolution order:
      1. $TAMPINHA_HOME/hindsight/config.json  (profile-scoped)
    ...
    profile_path = get_tampinha_home() / "hindsight" / "config.json"
    ...
    return {
        ...
        "banks": {
            "tampinha": {
                "bankId": os.environ.get("HINDSIGHT_BANK_ID", "tampinha"),
...
def _event_timestamp() -> str:
    """Return the configured Tampinha event time with an explicit UTC offset."""
    event_time = _tampinha_now()
    # tampinha_time.now() guarantees an aware datetime. Keep this fallback so a
...
def _embedded_profile_name(config: dict[str, Any]) -> str:
    """Return the Hindsight embedded profile name for this Tampinha config."""
    profile = config.get("profile", "tampinha")
    return str(profile or "tampinha")
...
def _load_simple_env(path) -> dict[str, str]:
    ...
    values: dict[str, str] = {}
    # utf-8-sig, not plain utf-8: this is also used on the Tampinha .env during
...
def _resolve_bank_id_template(template: str, fallback: str, **placeholders: str) -> str:
    ...
    Supported placeholders (each is sanitized before substitution):
      {profile}   — active Tampinha profile name (from agent_identity)
      {workspace} — Tampinha workspace name (from agent_workspace)
      {platform}  — "cli", "telegram", "discord", etc.
      {user}      — platform user id (gateway sessions)
      {session}   — current session id

    Missing/empty placeholders are rendered as the empty string and then
    collapsed — e.g. ``tampinha-{user}`` with no user becomes ``tampinha``.
...
class HindsightMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        self._retain_context = "conversation between Tampinha Agent and the User"
    ...
    def save_config(self, values, tampinha_home):
        """Write config to $TAMPINHA_HOME/hindsight/config.json."""
        import json
        from pathlib import Path
        config_dir = Path(tampinha_home) / "hindsight"
    ...
    def post_setup(self, tampinha_home: str, config: dict) -> None:
        ...
        from tampinha_cli.memory_setup import _CANCELLED, _curses_select, _print_cancelled_setup
        ...
        if mode == "cloud":
            ...
            if llm_key:
                ...
                env_path = Path(tampinha_home) / ".env"
                existing_llm_key = ""
                if env_path.exists():
        ...
        provider_config.setdefault("bank_id", "tampinha")
        ...
        self.save_config(provider_config, tampinha_home)
        ...
        if mode == "local_embedded":
            ...
            config_path = Path(tampinha_home) / "hindsight" / "config.json"
            try:
            ...
            if not llm_api_key:
                llm_api_key = _load_simple_env(Path(tampinha_home) / ".env").get("HINDSIGHT_LLM_API_KEY", "")
            if not llm_api_key:
    ...
    def _offer_starter_template(self, mode: str, provider_config: dict, env_writes: dict) -> None:
        """Offer to seed the bank with a Tampinha starter template (best-effort)."""
        from tampinha_cli.memory_setup import _CANCELLED, _curses_select
        ...
        bank_id = provider_config.get("bank_id", "tampinha")
    ...
    def get_config_schema(self):
        return [
            ...
            {"key": "bank_id", "description": "Memory bank name (static fallback when bank_id_template is unset)", "default": "tampinha"},
            {"key": "bank_id_template", "description": "Optional template to derive bank_id dynamically. Placeholders: {profile}, {workspace}, {platform}, {user}, {session}. Example: tampinha-{profile}", "default": ""},
            ...
            {"key": "retain_context", "description": "Context label for retained memories", "default": "conversation between Tampinha Agent and the User"},
        ...
        if self._client is None:
            if self._mode == "local_embedded":
                ...
                logger.debug("Creating HindsightEmbedded client (profile=%s, provider=%s)",
                             self._config.get("profile", "tampinha"), llm_provider)
                kwargs = dict(
                    profile=self._config.get("profile", "tampinha"),
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        try:
            ...
            if Version(installed) < Version(_MIN_CLIENT_VERSION):
                ...
                # durable data-volume target instead of /opt/tampinha (NS-605).
                from tools.lazy_deps import install_specs
        ...
        banks = cfg_get(self._config, "banks", "tampinha", default={})
        static_bank_id = self._config.get("bank_id") or banks.get("bankId", "tampinha")
        ...
        self._retain_context = self._config.get("retain_context", "conversation between Tampinha Agent and the User")
        ...
        # Tampinha emits a "👁️ Hindsight — recalled N memories" status line so the
        ...
        if self._mode == "local_embedded":
            ...
            if hasattr(os, "geteuid") and os.geteuid() == 0:
                msg = (
                    ...
                    "memory daemon. Run Tampinha as a non-root user, or switch "
                    "to cloud / local_external mode via 'tampinha memory setup'."
                )
                logger.warning(msg)
                # Surface to the terminal too — a daemon that never starts
                # would otherwise fail silently and the user would only see
                # Tampinha get sluggish. (issue #13125)
                try:
    ...
    def on_session_switch(
        ...
        **kwargs,
        ...
        document. See tampinha-agent#6672.
        ...
        if self._client is not None:
            try:
                if self._mode == "local_embedded":
                    ...
                    # When Tampinha created/used that client on the shared async loop,
