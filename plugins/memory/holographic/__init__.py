"""tampinha-memory-store — holographic memory plugin using MemoryProvider interface.
...
Config in $TAMPINHA_HOME/config.yaml (profile-scoped):
  plugins:
    tampinha-memory-store:
      db_path: $TAMPINHA_HOME/memory_store.db   # omit to use the default
      auto_extract: false
      default_trust: 0.5
...
def _load_plugin_config() -> dict:
    try:
        ...
        return cfg_get(all_config, "plugins", "tampinha-memory-store", default={}) or {}
    except Exception:
        return {}
...
class HolographicMemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, tampinha_home):
        """Write config to config.yaml under plugins.tampinha-memory-store."""
        from pathlib import Path
        config_path = Path(tampinha_home) / "config.yaml"
        try:
            import yaml
            ...
            existing["plugins"]["tampinha-memory-store"] = values
    ...
    def get_config_schema(self):
        ...
        _default_db = f"{display_tampinha_home()}/memory_store.db"
        return [
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        _tampinha_home = str(get_tampinha_home())
        _default_db = _tampinha_home + "/memory_store.db"
        db_path = self._config.get("db_path", _default_db)
        # Expand $TAMPINHA_HOME in user-supplied paths so config values like
        # "$TAMPINHA_HOME/memory_store.db" or "~/.tampinha/memory_store.db" both
        # resolve to the active profile's directory.
        if isinstance(db_path, str):
            db_path = db_path.replace("$TAMPINHA_HOME", _tampinha_home)
            db_path = db_path.replace("${TAMPINHA_HOME}", _tampinha_home)
