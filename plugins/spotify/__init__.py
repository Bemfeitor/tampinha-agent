...
``_check_spotify_available()`` — when the user has not run ``tampinha auth
spotify``, the tools remain registered (so they appear in ``tampinha
...
The Spotify auth flow (``tampinha auth spotify``), CLI plumbing, and docs
