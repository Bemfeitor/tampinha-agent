"""
Email platform adapter for the Tampinha gateway.

Allows users to interact with Tampinha by sending emails.
...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    try:
        ...
        imap.xatom(
            "ID",
            f'("name" "tampinha-agent" "version" "{_tampinha_version}" '
            '"vendor" "NousResearch" '
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # send-helper, and `tampinha config show` already read. Without the
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if missing:
            message = (
                ...
                + ", ".join(missing)
                + ". Set it via `tampinha gateway setup` (env) or platforms.email "
                "in config.yaml."
            )
    ...
    def _send_email(
        ...
        subject = ctx.get("subject", "Tampinha Agent")
        ...
        msg_id = f"<tampinha-{uuid.uuid4().hex[:12]}@{self._message_id_domain()}>"
...
# tampinha_cli/gateway.py, and the _send_email dispatch in
