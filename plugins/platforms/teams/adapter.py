"""
Microsoft Teams platform adapter for Tampinha Agent.
...
Requires:
    ...
    manually: ``<tampinha-venv>/bin/pip install microsoft-teams-apps aiohttp``)
...
def _suppress_third_party_dotenv() -> Iterator[None]:
    ...
    Tampinha owns dotenv loading; third-party import side effects must not.
    """
    try:
...
class TeamsAdapter(BasePlatformAdapter):
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            self._app = App(
                ...
                client=ClientOptions(headers={"User-Agent": "Tampinha"}),
            )
    ...
    async def _on_card_action(
        ...
        data = action.data or {}
        tampinha_action = data.get("tampinha_action", "")
        session_key = data.get("session_key", "")

        if not tampinha_action or not session_key:
        ...
        choice = choice_map.get(tampinha_action)
        if not choice:
            return InvokeResponse(
                status=200,
                body=AdaptiveCardActionMessageResponse(value="Unknown action."),
    ...
    async def send_exec_approval(
        ...
        actions = [ExecuteAction(
            title="Allow Once", verb="tampinha_approve",
            data={**btn_data_base, "tampinha_action": "approve_once"}, style="positive",
        )]
        if not smart_denied and allow_session:
            actions.append(ExecuteAction(
                title="Allow Session", verb="tampinha_approve",
                data={**btn_data_base, "tampinha_action": "approve_session"},
            ))
            if allow_permanent:
                actions.append(ExecuteAction(
                    title="Always Allow", verb="tampinha_approve",
                    data={**btn_data_base, "tampinha_action": "approve_always"},
                ))
        actions.append(ExecuteAction(
            title="Deny", verb="tampinha_approve",
            data={**btn_data_base, "tampinha_action": "deny"}, style="destructive",
        ))
        body = [
...
def interactive_setup() -> None:
    ...
    print_info("  teams app create --name \"Tampinha\" --endpoint \"https://<tunnel>/api/messages\"")
    print()
    ...
    print()
    print_success("Teams configuration saved to ~/.tampinha/.env")
...
def _install_hint() -> str:
    ...
    ``feature_install_command(venv_pip=True)`` targets the actual Tampinha
