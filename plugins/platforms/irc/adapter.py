"""
IRC Platform Adapter for Tampinha Agent.

A plugin-based gateway adapter that connects to an IRC server and relays
messages to/from the Tampinha agent.  Zero external dependencies — uses
...
class IRCAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config, **kwargs):
        ...
        try:
            ...
            self.port = 6697
        self.nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "tampinha-bot")
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        await self._send_raw(f"USER {self.nickname} 0 * :Tampinha Agent")
    ...
    async def disconnect(self) -> None:
        ...
        if self._writer and not self._writer.is_closing():
            try:
                await self._send_raw("QUIT :Tampinha Agent shutting down")
    ...
    async def _handle_line(self, raw: str) -> None:
        ...
        if command == "433":
            # Retry with incrementing suffix: tampinha_, tampinha_1, tampinha_2...
...
def check_requirements() -> bool:
    ...
    # tampinha setup / requirements check path.
...
def interactive_setup() -> None:
    """Interactive `tampinha gateway setup` flow for the IRC platform.

    Lazy-imports ``tampinha_cli.setup`` helpers so the plugin stays importable
    ...
    print_info("Connect Tampinha to an IRC network. Uses Python stdlib — no extra packages needed.")
    ...
    channel = prompt(
        "Channel to join (e.g. #tampinha — comma-separate for multiple)",
    ...
    print()
    print_success("IRC configuration saved to ~/.tampinha/.env")
    print_info("Restart the gateway for changes to take effect: tampinha gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "tampinha-bot")
    ...
    nick_base = nickname.rstrip("_0123456789-")[:24] or "tampinha-bot"
    ...
    try:
        ...
        await _raw(f"USER {standalone_nick} 0 * :Tampinha Agent (cron)")
