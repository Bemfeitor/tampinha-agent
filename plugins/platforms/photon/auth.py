...
Credential storage mirrors every other Tampinha channel:

    * runtime SDK creds  -> ``~/.tampinha/.env``  (``PHOTON_PROJECT_ID`` =
      project id, ``PHOTON_PROJECT_SECRET``) via ``save_env_value``
    * management metadata -> ``~/.tampinha/auth.json`` under
...
try:
    import httpx
except ImportError:  # pragma: no cover - httpx is a tampinha dependency
...
# registers Tampinha as its own client_id.
...
# Default name of the project Tampinha provisions for the operator.
DEFAULT_PROJECT_NAME = "Tampinha Agent"
...
# ---------------------------------------------------------------------------
# auth.json helpers — share the file with the rest of tampinha-agent.

def _auth_json_path() -> Path:
    """Resolve ``~/.tampinha/auth.json`` honouring the active Tampinha profile."""
    try:
        from tampinha_constants import get_tampinha_home
        return Path(get_tampinha_home()) / "auth.json"
    except Exception:
        return Path(os.path.expanduser("~/.tampinha")) / "auth.json"
...
def _save_auth(data: Dict[str, Any]) -> None:
    ...
    # tampinha_cli/auth.py:_save_auth_store (#19673, #21148).
    fd = os.open(
        str(tmp),
...
def store_photon_token(token: str) -> None:
    ...
    from tampinha_cli.auth import _auth_store_lock
...
def load_project_credentials() -> Tuple[Optional[str], Optional[str]]:
    ...
    Precedence: process env (``~/.tampinha/.env`` is loaded into the gateway's
...
def store_project_credentials(
    *,
    ...
    The runtime SDK creds land in ``~/.tampinha/.env`` via the same
...
def _persist_runtime_env(spectrum_project_id: str, project_secret: str) -> None:
    """Write the SDK creds to ``~/.tampinha/.env`` (canonical runtime store).
    ...
    try:
        ...
        logger.warning("photon: tampinha_cli.config unavailable — skipping .env write")
        return
    try:
...
def print_credential_summary(emit: Any = print) -> None:
    ...
    labels["device_token"] = (
        ...
        else "✗ missing (run `tampinha photon setup`)"
    )
    ...
    labels["phone_number"] = (
        phone if phone else "✗ missing (run `tampinha photon setup --phone ...`)"
    )
    labels["assigned_phone_number"] = (
        assigned if assigned else "✗ missing (run `tampinha photon setup`)"
    )
...
def credential_summary() -> Dict[str, str]:
    ...
    def _present_phone() -> str:
        ...
        return phone or "✗ missing (run `tampinha photon setup --phone ...`)"

    def _present_assigned_phone() -> str:
        _phone, assigned = load_user_numbers()
        return assigned or "✗ missing (run `tampinha photon setup`)"
