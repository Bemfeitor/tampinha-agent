"""SimpleX Chat platform adapter (Tampinha plugin).
...
This adapter ships as a Tampinha platform plugin under
``plugins/platforms/simplex/``. The Tampinha plugin loader scans the
...
Optional environment variables:
    ...
    TAMPINHA_SIMPLEX_TEXT_BATCH_DELAY
...
discoverable and ``tampinha setup`` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside Tampinha — no
...
class SimplexAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig, **kwargs):
        ...
        self._text_batch_delay = float(
            os.getenv("TAMPINHA_SIMPLEX_TEXT_BATCH_DELAY", "0.8")
        )
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``tampinha setup gateway`` → SimpleX.

    Prompts for the WebSocket URL and the optional allowlist / groups /
    auto-accept / home channel. Writes to ``~/.tampinha/.env`` via
    ``tampinha_cli.config``.
    """
    print()
    ...
    try:
        ...
        print(
            "tampinha_cli.config not available; set SIMPLEX_* vars manually in "
            "~/.tampinha/.env"
        )
        return
