"""ntfy platform adapter (Tampinha plugin).
...
a Tampinha dependency.

This adapter ships as a Tampinha platform plugin under
``plugins/platforms/ntfy/``. The Tampinha plugin loader scans the
    ...
    platforms:
      ntfy:
        ...
        extra:
          ...
          topic: "tampinha-in"              # subscribe topic (incoming)
          publish_topic: "tampinha-out"     # optional — defaults to topic
...
_ECHO_TAG = "tampinha-agent"  # tag added to outgoing messages for echo-loop prevention
...
async def _standalone_send(
    ...
    *,
    ...
    ``tampinha cron`` running standalone). Without this hook,
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install httpx   # already a Tampinha dependency",
        # Env-driven auto-configuration: seeds PlatformConfig.extra so
        # env-only setups show up in `tampinha gateway status` without
