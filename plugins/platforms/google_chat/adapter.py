...
# ``model_tools`` import time, so ``tampinha status``, ``tampinha chat``, etc.
...
# ``tampinha_plugins.platforms__google_chat.adapter`` once the plugin
...
#     fresh "Tampinha is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a tampinha MessageType.
...
def card_spec_to_cards_v2(card_spec: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {"cardId": str(card_spec.get("card_id") or "tampinha-card"), "card": card}
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${TAMPINHA_HOME}/google_chat_thread_counts.json so
        ...
        try:
            from tampinha_constants import get_tampinha_home as _get_tampinha_home
            _tampinha_home = _get_tampinha_home()
        except (ModuleNotFoundError, ImportError):
            _tampinha_home = _Path.home() / ".tampinha"
        self._thread_count_store = _ThreadCountStore(
            _tampinha_home / "google_chat_thread_counts.json"
        )
        ...
        # they don't sit in the chat forever as "Tampinha is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("TAMPINHA_HOME", str(_Path.home() / ".tampinha"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${TAMPINHA_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a tampinha MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Tampinha is thinking…' marker message.
        ...
        "Tampinha is thinking…" stuck in chat forever, plus one card that
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "Tampinha is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "Tampinha is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "Tampinha is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``tampinha setup``.

    The setup wizard at ``tampinha_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.tampinha/.env`` so the next
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. tampinha-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.tampinha/.env")
    print_info("Restart the gateway: tampinha gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``tampinha cron`` running as a
    separate process from ``tampinha gateway``).  Without this hook,
...
def register(ctx) -> None:
    """Plugin entry point — called by the Tampinha plugin system at startup.
    ...
    ctx.register_platform(
        ...
        install_hint="Run `tampinha setup` to install Google Chat support.",
        ...
        # the "Tampinha is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'Tampinha is thinking…' marker message "
