"""
LINE Messaging API platform adapter for Tampinha Agent.
...
to Tampinha Agent. It deliberately ports the *strongest* idea from each into
...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``TAMPINHA_HOME``. PR #8398.
        """
        from aiohttp import web
        ...
        try:
            ...
            tampinha_home = Path(get_tampinha_home()).resolve()
        except Exception:
            tampinha_home = Path.home().joinpath(".tampinha").resolve()
...
def is_connected(config) -> bool:
    """Surface in ``tampinha status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``tampinha status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``tampinha setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to ``~/.tampinha/.env`` via ``tampinha_cli.config``.
    """
    print()
    ...
    try:
        from tampinha_cli.config import get_env_value as _get_env, save_env_value as _set_env
    except ImportError:
        print("tampinha_cli.config not available; set LINE_* vars manually in ~/.tampinha/.env")
        return
