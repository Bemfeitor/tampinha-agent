...
three virtual model IDs so the ``tampinha tools`` model picker and the
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        if not api_key:
            return error_response(
                error=(
                    "OPENAI_API_KEY not set. Run `tampinha tools` → Image "
                    "Generation → OpenAI to configure, or `tampinha setup` "
                    "to add the key."
                ),
