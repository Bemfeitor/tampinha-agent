...
``<tampinha home>/plugins/<name>/``. That tree is the plugin *install* dir —
``tampinha plugins remove`` deletes it and ``tampinha plugins update`` git-pulls
...
``<tampinha home>/plugin-data/<name>/``, owned by the user, untouched by
...
through ``agent.secret_scope`` / ``.env`` like everywhere else in Tampinha.
...
# Mirrors the plugin-name shape `tampinha plugins install` accepts. Anything
...
def plugin_data_dir(name: str) -> Path:
    ...
    ``<tampinha home>/plugin-data/<name>/`` — survives plugin update and
    removal, and follows the active profile because it resolves through
    :func:`tampinha_constants.get_tampinha_home` on every call. Don't cache the
    result across profile switches.
    """
    from tampinha_constants import get_tampinha_home

    root = get_tampinha_home() / "plugin-data" / _validate_name(name)
