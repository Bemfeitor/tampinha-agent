...
def render_skill_page(
    ...
    tampinha_meta = (fm.get("metadata") or {}).get("tampinha") or {}
    tags = tampinha_meta.get("tags") or []
    related = tampinha_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `tampinha skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
    ...
    return (
        ...
        "\n"
        ":::info\n"
        "The following is the complete skill definition that Tampinha loads when this skill is triggered. This is what the agent sees as instructions when the skill is active.\n"
        ":::\n"
        "\n"
        f"{body_clean}\n"
    )
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with Tampinha Agent"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "Tampinha ships with a large built-in skill library copied into `~/.tampinha/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "Tampinha also syncs bundled skills on `tampinha update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.tampinha/skills/` tree, it is still shipped with Tampinha; restore it with `tampinha skills reset <name> --restore`.",
        "",
...
def build_catalog_md_optional(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Official optional skills shipped with tampinha-agent — install via tampinha skills install official/<category>/<skill>"',
        "---",
        "",
        "# Optional Skills Catalog",
        "",
        "Optional skills ship with tampinha-agent under `optional-skills/` but are **not active by default**. Install them explicitly:",
        "",
        "```bash",
        "tampinha skills install official/<category>/<skill>",
        "```",
        "",
        "For example:",
        "",
        "```bash",
        "tampinha skills install official/blockchain/solana",
        "tampinha skills install official/mlops/flash-attention",
        "```",
        "",
        ...
        "",
        "```bash",
        "tampinha skills uninstall <skill-name>",
        "```",
        "",
    ]
    for category in sorted(by_cat):
