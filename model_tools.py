...
def _is_dispatcher_owned_worker() -> bool:
    """False when TAMPINHA_KANBAN_* is present but this execution does not own it
...
def _run_async(coro):
    ...
    if loop and loop.is_running():
        ...
        # async tools resolve get_tampinha_home() under the active profile.
...
#   - cli.py, tampinha_cli/*      -> inline on startup (no event loop)
    ...
    if quiet_mode:
        ...
        if profile_scope != CHECK_FN_CACHE_BYPASS:
            cache_key = (
                ...
                cfg_fp,
                bool(os.environ.get("TAMPINHA_KANBAN_TASK")),
...
def _compute_tool_definitions(
    ...
    if enabled_toolsets is not None:
        ...
        if (
            ...
            # Dispatcher-spawned workers are scoped by TAMPINHA_KANBAN_TASK and
    ...
    # This ensures that even if a composite toolset (like tampinha-cli)
    ...
    if disabled_toolsets:
        for toolset_name in disabled_toolsets:
            if validate_toolset(toolset_name):
                ...
                if toolset_name.startswith("tampinha-") or (get_toolset(toolset_name) or {}).get("posture"):
                    # Platform bundles (tampinha-*) include _TAMPINHA_CORE_TOOLS, and
                    ...
                    if (not quiet_mode and toolset_name.startswith("tampinha-")
    ...
    # window). Core Tampinha tools (toolsets._TAMPINHA_CORE_TOOLS) are NEVER
...
def _resolve_active_context_length() -> int:
    ...
    """
    try:
        from tampinha_cli.config import load_config as _load
        cfg = _load() or {}
...
def _normalize_json_strings_for_schema(value: Any, schema: Any) -> Any:
    ...
    Ported from cline/cline#11803, adapted to tampinha-agent's coercion layer.
