#!/usr/bin/env python3
"""
SQLite State Store for Tampinha Agent.
...
# transitive dependency via tampinha_state_common). run_agent keeps its own
# predating copy — tampinha_state cannot import run_agent (circular) — guarded
...
from tampinha_cli.sqlite_runtime import (
    is_sqlite_wal_reset_vulnerable as _is_sqlite_wal_reset_vulnerable,
)
from typing import Any, Callable, Dict, List, Optional, Set, Tuple, TypeVar

from tampinha_state_common import (  # noqa: F401  (re-exported for back-compat)
    _BRANCH_CHILD_SQL,
...
from tampinha_state_portability import SessionPortabilityMixin
from tampinha_state_schema import SessionSchemaMixin
from tampinha_state_search import SessionSearchMixin
...
def _workspace_key_clause(key: str) -> Tuple[str, List[str]]:
    ...
    Used by ``tampinha -c``/``--resume`` to continue the most recent session in
...
DEFAULT_DB_PATH = get_tampinha_home() / "state.db"
...
def _default_db_path() -> Path:
    ...
    freezes the developer's real ``~/.tampinha`` even when a test fixture later
    redirects ``TAMPINHA_HOME`` — importing this module during collection was
    ...
    2. Otherwise resolve ``get_tampinha_home()`` fresh so a runtime
       ``TAMPINHA_HOME`` redirect takes effect regardless of import order.
    """
    if DEFAULT_DB_PATH != _IMPORT_DEFAULT_DB_PATH:
        return DEFAULT_DB_PATH
    return get_tampinha_home() / "state.db"
...
# ~/.tampinha/state.db accumulated pytest fixture rows — sessions with
...
# FTS write corruption").  The hermetic conftest redirects TAMPINHA_HOME per
# test, but any escape (a session-scoped fixture running before the autouse
# fixture, a subprocess child launched without TAMPINHA_HOME, a stale worktree
# without the re-pin, or a developer shell that exports TAMPINHA_HOME to the
...
_STATE_DB_GUARD_BYPASS_ENV = "TAMPINHA_STATE_DB_GUARD_BYPASS"

#: Additional production roots to refuse (beyond the platform default
#: ``~/.tampinha``).  The test conftest injects the pre-sandbox production
#: root here so custom-``TAMPINHA_HOME`` deployments are covered too.
_STATE_DB_GUARD_EXTRA_DENY_ROOTS: Tuple[Path, ...] = ()


def _real_platform_state_root() -> Optional[Path]:
    """Resolve the REAL platform-default Tampinha root for the guard.

    Deliberately avoids ``Path.home()`` / ``tampinha_constants``: tests
    routinely monkeypatch ``Path.home`` to a tempdir, and ``tampinha_state``
    ...
    try:
        if sys.platform == "win32":
            ...
            root = (
                Path(base) / "tampinha"
                if base
                else Path(os.path.expanduser("~")) / "AppData" / "Local" / "tampinha"
            )
        else:
            root = Path(os.path.expanduser("~")) / ".tampinha"
        return root.resolve()
    except Exception:
        return None
...
#: redirects ``TAMPINHA_HOME`` to the per-session tmp isolation root.  Its
...
#: is OURS: it declares "this process tree is running under Tampinha test
#: isolation", and it inherits into subprocess children by default — so a
#: child that received the patched ``TAMPINHA_HOME`` also received the marker,
#: and a child that resolves a production DB while carrying it is, by
#: definition, an isolation escape (#82770).
_TEST_ISOLATION_MARKER_ENV = "TAMPINHA_TEST_ISOLATION"
...
def _has_pytest_ancestor() -> bool:
    ...
    ``TAMPINHA_HOME`` redirect: that child aims at the production DB *and*
...
def _in_test_context() -> bool:
    ...
    so a real ``tampinha`` invocation pays for at most one walk.
    """
...
def _is_production_state_db(resolved: Path, root: Path) -> bool:
    """True when *resolved* is a DB file of the real Tampinha home *root*.
    ...
    under ``~/.tampinha/tampinha-agent/...``) so hermetic tests using unusual
...
def _ensure_test_isolation(db_path: Path) -> None:
    ...
    pytest and for hermetic (tmp ``TAMPINHA_HOME``) paths.
    ...
    a rebuilt environment loses ``PYTEST_*`` and ``TAMPINHA_HOME`` together,
    ...
    for root in _production_state_roots():
        if _is_production_state_db(resolved, root):
            raise RuntimeError(
                ...
                f"state.db at {resolved} (under real Tampinha root {root}). "
                "Tests must run against a temporary TAMPINHA_HOME — pass an "
                "explicit tmp db_path or let the hermetic conftest redirect "
                "TAMPINHA_HOME. If this test genuinely needs the live "
...
# tampinha_cli/kanban_db.py for ~30 call sites) would re-log the same
...
def _apply_wal_size_limit(conn: sqlite3.Connection) -> None:
    ...
    3.0 GB ``state.db``: ``tampinha sessions optimize`` (FTS merge + VACUUM)
    ...
    ``tampinha_cli/kanban_db.py`` already bounds its WAL growth with
...
def apply_wal_with_fallback(
    ...
    *,
    ...
    Shared by :class:`SessionDB` and ``tampinha_cli.kanban_db.connect`` so
...
def _wal_reset_repair_hint() -> str:
    ...
    ``tampinha update`` can actually do for this install (#75153).
    """
    try:
        ...
        if method in {"git", "unknown"}:
            return f"Tampinha-managed installs can repair the embedded runtime with `{cmd}`"
        if method == "docker":
    ...
    return (
        ...
        "(or backports 3.50.7 / 3.44.6) and restart Tampinha"
    )
...
def _log_wal_reset_bug_once(
    ...
    *,
    ...
    # Check whether this is a Tampinha-managed install (uv-managed venv)
    ...
    logger.warning(
        ...
        "%s. See `tampinha doctor`. This warning fires once per "
...
def _log_journal_mode_upgrade_once(db_label: str, previous_mode: str) -> None:
    ...
    ``tampinha_cli/managed_uv._default_live_venv``, which treats a database
...
def _log_wal_fallback_once(db_label: str, exc: Exception) -> None:
    ...
    connection on every operation — see tampinha_cli/kanban_db.py) would
...
def apply_database_pragmas(
    ...
    * ``synchronous`` — durability level: ``OFF``/``NORMAL``/``FULL``/``EXTRA``
    ...
    """
    try:
        # Local import avoids a circular import with tampinha_cli.config.
...
def classify_persistence_error(exc_or_str) -> str:
    ...
    * ``"corrupt"`` — the database file itself is structurally damaged
      ...
      (``tampinha doctor`` / automatic schema surgery).
...
# threads inside ONE interpreter, yet a normal Tampinha host runs several
# independent processes against the same ``state.db``: the gateway service,
# the Desktop app's own ``tampinha serve`` backend, interactive CLI sessions,
...
# included because rollback-journal (DELETE) mode — Tampinha's fallback on
...
def _db_fingerprint(db_path: Path) -> "Optional[str]":
    ...
    RESERVED lock (see ``tampinha_cli.sqlite_safe_read`` rule 1). This function
    ...
    try:
        ...
        try:
            ...
            # Scaffold/embed installs ship tampinha_state without tampinha_cli. No
...
def _backup_db_file(db_path: Path) -> "Tuple[Optional[Path], Optional[str]]":
    ...
    connection's POSIX advisory locks (see ``tampinha_cli.sqlite_safe_read``).
...
def preflight_db_writability(
    ...
    *,
    ...
    - **Repairs** permissions with ``chmod u+rw`` when the file lives inside
      the Tampinha home tree (``get_tampinha_home()``) — the safe repair scope:
      Tampinha owns those files, and the OS makes ``chmod`` fail on files the
    ...
    to check). Shared by :class:`SessionDB` and ``tampinha_cli.kanban_db``.
    """
    raw = str(db_path)
    ...
    try:
        home: Optional[Path] = Path(get_tampinha_home()).resolve()
    ...
    def _ensure_writable(p: Path, *, is_dir: bool = False) -> None:
        ...
        raise sqlite3.OperationalError(
            ...
            f"user. Tampinha needs read-write access to open the database. "
    ...
    try:
        ...
        for fts_table in ("messages_fts", "messages_fts_trigram", "messages_fts_cjk"):
            try:
                ...
                # (tampinha_state.py:645-723). The supported degraded-runtime
                # path (SessionDB._is_fts5_unavailable_error + the
                # regression suite in tests/test_tampinha_state.py:600-632)
        ...
        probe_session_id = f"_tampinha_fts_health_probe_{time.time_ns()}"
        try:
...
def _live_writer_holds_db(db_path: Path) -> bool:
    ...
    bug and on NFS/SMB, Tampinha deliberately runs ``state.db`` in
        ...
        if not holding_lock:
            ...
            # later BEGIN EXCLUSIVE acquisition.
                ...
                report["error"] = (
                    ...
                    "Stop the gateway (tampinha gateway stop) and retry."
                )
                logger.error("state.db repair skipped: %s", report["error"])
            else:
...
# (``~/.tampinha/lib/libfts5_cjk.so``, built by ``native/fts5_cjk/build.sh``).
...
# next ``tampinha sessions optimize-storage`` on a capable host).
#
...
def fts5_cjk_so_path() -> Path:
    ...
    env = os.getenv("TAMPINHA_FTS5_CJK_SO")
    if env:
        return Path(env).expanduser()
    return get_tampinha_home() / "lib" / "libfts5_cjk.so"


def _cjk_fts_config_enabled() -> bool:
    """config.yaml ``sessions.cjk_fts`` (default on), via its env bridge."""
    return os.getenv("TAMPINHA_CJK_FTS", "1").strip().lower() not in (
        "0", "false", "off", "no",
    )
...
def _connect_tracked_db(path, tracking_path=None, **kwargs):
    ...
    (scaffold/embed installs that ship tampinha_state without tampinha_cli). A
    ...
    try:
        ...
        logger.debug(
            "tampinha_cli.sqlite_safe_read unavailable; opening %s untracked "
    ...
    # patch tampinha_state.sqlite3.connect keep control of connection creation;
...
def is_zeroed_state_db(
    ...
    Prefer ``tampinha_cli.backup.is_zeroed_sqlite_file`` when available; this
    ...
    try:
        ...
        if not acquired:
            ...
            logger.error(
                ...
                "state-snapshots via `tampinha snapshot list` / "
                "`tampinha snapshot restore <id>`.",
                path,
            )
            return None
...
# ── Read-only health/stats probes (tampinha doctor, dashboards) ──────────
...
_TAMPINHA_CMDLINE_MARKERS = ("tampinha_cli.main", "tampinha_cli/main", "tampinha serve",
                           "tampinha-agent", "tampinha gateway", "tampinha chat")


def _looks_like_tampinha(cmdline: str) -> bool:
    """Heuristic: does this cmdline look like a Tampinha process?
    ...
    We only flag processes that look like Tampinha, not every system daemon.
    """
    lower = cmdline.lower()
    return any(marker in lower for marker in _TAMPINHA_CMDLINE_MARKERS)
...
class SessionDB(SessionSearchMixin, SessionSchemaMixin, SessionPortabilityMixin):
    ...
    # With multiple tampinha processes (gateway + CLI sessions + worktree agents)
    ...
    # legitimately held for multi-second stretches by sibling Tampinha
    ...
    # `tampinha update` leaves mixed-version processes sharing the DB until
        ...
        try:
            ...
            if (
                ...
                msg = (
                    ...
                    f"Restore from {snaps} via `tampinha snapshot list` / "
                    f"`tampinha snapshot restore <id>` if available. "
            ...
            # NOTE: the v23 FTS optimization is OPT-IN (`tampinha db optimize`),
            ...
            # ``tampinha_state._set_last_init_error(None)`` explicitly.
    ...
    def _warn_fts5_unavailable(self, exc: sqlite3.OperationalError) -> None:
        ...
        logger.warning(
            ...
            "disabled. Run `tampinha update` to rebuild the venv with a "
            ...
            if cjk_present:
                ...
                if live:
                    ...
                    logger.warning(
                        ...
                        "run `tampinha sessions optimize-storage` on a host "
        ...
        while True:
            try:
                ...
                if "locked" in err_msg or "busy" in err_msg:
                    ...
                    raise sqlite3.OperationalError(
                        f"database is locked (another Tampinha process held the "
            ...
            try:
                ...
                for pid_str in os.listdir("/proc"):
                    ...
                    try:
                        ...
                        # so check whether this is a Tampinha process —
                        # only flag uninspectable holders that look like
                        # another Tampinha instance, not every system daemon.
                        cmdline = _read_proc_cmdline(pid)
                        if cmdline is not None and _looks_like_tampinha(cmdline):
    ...
    def _reap_inactive_orphan_desktop_holders(
        ...
        if not sys.platform.startswith("linux") or psutil is None:
            return []
        try:
            from tampinha_cli.dashboard_procs import _is_ephemeral_port_zero_backend
        except Exception:
            return []
    ...
    #
    # `optimize_fts_storage()` (the `tampinha sessions optimize-storage`
    ...
    # ── Opt-in v23 FTS storage optimization (`tampinha sessions optimize-storage`) ──
    #
    ...
    # offline repair path behind ``tampinha sessions repair-routing``.
    ...
    def set_session_yolo(self, session_id: str, enabled: bool) -> None:
        ...
        ``--yolo`` launch — survives ``tampinha --resume`` into a fresh
    ...
    def sweep_orphaned_sessions(
        ...
        *,
        ...
        ``tampinha serve``, desktop WS sidecar), each backend registers a row
    ...
    # of truth: tampinha_state_common._RECOVERABLE_END_REASONS, interpolated
    ...
    def _rows_to_conversation(
        ...
        *,
        ...
        if repair_alternation and messages:
            # Lazy import: tampinha_state already depends on agent.* (see
    ...
    def search_sessions(
        ...
        ``tampinha -c``/``--resume`` so the "last" session is the last one in
    ...
    def delete_session_if_empty(
        ...
        and ``tampinha sessions list`` output. (Pattern ported from
    ...
    def retag_kanban_worker_sessions(self, workspaces_root: str) -> int:
        ...
        Workers used to spawn without ``TAMPINHA_SESSION_SOURCE``, so their runs
    ...
    def apply_telegram_topic_migration(self) -> None:
        ...
        reconciliation. Operators must be able to upgrade Tampinha, keep the old
    ...
    def bind_telegram_topic(
        ...
        *,
        ...
        """Bind one Telegram DM topic thread to one Tampinha session.

        A Tampinha session may only be linked to one Telegram topic in MVP.
    ...
    def is_telegram_session_linked_to_topic(self, *, session_id: str) -> bool:
        """Return True if a Tampinha session is already bound to any Telegram DM topic.
    ...
    def vacuum(self) -> int:
        ...
        with self._lock:
            ...
            # TRUNCATE: a manual `tampinha sessions vacuum` runs in a transient
