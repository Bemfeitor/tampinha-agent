...
@method("reload.env")
def _(rid, params: dict) -> dict:
    """Re-read ``~/.tampinha/.env`` into the gateway process via
    ``tampinha_cli.config.reload_env``, matching classic CLI's ``/reload``
    ...
    """
    try:
        from tampinha_cli.config import reload_env
...
@method("cli.exec")
def _(rid, params: dict) -> dict:
    """Run `python -m tampinha_cli.main` with argv; capture stdout/stderr (non-interactive only)."""
    ...
    try:
        ...
        r = subprocess.run(
            [sys.executable, "-m", "tampinha_cli.main", *argv],
            capture_output=True,
            text=True,
            ...
            cwd=os.getcwd(),
            # cli.exec runs `python -m tampinha_cli.main` (can drive the agent) →
            # needs provider credentials. Tier-1 secrets still stripped (#29157).
            env=tampinha_subprocess_env(inherit_credentials=True),
...
def _(rid, params: dict) -> dict:
    ...
    if name == "moa":
        ...
        try:
            from tampinha_cli.moa_config import moa_usage, normalize_moa_config
    ...
    if name == "loop":
        ...
        if not session:
            return _err(rid, 4001, "no active session")
        try:
            from tampinha_cli.loops import LoopManager, dispatch_loop_command
        except Exception as exc:
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        # Re-bind TAMPINHA_HOME to the session's profile so get_skill_commands()
        ...
        _home_token = (
            set_tampinha_home_override(_profile_home) if _profile_home else None
        )
        try:
...
def _(rid, params: dict) -> dict:
    try:
        ...
        api_key = get_secret("TAMPINHA_API_KEY", "") or cfg.get("api_key", "")
        masked = f"****{api_key[-4:]}" if len(api_key) > 4 else "(not set)"
        base_url = os.environ.get("TAMPINHA_BASE_URL", "") or cfg.get("base_url", "")
        ...
        sections = [
            ...
            },
            {
                ...
                "rows": [
                    ...
                    ["Config File", str(_tampinha_home / "config.yaml")],
                ],
            },
        ]
...
def _(rid, params: dict) -> dict:
    ...
    # Optional profile scoping: cronjob() keys off TAMPINHA_HOME, so scoping the
    ...
    if profile:
        try:
            ...
            from tampinha_constants import set_tampinha_home_override
    ...
    try:
        ...
        if token is not None:
            try:
                from tampinha_constants import reset_tampinha_home_override
...
def _(rid, params: dict) -> dict:
    ...
    gateway. Shares its renderer with the ``tampinha journey`` CLI.
    """
    try:
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if action == "install":
            from tampinha_cli.skills_hub import do_install
        ...
        if action == "browse":
            from tampinha_cli.skills_hub import browse_skills
        ...
        if action == "inspect":
            from tampinha_cli.skills_hub import inspect_skill
...
def _(rid, params: dict) -> dict:
    ...
    keys], transport}]}`` — the same catalog `tampinha mcp` offers, so
    ...
    try:
        ...
        from tampinha_cli import mcp_catalog
...
# (tampinha_cli/web_routers/mcp.py) so a desktop plugin can manage MCP servers for
# ANY profile, not just the launch profile. Each accepts an optional ``profile``
# param that scopes TAMPINHA_HOME via set_tampinha_home_override (omitted/None = the
# launch profile) in a try/finally, exactly like ``skills.manage`` / ``mcp.catalog``.
# All persistence reuses tampinha_cli/mcp_config.py helpers — no logic is duplicated.
...
def _(rid, params: dict) -> dict:
    ...
    if err:
        return err
    try:
        from tampinha_cli.mcp_config import _get_mcp_servers
...
def _(rid, params: dict) -> dict:
    ...
    if err:
        return err
    try:
        from tampinha_cli.config import load_config, save_config, save_env_value
        from tampinha_cli.mcp_config import (
...
def _(rid, params: dict) -> dict:
    ...
    if err:
        return err
    try:
        from tampinha_cli.mcp_config import _remove_mcp_server
...
def _(rid, params: dict) -> dict:
    ...
    the SAME interactive MCP OAuth machinery ``tampinha mcp login`` uses
    ...
    try:
        ...
        tampinha_home = str(get_tampinha_home().expanduser().resolve(strict=False))
        result = mcp_oauth_sessions.start_flow(tampinha_home, name, cfg)
        return _ok(
            rid,
            {
                "ok": True,
...
def _(rid, params: dict) -> dict:
    ...
    primitives as ``tampinha plugins`` / the dashboard, so the three surfaces
    ...
    Actions:
      ...
      - ``install`` → git-clone into ``~/.tampinha/plugins/`` (non-interactive).
    ...
    plugins live under each profile's TAMPINHA_HOME, so a client can list or
                ...
                out.append(
                    {
                        ...
                        # skills/MCP format) vs a native Tampinha plugin.
