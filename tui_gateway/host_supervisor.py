...
def _default_registry_path() -> Path:
    return get_tampinha_home() / "state" / _REGISTRY_NAME
...
class HostSupervisor:
    ...
    def __init__(
        ...
        *,
        ...
        expected_tampinha_home: str | None = None,
        autostart: bool = True,
    ) -> None:
        ...
        self.expected_tampinha_home = expected_tampinha_home if expected_tampinha_home is not None else str(get_tampinha_home())
    ...
    def _spawn_locked(self, *, reason: str) -> None:
        ...
        if self.env:
            env.update(self.env)
        env["TAMPINHA_COMPUTE_HOST_HEARTBEAT_SECS"] = str(self.heartbeat_secs)
    ...
    def _validate_hello(self) -> None:
        ...
        got_home = str(hello.get("tampinha_home") or "")
        if got_home and got_home != self.expected_tampinha_home:
            raise RuntimeError(f"compute host TAMPINHA_HOME mismatch: {got_home} != {self.expected_tampinha_home}")
