...
# 5. Symlinks the 'tampinha' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}⚕ Tampinha Agent Setup${NC}"
echo ""
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/tampinha-uv-install.$$.log")"
        _uv_installer="$(mktemp 2>/dev/null || echo "/tmp/tampinha-uv-installer.$$.sh")"
...
# ============================================================================
# PATH setup — symlink tampinha into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up tampinha command..."

TAMPINHA_BIN="$SCRIPT_DIR/venv/bin/tampinha"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$TAMPINHA_BIN" "$COMMAND_LINK_DIR/tampinha"
echo -e "${GREEN}✓${NC} Symlinked tampinha → $COMMAND_LINK_DISPLAY_DIR/tampinha"
    ...
    if [ -n "$SHELL_CONFIG" ]; then
        ...
        if ! echo "$PATH" | tr ':' '\n' | grep -q "^$HOME/.local/bin$"; then
            if ! grep -q '\.local/bin' "$SHELL_CONFIG" 2>/dev/null; then
                echo "" >> "$SHELL_CONFIG"
                echo "# Tampinha Agent — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
...
# ============================================================================
# Seed bundled skills into ~/.tampinha/skills/
# ============================================================================

TAMPINHA_SKILLS_DIR="${TAMPINHA_HOME:-$HOME/.tampinha}/skills"
mkdir -p "$TAMPINHA_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.tampinha/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$TAMPINHA_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  tampinha status        # Check configuration"
if is_termux; then
    echo "  tampinha gateway       # Run gateway in foreground"
else
    echo "  tampinha gateway install # Install gateway service (messaging + cron)"
fi
echo "  tampinha cron list     # View scheduled jobs"
echo "  tampinha doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m tampinha_cli.main setup
fi
