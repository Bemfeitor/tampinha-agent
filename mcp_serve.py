"""
Tampinha MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Tampinha-specific extra)

Usage:
    tampinha mcp serve
    tampinha mcp serve --verbose
...
logger = logging.getLogger("tampinha.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using TAMPINHA_HOME."""
    try:
        from tampinha_constants import get_tampinha_home
        return get_tampinha_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("TAMPINHA_HOME", Path.home() / ".tampinha")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from tampinha_constants import get_tampinha_home
        directory_file = get_tampinha_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("TAMPINHA_HOME", Path.home() / ".tampinha")
...
class EventBridge:
    ...
    This is the Tampinha equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _establish_baseline_with_db(self, db) -> None:
        ...
        try:
            from tampinha_constants import get_tampinha_home
            db_file = get_tampinha_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("TAMPINHA_HOME", Path.home() / ".tampinha")) / "state.db"
        try:
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "MCPServer":
    """Create and return the Tampinha MCP server with all tools registered."""
    ...
    mcp = MCPServer(
        "tampinha",
        instructions=(
            "Tampinha Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Tampinha MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
