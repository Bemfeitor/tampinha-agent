"""CLI entry point for the tampinha-agent ACP adapter.

Loads environment variables from ``~/.tampinha/.env``, configures logging
...
try:
    ...
    # shadowing Tampinha's own modules — ``tampinha acp`` can be started from any
...
def _load_env() -> None:
    """Load .env from TAMPINHA_HOME (default ``~/.tampinha``)."""
    from tampinha_cli.env_loader import load_tampinha_dotenv

    tampinha_home = get_tampinha_home()
    loaded = load_tampinha_dotenv(tampinha_home=tampinha_home)
    if loaded:
        for env_file in loaded:
            logging.getLogger(__name__).info("Loaded env from %s", env_file)
    else:
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", tampinha_home / ".env"
        )
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="tampinha-acp",
        description="Run Tampinha Agent as an ACP stdio server.",
    )
    parser.add_argument("--version", action="store_true", help="Print Tampinha version and exit")
    parser.add_argument(
        "--check",
        action="store_true",
    ...
    parser.add_argument(
        "--setup",
        action="store_true",
        help="Run interactive Tampinha provider/model setup for ACP terminal auth",
    )
    parser.add_argument(
        "--setup-browser",
        action="store_true",
        help="Install agent-browser + Playwright Chromium into ~/.tampinha/node/ "
...
def _run_check() -> None:
    ...
    print("Tampinha ACP check OK")


def _run_setup() -> None:
    from tampinha_cli.main import main as tampinha_main

    old_argv = sys.argv[:]
    try:
        sys.argv = [old_argv[0] if old_argv else "tampinha", "model"]
        tampinha_main()
    finally:
        sys.argv = old_argv
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Starting tampinha-agent ACP adapter")
    ...
    if os.environ.get("TAMPINHA_ACP_SKIP_CONFIGURED_MCP", "").strip() != "1":
        try:
