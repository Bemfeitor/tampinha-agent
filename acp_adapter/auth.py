"""ACP auth helpers — detect and advertise Tampinha authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "tampinha-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active Tampinha runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if Tampinha can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for Tampinha.
    ...
    not have Tampinha provider credentials configured yet, so Tampinha always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate Tampinha using the currently configured "
    ...
    methods.append(
        TerminalAuthMethod(
            ...
            name="Configure Tampinha provider",
            description=(
                "Open Tampinha' interactive model/provider setup in a terminal. "
                "Use this when Tampinha has not been configured on this machine yet."
            ),
            type="terminal",
            args=["--setup"],
        )
