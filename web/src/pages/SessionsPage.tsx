...
const SOURCE_CONFIG: Record<string, { icon: typeof Terminal; color: string }> =
  {
    ...
    tampinha_flow: { icon: Play, color: "text-warning" },
...
const NO_MATCHING_SESSION_SOURCE = "__tampinha_dashboard_no_matching_source__";
    ...
    async (id: string) => {
      try {
        const res = await fetch(api.exportSessionUrl(id), {
          credentials: "include",
          headers: {
            "X-Tampinha-Session-Token":
              (window as unknown as { __TAMPINHA_SESSION_TOKEN__?: string })
                .__TAMPINHA_SESSION_TOKEN__ ?? "",
          },
        });
