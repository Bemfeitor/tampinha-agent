...
import { api, TAMPINHA_BASE_PATH } from "@/lib/api";
...
export const MANIFEST_CACHE_KEY = "tampinha:plugin-manifests";
...
export function usePlugins() {
  ...
  useEffect(() => {
    ...
    for (const manifest of manifests) {
      ...
      if (manifest.css) {
        const cssUrl = `${TAMPINHA_BASE_PATH}/dashboard-plugins/${manifest.name}/${manifest.css}`;
      ...
      const baseUrl = `${TAMPINHA_BASE_PATH}/dashboard-plugins/${manifest.name}/${manifest.entry}`;
      const scriptSrc = import.meta.env.DEV
        ? `${baseUrl}?tampinha_dv=${Date.now()}`
        : baseUrl;
      ...
      script.setAttribute("data-tampinha-plugin", manifest.name);
