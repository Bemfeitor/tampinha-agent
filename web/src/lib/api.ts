import { buildHermesWebSocketUrl } from "@tampinha/shared";

// The dashboard can be served either at the root of its host (e.g.
// https://kanban.tilos.com/) or under a URL prefix when reverse-proxied
// (e.g. https://mission-control.tilos.com/hermes/). The Python backend
// injects ``window.__TAMPINHA_BASE_PATH__`` into index.html based on the
// incoming ``X-Forwarded-Prefix`` header so the SPA can address its own
// ``/api/...`` and ``/dashboard-plugins/...`` URLs correctly without a
// rebuild. Empty string means "served at root".
function readBasePath(): string {
  if (typeof window === "undefined") return "";
  const raw = window.__TAMPINHA_BASE_PATH__ ?? "";
  if (!raw) return "";
...
export const TAMPINHA_BASE_PATH = readBasePath();
const BASE = TAMPINHA_BASE_PATH;
...
declare global {
  interface Window {
    __TAMPINHA_SESSION_TOKEN__?: string;
    __TAMPINHA_BASE_PATH__?: string;
    ...
    __TAMPINHA_AUTH_REQUIRED__?: boolean;
  }
}
const SESSION_HEADER = "X-Tampinha-Session-Token";
...
export async function fetchJSON<T>(
  ...
  const token = window.__TAMPINHA_SESSION_TOKEN__;
  if (token) {
  ...
  if (res.status === 401) {
    ...
    // (``tampinha update``, ``tampinha gateway restart``, etc.). A tab kept
    // open across the restart holds the OLD token in
    // ``window.__TAMPINHA_SESSION_TOKEN__`` from the previous HTML render,
    ...
    if (!window.__TAMPINHA_AUTH_REQUIRED__ && !options?.allowUnauthorized) {
  ...
  if (res.ok) {
    ...
    // current ``window.__TAMPINHA_SESSION_TOKEN__`` is valid, so the next
 ...
 *
 * The dashboard's gated-mode WS auth (``tampinha_cli.web_server._ws_auth_ok``)
...
export async function buildWsAuthParam(): Promise<[string, string]> {
  if (window.__TAMPINHA_AUTH_REQUIRED__) {
 ...
 *  - loopback / ``--insecure``: attach the ``X-Tampinha-Session-Token`` header.
 *  - gated OAuth: no token header (it's absent by design); the
 *    ``tampinha_session_at`` cookie rides along via ``credentials: 'include'``.
 *
 ...
 * ``window.__TAMPINHA_SESSION_TOKEN__`` directly, so the gated-mode ticket
...
export const api = {
  ...
  updateHermes: () =>
    fetchJSON<ActionResponse>("/api/tampinha/update", { method: "POST" }),
  checkHermesUpdate: (force = false) =>
    fetchJSON<UpdateCheckResponse>(
      `/api/tampinha/update/check${force ? "?force=true" : ""}`,
    ),
...
export interface SkillHubSource {
  ...
  /** tampinha-index only: whether the centralized index loaded. */
  available?: boolean;
}
...
export interface StatusResponse {
   ...
   * updates are handled by the outer launcher instead of ``tampinha update``. */
  can_update_tampinha?: boolean;
  config_path: string;
  ...
  /** NS-656: disk-usage rollup for the TAMPINHA_HOME volume. Absent on
   * older gateways. */
 ...
 * sample of the TAMPINHA_HOME volume — no staleness dimension, so no
 * sampled_at. */
