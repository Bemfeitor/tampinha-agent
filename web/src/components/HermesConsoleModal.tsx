...
export function HermesConsoleModal({ open, onClose }: HermesConsoleModalProps) {
  ...
  const promptRef = useRef("tampinha> ");
  const inputPromptRef = useRef("tampinha> ");
  ...
  const handleFrame = useCallback(
    (frame: ConsoleFrame) => {
      ...
      if (frame.type === "ready") {
        const nextPrompt = frame.prompt || "tampinha> ";
  ...
  useEffect(() => {
    ...
    writeLine(term, "\x1b[2mConnecting to Tampinha Console...\x1b[0m");
  ...
  return createPortal(
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="tampinha-console-title"
    >
      <div
        className={cn(
          themedBody,
