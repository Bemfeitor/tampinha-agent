...
const BACKEND = process.env.TAMPINHA_DASHBOARD_URL ?? "http://127.0.0.1:9119";

/**
 * In production the Python `tampinha dashboard` server injects a one-shot
 * session token into `index.html` (see `tampinha_cli/web_server.py`). The
 * Vite dev server serves its own `index.html`, so unless we forward that
 * token, every protected `/api/*` call 401s.
 *
 * This plugin fetches the running dashboard's `index.html` on each dev page
 * load, scrapes the `window.__TAMPINHA_SESSION_TOKEN__` assignment, and
 * re-injects it into the dev HTML. No-op in production builds.
 */
function hermesDevToken(): Plugin {
  const TOKEN_RE = /window\.__TAMPINHA_SESSION_TOKEN__\s*=\s*"([^"]+)"/;
  const EMBEDDED_RE =
    /window\.__TAMPINHA_DASHBOARD_EMBEDDED_CHAT__\s*=\s*(true|false)/;

  return {
    name: "tampinha:dev-session-token",
    apply: "serve",
    async transformIndexHtml() {
      try {
        ...
        if (!match) {
          console.warn(
            `[tampinha] Could not find session token in ${BACKEND} — ` +
              `is \`tampinha dashboard\` running? /api calls will 401.`,
          );
          return;
        }
        ...
        return [
          {
            ...
            injectTo: "head",
            children:
              `window.__TAMPINHA_SESSION_TOKEN__="${match[1]}";` +
              `window.__TAMPINHA_DASHBOARD_EMBEDDED_CHAT__=${embeddedJs};`,
          },
        ];
      } catch (err) {
        console.warn(
          `[tampinha] Dashboard at ${BACKEND} unreachable — ` +
            `start it with \`tampinha dashboard\` or set TAMPINHA_DASHBOARD_URL. ` +
...
export default defineConfig({
  ...
  resolve: {
    alias: {
      ...
      "@tampinha/shared": path.resolve(__dirname, "../apps/shared/src"),
    },
  ...
  server: {
    proxy: {
      ...
      // Same host as `tampinha dashboard` must serve these; Vite has no
