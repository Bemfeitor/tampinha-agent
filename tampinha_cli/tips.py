...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or tampinha -c.",
    ...
    "Ctrl+Z suspends Tampinha to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "tampinha -c resumes your most recent CLI session. tampinha -c \"project name\" resumes by title.",
    "tampinha -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "tampinha -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "tampinha chat -t web,terminal enables only specific toolsets for a focused session.",
    "tampinha chat -s github-pr-workflow preloads a skill at launch.",
    "tampinha chat -q \"query\" runs a single non-interactive query and exits.",
    "tampinha chat --max-turns 1000 overrides the default 500-iteration limit per turn.",
    "tampinha chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "tampinha --yolo bypasses all dangerous command approval prompts for the entire session.",
    "tampinha chat --source telegram tags the session for filtering in tampinha sessions list.",
    "tampinha -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "tampinha doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "tampinha dump outputs a compact setup summary — great for bug reports.",
    "tampinha config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "tampinha config edit opens config.yaml in your default editor.",
    "tampinha config check scans for missing or stale configuration options.",
    "tampinha sessions browse opens an interactive session picker with search.",
    "tampinha sessions stats shows session counts by platform and database size.",
    "tampinha sessions prune --older-than 30 cleans up old sessions.",
    "tampinha skills search react --source skills-sh searches the skills.sh public directory.",
    "tampinha skills check scans installed hub skills for upstream updates.",
    "tampinha skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "tampinha skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "tampinha mcp add github --command npx adds MCP servers from the command line.",
    "tampinha mcp serve runs Tampinha itself as an MCP server for other agents.",
    "tampinha auth add lets you add multiple API keys for credential pool rotation.",
    "tampinha completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "tampinha logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "tampinha backup creates a zip backup of your entire Tampinha home directory.",
    "tampinha profile create coder creates an isolated profile that becomes its own command.",
    "tampinha profile create work --clone copies your current config and keys to a new profile.",
    "tampinha update syncs new bundled skills to ALL profiles automatically.",
    "tampinha gateway install sets up Tampinha as a system service (systemd/launchd).",
    "tampinha memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "tampinha webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: tampinha tools disables unused tools, tampinha skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "tampinha models routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main chat model.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as tampinha -w).",
    ...
    "TAMPINHA_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Tampinha tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'tampinha profile create coder' creates the 'coder' command.",
    "tampinha profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, Tampinha prints a resume command with session ID and stats.",
    "tampinha sessions export backup.jsonl exports all sessions for backup or analysis.",
    "tampinha -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "tampinha skills install official/security/1password installs optional skills from the repo.",
    ...
    "The plan skill saves markdown plans under .tampinha/plans/ in the active workspace.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: tampinha cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "Tampinha runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    "tampinha gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to Tampinha via a local macOS server.",
    ...
    "Context files (.tampinha.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "Tampinha loads project context from .tampinha.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    # --- MCP ---
    "tampinha mcp opens an interactive picker of Nous-approved MCPs you can install in one keystroke.",
    "tampinha mcp catalog lists Nous-approved MCP servers shipped with the repo.",
    "tampinha mcp install <name> installs a catalog entry, prompts for credentials, and lets you pick which of its tools to enable.",
    ...
    "MCP servers auto-generate toolsets at runtime — tampinha tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.tampinha/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "tampinha chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "tampinha plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "tampinha acp runs Tampinha as an ACP server for VS Code, Zed, and JetBrains integration.",
    "Custom providers: save named endpoints in config.yaml under custom_providers.",
    "TAMPINHA_EPHEMERAL_SYSTEM_PROMPT injects a system prompt that's never persisted to history.",
    "credential_pool_strategies supports fill_first, round_robin, least_used, and random rotation.",
    "tampinha auth add nous or tampinha auth add openai-codex sets up OAuth-based providers.",
    ...
    "tampinha status --deep runs deeper diagnostic checks across all components.",
    ...
    "Cron scripts live in ~/.tampinha/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make Tampinha your own.",
    ...
    "Before a gateway session reset, Tampinha auto-flushes important facts to memory in the background.",
    ...
    "The skills audit log at ~/.tampinha/skills/.hub/audit.log tracks every install and removal operation.",
    ...
    "Profiles scope Tampinha state via TAMPINHA_HOME; host tool subprocesses keep your real HOME unless terminal.home_mode is profile.",
    "TAMPINHA_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in TAMPINHA_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.tampinha/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'tampinha profile create ls' would be rejected.",
    "tampinha profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "tampinha claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "TAMPINHA_DEV=1 bypasses container mode detection for local development.",
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via tampinha tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.tampinha/.env.",
    "Skills from trusted repos (NousResearch) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.tampinha/skills/.hub/quarantine/ holds skills pending security review.",

    # --- Advanced Slash Commands ---
    '/steer <prompt> injects a note after the next tool call — nudge direction mid-task without interrupting.',
    '/goal <text> sets a standing Ralph-loop objective — Tampinha auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of Tampinha config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while Tampinha is working.',
    ...
    '/reload reloads ~/.tampinha/.env into the running session — pick up new API keys without restarting.',
    ...
    "TAMPINHA_CRON_MAX_PARALLEL (default 4) caps how many cron jobs run per tick so bursts don't saturate your keys.",

    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.tampinha/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.tampinha/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',

    # --- Curator ---
    'tampinha curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "tampinha curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'tampinha curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',

    # --- Credential Pools & Routing ---
    'tampinha auth reset <provider> clears all cooldowns and exhaustion flags on a credential pool.',
    ...
    # --- TUI & Dashboard ---
    'TAMPINHA_TUI_RESUME=1 auto-re-attaches to the most recent TUI session on launch — handy after SSH drops.',
    "TAMPINHA_TUI_THEME=light|dark|<hex> forces the TUI theme on terminals that don't set COLORFGBG.",
    ...
    'tampinha dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    'tampinha dashboard embeds the full Tampinha TUI in your browser via xterm.js and a WebSocket PTY.',
    'Drop a YAML in ~/.tampinha/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in: manifest.json + JS bundle in ~/.tampinha/dashboard-plugins/ — no npm build required.',
    ...
    'TAMPINHA_BACKGROUND_NOTIFICATIONS=result only pings when background tasks finish (vs concise/all/error/off).',
    'TAMPINHA_WRITE_SAFE_ROOT restricts write_file/patch to directory prefixes; multiple paths via os.pathsep (: or ;).',
    'TAMPINHA_IGNORE_RULES skips auto-injection of AGENTS.md, SOUL.md, .cursorrules, memory, and preloaded skills.',
    'TAMPINHA_ACCEPT_HOOKS auto-approves unseen shell hooks declared in config.yaml without a TTY prompt.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.tampinha/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.tampinha/skills/ so drop-in skills appear without restarting the session.',
    ...
    'tampinha -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'tampinha chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'tampinha chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'tampinha chat --ignore-user-config skips the active user config — reproducible bug reports and CI runs.',
    "tampinha chat --source tool tags programmatic chats so they don't clutter tampinha sessions list.",
    'tampinha dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'tampinha sessions rename <ID> "new title" renames any past session; tampinha sessions delete <ID> removes one.',
    'tampinha import restores a full Tampinha backup zip; session JSON/JSONL exports import from the dashboard Sessions page.',
    'tampinha fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'tampinha pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'tampinha setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'tampinha status --deep runs the full health sweep across every component; plain tampinha status is the quick view.',

    # --- Agent Behavior Env Vars ---
    'TAMPINHA_AGENT_TIMEOUT=0 disables the gateway inactivity kill for a running agent — use for long research runs.',
    'TAMPINHA_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.tampinha/plugins/ — trust-gated by design.',
    "TAMPINHA_DISABLE_FILE_STATE_GUARD=1 turns off the 'file changed since you read it' guard on patch and write_file.",
    'TAMPINHA_ALLOW_PRIVATE_URLS=true lets web tools hit localhost and private networks — off by default in gateway mode.',
    'TAMPINHA_OPTIONAL_SKILLS=name1,name2 auto-installs extra optional-catalog skills on first run per profile.',
    'TAMPINHA_BUNDLED_SKILLS points at a custom bundled-skill tree — used by Homebrew and Nix packaging.',
    'TAMPINHA_DUMP_REQUEST_STDOUT=1 dumps every API request payload to stdout instead of log files.',
    'TAMPINHA_OAUTH_TRACE=1 logs redacted OAuth token exchange and refresh attempts for debugging provider auth.',
    'TAMPINHA_STREAM_RETRIES (default 3) controls mid-stream reconnect attempts on transient network errors.',

    # --- Gateway Behavior Env Vars ---
    'TAMPINHA_GATEWAY_BUSY_ACK_ENABLED=false silences the ⚡/⏳/⏩ ack messages when a user messages a busy agent.',
    'TAMPINHA_AGENT_NOTIFY_INTERVAL (default 180s) sets how often the gateway pings with progress on long turns.',
    'TAMPINHA_RESTART_DRAIN_TIMEOUT (default 900s) caps how long /restart waits for in-flight runs before forcing.',
    'TAMPINHA_CHECKPOINT_TIMEOUT (default 30s) caps filesystem checkpoint creation — raise it on huge monorepos.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes Tampinha block commands when the tirith scanner itself errors out.',
    ...
    '--source tool chats are excluded from tampinha sessions list by default — set --source explicitly to see them.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.tampinha/dashboard-plugins/.',
]
...
# any project or none — Tampinha is not a coding-only agent.
# ---------------------------------------------------------------------------
