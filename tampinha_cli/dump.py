"""
Dump command for tampinha CLI.

Outputs a compact, plain-text summary of the user's Tampinha setup
...
from tampinha_cli.config import get_tampinha_home, get_env_path, get_project_root, load_config
...
def _dotenv_key_names() -> set[str]:
    """Return the set of env-var names assigned a non-empty value in ~/.tampinha/.env.
    ...
    exports. ``tampinha debug share`` runs in a terminal, so ``os.getenv`` reflects
...
def _get_git_commit(project_root: Path) -> str:
    ...
    to ``<project_root>/.tampinha_build_sha`` by the Dockerfile's
    ``TAMPINHA_GIT_SHA`` build-arg (see ``tampinha_cli/build_info.py``).
...
def _redact(value: str) -> str:
    ...
    ``tampinha dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from tampinha_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(tampinha_home: Path) -> int:
...
def _cron_summary(tampinha_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = tampinha_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"tampinha_home:      {display_tampinha_home()}")
    ...
    for env_var, label in api_keys:
        ...
        # Set in this (shell) process but absent from ~/.tampinha/.env: a managed
        ...
        # A credential added via `tampinha auth add openrouter` lives in the
        # credential pool, not as an env var — surface it so the dump doesn't
        # misleadingly read "not set" while `tampinha auth list` shows it (#42130).
    ...
    toolsets = config.get("toolsets", ["tampinha-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(tampinha_home)}")
    lines.append(f"  skills:             {_count_skills(tampinha_home)}")
