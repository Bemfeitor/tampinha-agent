"""
Doctor command for tampinha CLI.

Diagnoses issues with Tampinha Agent setup.
"""
...
from tampinha_constants import agent_browser_runnable

PROJECT_ROOT = get_project_root()
TAMPINHA_HOME = get_tampinha_home()
_DHH = display_tampinha_home()  # user-facing display path (e.g. ~/.tampinha or ~/.tampinha/profiles/coder)

# Load environment variables from ~/.tampinha/.env so API key checks work
_env_path = get_env_path()
load_tampinha_dotenv(tampinha_home=_env_path.parent, project_env=PROJECT_ROOT / ".env")

from tampinha_cli.colors import Colors, color
from tampinha_cli.models import _TAMPINHA_USER_AGENT
from tampinha_cli.vercel_auth import describe_vercel_auth
from tampinha_constants import OPENROUTER_MODELS_URL
...
from tampinha_constants import is_termux as _is_termux
...
def _sqlite_upgrade_hint(install_method: str | None = None) -> str:
    ...
    if method == "docker":
        ...
        action = f"run `{command}`, then recreate all Tampinha containers"
...
def _tampinha_database_paths(tampinha_home: Path) -> list[tuple[str, Path]]:
    """Return (display name, path) pairs for Tampinha-managed SQLite databases."""
    # backup.py owns the canonical list of per-profile stores; reuse it.
    from tampinha_cli.backup import _QUICK_STATE_FILES

    entries = [
        (name, tampinha_home / name)
        for name in _QUICK_STATE_FILES
        if name.endswith(".db")
    ]
    # Non-default kanban boards each keep their own kanban.db.
    for board_db in sorted((tampinha_home / "kanban" / "boards").glob("*/kanban.db")):
        entries.append((str(board_db.relative_to(tampinha_home)), board_db))
    return entries
...
def _read_journal_mode(db_path: Path) -> tuple[str | None, str | None]:
    ...
    locks a live connection is holding (see ``tampinha_cli.sqlite_safe_read``).
    ...
    """
    from tampinha_cli.sqlite_safe_read import (
        has_live_connection,
...
def _format_db_size(db_path: Path) -> str:
    ...
    from tampinha_cli.backup import _format_size
...
def _report_database_journal_modes(
    tampinha_home: Path | None = None,
    version_info: tuple[int, ...] | None = None,
) -> None:
    """List each database's journal mode; warn on WAL under a vulnerable SQLite."""
    from tampinha_state import _wal_reset_repair_hint, is_sqlite_wal_reset_vulnerable

    vulnerable = is_sqlite_wal_reset_vulnerable(version_info)
    home = tampinha_home if tampinha_home is not None else TAMPINHA_HOME
    try:
        databases = _tampinha_database_paths(home)
    except Exception as exc:
        check_warn(f"Could not list Tampinha databases: {exc}")
        return
    exposed = []
...
def _has_provider_env_config(content: str) -> bool:
    """Return True when ~/.tampinha/.env contains provider auth/base URL settings."""
...
def _is_kanban_worker_env_gate(item: dict) -> bool:
    ...
    if item.get("name") != "kanban":
        return False
    if os.environ.get("TAMPINHA_KANBAN_TASK"):
        return False
...
def _doctor_tool_availability_detail(toolset: str) -> str:
    ...
    if toolset == "kanban" and not os.environ.get("TAMPINHA_KANBAN_TASK"):
...
def _has_healthy_oauth_fallback_for_apikey_provider(provider_label: str) -> bool:
    ...
    if normalized == "minimax":
        try:
            from tampinha_cli.auth import get_minimax_oauth_auth_status
...
from tampinha_cli.sizefmt import format_bytes as _human_bytes
...
def _render_state_db_stats(stats: dict, holders=None) -> list:
    ...
    if isinstance(deferral, dict):
        ...
        lines.append((
            ...
            "(stop the listed processes, then run 'tampinha sessions "
    ...
    if logical is not None and logical > STATE_DB_SIZE_WARN_BYTES:
        ...
        if stats.get("fts_rebuild_pending") or legacy_trigram:
            detail += (
                "; run 'tampinha sessions optimize-storage' offline "
...
_DEPRECATED_ENV_VARS: tuple[tuple[str, str], ...] = (
    ...
    # ignored. TAMPINHA_TOOL_PROGRESS_MODE is still read by the gateway as a
    # back-compat fallback but remains deprecated.
    ("TAMPINHA_TOOL_PROGRESS", "display.tool_progress in config.yaml — ignored/unsupported since config floor v12"),
    ("TAMPINHA_TOOL_PROGRESS_MODE", "display.tool_progress in config.yaml"),
...
def collect_relay_plugin_cutover_findings(
    ...
    """Return actionable findings for the removed Tampinha Relay plugin."""
    from tampinha_cli.relay_plugin_cutover import (
...
def _check_version_consistency(issues: list[str]) -> None:
    """Verify pyproject.toml version matches tampinha_cli.__version__.

    A git conflict resolution (reset/merge) can revert one file without the
    other, leaving ``tampinha --version`` reporting a stale version while
    ...
    """
    try:
        from tampinha_cli import __version__ as init_version
    except Exception:
        return
    ...
    if pyproject_version == init_version:
        ...
        _fail_and_issue(
            ...
            f"(pyproject.toml {pyproject_version} != tampinha_cli/__init__.py {init_version})",
            "Re-sync version files (e.g. run 'tampinha update', or set "
            "tampinha_cli/__init__.py __version__ to match pyproject.toml)",
            issues,
        )
...
def _check_s6_supervision(issues: list[str]) -> None:
    ...
    Reports:
      - Whether the main-tampinha and dashboard static services are up
    ...
    for static in ("main-tampinha", "dashboard"):
    ...
    if not profiles:
        check_info("No per-profile gateways registered yet — create one with `tampinha profile create <name>`")
        return
...
def check_certificates(should_fix: bool = False, issues: "list | None" = None) -> None:
    ...
    if not should_fix:
        ...
        if issues is not None:
            issues.append(
                "Repair the CA bundle: run `tampinha doctor --fix`, or "
...
def _build_apikey_providers_list() -> list:
    ...
    try:
        ...
        try:
            from tampinha_cli.providers import normalize_provider as _normalize_provider
...
def managed_scope_check() -> None:
    ...
    resolved from the TAMPINHA_MANAGED_DIR override (rather than the system
    ...
    if os.environ.get("TAMPINHA_MANAGED_DIR", "").strip():
        check_info(f"managed dir set via TAMPINHA_MANAGED_DIR={managed_dir}")
...
def check_macos_tcc_grants() -> None:
    ...
    Post-#73681 builds pin ``designated => identifier "com.nousresearch.tampinha"``
    ...
    if "cdhash" in dr.lower():
        check_warn(
            ...
            "Run `tampinha update` to get the stable identifier-pinned signing "
            "identity, then re-grant permissions once.",
        )
        return
    if "certificate" in dr.lower():
        # Certificate-anchored DR (tampinha desktop --setup-tcc-identity, or a
        ...
        check_ok(
            ...
            "anchor, see `tampinha desktop --setup-tcc-identity`)",
        )
    check_info(
        "If macOS still re-prompts for permissions (toggle shows ON): the stored "
        "grant is stale — run `tccutil reset ScreenCapture com.nousresearch.tampinha` "
        "(repeat per affected service), toggle it ON in System Settings, then "
        "fully quit & relaunch Tampinha once."
    )
...
def _desktop_app_bundle() -> Path | None:
    ...
    (``apps/desktop/release/mac-<arch>/Tampinha.app``) — the only layout whose
    ...
    ``_desktop_packaged_executable``'s selection. ``/Applications/Tampinha.app``
    is deliberately not probed: it is the separately-signed Tampinha-Setup
    launcher (``com.nousresearch.tampinha.setup``, certificate-anchored), whose
    grants are stable by construction and unaffected by rebuilds.
    """
    root = Path(__file__).resolve().parents[1]
    release_dir = root / "apps" / "desktop" / "release"
    candidates = [p for p in release_dir.glob("mac*/Tampinha.app") if p.is_dir()]
    if not candidates:
        return None
...
def check_macos_tcc_anchor(should_fix: bool = False) -> None:
    ...
    """
    try:
        from tampinha_cli import macos_tcc_anchor as tcc
...
def check_macos_full_disk_access() -> None:
    ...
    check_info(
        ...
        "app Full Disk Access and Tampinha will never trip per-folder dialogs "
        ...
        "    then enable your terminal (and Tampinha.app if you use Desktop), "
        "and restart them once. With Tampinha' stable signing identities the "
...
def run_doctor(args):
    ...
    # checks (like cronjob management) should see the same context as `tampinha`.
    os.environ.setdefault("TAMPINHA_INTERACTIVE", "1")

    # Handle `tampinha doctor --ack <id>` as a fast path. Persist the ack and
    # return without running the rest of the diagnostics — the user has
    # already seen the advisory and just wants to silence it.
    if ack_target:
        from tampinha_cli.security_advisories import (
            ADVISORIES,
            ack_advisory,
        )
        ...
        if ack_advisory(ack_target):
            ...
            print(color(
                ...
                f"Check ~/.tampinha/config.yaml is writable.",
                Colors.RED,
            ))
            sys.exit(1)
        return
    ...
    print(color("│                 🩺 Tampinha Doctor                        │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
        ...
        if fresh_hits:
            for hit in fresh_hits:
                ...
                manual_issues.append(
                    ...
                    f"`tampinha doctor --ack {hit.advisory.id}`."
                )
    ...
    try:
        import sqlite3
        from tampinha_state import is_sqlite_wal_reset_vulnerable, sqlite_source_id
        ...
        if is_sqlite_wal_reset_vulnerable():
            # Warn-only: Tampinha already refuses to enable WAL on fresh DBs.
    ...
    # Detect drift between pyproject.toml and tampinha_cli/__init__.py versions
    ...
    managed_scope_check()
    # Check ~/.tampinha/.env (primary location for user config)
    env_path = TAMPINHA_HOME / '.env'
    if env_path.exists():
        ...
        # matches tampinha_cli.env_loader._load_dotenv_with_fallback.
        try:
        ...
        if _has_provider_env_config(content):
            ...
            issues.append("Run 'tampinha setup' to configure API keys")
    else:
        ...
        if fallback_env.exists():
            ...
            if should_fix:
                ...
                check_info("Run 'tampinha setup' to configure API keys")
                fixed_count += 1
            else:
                check_info("Run 'tampinha setup' to create one")
                issues.append("Run 'tampinha setup' to create .env")
    
    # Check ~/.tampinha/config.yaml (primary) or project cli-config.yaml (fallback)
    config_path = TAMPINHA_HOME / 'config.yaml'
    if config_path.exists():
        ...
        try:
            ...
            try:
                ...
                pass
            try:
                from tampinha_cli.config import get_compatible_custom_providers as _compatible_custom_providers
            ...
            if isinstance(user_providers, dict):
                from tampinha_cli.config import is_provider_enabled
                ...
                if catalog_provider is None or (
                    ...
                    _fail_and_issue(
                        ...
                        (
                            ...
                            f"Fix: run 'tampinha config set model.provider <valid_provider>'"
                        ),
                        issues,
                    )
            ...
            if runtime_provider and runtime_provider not in ("auto", "custom"):
                try:
                    if runtime_provider == "openrouter":
                        ...
                        from tampinha_cli.auth import PROVIDER_REGISTRY, get_auth_status
                    ...
                    if not configured:
                        _fail_and_issue(
                            ...
                            "(check ~/.tampinha/.env or run 'tampinha setup')",
                            (
                                f"No credentials found for provider '{runtime_provider}'. "
                                f"Run 'tampinha setup' or set the provider's API key in {_DHH}/.env, "
                                f"or switch providers with 'tampinha config set model.provider <name>'"
                            ),
                            issues,
                        )
                except Exception:
        ...
        if fallback_config.exists():
            ...
            if should_fix:
                ...
                if example_config.exists():
                    ...
                    from tampinha_cli.config import DEFAULT_CONFIG, save_config
    ...
    if config_path.exists():
        try:
            from tampinha_cli.config import check_config_version, migrate_config
            ...
            if current_ver < latest_ver:
                ...
                if should_fix:
                    try:
                        ...
                        issues.append("Run 'tampinha setup' to migrate config")
                else:
                    issues.append("Run 'tampinha doctor --fix' or 'tampinha setup' to migrate config")
            else:
                ...
                if should_fix:
                    ...
                    for k in stale_root_keys:
                        if not model_section.get(k):
                            ...
                            raw_config.pop(k)
                    from tampinha_cli.config import atomic_config_write
                    atomic_config_write(config_path, raw_config)
                    check_ok("Migrated stale root-level keys into model section")
                    fixed_count += 1
                else:
                    issues.append("Stale root-level provider/base_url in config.yaml — run 'tampinha doctor --fix'")
        except Exception:
            pass

        # Detect stale TAMPINHA_MAX_ITERATIONS ghost in .env shadowing
        ...
        # bridge normally derives TAMPINHA_MAX_ITERATIONS from agent.max_turns
        ...
        try:
            from tampinha_cli.config import load_env, read_user_config_raw, remove_env_value
            ...
            env_ghost = load_env().get("TAMPINHA_MAX_ITERATIONS")
            drift = (
            ...
            if drift:
                check_warn(
                    f"TAMPINHA_MAX_ITERATIONS={env_ghost} in .env shadows "
                    f"agent.max_turns={cfg_max_turns} in config.yaml",
                    "(stale ghost from an earlier `tampinha setup` run)",
                )
                if should_fix:
                    if remove_env_value("TAMPINHA_MAX_ITERATIONS"):
                        check_ok(
                            "Removed stale TAMPINHA_MAX_ITERATIONS from .env "
                            f"(config.yaml agent.max_turns={cfg_max_turns} is now authoritative)"
                        )
                        fixed_count += 1
                    else:
                        check_warn("Could not remove TAMPINHA_MAX_ITERATIONS from .env")
                        manual_issues.append(
                            "Manually delete the TAMPINHA_MAX_ITERATIONS line from "
                            f"{_DHH}/.env — config.yaml agent.max_turns is authoritative."
                        )
                else:
                    issues.append(
                        "Stale TAMPINHA_MAX_ITERATIONS in .env shadows config.yaml — "
                        "run 'tampinha doctor --fix'"
                    )
        except Exception:
            pass
        ...
        try:
            from tampinha_cli.config import load_env as _load_env_depr
            from tampinha_cli.config import read_user_config_raw as _read_raw_depr
    ...
    try:
        ...
        if codex_status.get("logged_in"):
            ...
            # Native OAuth uses Tampinha' own device-code flow — the Codex CLI is
    ...
    tampinha_home = TAMPINHA_HOME
    if tampinha_home.exists():
    ...
    for subdir_name in expected_subdirs:
        subdir_path = tampinha_home / subdir_name
    ...
    soul_path = tampinha_home / "SOUL.md"
    if soul_path.exists():
        ...
        check_warn(f"{_DHH}/SOUL.md not found", "(create it to give Tampinha a custom personality)")
        if should_fix:
            soul_path.parent.mkdir(parents=True, exist_ok=True)
            soul_path.write_text(
                "# Tampinha Agent Persona\n\n"
                "<!-- Edit this file to customize how Tampinha communicates. -->\n\n"
                "You are Tampinha, a helpful AI assistant.\n",
                encoding="utf-8",
            )
    ...
    memories_dir = tampinha_home / "memories"
    ...
    state_db_path = tampinha_home / "state.db"
    if state_db_path.exists():
        try:
            ...
            from tampinha_state import _db_opens_cleanly, repair_state_db_schema
            ...
            if _write_reason is not None:
                ...
                if should_fix:
                    ...
                    issues.append(
                        "state.db FTS write corruption — run 'tampinha doctor --fix' "
                        "(or 'tampinha sessions repair') to rebuild the FTS index"
                    )
        except Exception as e:
            from tampinha_state import is_malformed_db_error, repair_state_db_schema
            ...
            if is_malformed_db_error(e):
                ...
                if should_fix:
                    ...
                    issues.append(
                        "state.db schema malformed — run 'tampinha doctor --fix' "
                        "(or 'tampinha sessions repair') to recover hidden sessions"
                    )
            else:
                check_warn(f"{_DHH}/state.db exists but has issues: {e}")

        # Health/stats snapshot (#statedb-visibility): a multi-GB state.db
        # with a runaway WAL was previously invisible to every Tampinha
        ...
        try:
            from tampinha_state import collect_state_db_stats, count_db_holders
                ...
                if _kind == "warn":
                    ...
                    if "auto_prune" in _detail:
                        issues.append(
                            ...
                            "in config.yaml"
                            + (
                                " and run 'tampinha sessions optimize-storage' "
    ...
    wal_path = tampinha_home / "state.db-wal"
    if wal_path.exists():
        try:
            ...
            if wal_size > 50 * 1024 * 1024:  # 50 MB
                ...
                if should_fix:
                    ...
                    issues.append("Large WAL file — run 'tampinha doctor --fix' to checkpoint")
    ...
    if sys.platform != "win32":
        ...
        for _venv_name in ("venv", ".venv"):
            _candidate = PROJECT_ROOT / _venv_name / "bin" / "tampinha"
        ...
        _cmd_link = _cmd_link_dir / "tampinha"

        if _venv_bin is None:
            check_warn(
                "Venv entry point not found",
                "(tampinha not in venv/bin/ or .venv/bin/ — reinstall with pip install -e '.[all]')"
            )
            manual_issues.append(
            ...
            if _cmd_link.is_symlink():
                ...
                if _target == _expected:
                    check_ok(f"{_cmd_link_display}/tampinha → correct target")
                else:
                    check_warn(
                        f"{_cmd_link_display}/tampinha points to wrong target",
                        f"(→ {_target}, expected → {_expected})"
                    )
                    if should_fix:
                        _cmd_link.unlink()
                        _cmd_link.symlink_to(_venv_bin)
                        check_ok(f"Fixed symlink: {_cmd_link_display}/tampinha → {_venv_bin}")
                        fixed_count += 1
                    else:
                        issues.append(f"Broken symlink at {_cmd_link_display}/tampinha — run 'tampinha doctor --fix'")
            elif _cmd_link.exists():
                # It's a regular file, not a symlink — possibly a wrapper script
                check_ok(f"{_cmd_link_display}/tampinha exists (non-symlink)")
            else:
                check_fail(
                    f"{_cmd_link_display}/tampinha not found",
                    "(tampinha command may not work outside the venv)"
                )
                if should_fix:
                    _cmd_link_dir.mkdir(parents=True, exist_ok=True)
                    _cmd_link.symlink_to(_venv_bin)
                    check_ok(f"Created symlink: {_cmd_link_display}/tampinha → {_venv_bin}")
                    fixed_count += 1
                    ...
                    issues.append(f"Missing {_cmd_link_display}/tampinha symlink — run 'tampinha doctor --fix'")
    ...
    try:
        from tampinha_constants import is_container as _is_container
    ...
    if terminal_env == "vercel_sandbox":
        ...
        if importlib.util.find_spec("vercel") is not None:
            ...
            _fail_and_issue(
                ...
                "(pip install 'tampinha-agent[vercel]')",
                "Install the Vercel optional dependency: pip install 'tampinha-agent[vercel]'",
                issues,
            )
    ...
    if _safe_which("node"):
        ...
        # — it resolves lazily via npx (or a global/Tampinha-managed install)
        ...
        if _resolved_ab and _is_npx_agent_browser_sentinel(_resolved_ab):
            ...
            if should_fix:
                ...
                # `tampinha update` does, so a session's first browser call
            ...
            # `tampinha update` wiped node_modules (issue #48521).
            check_warn(
        ...
        if agent_browser_ok and not _is_termux():
            try:
                ...
                # to eagerly load in every `tampinha doctor` invocation.
    ...
    if _npm_bin:
        ...
        # The WhatsApp bridge may live under a writable TAMPINHA_HOME mirror
            ...
            try:
                ...
                if total == 0:
                    ...
                    if audit_extra and audit_extra[0] == "--workspace":
                        ...
                        # tree — in that case it is an npm bug, not a Tampinha one.
                        check_info(
            ...
            if r.status_code == 402:
                return _ConnectivityResult(
                    ...
                    ["OpenRouter account has insufficient credits. "
                     "Fix: run 'tampinha config set model.provider <provider>' "
    ...
    def _probe_apikey_provider(pname, env_vars, default_url, base_env,
        ...
        try:
            ...
            headers = {
                ...
                "User-Agent": _TAMPINHA_USER_AGENT,
            }
            ...
            # GOOGLE_API_KEY/GEMINI_API_KEY always shows red in ``tampinha doctor``.
    ...
    # never the legitimate source for `tampinha doctor`.
    ...
    try:
        ...
        if api_disabled or web_not_ready:
            issues.append("Run 'tampinha setup' to configure missing API keys for full tool access")
    except Exception as e:
        check_warn("Could not check tool availability", f"({e})")
    
    _section("Skills Hub")
    hub_dir = TAMPINHA_HOME / "skills" / ".hub"
    if hub_dir.exists():
        ...
        check_warn("Skills Hub directory not initialized", "(run: tampinha skills list)")
    ...
    try:
        from tampinha_cli.config import read_user_config_raw as _read_raw_mem
        _mem_cfg_path = TAMPINHA_HOME / "config.yaml"
    ...
    if not _active_memory_provider:
        ...
        try:
            ...
            if not _honcho_cfg_path.exists():
                ...
                if hcfg.api_key or hcfg.base_url:
                    ...
                    check_warn("Honcho config not found", "run: tampinha memory setup")
            elif not hcfg.enabled:
                check_info(f"Honcho disabled (set enabled: true in {_honcho_cfg_path} to activate)")
            elif not (hcfg.api_key or hcfg.base_url):
                _fail_and_issue(
                    "Honcho API key or base URL not set",
                    "run: tampinha memory setup",
                    "No Honcho API key — run 'tampinha memory setup'",
                    issues,
                )
            else:
        ...
        try:
            ...
            if mem0_key:
                ...
                _fail_and_issue(
                    ...
                    "(set MEM0_API_KEY in .env or run tampinha memory setup)",
        ...
        try:
            ...
            if _provider and _provider.is_available():
                ...
                check_warn(f"{_active_memory_provider} configured but not available", "run: tampinha memory status")
            else:
                check_warn(f"{_active_memory_provider} plugin not found", "run: tampinha memory setup")
        except Exception as _e:
            check_warn(f"{_active_memory_provider} check failed", str(_e))

    try:
        from tampinha_cli.profiles import list_profiles, _get_wrapper_dir, profile_exists
        import re as _re
            ...
            if wrapper_dir.is_dir():
                for wrapper in wrapper_dir.iterdir():
                    ...
                    try:
                        ...
                        if "tampinha -p" in content:
                            _m = _re.search(r"tampinha -p (\S+)", content)
    ...
    # `tampinha doctor --live` (real network calls; bounded + read-only).
    try:
        from tampinha_cli.doctor_live import maybe_run_live_checks
    ...
    if should_fix and fixed_count > 0:
        ...
        print()
        if not should_fix:
            print(color("  Tip: run 'tampinha doctor --fix' to auto-fix what's possible.", Colors.DIM))
    else:
        print(color("─" * 60, Colors.GREEN))
