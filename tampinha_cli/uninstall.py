"""
Tampinha Agent Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep ~/.tampinha/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove Tampinha PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing tampinha-agent or tampinha PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# Tampinha Agent" comment and following line
                if '# Tampinha Agent' in line or '# tampinha-agent' in line:
                    skip_next = True
                    continue
                if skip_next and ('tampinha' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing tampinha
                if 'tampinha' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
            ...
            if new_content != original_content:
                ...
                # This is the user's own shell rc, not a Tampinha-owned file, and
...
def remove_wrapper_script():
    """Remove the tampinha wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "tampinha",
        Path.home() / ".local" / "bin" / "tampinha-acp",
        Path.home() / ".local" / "bin" / "tampinha-agent",
        Path("/usr/local/bin/tampinha"),
        Path("/usr/local/bin/tampinha-acp"),
        Path("/usr/local/bin/tampinha-agent"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains tampinha_cli reference)
                content = wrapper.read_text(encoding="utf-8")
                if 'tampinha_cli' in content or 'tampinha-agent' in content:
                    wrapper.unlink()
...
def remove_node_symlinks(tampinha_home: Path) -> list:
    ...
    symlinks node/npm/npx into the same directory as the ``tampinha`` command:
    ...
    the FHS fix).  Only symlinks that resolve into this Tampinha home's ``node``
    directory are removed — links the user has repointed elsewhere (nvm, fnm,
    etc.) are left untouched.
    """
    node_dir = (tampinha_home / "node").resolve()
    removed = []
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``tampinha gateway run`` processes
    ...
    try:
        from tampinha_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from tampinha_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``TAMPINHA_HOME`` and ``TAMPINHA_GIT_BASH_PATH``
...
#      like ``%LOCALAPPDATA%\tampinha\git\cmd``, ``%LOCALAPPDATA%\tampinha\git\bin``,
#      ``%LOCALAPPDATA%\tampinha\git\usr\bin``, ``%LOCALAPPDATA%\tampinha\node``.
#      Again not in any rc file — only accessible via the registry or the
#      .NET [Environment] API.
#   3. Downloads PortableGit to ``%LOCALAPPDATA%\tampinha\git\`` and Node to
#      ``%LOCALAPPDATA%\tampinha\node\`` as user-scoped, isolated copies.
#      These are ~200MB combined and serve no purpose after uninstall.
#   4. On the ``tampinha dashboard`` + gateway paths, drops files into
#      ``%LOCALAPPDATA%\tampinha\gateway-service\`` and sometimes
...
def _tampinha_path_markers(tampinha_home: Path, *, include_managed_bin: bool = False) -> list[str]:
    """Path-entry substrings that identify Tampinha-owned User-PATH entries.

    ``include_managed_bin`` adds the managed binary dir (``<root>\\bin``,
    holding the tampinha launchers and the managed uv) — only wanted when
    ...
    """
    root = str(tampinha_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare tampinha-agent install dir.
    markers = [root + "\\tampinha-agent", root + "\\git", root + "\\node", root + "\\venv"]
    if include_managed_bin:
        markers.append(root + "\\bin")
    # Also match if TAMPINHA_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "tampinha".  We don't want to catch
    # unrelated entries like "chermes-foo" or "ephermeral", so we look for
    # backslash-tampinha as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(tampinha_home: Path, *, include_managed_bin: bool = False) -> list[str]:
    """Strip Tampinha-owned entries from User-scope PATH in the registry.
    ...
    ``include_managed_bin`` adds ``<tampinha_home>\\bin`` (the managed binary
    dir holding the tampinha launchers and the managed uv) to the sweep. Only
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _tampinha_path_markers(tampinha_home, include_managed_bin=include_managed_bin)
            kept: list[str] = []
...
def remove_tampinha_env_vars_windows() -> list[str]:
    """Delete TAMPINHA_HOME and TAMPINHA_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("TAMPINHA_HOME", "TAMPINHA_GIT_BASH_PATH"):
                try:
...
def remove_windows_bin_launchers(*, windows: bool | None = None) -> list[Path]:
    """Delete the ``tampinha`` launchers install.ps1 staged in the managed
    binary dir (the default Tampinha root's ``bin``, next to the managed uv).

    Every uninstall mode deletes the code checkout, so the launchers —
    which invoke ``<checkout>\\venv\\Scripts`` — would otherwise dangle:
    ``tampinha`` in a new terminal resolves to a launcher whose target is
    ...
    ``_install_repair._quarantine_running_tampinha_exe`` relies on), so
    ...
    *windows* is an injectable platform verdict for tests (same pattern as
    ...
    try:
        ...
        from tampinha_cli._install_repair import _WINDOWS_BIN_LAUNCHERS
        from tampinha_constants import get_default_tampinha_root

        bin_dir = get_default_tampinha_root() / "bin"
    except Exception as e:
...
def _is_default_tampinha_home(tampinha_home: Path) -> bool:
    """Return True when ``tampinha_home`` points at the default (non-profile) root."""
    try:
        from tampinha_constants import get_default_tampinha_root
        return tampinha_home.resolve() == get_default_tampinha_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its TAMPINHA_HOME directory.

    We shell out to ``tampinha -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current TAMPINHA_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m tampinha_cli.main` so we don't depend on a `tampinha`
    #    wrapper that may be half-removed mid-uninstall.
    tampinha_invocation = [_sys.executable, "-m", "tampinha_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                tampinha_invocation + ["gateway", subcmd],
                capture_output=True,
    ...
    # 3. Wipe the profile's TAMPINHA_HOME directory.
    try:
...
def run_gui_uninstall(args):
    ...
    Mirrors ``tampinha uninstall --gui``. Removes the desktop app's built
    artifacts, the packaged app bundle (best-effort), and the Electron
    userData dir — nothing under ``$TAMPINHA_HOME`` config/sessions/.env, and
    never the Python agent or its venv.
    """
    from tampinha_cli.gui_uninstall import (
        agent_is_installed,
        gui_install_summary,
        uninstall_gui,
    )
    ...
    summary = gui_install_summary(tampinha_home)
    skip_confirm = bool(getattr(args, "yes", False))

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│         ⚕ Tampinha Chat GUI Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()

    if not summary["gui_installed"]:
        print("No Tampinha Chat GUI installation was found.")
        print(f"  Checked: {tampinha_home}, and the standard app locations for this OS.")
        return

    print(color("This removes the Chat GUI only. The Tampinha agent stays installed.", Colors.CYAN))
    print()
    ...
    print()
    if agent_is_installed(tampinha_home):
        print(color("Kept intact:", Colors.GREEN, Colors.BOLD))
        print(f"  • The Tampinha agent at {tampinha_home / 'tampinha-agent'}")
        print(f"  • Your config, sessions, and secrets under {tampinha_home}")
        print()
    ...
    print(color("└─────────────────────────────────────────────────────────┘", Colors.GREEN, Colors.BOLD))
    print()
    print("The Tampinha agent is still installed. Run 'tampinha' to use the CLI,")
    print("or 'tampinha uninstall' to remove the agent too.")
    print()
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.tampinha/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.tampinha/ for future reinstall
    """
    ...
    # offer to clean them up too instead of leaving zombie TAMPINHA_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_tampinha_home(tampinha_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    # Non-interactive fast path (``--yes``): no prompts. ``--full`` selects a
    # full wipe (code + ~/.tampinha data); otherwise keep-data. Named profiles
    ...
    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚕ Tampinha Agent Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(f"  Config:  {tampinha_home / 'config.yaml'}")
    print(f"  Secrets: {tampinha_home / '.env'}")
    print(f"  Data:    {tampinha_home / 'cron/'}, {tampinha_home / 'sessions/'}, {tampinha_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their TAMPINHA_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Tampinha data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the Tampinha code but keep your configuration and data.")
...
def _print_uninstall_dry_run(*, project_root: Path, tampinha_home: Path, full_uninstall: bool) -> None:
    ...
    print("  • Tampinha PATH entries from shell configs / Windows User PATH")
    print("  • Tampinha wrapper scripts and Tampinha-managed node/npm/npx symlinks")
    print("  • Desktop Chat GUI artifacts")
    print(f"  • Code checkout: {project_root}")
    if full_uninstall:
        print(f"  • Tampinha config/data: {tampinha_home}")
        if _is_default_tampinha_home(tampinha_home):
        ...
        print(f"  • Keep Tampinha config/data: {tampinha_home}")
    print()
...
def _perform_uninstall(
    *,
    ...
    ``tampinha`` wrapper + node symlinks → remove the desktop Chat GUI artifacts →
    delete the code checkout → (Windows) remove PortableGit/Node → optionally
    wipe ``$TAMPINHA_HOME`` data and named profiles on full uninstall.
    """
    print()
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in tampinha_home so the marker matching is
        # against fully resolved paths — installer writes literal strings
        # like C:\Users\<u>\AppData\Local\tampinha\git\cmd, not %LOCALAPPDATA%.
        # The managed binary dir (tampinha\bin: launchers + managed uv) leaves
        ...
        sweep_managed_bin = full_uninstall and _is_default_tampinha_home(tampinha_home)
        removed_path_entries = remove_path_from_windows_registry(
            Path(os.path.expandvars(str(tampinha_home))),
        ...
        if removed_path_entries:
            ...
            log_info("No Tampinha-owned PATH entries in User environment")

        log_info("Removing TAMPINHA_HOME / TAMPINHA_GIT_BASH_PATH User env vars...")
        removed_env = remove_tampinha_env_vars_windows()
        if removed_env:
            for name in removed_env:
                log_success(f"Removed User env var: {name}")
        else:
            log_info("No Tampinha-set User env vars to remove")
    
    # 3. Remove wrapper script
    log_info("Removing tampinha command...")
    ...
    #     would dangle — `tampinha` in a new terminal would resolve and then
    #     error on its missing venv target, worse than command-not-found.
    if _is_windows():
        log_info("Removing Windows tampinha launchers...")
        ...
        if removed_launchers:
            ...
            log_info("No Windows tampinha launchers found")

    # 3b. Remove node/npm/npx symlinks the installer left in ~/.local/bin
    #     (only when they still point into this Tampinha home's node dir, so we
    #     never clobber an existing nvm / user-managed Node).
    log_info("Removing Tampinha-managed node/npm/npx symlinks...")
    removed_node_links = remove_node_symlinks(tampinha_home)
    if removed_node_links:
        for link in removed_node_links:
            log_success(f"Removed {link}")
    else:
        log_info("No Tampinha-managed node/npm/npx symlinks found")
    ...
    #     step-5 rmtree(tampinha_home) would sweep the in-tree artifacts anyway,
    #     but the packaged app + Electron userData live OUTSIDE TAMPINHA_HOME and
    #     must be cleaned explicitly here.
    log_info("Removing desktop Chat GUI artifacts...")
    try:
        from tampinha_cli.gui_uninstall import uninstall_gui
        gui_removed = uninstall_gui(tampinha_home)
        if not gui_removed:
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.tampinha/, just remove the tampinha-agent subdir
            if tampinha_home in project_root.parents or project_root.parent == tampinha_home:
    ...
    #     under TAMPINHA_HOME but they're install tooling, not config — safe to
    #     remove even in "keep data" mode.  If we're doing a full uninstall
    #     the step-5 rmtree(tampinha_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(tampinha_home)
    ...
    # 5. Optionally remove ~/.tampinha/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile TAMPINHA_HOME dirs live under
        ...
        try:
            if tampinha_home.exists():
                shutil.rmtree(tampinha_home)
                log_success(f"Removed {tampinha_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {tampinha_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {tampinha_home}")
    ...
    print()
    print("Thank you for using Tampinha Agent! ⚕")
    print()
...
def main(argv=None) -> int:
    """Module entrypoint: ``python -m tampinha_cli.uninstall --mode <gui|lite|full>``.
    ...
    so ``import tampinha_cli`` resolves from source while the venv is torn down.

    This module imports only stdlib + ``tampinha_constants`` + ``tampinha_cli.colors``
    (and lazily ``tampinha_cli.gui_uninstall``), so it runs fine under a bare
    system Python with no site-packages from the venv.
    """
    import argparse

    parser = argparse.ArgumentParser(prog="python -m tampinha_cli.uninstall")
    parser.add_argument(
        "--mode",
