"""Shell completion script generation for tampinha CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_tampinha_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# Tampinha Agent bash completion
# Add to ~/.bashrc:
#   eval "$(tampinha completion bash)"

_tampinha_profiles() {{
    local profiles_dir="$HOME/.tampinha/profiles"
...
_tampinha_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_tampinha_profiles)" -- "$cur"))
        return
    fi
...
complete -F _tampinha_completion tampinha
"""
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef tampinha
# Tampinha Agent zsh completion
# Add to ~/.zshrc:
#   eval "$(tampinha completion zsh)"

_tampinha_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.tampinha/profiles" ]]; then
        profiles+=($HOME/.tampinha/profiles/*(N/:t))
    fi
...
_tampinha() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_tampinha_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'tampinha command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Tampinha Agent fish completion",
        "# Add to your config:",
        "#   tampinha completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __tampinha_profiles",
        "    echo default",
        "    if test -d $HOME/.tampinha/profiles",
        "        for d in $HOME/.tampinha/profiles/*/",
        "            basename $d",
        "        end",
        "    end",
        "end",
        ...
        "complete -c tampinha -f",
        "",
        "# Complete profile names after -p / --profile",
        "complete -c tampinha -f -s p -l profile"
        " -d 'Profile name' -xa '(__tampinha_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__tampinha_profiles)' -d 'Profile name'"
                )
