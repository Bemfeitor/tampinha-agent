"""
Backup and import commands for tampinha CLI.

`tampinha backup` creates a zip archive of the entire ~/.tampinha/ directory
(excluding the tampinha-agent repo and transient files).

`tampinha import` restores from a backup zip, overlaying onto the current
TAMPINHA_HOME root.
"""
...
from tampinha_constants import get_default_tampinha_root, get_tampinha_home, display_tampinha_home
from utils import (
    _preserve_file_mode,
...
from tampinha_cli.sizefmt import format_bytes as _format_size
...
# Where ``tampinha backup --quick`` / ``/snapshot`` / the pre-update safety net
...
# ``tampinha-agent`` is special-cased to root level only in ``_should_exclude``
# so that skill directories like ``skills/autonomous-ai-agents/tampinha-agent/``
...
# TAMPINHA_HOME gets walked file-by-file, ballooning a backup to hundreds of
...
_EXCLUDED_DIRS = {
    "tampinha-agent",     # the codebase repo — re-clone instead
    ...
    "browser-profile",
    # Python dependency trees (plugin / MCP-server venvs under TAMPINHA_HOME) —
...
# File names that ``tampinha import`` must never overwrite, matched by basename so
...
# Reserved archive subtree for provider state that lives OUTSIDE TAMPINHA_HOME
...
class BackupInProgressError(RuntimeError):
    """Raised when another process already owns the Tampinha backup slot."""
...
@contextmanager
def _backup_operation_lock(tampinha_home: Path, timeout_seconds: float = 0.25):
    """Acquire one cross-process backup slot for full and quick snapshots."""
    lock_path = tampinha_home / ".backup.lock"
            ...
            while True:
                try:
                    ...
                    if time.monotonic() >= deadline:
                        raise BackupInProgressError("another Tampinha backup is already running")
                    time.sleep(0.05)
        else:
            import fcntl
...
def _collect_memory_provider_external_paths() -> List[Path]:
    ...
    outside TAMPINHA_HOME, resolved from config only (no network, no init).
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to tampinha root) should be skipped."""
    parts = rel_path.parts

    for part in parts:
        if part not in _EXCLUDED_DIRS:
            continue
        # ``tampinha-agent`` only matches at the root level (first component).
        # Nested directories with the same name — e.g.
        # ``skills/autonomous-ai-agents/tampinha-agent/`` — must be preserved.
        if part == "tampinha-agent" and part != parts[0]:
            continue
        return True
...
def _should_skip_backup_file(abs_path: Path, rel_path: Path, out_path: Path) -> bool:
    ...
    # write can copy data from outside TAMPINHA_HOME.
...
# to the user as a hung `tampinha update` (#70553 follow-up). Sessions databases
...
def verify_sqlite_integrity(
    ...
    *,
    ...
    if check_header:
        ...
        # tampinha_cli.sqlite_safe_read). Verification targets snapshots and
...
def run_backup(args) -> None:
    """Create a zip backup of the Tampinha home directory."""
    tampinha_root = get_default_tampinha_root()

    if not tampinha_root.is_dir():
        print(f"Error: Tampinha home directory not found at {tampinha_root}")
        sys.exit(1)

    try:
        with _backup_operation_lock(tampinha_root):
            _run_backup_locked(args, tampinha_root)
    except BackupInProgressError as exc:
        print(f"Error: {exc}")
        raise SystemExit(2) from exc


def _run_backup_locked(args, tampinha_root: Path) -> None:
    ...
    try:
        if args.output:
            ...
            if out_path.is_dir():
                ...
                out_path = out_path / f"tampinha-backup-{stamp}.zip"
        else:
            stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
            out_path = Path.home() / f"tampinha-backup-{stamp}.zip"
    ...
    print(f"Scanning {display_tampinha_home()} ...")
    files_to_add: list[tuple[Path, Path]] = []  # (absolute, relative)
    skipped_dirs = set()

    for dirpath, dirnames, filenames in os.walk(tampinha_root, followlinks=False):
        dp = Path(dirpath)
        rel_dir = dp.relative_to(tampinha_root)

        # Prune excluded directories in-place so os.walk doesn't descend
        # ``tampinha-agent`` is only pruned at the root level; nested dirs
        # with the same name (e.g. in skills/) must be preserved.
        is_root = rel_dir == Path(".")
        orig_dirnames = dirnames[:]
        dirnames[:] = [
            d for d in dirnames
            if d not in _EXCLUDED_DIRS or (d == "tampinha-agent" and not is_root)
        ]
        ...
        for fname in filenames:
            fpath = dp / fname
            rel = fpath.relative_to(tampinha_root)
    ...
    # outside TAMPINHA_HOME, so the walk above never sees it. Ask the active
    ...
    if external_to_add:
        print(
            ...
            f"stored outside {display_tampinha_home()}."
        )
    ...
    if not errors:
        print(f"\nRestore with: tampinha import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a Tampinha backup.
    ...
    # Look for telltale files that a tampinha home would have
    ...
    if not found:
        return False, (
            "zip does not appear to be a Tampinha backup "
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.tampinha/config.yaml` instead of `config.yaml`.
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a tampinha dir name
        if prefix in {".tampinha", "tampinha"}:
            return prefix + "/"
...
def _extract_member_atomically(
    ...
    with nothing behind it — during ``tampinha import``, which is the
    ...
    so without the chown a ``sudo tampinha import`` would silently re-own every
    ...
    if mode is None:
        ...
        # take over an existing setuid/setgid file, so ``tampinha import`` would
        # hand whoever produced the zip the identity that file runs as.  Nothing
        # constrains that to Tampinha' own state either: the ``_external/`` branch
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_tampinha_home()}")

        if prefix:
            print(f"Detected archive prefix: {prefix!r} (will be stripped)")

        # Check for existing installation
        has_config = (tampinha_root / "config.yaml").exists()
        has_env = (tampinha_root / ".env").exists()

        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has Tampinha configuration.")
        ...
        for member in members:
            ...
            # location (e.g. ~/.honcho/config.json), NOT under TAMPINHA_HOME.
            ...
            target = tampinha_root / rel

            # Security: reject absolute paths and traversals
            try:
                target.resolve().relative_to(tampinha_root.resolve())
            except ValueError:
        ...
        if restored_external:
            print(
                ...
                f"their original location(s) outside {display_tampinha_home()}."
            )
        ...
        profiles_dir = tampinha_root / "profiles"
        restored_profiles = []
        if profiles_dir.is_dir():
            try:
                ...
                # tampinha_cli.profiles might not be available (fresh install)
                if any(profiles_dir.iterdir()):
                    print("\n  Profiles detected but aliases could not be created.")
                    print("  Run: tampinha profile list  (after installing tampinha)")

        # Guidance
        print()
        if not (tampinha_root / "tampinha-agent").is_dir():
            print("Note: The tampinha-agent codebase was not included in the backup.")
            print("  If this is a fresh install, run: tampinha update")

        if restored_profiles:
            gw_profiles = [n for n, _ in restored_profiles]
            print("\nTo re-enable gateway services for profiles:")
            for pname in gw_profiles:
                print(f"  tampinha -p {pname} gateway install")
        ...
        try:
            from tampinha_cli.gateway import ensure_gateway_service, _is_service_running
        ...
        print("Done. Your Tampinha configuration has been restored.")


# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and tampinha backup --quick)
# ---------------------------------------------------------------------------

# Critical state files to include in quick snapshots (relative to TAMPINHA_HOME).
...
# — `tampinha update` snapshots this set before pulling so approved-user lists
...
_QUICK_STATE_FILES = (
    ...
    # are at $TAMPINHA_HOME/<name> for the default/root profile; on non-root
    # profiles the real path is outside TAMPINHA_HOME and the entry is silently
...
def _quick_snapshot_root(tampinha_home: Optional[Path] = None) -> Path:
...
def _create_quick_snapshot_locked(
    ...
    if failed_dbs:
        ...
        print(
            ...
            f"{root} and run: tampinha snapshot list"
        )
        logger.error(
...
def list_quick_snapshots(
    ...
    root = _quick_snapshot_root(tampinha_home)
    if not root.exists():
        return []
...
# Relative path of the cron job database inside TAMPINHA_HOME. Kept in sync with
...
def restore_cron_jobs_if_emptied(
    ...
    """Safety net for silent cron-job loss across ``tampinha update``.
    ...
    Args:
        ...
        tampinha_home: Override for the Tampinha home directory (tests).
...
def restore_cron_jobs_all_profiles(
    ...
    for name, snap_id in profile_snapshots.items():
        ...
        if profile_home is None:
            continue
        try:
            result = restore_cron_jobs_if_emptied(snap_id, tampinha_home=profile_home)
        except Exception as exc:
...
def prune_quick_snapshots(
    ...
    return _prune_quick_snapshots(_quick_snapshot_root(tampinha_home), keep=keep)


def run_quick_backup(args) -> None:
    """CLI entry point for tampinha backup --quick."""
    label = getattr(args, "label", None)
    snap_id = create_quick_snapshot(label=label)
    if snap_id:
        print(f"State snapshot created: {snap_id}")
        snaps = list_quick_snapshots()
        print(f"  {len(snaps)} snapshot(s) stored in {display_tampinha_home()}/state-snapshots/")
...
def _write_full_zip_backup(out_path: Path, tampinha_root: Path) -> Optional[Path]:
    """Single-flight wrapper for automatic full zip backups."""
    try:
        with _backup_operation_lock(tampinha_root):
            return _write_full_zip_backup_locked(out_path, tampinha_root)
    except BackupInProgressError as exc:
        logger.warning("Full-zip backup skipped: %s", exc)
        return None


def _write_full_zip_backup_locked(out_path: Path, tampinha_root: Path) -> Optional[Path]:
    """Write a full zip snapshot of ``tampinha_root`` to ``out_path``.
...
def _pre_update_backup_dir(tampinha_home: Optional[Path] = None) -> Path:
    home = tampinha_home or get_tampinha_home()
...
def create_pre_update_backup(
    ...
    """Create a full zip backup of TAMPINHA_HOME under ``backups/``.

    Mirrors :func:`run_backup` (same exclusion rules, same SQLite safe-copy)
    but writes to ``<TAMPINHA_HOME>/backups/pre-update-<timestamp>.zip`` and
    auto-prunes old pre-update backups.

    Returns the path to the created zip, or ``None`` if no files were
    found or the backup could not be created.  Never raises — the caller
    (``tampinha update``) should continue even if the backup fails.
    """
    tampinha_root = tampinha_home or get_default_tampinha_root()
    if not tampinha_root.is_dir():
        return None

    backup_dir = _pre_update_backup_dir(tampinha_root)
    try:
    ...
    result = _write_full_zip_backup(out_path, tampinha_root)
    if result is None:
        return None
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `tampinha claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of TAMPINHA_HOME under ``backups/`` before a
    ``tampinha claw migrate`` apply.

    Shares implementation with :func:`create_pre_update_backup` via
    ``_write_full_zip_backup`` — same exclusions, same SQLite safe-copy,
    restorable with ``tampinha import <archive>``.  Writes to
    ``<TAMPINHA_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
    ...
    # Reuses the shared backups/ directory so `tampinha import` and the
