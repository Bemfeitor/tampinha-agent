"""``tampinha logs`` — view and filter Tampinha log files.
...
Usage examples::
    ...
    tampinha logs                    # last 50 lines of agent.log
    tampinha logs -f                 # follow agent.log in real time
    tampinha logs errors             # last 50 lines of errors.log
    tampinha logs gateway -n 100    # last 100 lines of gateway.log
    tampinha logs gui -f            # follow gui.log (dashboard/pty/ws)
    tampinha logs desktop -f        # follow desktop.log (Electron app boot/backend)
    tampinha logs --level WARNING    # only WARNING+ lines
    tampinha logs --session abc123   # filter by session ID substring
    tampinha logs --component tools  # only tool-related lines
    tampinha logs --since 1h         # lines from the last hour
    tampinha logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_tampinha_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print("(Logs are created when Tampinha runs — try 'tampinha chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_tampinha_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_tampinha_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_tampinha_home()}/logs/")
        return

    print(f"Log files in {display_tampinha_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'tampinha chat' to generate logs)")
