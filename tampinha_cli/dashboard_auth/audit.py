...
Profile-aware location: ``$TAMPINHA_HOME/logs/dashboard-auth.log``.
...
from ``tampinha_constants`` or other tampinha_cli modules — so it can be
...
def _resolve_log_path() -> Path:
    """``$TAMPINHA_HOME/logs/dashboard-auth.log``.

    Uses ``tampinha_constants.get_tampinha_home()`` (a leaf module — no import
    ...
    return get_tampinha_home() / "logs" / "dashboard-auth.log"
