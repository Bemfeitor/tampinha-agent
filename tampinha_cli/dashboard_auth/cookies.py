...
Three cookies in play:
  - tampinha_session_at:   the OAuth access token
                         (HttpOnly, lifetime = token TTL, ~15 min)
  - tampinha_session_rt:   the OAuth refresh token
                         (HttpOnly, lifetime = 24h, ROTATING + reuse-detected)
                         Nous Portal issues a rotating refresh token for the
                         dashboard auth-code grant (Portal NAS #293 / tampinha
  ...
  - tampinha_session_pkce: short-lived PKCE state + CSRF nonce + provider
...
https://datatracker.ietf.org/doc/html/draft-west-cookie-prefixes):
  ...
  * Gated HTTPS, direct deploy (Path=/) — ``__Host-`` prefix. Binds the
    ...
    guarantee.
  * Gated HTTPS, behind a reverse-proxy prefix (Path=/tampinha) —
    ``__Secure-`` prefix. ``__Host-`` is disallowed when Path != "/";
    ``__Secure-`` keeps the Secure-required hardening without the
    Path constraint, and the explicit ``Path=/tampinha`` covers
...
setter wrote ``__Secure-tampinha_session_at`` would never find the value.
...
SESSION_AT_COOKIE = "tampinha_session_at"
SESSION_RT_COOKIE = "tampinha_session_rt"
...
SESSION_PROVIDER_COOKIE = "tampinha_session_provider"
PKCE_COOKIE = "tampinha_session_pkce"
...
SSO_ATTEMPT_COOKIE = "tampinha_sso_attempt"
...
def _cookie_path(prefix: str) -> str:
    ...
    Under ``X-Forwarded-Prefix: /tampinha`` we want ``Path=/tampinha`` so:
...
def set_session_cookies(
    ...
    *,
    ...
    issues a 24h rotating refresh token (tampinha #37247); a provider that
    ...
    ``prefix`` is the normalised X-Forwarded-Prefix value (e.g. ``/tampinha``)
