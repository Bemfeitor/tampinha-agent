...
Pure logic: walks a Tampinha config dict, returns issues for any reference
to a retired xAI model. No I/O, no CLI dependencies — testable in isolation
and reusable from both `tampinha doctor` and a future `tampinha migrate xai`.
"""
...
class RetirementIssue:
    """A reference to a retired xAI model found in a Tampinha config."""
...
def find_retired_xai_refs(config: Dict[str, Any]) -> List[RetirementIssue]:
    """Walk all model slots in a Tampinha config and return retirement issues.
...
def apply_migration(
    ...
    from tampinha_cli.config import require_readable_config_before_write
    ...
    # None``, so the next `tampinha migrate xai` reports nothing to migrate
