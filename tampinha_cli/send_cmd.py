"""CLI subcommand: ``tampinha send`` — pipe text from shell scripts to any
...
def _read_message_body(
    ...
    if file_path:
        ...
        try:
            ...
            print(
                f"tampinha send: {file_path} is not a text file. --file reads the "
                ...
                f'  tampinha send --to telegram "MEDIA:{file_path}"\n'
                f'  tampinha send --to telegram "optional caption MEDIA:{file_path}"\n'
                "Add [[as_document]] to deliver an image as an uncompressed file:\n"
                f'  tampinha send --to telegram "[[as_document]] MEDIA:{file_path}"',
                file=sys.stderr,
            )
            sys.exit(_USAGE_EXIT)
        except OSError as exc:
            print(f"tampinha send: cannot read {file_path}: {exc}", file=sys.stderr)
            sys.exit(_USAGE_EXIT)
    ...
    *,
    ...
    if json_mode:
        ...
        if payload.get("error"):
            print(f"tampinha send: {payload['error']}", file=sys.stderr)
...
def _list_targets(platform_filter: Optional[str], *, json_mode: bool) -> int:
    ...
    try:
        ...
        print(f"tampinha send: failed to load channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT

    try:
        raw = load_directory()
    except Exception as exc:
        print(f"tampinha send: failed to read channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT
    ...
    # setup used only for outbound `tampinha send`) would otherwise be
    ...
    if platform_filter:
        ...
        if not filtered:
            print(
                f"tampinha send: no targets found for platform '{platform_filter}'. "
    ...
    if not platforms:
        ...
        print("Set one up with `tampinha gateway setup`, or run the gateway once so")
        print("channel discovery can populate ~/.tampinha/channel_directory.json.")
        return _SUCCESS_EXIT
...
def _load_tampinha_env() -> None:
    """Populate ``os.environ`` from ``~/.tampinha/.env`` AND bridge top-level
    ...
    startup that ``tampinha send`` must replicate when invoked standalone:

    1. ``load_dotenv(~/.tampinha/.env)`` — brings bot tokens into the env.
    2. Bridge top-level simple values from ``~/.tampinha/config.yaml`` into
       ...
       via ``tampinha config set``.

    See ``gateway/run.py`` for the canonical version of this bridge — we
    intentionally reimplement the minimum needed here so ``tampinha send``
...
def cmd_send(args: argparse.Namespace) -> None:
    ...
    # Bridge ~/.tampinha/.env and ~/.tampinha/config.yaml into os.environ so the
    ...
    if not target:
        print(
            "tampinha send: --to PLATFORM[:channel[:thread]] is required\n"
            "Examples:\n"
            "  tampinha send --to telegram \"hello\"\n"
            "  tampinha send --to discord:#ops --file report.md\n"
            "  tampinha send --list      # list available targets",
            file=sys.stderr,
        )
        sys.exit(_USAGE_EXIT)
    ...
    if message is None or not message.strip():
        print(
            "tampinha send: no message provided. Pass text as a positional "
    ...
    # Import lazily so `tampinha send --help` stays fast and does not pull in
...
def register_send_subparser(subparsers) -> argparse.ArgumentParser:
    ...
    parser = subparsers.add_parser(
        ...
        description=(
            "Pipe text from any shell script to any messaging platform Tampinha "
            "is already configured for. Reuses the gateway's platform "
            "credentials (~/.tampinha/.env + ~/.tampinha/config.yaml) — no LLM, "
            "no agent loop, no running gateway required for bot-token "
            "platforms like Telegram/Discord/Slack/Signal."
        ),
        epilog=(
            "Examples:\n"
            "  tampinha send --to telegram \"deploy finished\"\n"
            "  echo \"RAM 92%\" | tampinha send --to telegram:-1001234567890\n"
            "  tampinha send --to discord:#ops --file /tmp/report.md\n"
            "  tampinha send --to slack:#eng --subject \"[CI]\" --file build.log\n"
            "  tampinha send --to telegram \"MEDIA:/tmp/chart.png\"   # send a media attachment\n"
            "  tampinha send --list                  # all platforms\n"
            "  tampinha send --list telegram         # filter by platform\n"
            "\n"
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="List available targets. Optional positional filter: `tampinha send --list telegram`.",
    )
