...
to the resolved absolute path of the client binary.  Tampinha' interpreter is
...
* Dynamically-linked builds look up ``libpython`` via
  ...
  no dylib — every tampinha command, including update/doctor, died in dyld
  (#95425).
...
from tampinha_cli.managed_uv import _RUNTIME_DIR_NAME
...
def _install_anchor(venv_dir: Path, source_file: Path) -> None:
    ...
    try:
        ...
        try:
            from tampinha_cli.managed_uv import _macos_sign_managed_python
...
def tcc_anchor_state(project_root: Path | None = None) -> tuple[str, str]:
    """Report the anchor state for ``tampinha doctor``.
