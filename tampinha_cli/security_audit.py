"""On-demand supply-chain audit for Tampinha Agent installs.

Scans three surfaces a Tampinha user actually controls and we can map to
upstream advisories without auth or extra binaries:

1. The Tampinha venv (every PyPI dist via ``importlib.metadata``).
2. Python deps declared by user-installed plugins under ``~/.tampinha/plugins``
...
def _discover_plugins(tampinha_home: Path) -> list[Component]:
    """Python deps declared by plugins under ``~/.tampinha/plugins``.
...
def _discover_components(
    *,
    ...
    home = tampinha_home or Path(get_tampinha_home())
...
def cmd_security_audit(args: argparse.Namespace) -> int:
    """Implementation of `tampinha security audit`."""
    ...
    components = _discover_components(
        skip_venv=skip_venv, skip_plugins=skip_plugins, skip_mcp=skip_mcp, tampinha_home=home
    )
