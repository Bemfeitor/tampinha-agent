...
Used by ``tampinha kanban specify [task_id | --all]``. Takes a task that
...
* This module intentionally mirrors ``tampinha_cli/goals.py`` — same aux
...
TAMPINHA_KANBAN_SPECIFY_MAX_TOKENS = max(
    1500,
    env_int("TAMPINHA_KANBAN_SPECIFY_MAX_TOKENS", 6000),
)

logger = logging.getLogger(__name__)


_SYSTEM_PROMPT = """You are the Kanban triage specifier for the Tampinha Agent board.
...
def _profile_author() -> str:
    """Mirror of ``tampinha_cli.kanban._profile_author``. Kept local to
    avoid a circular import when kanban.py imports this module."""
    return (
        os.environ.get("TAMPINHA_PROFILE")
    ...
    *,
    ...
    try:
        ...
        resp = call_llm(
            ...
            temperature=0.3,
            max_tokens=TAMPINHA_KANBAN_SPECIFY_MAX_TOKENS,
