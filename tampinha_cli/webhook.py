"""tampinha webhook — manage dynamic webhook subscriptions from the CLI.

Usage:
    tampinha webhook subscribe <name> [options]
    tampinha webhook list
    tampinha webhook remove <name>
    tampinha webhook test <name> [--payload '{"key": "value"}']

Subscriptions persist to ~/.tampinha/webhook_subscriptions.json and are
...
def _subscriptions_path() -> Path:
    return _tampinha_home() / _SUBSCRIPTIONS_FILENAME
...
def _setup_hint() -> str:
    _dhh = display_tampinha_home()
    return f"""
  Webhook platform is not enabled. To set it up:

  1. Run the gateway setup wizard:
     tampinha gateway setup
  ...
  Then start the gateway: tampinha gateway run
"""
...
def webhook_command(args):
    """Entry point for 'tampinha webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: tampinha webhook {subscribe|list|remove|test}")
        print("Run 'tampinha webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print("  The gateway must be running to receive events (tampinha gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: tampinha webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from tampinha webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (tampinha gateway run)")
