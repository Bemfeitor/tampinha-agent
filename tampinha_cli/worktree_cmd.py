"""``tampinha worktree`` — audit and reclaim accumulated git worktrees/branches.

Attended counterpart of the silent startup pruner (see
``tampinha_cli/worktree_gc.py`` for the policy and shared invariants). Usage:

    tampinha worktree list             # audit: verdict + reason per tree
    tampinha worktree prune            # reap safe trees + merged branches
    tampinha worktree prune --dry-run  # show the plan, change nothing
    tampinha worktree prune --trees-only / --branches-only
"""
...
def cmd_worktree(args) -> int:
    ...
    if action == "list":
        ...
        print(
            ...
            f"{_fmt_size(reapable_mb)} reclaimable now via `tampinha worktree prune`."
        )
