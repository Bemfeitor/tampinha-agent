...
def _is_tampinha_gateway_run_argv(command: Sequence[str]) -> bool:
    """True for Tampinha ``gateway run`` argv this wrapper is allowed to upgrade.

    The wrapper is generic. Only historical/current Tampinha gateway shapes
...
def _prepare_child_command(
    ...
    ``--external-supervisor`` on the inner ``gateway run`` so ``tampinha update``
    ...
    if not _is_launchd_supervised(environ):
        return argv
    if not _is_tampinha_gateway_run_argv(argv):
        return argv
