"""Image-authored deployment provenance for immutable Tampinha runtimes.

The published image bakes ``/etc/tampinha/image-provenance.json`` outside both
``$TAMPINHA_HOME`` and the mutable checkout.  A bind-mounted checkout (including
...
IMAGE_PROVENANCE_PATH = Path("/etc/tampinha/image-provenance.json")
