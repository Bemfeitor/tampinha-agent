"""Tampinha skin/theme engine — the theme SDK for every surface.

A data-driven skin system that lets users (and Tampinha itself) customize the
visual appearance across the CLI, the TUI, and the desktop GUI from a single
file. Skins are defined as YAML files in ~/.tampinha/skins/ or as built-in presets.
...
``resolve_skin`` / ``skin.changed``). A skin dropped in ~/.tampinha/skins/ therefore
...
.. code-block:: yaml
    ...
    branding:
      agent_name: "Tampinha Agent"          # Banner title, status display
      welcome: "Welcome message"          # Shown at CLI startup
      goodbye: "Goodbye! ⚕"              # Shown on exit
      response_label: " ⚕ Tampinha "       # Response box header label
...
.. code-block:: python
    ...
    from tampinha_cli.skin_engine import get_active_skin, list_skins, set_active_skin

    skin = get_active_skin()
    print(skin.colors["banner_title"])    # "#FFD700"
    print(skin.get_branding("agent_name"))  # "Tampinha Agent"

    set_active_skin("ares")               # Switch to built-in ares skin
    set_active_skin("mytheme")            # Switch to user skin from ~/.tampinha/skins/

BUILT-IN SKINS
==============

- ``default`` — Classic Tampinha gold/kawaii (the current look)
...
Drop a YAML file in ``~/.tampinha/skins/<name>.yaml`` following the schema above.
...
class SkinConfig:
    ...
    banner_logo: str = ""    # Rich-markup ASCII art logo (replaces TAMPINHA_AGENT_LOGO)
    banner_hero: str = ""    # Rich-markup hero art (replaces TAMPINHA_CADUCEUS)
...
_BUILTIN_SKINS: Dict[str, Dict[str, Any]] = {
    "default": {
        "name": "default",
        "description": "Classic Tampinha — gold and kawaii",
        # Dark-authored. Values match the TUI's DARK_THEME so the classic CLI
        # and the TUI render the same Tampinha gold.
        "colors": {
        ...
        "branding": {
            "agent_name": "Tampinha Agent",
            "welcome": "Welcome to Tampinha Agent! Type your message or /help for commands.",
            "goodbye": "Goodbye! ⚕",
            "response_label": " ⚕ Tampinha ",
            "prompt_symbol": "❯",
...
def _skins_dir() -> Path:
    ...
    return get_tampinha_home() / "skins"
