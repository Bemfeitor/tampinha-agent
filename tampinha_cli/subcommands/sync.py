"""``tampinha sync`` subcommand parser — Skill Sync.

Cloned from ``tampinha_cli/subcommands/cron.py`` — same injected-handler shape
...
Skill Sync covers two surfaces, both under this one command for launch:
  ...
  Personal — your own skills, across your own devices:
    tampinha sync status                 show gate/opt-in/head state
    tampinha sync pull                   pull and materialize opted-in skills
    tampinha sync push                   push opted-in skills
    tampinha sync now                    reconcile: pull then push
    tampinha sync enable <skill>         opt a skill into sync
    tampinha sync disable <skill>        opt a skill out of sync
    tampinha sync device [--name]        show or set this device's label

  Organisation — skills shared with your team:
    tampinha sync propose <skill>        share a skill with your organisation
...
def build_sync_parser(subparsers, *, cmd_sync: Callable) -> None:
    ...
    sync_parser = subparsers.add_parser(
        ...
        epilog=(
            "Examples:\n"
            "  tampinha sync status            what is synced, and from where\n"
            "  tampinha sync enable my-skill   include a skill in your sync\n"
            "  tampinha sync now               pull, then push\n"
            "  tampinha sync propose my-skill  share a skill with your team\n"
        ),
