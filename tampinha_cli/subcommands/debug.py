"""``tampinha debug`` subcommand parser.
...
def build_debug_parser(subparsers, *, cmd_debug: Callable) -> None:
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Tampinha Agent. Use 'tampinha debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    tampinha debug share              Upload debug report (asks for confirmation)
    tampinha debug share --yes        Skip confirmation (for scripts/CI)
    tampinha debug share --lines 500  Include more log lines
    tampinha debug share --expire 30  Keep paste for 30 days
    tampinha debug share --local      Print report locally (no upload)
    tampinha debug share --no-redact  Disable upload-time secret redaction
    tampinha debug share --nous       Upload to Nous-internal storage (private)
    tampinha debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'tampinha debug share'",
    )
