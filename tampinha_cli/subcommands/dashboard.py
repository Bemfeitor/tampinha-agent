"""``tampinha dashboard`` / ``tampinha serve`` subcommand parsers.
...
``tampinha_cli/main.py:main()`` (god-file Phase 2); handler injected to avoid
importing ``main``.
"""
...
def _add_server_runtime_args(parser) -> None:
    ...
    # `tampinha dashboard` / `tampinha serve` cmdlines and SIGTERM them directly —
    # the same path `tampinha update` uses to clean up stale servers.
    parser.add_argument(
        "--stop",
        action="store_true",
        help="Stop all running Tampinha web server processes and exit",
    )
    parser.add_argument(
        "--status",
        action="store_true",
        help="List running Tampinha web server processes and exit",
    )
...
def build_dashboard_parser(
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the Tampinha Agent web dashboard for managing config, API keys, and sessions",
    )
    _add_server_runtime_args(dashboard_parser)
    dashboard_parser.add_argument(
        "--no-open", action="store_true", help="Don't open browser automatically"
    )
    # Backward-compat shim: older Tampinha desktop app shells (<= 0.15.x) spawn the
    # backend as `tampinha dashboard --no-open --tui --host ... --port ...`. The
    ...
    # showed "Tampinha couldn't start" with no actionable cause. Accept and silently
    ...
    # browser. It exists so the Tampinha Desktop app (and headless remote
    ...
    serve_parser = subparsers.add_parser(
        "serve",
        help="Start the Tampinha backend server (headless; powers the desktop app and remote backends)",
        description=(
            "Run the Tampinha backend server — the JSON-RPC/WebSocket gateway the "
    ...
    # (cmd_dashboard exports TAMPINHA_SERVE_HEADLESS=1). `dashboard` leaves it
    # unset and serves the browser UI as before.
    serve_parser.set_defaults(func=cmd_dashboard, no_open=True, headless_backend=True)

    # `tampinha dashboard register` — register a self-hosted dashboard OAuth
    # client with Nous Portal and write the client_id into ~/.tampinha/.env.
    # Nested subparser so bare `tampinha dashboard` keeps launching the server
    ...
    dashboard_register_parser = dashboard_subparsers.add_parser(
        ...
        description=(
            ...
            "TAMPINHA_DASHBOARD_OAUTH_CLIENT_ID into ~/.tampinha/.env, and prints "
            "how to engage the login gate. Requires being logged in (tampinha setup)."
        ),
    )
    ...
    dashboard_register_parser.add_argument(
        ...
        help=(
            ...
            "portal. Also settable via TAMPINHA_DASHBOARD_PORTAL_URL. Mainly for "
