"""``tampinha pause`` / ``tampinha resume`` — the global emergency stop.

``tampinha pause`` writes the ESTOP sentinel at ``$TAMPINHA_HOME/ESTOP``, which
halts cron dispatch, kanban dispatch, and new gateway turns on their next
check. In-flight work is never killed. ``tampinha resume`` removes the
...
def cmd_pause(args: argparse.Namespace) -> int:
    ...
    state = get_state() or {}
    verb = "Still paused" if already else "Tampinha paused"
    ...
    print(
        ...
        "    In-flight work keeps running. Run `tampinha resume` to lift the pause."
    )
    return 0
...
def cmd_resume(args: argparse.Namespace) -> int:
    ...
    if disengage():
        print("▶️  Tampinha resumed — dispatch picks up on the next tick.")
    else:
        print(f"Tampinha is not paused (no sentinel at {sentinel_path()}).")
    return 0
...
def build_pause_parser(subparsers) -> None:
    ...
    pause_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "`tampinha resume`. In-flight work is never killed."
        ),
    )
    ...
    resume_parser = subparsers.add_parser(
        "resume",
        help="Lift the emergency stop set by `tampinha pause`",
