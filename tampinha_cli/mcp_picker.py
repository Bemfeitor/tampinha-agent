"""MCP picker — interactive `tampinha mcp picker` (also the default `tampinha mcp`).

Lists every catalog entry plus any custom MCP servers the user has added via
``tampinha mcp add``, lets them pick one, and routes to install / enable /
disable / uninstall / configure-tools flows.

Mirrors the `tampinha plugin` picker UX: arrow keys to navigate, ENTER on a row
...
from tampinha_cli.cli_output import prompt_yes_no
from tampinha_cli.curses_ui import curses_single_select
from tampinha_cli.mcp_catalog import (
    CatalogEntry,
    CatalogError,
    catalog_diagnostics,
    install_entry,
...
def _enable_disable(name: str, *, enable: bool) -> None:
    ...
    print(color(
        ...
        "Start a new Tampinha session for changes to take effect.",
        Colors.GREEN,
    ))
...
def _configure_tools(name: str) -> None:
    ...
    """
    import argparse
    from tampinha_cli.mcp_config import cmd_mcp_configure
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    as the default output of `tampinha mcp catalog`."""
    if not rows:
        print()
    ...
    print()
    print(color(
        "  Install: tampinha mcp install <name>    Picker: tampinha mcp",
        Colors.DIM,
    ))

    # Surface manifest-version warnings so users know when their Tampinha is
    ...
    if future:
        ...
        for name, _, msg in future:
            print(color(
                f"  ⚠ '{name}' requires a newer Tampinha — run `tampinha update` "
...
def show_catalog() -> None:
    """`tampinha mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`tampinha mcp picker` (and default `tampinha mcp`) — interactive selector.
...
def install_by_name(identifier: str) -> int:
    """`tampinha mcp install <name>` — non-interactive entry-point.
    ...
    if entry is None:
        print(color(
            ...
            "Run `tampinha mcp catalog` to see available entries.",
            Colors.RED,
        ))
        return 1
    try:
