"""Profile distributions — shareable, packaged Tampinha profiles via git.

A distribution is a Tampinha profile published as a git repository (or
...
* ``tampinha profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``tampinha skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``tampinha profile``, not a parallel tree):

    tampinha profile install <source> [--name N] [--alias] [--force] [--yes]
    tampinha profile update  <name>  [--force-config] [--yes]
    tampinha profile info    <name>
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "tampinha_state.db", "response_store.db",
    "response_store.db-shm", "response_store.db-wal",
    "gateway.pid", "gateway_state.json", "processes.json",
    "auth.lock", "active_profile", ".update_check",
    "errors.log", ".tampinha_history",
    # User data
    ...
    "backups", "cache",
    # Infrastructure
    "tampinha-agent", ".worktrees", "profiles", "bin", "node_modules",
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            tampinha_requires=str(data.get("tampinha_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.tampinha_requires:
            out["tampinha_requires"] = self.tampinha_requires
        if self.author:
...
def check_tampinha_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires Tampinha {op}{target}, "
...
def _env_template_from_manifest(manifest: DistributionManifest) -> str:
    ...
    lines = [
        "# Environment variables required by this Tampinha distribution.",
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``tampinha profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a Tampinha profile distribution."
            )
...
def plan_install(
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a Tampinha distribution."
        )

    # Version check up-front so we fail fast
    check_tampinha_requires(manifest.tampinha_requires, tampinha_version)
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.tampinha).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
...
def install_distribution(
    ...
    with tempfile.TemporaryDirectory(prefix="tampinha_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `tampinha profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`tampinha profile install <source> --name {canon} --force`."
        )

    with tempfile.TemporaryDirectory(prefix="tampinha_dist_update_") as tmp:
        plan = plan_install(
