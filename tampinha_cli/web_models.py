"""Pydantic request/response models for the Tampinha dashboard web server.

Extracted verbatim from ``tampinha_cli/web_server.py`` (pure schema move).
``web_server`` re-exports every name here, so existing imports like
``from tampinha_cli.web_server import ConfigUpdate`` keep working.
"""
...
class ModelAssignment(BaseModel):
    ...
    # ``tampinha model`` custom flow collects.
    api_key: str = ""
...
class ImportRequest(BaseModel):
    archive: str
    # Pass --force to `tampinha import`. The spawned action runs with
...
class ProfileCreate(BaseModel):
    ...
    # via a subprocess scoped to the profile (`tampinha -p <name> skills install`)
    # because skills_hub.SKILLS_DIR is import-time-bound and the TAMPINHA_HOME
...
class ProfileExport(BaseModel):
    ...
    # Where to write the archive. Empty → a staging path under TAMPINHA_HOME.
    output: str = ""
