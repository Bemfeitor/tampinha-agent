...
The Tampinha docs site hosts a JSON manifest of curated models for providers
...
1. ``get_catalog()`` — returns a parsed manifest dict.
   ...
   - Reads disk cache at ``~/.tampinha/cache/model_catalog.json``.
...
def _cache_path() -> Path:
    ...
    return get_tampinha_home() / "cache" / "model_catalog.json"
...
def get_default_model_from_cache(provider: str) -> str | None:
    ...
    ``"default": true``; that entry is the model Tampinha silently lands on when
    ...
    network-free. The cache is kept fresh by the picker/`tampinha update` paths;
...
def seed_cache_from_checkout(project_root: "Path | str") -> bool:
    ...
    ``tampinha update`` pulls the latest repo, so the freshly-pulled
    ...
    to ``~/.tampinha/cache/model_catalog.json`` via the same atomic writer the
...
def reset_cache() -> None:
    """Clear the in-process cache. Used by tests and ``tampinha model --refresh``."""
    global _catalog_cache, _catalog_cache_source_mtime
    _catalog_cache = None
