"""Dependency-light venv recovery that runs BEFORE tampinha_cli.main's imports.

The ``tampinha`` console entry point is ``tampinha_cli.main:main``.  Importing
``tampinha_cli.main`` pulls in third-party packages at module level (``dotenv``
via ``tampinha_cli.env_loader``, ``yaml`` via ``tampinha_cli.config``, ...).  In
...
a corrupted venv.  ``tampinha_cli.main`` imports and calls
:func:`recover_if_needed` at the very top of its module body, before any
third-party import.

Scope: this early pass only repairs enough for ``tampinha_cli.main`` to become
...
#
# ``tampinha update`` renames the live ``tampinha*.exe`` shims aside
# (``tampinha.exe.old.<unix-ms>``) so uv can write replacements. Putting them BACK
# is the safety-critical direction: losing that rename leaves the install with
# no ``tampinha`` on PATH, and the command that would repair it IS ``tampinha
# update`` (#75584).
#
...
def restore_quarantined_shims(
    ...
    *,
    ...
    Messages go to stderr by default -- the startup sweep runs on EVERY tampinha
    invocation, and ``tampinha acp`` speaks JSON-RPC on stdout.
    """
    if stream is None:
        stream = sys.stderr
...
def _find_uv_binary() -> str | None:
    ...
    the only sanctioned installer is uv itself, which Tampinha already vendors
    (``~/.tampinha/bin/uv.exe``) or the user has on PATH.  Stdlib-only.
    """
    exe = "uv.exe" if sys.platform == "win32" else "uv"
    candidates = [
        Path.home() / ".tampinha" / "bin" / exe,
...
def _run_repair_install(specs: list[str], project_root: Path) -> bool:
    ...
    Streams nothing to stdout (``tampinha acp`` speaks JSON-RPC on stdout);
...
def _pytest_owns_live_checkout(root: Path) -> bool:
    ...
    Lifecycle tests spawn real subprocesses that import ``tampinha_cli.main``
...
def recover_if_needed(
    ...
    """Repair wiped core packages so ``tampinha_cli.main`` can import at all.

    Fast path (no marker present) is two ``lstat`` calls.  Only acts when a
    recovery marker from a prior ``tampinha update`` exists AND an import probe
    ...
    try:
        ...
        # launch is itself `tampinha update`: CLI and Desktop retries preserve
...
# every `tampinha` launch: past this many attempts the early pass hands the
...
def _complete_pending_core_install(root: Path, core_marker: Path) -> bool:
    ...
    a prior ``tampinha update`` (or the self-lock preflight, #83569) left the
    ...
    ``tampinha_cli.main``'s third-party imports, maps nothing yet, so the
    ...
    failure does not block every launch (``tampinha acp`` included).
    ...
    """
    try:
        from tampinha_cli import _install_repair as ir
        ...
        try:
            ...
            print(
                "  The next launch will retry; tampinha will keep working from "
