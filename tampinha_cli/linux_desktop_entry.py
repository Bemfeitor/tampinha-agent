"""Install and remove the Linux desktop entry (``tampinha.desktop``).

``tampinha desktop`` builds and launches the Electron app. On Linux, a
...
``tampinha uninstall --gui`` removes the entry again.

Two values must be absolute for the entry to work:

  - ``Exec`` — the launcher runs without shell ``PATH`` customizations, so
    a bare ``tampinha desktop`` fails when tampinha lives in ``~/.local/bin``
...
DESKTOP_ENTRY_NAME = "tampinha.desktop"
...
def desktop_entry_path() -> Path:
    """Where the ``tampinha.desktop`` entry lives."""
...
def resolve_exec_command() -> str:
    """Build the absolute ``Exec=`` command line for ``tampinha desktop``.

    Prefer the real ``tampinha`` executable (argv[0] or PATH). When Tampinha
    runs as a module with no launcher installed, use the current
    interpreter, also absolute.
    """
    from tampinha_cli.relaunch import resolve_tampinha_bin
    ...
    if bin_path:
        ...
        if _needs_interpreter(resolved):
            ...
            # a NON-venv interpreter (e.g. the repo's `tampinha` script with
            ...
            # sys.executable is the interpreter actually running Tampinha (the
            # venv one), so prefix it explicitly.
            argv = [str(Path(sys.executable).resolve()), str(resolved), "desktop"]
        else:
            argv = [str(resolved), "desktop"]
    else:
        argv = [str(Path(sys.executable).resolve()), "-m", "tampinha_cli.main", "desktop"]
    return " ".join(_quote_exec_arg(a) for a in argv)


def _needs_interpreter(bin_path: Path) -> bool:
    """Whether ``bin_path`` is a Python script that must run under
    ``sys.executable`` to see Tampinha' venv (rather than its own shebang)."""
    try:
...
def render_desktop_entry(exec_command: str, icon: str) -> str:
    return (
        ...
        "Type=Application\n"
        "Name=Tampinha\n"
        "GenericName=Tampinha Desktop\n"
        "Comment=Launch Tampinha Desktop\n"
        f"Exec={exec_command}\n"
        f"Icon={icon}\n"
        "Terminal=false\n"
        "Categories=Utility;\n"
        "StartupNotify=true\n"
        "StartupWMClass=Tampinha\n"
    )
...
def install_desktop_entry(project_root: Path) -> Optional[Path]:
    """Write (or refresh) the Tampinha desktop entry. Return its path.
    ...
    icon_value = str(icon) if icon.is_file() else "tampinha"
