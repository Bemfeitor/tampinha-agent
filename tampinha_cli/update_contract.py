...
``tampinha update`` mutation (CLI apply, CLI --check, dashboard update
endpoint). The decision layers:

1. **Baked provenance marker** (``/etc/tampinha/image-provenance.json``,
   written by the image build — see :mod:`tampinha_cli.image_provenance`):
