"""Safe Tampinha Console command engine.

This module backs ``tampinha console`` and is intentionally narrower than the
full Tampinha CLI. It exposes a curated set of native adapters that can later be
...
def _strip_console_status_footer(text: str) -> str:
    ...
    if not (
        prev.startswith("Run 'tampinha doctor'")
        and last.startswith("Run 'tampinha setup'")
    ):
        return text.rstrip()
...
def _split_line(line: str) -> list[str]:
    try:
        ...
        from tampinha_cli._subprocess_compat import split_command_line
...
def _parser_root() -> tuple[_ArgumentParser, argparse._SubParsersAction]:
    parser = _ArgumentParser(prog="tampinha", add_help=False)
...
def _clean_summary(text: str | None) -> str:
    ...
    if not summary:
        return ""
    if summary.startswith("Run `tampinha "):
        return ""
    return summary
...
def _dispatch_extracted_subcommand(
    *,
    ...
    parser, subparsers = _parser_root()
    module = importlib.import_module(module_name)
    main_module = importlib.import_module("tampinha_cli.main")
    builder = getattr(module, builder_name)
...
def _register_command_family(
    ...
    *,
    ...
    for child_path in paths:
        ...
        command_summary = summary or (summaries or {}).get(full_path) or f"Run `tampinha {usage}`."
        engine.register(
            full_path,
            usage,
            command_summary,
            ...
            confirmation=confirmation or f"Run `tampinha {usage}`?",
        )


class HermesConsoleEngine:
    """Curated line-command executor for Tampinha Console."""
    ...
    def execute(self, line: str, *, confirmed: bool = False) -> ConsoleResult:
        ...
        try:
            ...
            if tokens and tokens[0] == "tampinha":
                tokens = tokens[1:]
            if not tokens:
            ...
            if _contains_shell_syntax(raw_line, tokens):
                raise ConsoleCommandError(
                    "Tampinha Console does not run shell syntax. Use one supported "
                    "Tampinha command at a time."
                )
    ...
    def _register_defaults(self) -> None:
        self.register(("status",), "status", "Show Tampinha component status.", _status)
        self.register(("version",), "version", "Show Tampinha version information.", _version)
        self.register(("doctor",), "doctor", "Run diagnostics without auto-fix.", _doctor)
        self.register(("logs",), "logs [name] [-n N]", "Show recent Tampinha logs.", _logs)
        ...
        self.register(
            ...
            _config_set,
            mutating=True,
            confirmation="Update Tampinha configuration?",
        )
    ...
    def _register_broad_cli_surface(self) -> None:
        """Register non-admin CLI commands that are safe for Tampinha Console."""

        extracted = {
            "dump": (
                "tampinha_cli.subcommands.dump",
                "build_dump_parser",
                "cmd_dump",
                [()],
                set(),
            ...
            "debug": (
                "tampinha_cli.subcommands.debug",
                "build_debug_parser",
                "cmd_debug",
                [("share",), ("delete",)],
                {("share",), ("delete",)},
            ...
            "prompt-size": (
                "tampinha_cli.subcommands.prompt_size",
            ...
            "insights": (
                "tampinha_cli.subcommands.insights",
            ...
            "security": (
                "tampinha_cli.subcommands.security",
            ...
            "backup": (
                "tampinha_cli.subcommands.backup",
                "build_backup_parser",
                "cmd_backup",
                [()],
                {()},
            ...
            "import": (
                "tampinha_cli.subcommands.import_cmd",
            ...
            "config": (
                "tampinha_cli.subcommands.config",
                "build_config_parser",
                "cmd_config",
                [("env-path",), ("check",)],
                set(),
            ...
            "tools": (
                "tampinha_cli.subcommands.tools",
                "build_tools_parser",
                "cmd_tools",
            ...
            "plugins": (
                "tampinha_cli.subcommands.plugins",
            ...
            "skills": (
                "tampinha_cli.subcommands.skills",
                "build_skills_parser",
                "cmd_skills",
                [
                    ("browse",),
            ...
            "memory": (
                "tampinha_cli.subcommands.memory",
                "build_memory_parser",
                "cmd_memory",
                [("status",), ("off",), ("reset",)],
                {("off",), ("reset",)},
            ...
            "pairing": (
                "tampinha_cli.subcommands.pairing",
            ...
            "webhook": (
                "tampinha_cli.subcommands.webhook",
            ...
            "hooks": (
                "tampinha_cli.subcommands.hooks",
                "build_hooks_parser",
                "cmd_hooks",
            ...
            "slack": (
                "tampinha_cli.subcommands.slack",
                "build_slack_parser",
                "cmd_slack",
                [("manifest",)],
                set(),
            ...
            "profile": (
                "tampinha_cli.subcommands.profile",
        ...
        self.register(
            ...
            _config_migrate,
            mutating=True,
            confirmation="Update Tampinha configuration with missing defaults?",
        )
        self.register(
            ("sessions", "export"),
        ...
        self.register(
            ...
            _adder_handler("send", (), "tampinha_cli.send_cmd", "register_send_subparser"),
            mutating=True,
        ...
        _register_command_family(
            ...
            root="portal",
            paths=portal_paths,
            summaries=_adder_summaries("tampinha_cli.portal_cli", "add_parser"),
            handler_factory=lambda fixed: _adder_handler(
                "portal",
                fixed,
                "tampinha_cli.portal_cli",
                "add_parser",
            ),
        )
        ...
        _register_command_family(
            ...
            summaries=_builder_summaries("tampinha_cli.projects_cmd", "build_parser"),
            mutating=[
                ("create",),
                ("add-folder",),
                ("remove-folder",),
        ...
        _register_command_family(
            ...
            summaries=_builder_summaries("tampinha_cli.kanban", "build_parser"),
            mutating=[
                ("init",),
                ("boards", "create"),
                ("boards", "rm"),
    ...
    def _resolve_command(self, tokens: Sequence[str]) -> tuple[ConsoleCommand, list[str]]:
        ...
        raise ConsoleCommandError(f"Unsupported Tampinha Console command: {probe}.{suffix}")

    def _rejection_for(self, tokens: Sequence[str]) -> str:
        first = tokens[0]
        if first.startswith("-"):
            return f"{first} is not available in Tampinha Console."
        blocked_top = {
            "acp",
            "chat",
            "claw",
        ...
        if first in blocked_top:
            return f"`tampinha {first}` is not available in Tampinha Console."
        blocked_pairs = {
            ("config", "edit"): "`config edit` opens an editor and is not available in Tampinha Console.",
            ("mcp", "serve"): "`mcp serve` starts a server and is not available in Tampinha Console.",
            ("profile", "alias"): "`profile alias` creates shell wrappers and is not available in Tampinha Console.",
            ("skills", "config"): "`skills config` is interactive and is not available in Tampinha Console.",
            ("skills", "publish"): "`skills publish` is not available in Tampinha Console.",
            ("portal", "login"): "`portal login` is interactive and is not available in Tampinha Console.",
            ("portal", "open"): "`portal open` opens a browser and is not available in Tampinha Console.",
            ("kanban", "tail"): "`kanban tail` streams output and is not available in Tampinha Console.",
            ("kanban", "watch"): "`kanban watch` streams output and is not available in Tampinha Console.",
            ("kanban", "daemon"): "`kanban daemon` starts a service and is not available in Tampinha Console.",
            ("kanban", "dispatcher"): "`kanban dispatcher` starts a worker and is not available in Tampinha Console.",
            ("kanban", "swarm"): "`kanban swarm` starts agent work and is not available in Tampinha Console.",
            ("kanban", "decompose"): "`kanban decompose` starts agent work and is not available in Tampinha Console.",
            ("kanban", "specify"): "`kanban specify` starts agent work and is not available in Tampinha Console.",
            ("kanban", "gc"): "`kanban gc` is not available in Tampinha Console.",
        }
        if len(tokens) >= 2:
        ...
        if tuple(tokens[:2]) in {("sessions", "delete"), ("sessions", "prune")}:
            return "`sessions delete` and `sessions prune` are not available in Tampinha Console."
        return ""
...
def _apply_confirmed_defaults(args: argparse.Namespace) -> None:
    ...
    if getattr(args, "auth_action", None) == "add":
        ...
        if auth_type in {"api-key", "api_key"} and not getattr(args, "api_key", None):
            raise ConsoleCommandError("auth add --type api-key requires --api-key in Tampinha Console.")
...
def _version(_engine: HermesConsoleEngine, args: list[str]) -> str:
    ...
    from tampinha_cli._startup_fast import print_fast_version_info
...
def _logs(_engine: HermesConsoleEngine, args: list[str]) -> str:
    if "-f" in args or "--follow" in args:
        raise ConsoleCommandError("`logs -f` is not available in Tampinha Console.")
    ...
    from tampinha_cli.logs import list_logs, tail_log
...
def _config_show(_engine: HermesConsoleEngine, args: list[str]) -> str:
    ...
    from tampinha_cli.config import show_config
...
def _config_path(_engine: HermesConsoleEngine, args: list[str]) -> str:
    ...
    from tampinha_cli.config import get_config_path
...
def _sessions_repair(_engine: HermesConsoleEngine, args: list[str]) -> str:
    ...
    def _run() -> None:
        from tampinha_state import DEFAULT_DB_PATH, _db_opens_cleanly, repair_state_db_schema
...
def _profile_status(_engine: HermesConsoleEngine, args: list[str]) -> str:
    ...
    return _dispatch_extracted_subcommand(
        ...
        fixed=(),
        args=(),
        module_name="tampinha_cli.subcommands.profile",
...
def _cron_list(_engine: HermesConsoleEngine, args: list[str]) -> str:
    ...
    ns = parser.parse_args(args)
    from tampinha_cli.cron import cron_list
...
def _cron_status(_engine: HermesConsoleEngine, args: list[str]) -> str:
    ...
    from tampinha_cli.cron import cron_status
...
def _cron_pause(_engine: HermesConsoleEngine, args: list[str]) -> str:
    ...
    try:
        job = pause_job(args[0], reason="paused from tampinha console")
...
def _cron_resume(_engine: HermesConsoleEngine, args: list[str]) -> str:
    ...
    from cron.jobs import AmbiguousJobReference, _tampinha_now, rearm_oneshot, resume_job

    try:
        if ns.at or ns.run_now:
            job = rearm_oneshot(ns.job, _tampinha_now().isoformat() if ns.run_now else ns.at)
        else:
...
def run_console_repl(
    *,
    ...
    """Run the local ``tampinha console`` REPL."""
    ...
    engine = HermesConsoleEngine()
    if interactive:
        print("Tampinha Console. Type `help` for commands, `exit` to quit.", file=stdout)

    while True:
        if interactive:
            print("tampinha> ", end="", file=stdout, flush=True)
        line = stdin.readline()
        if line == "":
            if interactive:
                print(file=stdout)
