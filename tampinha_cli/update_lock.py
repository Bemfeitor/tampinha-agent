"""Cross-process mutual exclusion for in-flight Tampinha updates.

Three different surfaces can start an update of the same install tree:

* ``tampinha update`` from a terminal,
* the dashboard's Update button (``POST /api/tampinha/update`` →
  ``_spawn_tampinha_action(["update"])``, detached),
* the desktop's Update button, which hands off to the Tauri
  ``tampinha-setup --update`` and, on its failure screen, to install-mode
...
once — so a dashboard-spawned ``tampinha update`` and an installer-driven
...
unchanged and remain byte-compatible with the Rust and Electron readers:
    ...
    <TAMPINHA_HOME>/.tampinha-update-in-progress   body: "<pid>\\n<started_at_unix>"
...
then spawns ``tampinha update`` as a child stage. Without a handoff the child
sees its own parent's live marker and refuses — the GUI update deadlocks
against itself on every attempt ("Tampinha is still running", retry forever).
...
* A live holder that is a *process ancestor* of ours is likewise our own
  ...
  ``tampinha-setup`` binary under ``~/.tampinha`` is only refreshed by a full
  installer run (``copy_self_to_tampinha_home`` deliberately no-ops during
  ...
  ancestry check those users get exit 2 ("Tampinha is still running") on every
  GUI update forever, with no Tampinha process actually running.
"""
...
MARKER_NAME = ".tampinha-update-in-progress"

# Set by an orchestrating updater (the Tauri `tampinha-setup --update` flow) to
# its own pid before spawning `tampinha update` as a child stage. The parent
...
HANDOFF_PID_ENV = "TAMPINHA_UPDATE_HANDOFF_PID"
...
# to show "Tampinha is still running" instead of a generic failure. Naming it
...
def update_marker_path() -> Path:
    ...
    Uses the *process* Tampinha home (never the context-local profile override):
    the Rust updater resolves ``$TAMPINHA_HOME`` or the platform default, and the
    ...
    return get_process_tampinha_home() / MARKER_NAME
...
def _is_ancestor_pid(pid: int) -> bool:
    ...
    The orchestrating updater spawns ``tampinha update`` as a (grand)child, so a
    ...
    parent chain. This heals the fleet of staged ``tampinha-setup`` binaries
...
def describe_holder(holder: UpdateHolder) -> str:
    ...
    return (
        f"✗ Another Tampinha update is already running (PID {holder.pid}, "
...
class UpdateLock:
    ...
    def acquire(self) -> bool:
        ...
        updater spawning `tampinha update` as a stage): we run under ITS claim
