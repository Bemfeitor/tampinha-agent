"""
Profile management for multiple isolated Tampinha instances.

Each profile is a fully independent TAMPINHA_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.tampinha/profiles/<name>/`` by default.

The "default" profile is ``~/.tampinha`` itself — backward compatible,
zero migration needed.

Usage::

    tampinha profile create coder          # fresh profile + bundled skills
    tampinha profile create coder --clone  # also copy config, .env, SOUL.md, skills
    tampinha profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    tampinha -p coder chat                 # or via flag
    tampinha profile use coder             # set as sticky default
    tampinha profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See tampinha_constants.get_subprocess_home().
    "home",
]
...
# default profile (``~/.tampinha``).  Named profiles never contain these
# directories at root, so the exclusion is gated to avoid silently dropping
# user data from a named-profile source.
#
# Rationale per item:
#   tampinha-agent  — git repo checkout (~84 MB source + ~3 GB venv)
...
#   backups             — `tampinha backup` archives
...
# Marker file written by `tampinha profile create --no-skills`.  When present in
# a profile's root, callers of seed_profile_skills() (fresh-create, `tampinha
...
# `tampinha skills install` or drop SKILL.md files into the profile's skills/.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Three categories:
      ...
      2. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         Tampinha infrastructure directories that only the default profile
         (``~/.tampinha``) ever contains.  Gated on ``source_dir`` actually
    ...
    is_default_source = source_resolved == _get_default_tampinha_home().resolve()
...
# Directories/files to exclude when exporting the default (~/.tampinha) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "tampinha-agent",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
# Allow-list for ``export_profile("default")``: when TAMPINHA_HOME equals the
...
# into the export. The set below identifies the *known Tampinha profile
# artifacts* at the root of TAMPINHA_HOME; everything else is excluded.
...
# (#58394). Add new artifacts here when introduced in ``tampinha_constants``.
...
_RESERVED_NAMES = frozenset({
    "tampinha", "default", "test", "tmp", "root", "sudo",
})

# Tampinha subcommands that cannot be used as profile names/aliases
_TAMPINHA_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the tampinha root, NOT to the current TAMPINHA_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where TAMPINHA_HOME points outside
    ``~/.tampinha``, profiles live under ``TAMPINHA_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_tampinha_home() / "profiles"


def _get_default_tampinha_home() -> Path:
    """Return the default (pre-profile) TAMPINHA_HOME path.

    In standard deployments this is ``~/.tampinha``.
    In Docker/custom deployments where TAMPINHA_HOME is outside ``~/.tampinha``
    (e.g. ``/opt/data``), returns TAMPINHA_HOME directly.
    """
...
def _get_active_profile_path() -> Path:
    ...
    return _get_default_tampinha_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(
            ...
            f"the Tampinha installation itself or a common system binary.  "
...
def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its TAMPINHA_HOME directory."""
    canon = normalize_profile_name(name)
    if canon == "default":
        return _get_default_tampinha_home()
...
def profile_matches_home(name: str, home: "Path | None" = None) -> bool:
    ...
    ``home`` defaults to the process's current Tampinha home
    (:func:`tampinha_constants.get_tampinha_home`).  Used by single-profile
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: alias-name validity, reserved names, tampinha subcommands, existing
    binaries in PATH.
    """
    canon = normalize_profile_name(name)
    try:
    ...
    if canon in _TAMPINHA_SUBCOMMANDS:
        return f"'{canon}' conflicts with a tampinha subcommand"
...
def create_wrapper_script(name: str, target: Optional[str] = None) -> Optional[Path]:
    ...
    if is_windows:
        ...
        try:
            tampinha_exe = shutil.which("tampinha") or "tampinha"
            wrapper_path.write_text(f'#!/bin/sh\nexec {shlex.quote(tampinha_exe)} -p {profile} "$@"\n', encoding="utf-8")
...
def _migrate_profile_config_if_outdated(profile_dir: Path) -> None:
    ...
    ``_config_version``) or that is simply older than the running Tampinha. If we
    ...
    try:
        ...
        # not be migrated. The next `tampinha doctor --fix` can still surface the
...
def find_alias_for_profile(profile_name: str) -> Optional[str]:
    ...
    alias whose body invokes ``tampinha -p <profile>``. When the alias name equals
    the profile name this is trivial, but a custom alias (``tampinha profile alias
...
# Real wrappers are a few hundred bytes of shell; the needle (``tampinha -p X``)
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``tampinha profile list`` for an unrelated profile.
    """
...
def _check_gateway_running(profile_dir: Path) -> bool:
    ...
    *this* reader — which is exactly the case for a dashboard process that is
    ...
    agree.  Parameterized by ``profile_dir`` so it never mutates ``TAMPINHA_HOME``.
    """
    try:
...
# ``config.yaml``. ``config.yaml`` is the user-facing Tampinha config
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_tampinha_home()
...
def profiles_to_serve(
    ...
    """Return the ``(profile_name, tampinha_home)`` pairs a gateway should serve.
    ...
    - ``multiplex=True``: returns the default profile plus every valid named
      profile under ``profiles/``, each paired with its own TAMPINHA_HOME. When
    ...
    The returned ``tampinha_home`` is the path to pass to
    ``set_tampinha_home_override`` when scoping a turn to that profile.
    """
    active = get_active_profile_name() or "default"
    if not multiplex:
        return [(active, get_profile_dir(active))]

    serve: List[Tuple[str, Path]] = [("default", _get_default_tampinha_home())]
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``tampinha update`` skips re-seeding this profile's
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.tampinha)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.tampinha/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
            symlinks=True,
    ...
    # Keys pages, `tampinha -p <name> auth add`) had no file until first
    ...
    if not env_path.exists():
        try:
            env_path.write_text(
                "# Per-profile secrets for this Tampinha profile.\n"
    ...
    if not soul_path.exists():
        try:
            from tampinha_cli.default_soul import DEFAULT_SOUL_MD
            soul_path.write_text(DEFAULT_SOUL_MD, encoding="utf-8")
        except Exception:
            pass  # best-effort — don't fail profile creation over this

    # Write the opt-out marker so seed_profile_skills() and `tampinha update`'s
    ...
    if no_skills:
        try:
            (profile_dir / NO_BUNDLED_SKILLS_MARKER).write_text(
                ...
                "(`tampinha profile create --no-skills`).\n"
                "Delete this file to re-enable sync on the next `tampinha update`.\n",
                encoding="utf-8",
            )
        except OSError:
            pass  # best-effort — the feature still works via the empty skills/ dir

    # Cloned configs can be older than the running Tampinha (or predate schema
    ...
    if description and description.strip():
        try:
            ...
            pass  # non-fatal — user can describe later with `tampinha profile describe`

    # Phase 4: when running inside a container under s6, register the
    # new profile's gateway as a runtime s6 service so
    # `tampinha -p <profile> gateway start` can supervise it via
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches TAMPINHA_HOME at module level.
    Returns the sync result dict, or None on failure.

    Profiles that opted out of bundled skills (via ``tampinha profile create
    ...
    only the essential skills (e.g. ``tampinha-agent``), reporting
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "TAMPINHA_HOME": str(profile_dir)},
...
def backfill_profile_envs(quiet: bool = False) -> List[str]:
    ...
    default_env = _get_default_tampinha_home() / ".env"
...
def _profile_bound_backend_pids(canon: str, profile_dir: Path) -> list[int]:
    """PIDs of running Tampinha *backends* bound to this profile.
    ...
    names it, so find it by inspection: a Tampinha backend subcommand
    (``serve``/``dashboard``/``gateway``) that is bound to *this* profile either
    by a ``--profile <canon>`` / ``-p <canon>`` selector or by a ``TAMPINHA_HOME``
    ...
    # Never terminate ourselves or a parent (e.g. `tampinha -p <canon> profile
    ...
    tampinha_markers = ("tampinha_cli.main", "tampinha-gateway", "tui_gateway")
    ...
    # something (e.g. Electron's `findOnPath('tampinha')` resolution) spawns the
    ...
    # interpreter, not "tampinha", so the checks below would otherwise miss it.
    ...
    # interpreter.
    _TAMPINHA_CONSOLE_SCRIPT_NAMES = frozenset({"tampinha", "tampinha-agent", "tampinha-acp"})
    pids: list[int] = []
    ...
    for proc in psutil.process_iter(["pid", "name", "username", "cmdline"]):
        try:
            ...
            # Must be a Tampinha process: either an entrypoint marker in argv, a
            # resolved executable named `tampinha`, or a python interpreter
            # directly exec'ing a `tampinha`-named console-script shim (argv[0]
            # is the interpreter, argv[1] is the shim's path).
            joined = " ".join(argv)
            exe_name = os.path.basename(argv[0]).lower()
            is_tampinha = (
                any(marker in joined for marker in tampinha_markers)
                or exe_name == "tampinha"
                or exe_name.startswith("tampinha")
            )
            if not is_tampinha and len(argv) >= 2 and _python_interpreter_re.match(exe_name):
                # Match against the actual known console-script entry points
                # (pyproject.toml [project.scripts]: tampinha, tampinha-agent,
                # tampinha-acp) rather than a bare `startswith("tampinha")` --
                ...
                # "tampinha-notes.py" or "tampinha-unrelated-tool" as the shim,
                ...
                is_tampinha = script_stem in _TAMPINHA_CONSOLE_SCRIPT_NAMES
            if not is_tampinha:
                continue
            ...
            # ...or by TAMPINHA_HOME env pointing at this profile dir.
            if not bound:
                try:
                    env_home = (proc.environ() or {}).get("TAMPINHA_HOME", "")
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.tampinha).\n"
            "To remove everything, use: tampinha uninstall"
        )
    ...
    # the rmtree below fail with ENOTEMPTY and — before the ensure_tampinha_home
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    ``TAMPINHA_HOME`` and binds the port resolved by ``gateway/config.py``
    ...
    try:
        ...
        from tampinha_cli.service_manager import get_service_manager
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set TAMPINHA_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("TAMPINHA_HOME")
    try:
        os.environ["TAMPINHA_HOME"] = str(profile_dir)
        from tampinha_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["TAMPINHA_HOME"] = old_home
        elif "TAMPINHA_HOME" in os.environ:
            del os.environ["TAMPINHA_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.tampinha/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: tampinha profile create {canon}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from TAMPINHA_HOME.

    Returns ``"default"`` if TAMPINHA_HOME is not set or points to ``~/.tampinha``.
    Returns the profile name if TAMPINHA_HOME points into ``~/.tampinha/profiles/<name>``.
    Returns ``"custom"`` if TAMPINHA_HOME is set to an unrecognized path.
    """
    from tampinha_constants import get_tampinha_home
    tampinha_home = get_tampinha_home()
    resolved = tampinha_home.resolve()

    default_resolved = _get_default_tampinha_home().resolve()
...
def _default_export_ignore(root_dir: Path):
    ...
    * **Root-level allow-list** — only entries whose name appears in
      ...
      TAMPINHA_HOME equals the cwd) is excluded. Blacklisting was tried
      first and proved unable to anticipate every non-Tampinha file the
      user may have lying alongside TAMPINHA_HOME (#58394).
...
def _scrub_export_secrets(staged: Path) -> None:
    ...
    ``tampinha sessions export --redact``. Runs on the *staged copy only* so the
    live profile is never rewritten. ``force=True`` ignores
    ``security.redact_secrets`` / ``TAMPINHA_REDACT_SECRETS`` — share archives
...
def export_profile(name: str, output_path: str, extra_files: Optional[Dict[str, str]] = None) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.tampinha itself — its parent is ~/ and its
        # directory name is ".tampinha", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: tampinha profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.tampinha itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.tampinha). "
            "Specify a different name: tampinha profile import <archive> --name <name>"
        )
    ...
    with tempfile.TemporaryDirectory(prefix="tampinha_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"tampinha_{old_name}"
    legacy_old_host = f"tampinha.{old_name}"
    new_host = f"tampinha_{new_name}"

    candidates = [
        new_dir / "honcho.json",
        _get_default_tampinha_home() / "honcho.json",
    ...
    for path in candidates:
        ...
        if isinstance(block, dict) and "aiPeer" not in block:
            if source_host.startswith("tampinha_"):
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a TAMPINHA_HOME path string.

    Called early in the CLI entry point, before any tampinha modules
    are imported, to set the TAMPINHA_HOME environment variable.

    When TAMPINHA_HOME is already set, the configured spelling IS the
