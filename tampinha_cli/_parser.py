"""
Top-level argparse construction for the tampinha CLI.
...
# argparse runs (it sets ``TAMPINHA_HOME`` and strips itself from ``sys.argv``),
...
# tests/tampinha_cli/test_top_level_value_flags_parity.py fails CI if the parser
...
def top_level_value_flag_sets() -> tuple[frozenset[str], frozenset[str]]:
    ...
    the exact drift that made ``tampinha --reasoning high chat …`` misread
...
def _inherited_flag(parser, *args, **kwargs):
    """Register a flag that ``tampinha_cli.relaunch`` should carry over when
...
_EPILOGUE = """
Examples:
    tampinha                        Start interactive chat
    tampinha chat -q "Hello"        Single query mode
    tampinha --tui                  Launch the modern TUI (or set display.interface: tui)
    tampinha --cli                  Force the classic REPL (overrides display.interface: tui)
    tampinha -c                     Resume the most recent session
    tampinha -c "my project"        Resume a session by name (latest in lineage)
    tampinha --resume <session_id>  Resume a specific session by ID
    tampinha --resume latest        Resume the most recent session (same as -c)
    tampinha --tui --resume latest --in ./dir   Resume ./dir's latest session in the TUI
    tampinha setup                  Run setup wizard
    tampinha logout                 Clear stored authentication
    tampinha auth add <provider>    Add a pooled credential
    tampinha auth list              List pooled credentials
    tampinha auth remove <p> <t>    Remove pooled credential by index, id, or label
    tampinha auth reset <provider>  Clear exhaustion status for a provider
    tampinha model                  Select default model
    tampinha fallback [list]        Show fallback provider chain
    tampinha fallback add           Add a fallback provider (same picker as `tampinha model`)
    tampinha fallback remove        Remove a fallback provider from the chain
    tampinha config                 View configuration
    tampinha config edit            Edit config in $EDITOR
    tampinha config set model gpt-4 Set a config value
    tampinha gateway                Run messaging gateway
    tampinha -s tampinha-agent-dev,github-auth
    tampinha -w                     Start in isolated git worktree
    tampinha gateway install        Install gateway background service
    tampinha sessions list          List past sessions
    tampinha sessions browse        Interactive session picker
    tampinha sessions rename ID T   Rename/title a session
    tampinha logs                   View agent.log (last 50 lines)
    tampinha logs -f                Follow agent.log in real time
    tampinha logs errors            View errors.log
    tampinha logs --since 1h        Lines from the last hour
    tampinha debug share             Upload debug report for support
    tampinha console                Open the safe Tampinha command console
    tampinha update                 Update to latest version
    tampinha dashboard              Start web UI dashboard (port 9119)
    tampinha dashboard --stop       Stop running dashboard processes
    tampinha dashboard --status     List running dashboard processes

For more help on a command:
    tampinha <command> --help
"""
...
def build_top_level_parser():
    ...
    parser = argparse.ArgumentParser(
        prog="tampinha",
        description="Tampinha Agent - AI assistant with tool-calling capabilities",
    ...
    # Mirrors `tampinha chat --model ... --provider ...` semantics.
    _inherited_flag(
        parser,
        "-m",
        "--model",
        ...
        help=(
            ...
            "Applies to -z/--oneshot and --tui. Also settable via TAMPINHA_INFERENCE_MODEL env var."
        ),
    )
    _inherited_flag(
        parser,
        ...
        help=(
            ...
            "under model.provider — use `tampinha setup` or edit the file to change it."
        ),
    )
    _inherited_flag(
        parser,
    ...
    _inherited_flag(
        ...
        help=(
            ...
            "without a TTY prompt.  Equivalent to TAMPINHA_ACCEPT_HOOKS=1 or "
    ...
    _inherited_flag(
        ...
        action="store_true",
        default=False,
        help="Ignore ~/.tampinha/config.yaml and fall back to built-in defaults (credentials in .env are still loaded)",
    )
    _inherited_flag(
        parser,
        "--ignore-rules",
    ...
    chat_parser = subparsers.add_parser(
        ...
        description="Start an interactive chat session with Tampinha Agent",
    )
    ...
    # top-level parser: when the user writes `tampinha -m foo chat`, argparse
    ...
    # `-w/--worktree`, `--yolo`, etc. (see tests/tampinha_cli/
    ...
    _inherited_flag(
        ...
        help=(
            "Auto-approve any unseen shell hooks declared in config.yaml "
            "without a TTY prompt (see also TAMPINHA_ACCEPT_HOOKS env var and "
    ...
    _inherited_flag(
        ...
        action="store_true",
        default=argparse.SUPPRESS,
        help="Ignore ~/.tampinha/config.yaml and fall back to built-in defaults (credentials in .env are still loaded). Useful for isolated CI runs, reproduction, and third-party integrations.",
    )
    _inherited_flag(
        chat_parser,
        "--ignore-rules",
    ...
    _inherited_flag(
        ...
        action="store_true",
        default=argparse.SUPPRESS,
        help="Troubleshooting mode: disable ALL customizations — user config, AGENTS.md/memory injection, plugins, and MCP servers (implies --ignore-user-config and --ignore-rules). Use to isolate whether a problem comes from your setup or from Tampinha itself.",
    )
    chat_parser.add_argument(
        "--source",
        default=None,
