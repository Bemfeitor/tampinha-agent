"""Bounded product contract for the first Tampinha shared-metrics slice."""
...
SCHEMA_KEY = "tampinha.metrics.schema_version"
SCHEMA_VERSION = "tampinha.metrics.event.v2"
MODEL_CALL_SCOPE = "tampinha.model_call"
MODEL_CALL_PROFILE_MODEL = "unknown"
TASK_SCOPE = "tampinha.task_run"
TOOL_CALL_SCOPE = "tampinha.tool_call"
CLIENT_ACTIVE_MARK = "tampinha.client.active"
TOOL_APPROVAL_MARK = "tampinha.tool_approval"
SKILL_LIFECYCLE_MARK = "tampinha.skill.lifecycle"
SKILL_LOAD_MARK = "tampinha.skill.load"
SUBSCRIBER_NAME = "tampinha.nemo_relay.shared_metrics"
CLIENT_ACTIVE_METRIC = "tampinha.client.active"
LEGACY_MODEL_CALL_METRIC = "tampinha.model_call.count"
MODEL_ROUTE_METRIC = "tampinha.model_route.count"
TASK_STARTED_METRIC = "tampinha.task_run.started"
TASK_FINISHED_METRIC = "tampinha.task_run.finished"
TOOL_CALL_METRIC = "tampinha.tool_call.count"
TOOL_APPROVAL_METRIC = "tampinha.tool_approval.count"
SKILL_LIFECYCLE_METRIC = "tampinha.skill.lifecycle.count"
SKILL_LOAD_METRIC = "tampinha.skill.load.count"
...
def client_install_method(value: Any) -> str:
    """Return an allowlisted Tampinha installation method."""
    normalized = str(value or "").strip().lower()
    if normalized == "nix":
        return "nixos"
...
def client_resource(
    ...
    *,
    ...
    normalized_version = str(tampinha_version or "").strip()
    ...
    return {
        ...
        "tampinha_version": normalized_version,
...
def client_resource_is_valid(resource: Any) -> bool:
    ...
    if not isinstance(resource, dict) or set(resource) != CLIENT_RESOURCE_KEYS:
        return False
    version = resource.get("tampinha_version")
    return (
...
def _auxiliary_model_call_dimensions(event: Any) -> dict[str, str] | None:
    """Project a terminal auxiliary route from its Tampinha logical scope."""
    metadata = getattr(event, "metadata", None)
    if (
    ...
    if relay_metadata - {"otel.status_code"} or metadata.get(
        ...
        return None
    call_role = metadata.get("tampinha.call_role")
...
def execution_surface(kwargs: dict[str, Any]) -> str:
    ...
    try:
        from tampinha_cli.platforms import get_all_platforms
...
def task_terminal_state(kwargs: dict[str, Any]) -> tuple[str, str, str]:
    """Map Tampinha terminal state to bounded task outcome dimensions."""
...
def tool_category(kwargs: dict[str, Any]) -> str:
    """Map Tampinha registry toolset metadata to a low-cardinality category."""
    ...
    if toolset.startswith(
        ("discord", "email", "feishu", "tampinha-yuanbao", "slack", "sms")
    ):
...
def tool_outcome(kwargs: dict[str, Any]) -> str:
    """Normalize the terminal Tampinha tool status without inspecting its result."""
...
def model_call_fields(kwargs: dict[str, Any]) -> dict[str, str]:
    """Return the terminal model identity and provider route known to Tampinha."""
