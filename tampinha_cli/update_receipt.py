...
1. **Update receipt** — a machine-readable JSON record of what one
   ``tampinha update`` run discovered, did, skipped (and why), written to
   ``<TAMPINHA_HOME>/logs/update_receipts/``. Silent-failure classes this
...
``tampinha_cli.build_info.get_code_identity()``: an image build reports
...
# Module-level current receipt. ``tampinha update`` is a single-threaded CLI
...
class UpdateReceipt:
    """Collects the observable facts of one ``tampinha update`` run."""
...
def _receipt_dir() -> Path:
    ...
    return get_tampinha_home() / "logs" / _RECEIPT_DIR_NAME
...
def finalize_pending_update_receipt(
    ...
    ``tampinha update`` has many early-termination paths (Windows
