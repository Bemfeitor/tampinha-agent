"""``tampinha portal`` — the human-readable entry point for Nous Portal.

Running ``tampinha portal`` with no subcommand performs the one-shot Portal
...
``tampinha auth add nous --type oauth`` (which still works), is identical to
``tampinha setup --portal``, and runs the same Nous flow as the first-time quick
setup.
...
already in ``tampinha auth`` or ``tampinha tools``. It's the onboarding + discovery
...
def _cmd_status(args) -> int:
    ...
    if provider == "nous":
        ...
        print(f"  Model:   currently {provider} (switch with `tampinha model`)")
...
def _cmd_tools(args) -> int:
    ...
    if not features.nous_auth_present:
        print(color("  Not logged into Nous Portal — sign in with `tampinha portal`.", Colors.YELLOW))
        print()
...
def _cmd_login(args) -> int:
    ...
    This is the human-readable front door for `tampinha auth add nous --type
    oauth`. It reuses the exact wiring behind `tampinha setup --portal` (which in
    ...
    """
    from tampinha_cli.setup import _run_portal_one_shot
...
def portal_command(args) -> int:
    """Top-level dispatch for `tampinha portal <subcommand>`."""
    sub = getattr(args, "portal_command", None)
    if sub in {None, "", "login"}:
        # Default to the one-shot onboarding — `tampinha portal` is the
        # human-readable alias for `tampinha auth add nous --type oauth` /
        # `tampinha setup --portal`.
    ...
    print("Run `tampinha portal -h` for usage.", file=sys.stderr)
    return 1


def add_parser(subparsers) -> None:
    """Register `tampinha portal` on the given argparse subparsers object."""
    portal_parser = subparsers.add_parser(
        "portal",
        help="Set up Nous Portal (login, model pick, Tool Gateway); see also `portal info`",
        description=(
            "Run `tampinha portal` with no subcommand to log in to Nous Portal "
            "and set it up — pick a model, set Nous as your provider, and offer "
            "the Tool Gateway (the human-readable alias for `tampinha auth add "
            "nous --type oauth`, identical to `tampinha setup --portal`). "
