"""
Skills configuration for Tampinha Agent.
`tampinha skills` enters this module.

Toggle individual skills or categories on/off, globally or per-platform.
Config stored in ~/.tampinha/config.yaml under:
...
from tampinha_cli.config import cfg_get, load_config, save_config
from tampinha_cli.colors import Colors, color
from tampinha_cli.platforms import PLATFORMS as _PLATFORMS
...
def save_disabled_skills(config: dict, disabled: Set[str], platform: Optional[str] = None):
    ...
    Essential skills (e.g. ``tampinha-agent``) are silently dropped from the
...
def skills_command(args=None):
    """Entry point for `tampinha skills`."""
    from tampinha_cli.curses_ui import curses_checklist
