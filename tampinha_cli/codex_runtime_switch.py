...
Toggles `model.openai_runtime` between "auto" (= chat_completions, Tampinha'
...
The actual runtime resolution happens in tampinha_cli.runtime_provider's
...
def parse_args(arg_string: str) -> tuple[Optional[str], list[str]]:
    ...
    if raw in {"off", "default", "disable", "tampinha"}:
        return "auto", []
...
def apply(
    ...
    *,
    ...
    # the world state (managed block in ~/.codex/config.toml, tampinha-tools
    ...
    if new_value == "codex_app_server":
        ...
        # Auto-migrate Tampinha' MCP servers + Codex's installed curated
        # plugins into ~/.codex/config.toml so the spawned codex subprocess
        # sees the same tool surface AND can call back into Tampinha for
        # browser/web/delegate_task/vision/memory tools (#7 fix).
        # Failures are non-fatal — the runtime change still proceeds.
        try:
            from tampinha_cli.codex_runtime_plugin_migration import migrate
            mig_report = migrate(config)
            # Tools/MCP servers (excluding the tampinha-tools callback,
            # which is internal plumbing — surface separately).
            user_servers = [
                s for s in mig_report.migrated if s != "tampinha-tools"
            ]
            if user_servers:
                msg_lines.append(
            ...
            # Permissions + Tampinha tool callback are always-on production
            ...
            if "tampinha-tools" in mig_report.migrated:
                msg_lines.append(
                    "Tampinha tool callback registered: codex can now use "
                ...
                msg_lines.append(
                    ...
                    "only on the default Tampinha runtime — they need the "
                    "agent loop context.)"
                )
        ...
        msg_lines.append(
            ...
            "Tampinha tools available via MCP callback)."
        )
        msg_lines.append(
        ...
        msg_lines.append("OpenAI/Codex turns will use the default Tampinha runtime.")
