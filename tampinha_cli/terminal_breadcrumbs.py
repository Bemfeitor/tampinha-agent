"""Per-terminal session breadcrumbs for ``tampinha -c`` / ``--continue``.

Each CLI session writes a tiny breadcrumb file
``$TAMPINHA_HOME/terminal-sessions/<terminal-id>`` containing
``{"session_id": ..., "cwd": ..., "ts": ...}``.  A bare ``tampinha -c`` then
...
def _breadcrumbs_dir() -> Path:
    ...
    return get_tampinha_home() / "terminal-sessions"
