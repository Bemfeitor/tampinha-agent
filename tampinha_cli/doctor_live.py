"""``tampinha doctor --live`` — opt-in bounded real-call tool-backend probes.
...
def _browser_available() -> bool:
    ...
    if shutil.which("agent-browser"):
        return True
    try:
        from tampinha_cli.doctor import TAMPINHA_HOME, PROJECT_ROOT

        if (PROJECT_ROOT / "node_modules" / "agent-browser").exists():
            return True
        for candidate in (TAMPINHA_HOME / "node" / "bin",
                          TAMPINHA_HOME / "node",
                          TAMPINHA_HOME / "node_modules" / ".bin"):
    ...
    # tampinha_cli.doctor uses so this probe can't diverge from it, including
...
def _probe_mcp_server(name: str, config: dict, timeout: float):
    ...
    Reuses the exact machinery behind ``tampinha mcp test``.
    """
