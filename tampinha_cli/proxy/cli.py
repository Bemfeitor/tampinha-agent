"""CLI handlers for the ``tampinha proxy`` subcommand."""
...
from tampinha_cli.proxy.server import (
    AIOHTTP_AVAILABLE,
    DEFAULT_HOST,
    DEFAULT_PORT,
    run_server,
...
def _print_aiohttp_missing() -> None:
    print(
        "tampinha proxy requires aiohttp. Run `tampinha setup` to install it.",
        file=sys.stderr,
    )
...
def cmd_proxy_start(args: Any) -> int:
    ...
    if not adapter.is_authenticated():
        auth_hint = getattr(adapter, "auth_hint", f"tampinha auth add {adapter.name}")
        print(
    ...
    print(
        f"Starting Tampinha proxy for {adapter.display_name}\n"
...
def cmd_proxy_status(args: Any) -> int:
    ...
    print("Tampinha proxy upstream adapters\n")
    ...
    print(
        "\nStart the proxy with: tampinha proxy start [--provider <name>]"
    )
    return 0
...
def cmd_proxy(args: Any) -> int:
    """Dispatch ``tampinha proxy <subcommand>``."""
    ...
    print(
        "tampinha proxy — local OpenAI-compatible proxy that attaches your\n"
        "OAuth-authenticated provider credentials to outbound requests.\n"
        "\n"
        "Subcommands:\n"
        "  tampinha proxy start [--provider nous|xai] [--host 127.0.0.1] [--port 8645]\n"
        "      Run the proxy in the foreground.\n"
        "  tampinha proxy status\n"
        "      Show which upstream adapters are ready.\n"
        "  tampinha proxy providers\n"
