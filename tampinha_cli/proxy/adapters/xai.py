...
# xAI's public API is OpenAI-compatible for the endpoints Tampinha commonly
# uses. The Responses endpoint is included because Tampinha' native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via Tampinha-managed OAuth credentials."""

    auth_hint = "tampinha auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`tampinha auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`tampinha auth reset xai-oauth` or re-authenticate with "
                    "`tampinha auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `tampinha auth add xai-oauth --type oauth`."
            )
