...
# coverage category (tampinha_cli.nous_account.TOOL_COVERAGE_CATEGORIES). Lets the
# `tampinha tools` picker scope its entitlement gate to the selected backend, so a
...
def _selected_provider(section: object, name_key: str = "provider") -> Optional[str]:
    ...
    with the runtime resolver is what stops ``tampinha status`` from lying.
    """
    if not isinstance(section, dict):
        return None
...
def _has_agent_browser() -> bool:
    ...
    try:
        ...
        # Tampinha-managed Node dirs (Windows installer / POSIX $TAMPINHA_HOME/node)
        ...
        managed_path = with_tampinha_node_path().get("PATH", "")
        if managed_path:
...
def _local_stt_backend_available() -> bool:
    ...
    """
    if get_env_value("TAMPINHA_LOCAL_STT_COMMAND"):
        return True
    try:
...
def get_nous_subscription_features(
    ...
    *,
    ...
    # Keyless Tavily is opt-in: selecting it in `tampinha tools` / setup writes
    ...
    try:
        ...
        local_stt_available = bool(_HAS_FASTER_WHISPER) or bool(
            get_env_value("TAMPINHA_LOCAL_STT_COMMAND")
        )
    except Exception:
        local_stt_available = bool(get_env_value("TAMPINHA_LOCAL_STT_COMMAND"))
...
def prompt_enable_tool_gateway(
    ...
    *,
    ...
    try:
        from tampinha_cli.setup import prompt_checklist
    except Exception:
        return set()
...
# ---------------------------------------------------------------------------
# Inline Nous Portal login for the Tool Gateway picker (`tampinha tools`)
# ---------------------------------------------------------------------------
...
def ensure_nous_portal_access(
    *,
    ...
    Used by ``tampinha tools`` when a user selects a Nous-managed Tool Gateway
    backend (e.g. "Firecrawl (Nous Portal)").  Unlike ``tampinha model``'s Nous
    login, this:
