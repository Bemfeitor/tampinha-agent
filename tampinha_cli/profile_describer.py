...
Used by ``tampinha profile describe <name> --auto`` and the dashboard's
...
Design notes
------------
- Mirrors the shape of ``tampinha_cli/kanban_specify.py``: lazy aux
...
_SYSTEM_PROMPT = """You are a profile-describer for the Tampinha Agent kanban board.
...
Rules:
  ...
  - Never write "Tampinha Agent profile" or other meta-narration.
    ...
    *,
    ...
    try:
        if canon == "default":
            ...
            profile_dir = Path(get_tampinha_home())
        else:
