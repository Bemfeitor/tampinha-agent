"""Default configuration data for Tampinha Agent.

Pure-data leaf module: DEFAULT_CONFIG and OPTIONAL_ENV_VARS, extracted
verbatim from tampinha_cli/config.py. Must not import from tampinha_cli.config.
"""
...
DEFAULT_CONFIG = {
    ...
    "toolsets": ["tampinha-cli"],
    # SQLite journal mode used by every Tampinha database opener. WAL is the
    ...
    # Soft file-descriptor limit for long-running Tampinha server processes.
    ...
    "session": {
        # Per-terminal `tampinha -c`: each CLI session drops a breadcrumb file
        # under $TAMPINHA_HOME/terminal-sessions/<terminal-id>, and a bare
    ...
    "agent": {
        ...
        # all normalized by tampinha_cli.config.resolve_turn_limit.
        "max_turns": None,
        ...
        # budget. CLI one-shot equivalent: `tampinha chat --run-budget N`.
        ...
        # an interactive `tampinha gateway restart` must never block for hours
        ...
        # the Tampinha-level retry loop that wraps the whole call.  Lower
        ...
        # prompt's environment-hints block. Lets a host that wraps Tampinha
        ...
        # identity slot (SOUL.md). Empty by default. The TAMPINHA_ENVIRONMENT_HINT
        # env var overrides this (build-time/container mechanism).
        "environment_hint": "",
        # Coding posture — on interactive coding surfaces (CLI, TUI, desktop
        # app, ACP) in a code workspace, Tampinha adds a coding operating brief
        ...
        # `tampinha status` / fleet monitoring). Retries never stop — this is a
        ...
        # ``TAMPINHA_LOCAL_STREAM_STALE_TIMEOUT`` overrides for escape-hatch use.
    ...
    "terminal": {
        ...
        # bot_relay from a short-lived `tampinha -p <bot> chat -Q` recipient
        ...
        #             TAMPINHA_HOME/home for persistent state (default)
        #   real    — force the real OS-user HOME
        #   profile — force TAMPINHA_HOME/home when it exists (old strict
        ...
        # skipped. When empty, Tampinha auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Tampinha sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when Tampinha
        ...
        #  "/home/user/.tampinha/cache/documents:/output"]
        ...
        # bundled Tampinha image, which drops to the `tampinha` user via
    ...
    "web": {
        ...
        # firecrawl, keenable). Set by the `tampinha tools` picker's
    ...
    "browser": {
        ...
        # browser tools) runs on a Tampinha-managed SNAPSHOT of the user's
        ...
        # fresh session launches — driven by Tampinha' packaged Chromium. Only
        ...
        # deletes the snapshot store (~/.tampinha/browser-profile/) so copied
        ...
        # approval does it run `tampinha browser close-profile` (terminates the
        ...
        "camofox": {
            # When true, Tampinha sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and Tampinha should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``tampinha chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.tampinha/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: tampinha sweeps the checkpoint base at startup
        ...
        # ``tampinha checkpoints prune`` command (add ``--keep-orphans`` to
    ...
    # AGENTS.md, CLAUDE.md, .tampinha.md, or .cursorrules before Tampinha applies
    ...
    # Single-query (``tampinha -q/-z "..."``) variant of mcp_discovery_timeout.
    ...
    # single read_file page exceeds these limits, Tampinha truncates the
    ...
    "compression": {
        ...
        "codex_app_server_auto": "native",  # Codex app-server (codex CLI runtime) thread
                                      ...
                                      # thread context, so Tampinha' summarizer cannot
                                      # shrink it (#36801). native = codex decides when
                                      # to compact its own thread (default); tampinha =
                                      # Tampinha' compression threshold triggers
        ...
        "codex_responses_native": False,  # Opt in to OpenAI's server-side compaction
                                      ...
                                      # route/model is unaffected. Tampinha' local
        ...
        "codex_responses_compact_threshold": 200000,  # Server-side compaction trigger
                                      ...
                                      # the server compacts before Tampinha does.
    ...
    "auxiliary": {
        ...
        # Invoked by ``tampinha kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``tampinha kanban decompose`` and the kanban
        ...
        # ``tampinha profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `tampinha model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # Which interface bare `tampinha` (and `tampinha chat`) launches by default:
        ...
        # REPL and `--tui` (or TAMPINHA_TUI=1) forces the TUI regardless of config.
        "interface": "cli",
        # When true, `tampinha --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `tampinha -c` muscle memory.  Default off so existing
        # users aren't surprised.  TAMPINHA_TUI_RESUME=<id> always wins.
        "tui_auto_resume_recent": False,
        # When true (default), `tampinha --tui` drops a one-time hint
        ...
        # model — see tampinha_cli/focus_view.py.
        "focus_view": False,
        ...
        # can set `path`/`paths` to TAMPINHA_HOME-relative YAML files/directories
        ...
        # e.g. `model · 68% · ~/projects/tampinha`. Per-platform overrides go under
        ...
        # CLI, TUI, and desktop app. Manage with `tampinha pets`. Disabled until
        ...
        "pet": {
            ...
            # get_tampinha_home()/pets/. Empty → first installed pet.
            "slug": "",
    ...
    "dashboard": {
        ...
        # protocol ping (see tampinha_cli/web_server.py — an event-loop stall
        ...
        # TAMPINHA_TUI_WS_ORPHAN_REAP_GRACE_S env var remains an internal
        ...
        # (TAMPINHA_TUI_SESSION_TTL_S, default 6h) are closed with
        ...
        # ``TAMPINHA_DASHBOARD_OAUTH_CLIENT_ID`` and
        # ``TAMPINHA_DASHBOARD_PORTAL_URL`` respectively — and the env var
        ...
        # (``TAMPINHA_DASHBOARD_BASIC_AUTH_USERNAME`` /
        ...
        # ``TAMPINHA_DASHBOARD_DRAIN_SECRET`` env var (the .env-is-for-secrets
        ...
        # Public URL override (env: ``TAMPINHA_DASHBOARD_PUBLIC_URL``).
    ...
    "tts": {
        ...
        "gemini": {
            ...
            # transcript section; Tampinha appends the live transcript when absent.
            "persona_prompt_file": "",
        },
        "xai": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.tampinha/cache/piper-voices/
    ...
    # "Hey Tampinha" hands-free wake word. Always-on, on-device hotword
    ...
    "wake_word": {
        ...
        "phrase": "hey tampinha",       # for "sherpa" this IS the detected phrase (any text works); for other engines it's a cosmetic label — detection is keyed by the model/keyword below
        ...
        "openwakeword": {
            # "hey_tampinha" (the bundled, works-out-of-the-box default) OR a
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.tampinha/plugins/.
    "context": {
        "engine": "compressor",
    ...
    "delegation": {
        ...
        # trim, the full text is spilled to ~/.tampinha/cache/delegation/
    ...
    # response. If not, Tampinha feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before Tampinha auto-pauses the goal and
    ...
    "moa": {
        ...
        # <tampinha_home>/moa-traces/<session_id>.jsonl. Off by default — turn it
    ...
    # always goes to ~/.tampinha/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Project-local skill discovery: when a session starts inside a git
        # checkout, ``<root>/.tampinha/skills/`` and ``<root>/.agents/skills/``
        ...
        # with ``tampinha skills trust`` (run from inside it). Set to false to
        ...
        # Managed by ``tampinha skills trust`` / ``tampinha skills untrust``.
        "trusted_project_dirs": [],
        # Substitute ${TAMPINHA_SKILL_DIR} and ${TAMPINHA_SESSION_ID} in SKILL.md
        ...
        # (`tampinha skills install`). Runs ALONGSIDE the built-in skills
        ...
        # ~/.tampinha/skills/.curator_ledger.jsonl with before/after file
        # hashes; file contents are stored content-addressed (deduped) under
        # ~/.tampinha/.curator_backups/blobs/. Enables `tampinha curator ledger`
        # and single-mutation `tampinha curator rollback <entry-id>`.
    ...
    # cron daemon.
    #
    # See `tampinha curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # class-level umbrellas. `tampinha curator run --consolidate` overrides
        ...
        # normally restored on every `tampinha update`, so pruning them only
        ...
        # skills are kept forever. When > 0, `tampinha curator purge` deletes
        ...
        # skipped), snapshot ~/.tampinha/skills/ into
        # ~/.tampinha/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `tampinha curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for tampinha-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *Tampinha Agent*" header.
    ...
    # dangerous command. -q runs export TAMPINHA_INTERACTIVE=1 (for interactive
    ...
    "approvals": {
        ...
        # modal; TAMPINHA_TUI_NO_CONFIRM=1 force-skips that modal regardless of
    ...
    # replace Tampinha' built-in platform hint for a single messaging platform
    ...
    # ``tampinha plugins enable|disable`` and intentionally omitted here so an
    ...
    # stored approval from ~/.tampinha/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or TAMPINHA_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # .cursorrules, project-local .tampinha config) always require human
        ...
        # longer trigger the startup banner. Add via `tampinha doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``tampinha_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow Tampinha to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # $TAMPINHA_HOME/plugins/<name>/) to relocate the trigger — e.g. "chronos",
        ...
        # Also overridable via TAMPINHA_CRON_MAX_PARALLEL env var.
        ...
        # TAMPINHA_CRON_SCRIPT_TIMEOUT. Keep this in sync with
        ...
        # TAMPINHA_CRON_SESSION_DB_TIMEOUT env var. 0 = unlimited (skip the bound).
        ...
        # overridable via TAMPINHA_CRON_MEDIA_SEND_TIMEOUT env var. Keep in
    ...
    # todos to ready, and fires `tampinha -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # one `tampinha` launches with no -p flag). This does not control the
        ...
        # decomposition is manual via `tampinha kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     tampinha-agent's own python (sys.executable). Maximum isolation
    ...
    #
    # Core Tampinha tools (terminal, read_file, write_file, patch,
    ...
    # Logging — controls file logging to ~/.tampinha/logs/.
    ...
    # update model picker lists without shipping a tampinha-agent release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": True,
        "url": "https://hermes-agent.nousresearch.com/docs/api/model-catalog.json",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `tampinha model` invocation; network failures
    ...
    # #8731).
    #
    # Provider keys accept the Tampinha provider id (as used elsewhere in
    ...
    "gateway": {
        ...
        # internal TAMPINHA_GATEWAY_PLATFORM_CONNECT_TIMEOUT env var, which still
        ...
        # producing ~/.tampinha/sessions/sessions.json entirely.
        ...
        # the TAMPINHA_SCALE_TO_ZERO env stamp) AND messaging is relay-only/absent
        ...
        # `launchctl kickstart ai.tampinha.gateway` that defenses 1-2 don't
        ...
        # `tampinha update` hang because it can never drain the gateway.
        "restart_loop_guard": {
            "max_restarts": 3,
            "window_seconds": 60,
            "max_gap_seconds": 300,
        ...
        # ``TAMPINHA_GATEWAY_MAX_STARTS`` / ``TAMPINHA_GATEWAY_START_WINDOW_S``
        ...
        # system-path denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.tampinha/.env,
        ...
        # behavior: files must live under the Tampinha cache, under
        ...
        # user. Bridged to TAMPINHA_MEDIA_DELIVERY_STRICT.
        "strict": False,
        # Extra directories from which model-emitted bare file paths may be
        # uploaded as native gateway attachments. Files inside the Tampinha
        # cache (~/.tampinha/cache/{documents,images,audio,video,screenshots})
        ...
        # to TAMPINHA_MEDIA_ALLOW_DIRS at gateway startup. Tilde paths are
        ...
        # TAMPINHA_MEDIA_TRUST_RECENT_FILES. Only consulted when ``strict``
        # is true; in default mode the denylist alone gates delivery.
        "trust_recent_files": True,
        # Recency window in seconds. 600 (10 min) comfortably covers a
        # multi-tool agent turn. Bridged to TAMPINHA_MEDIA_TRUST_RECENT_SECONDS.
    ...
    # Session storage — controls automatic cleanup of ~/.tampinha/state.db.
    ...
    "sessions": {
        ...
        # the default of ``tampinha sessions prune``.
        "retention_days": 90,
        ...
        # rewrites ``~/.tampinha/sessions/session_{sid}.json`` on every turn
        ...
        # user runs `tampinha sessions optimize-storage`, because the rebuild is
        # disk-heavy and long on large DBs (see that command's disk preflight).
        #
        #   "advise" (default): `tampinha update` prints a one-line notice with
        ...
        # ~/.tampinha/lib/libfts5_cjk.so), 1-2 char CJK terms (일본, 项目, ...)
        ...
        # serve the cjk index. Bridged to TAMPINHA_CJK_FTS (internal carrier).
        "cjk_fts": True,
        ...
        # TAMPINHA_SEARCH_SLOW_MS (internal carrier).
        "search_slow_ms": 1000,
        ...
        # (non-streaming) export such as `tampinha sessions export`. Checked
    ...
    # ``tampinha doctor`` behaviour.
    "doctor": {
        # Per-probe timeout (seconds) for the opt-in `tampinha doctor --live`
        # real-call backend probes (Firecrawl/FAL/browser/MCP/TTS/STT).
        "live_probe_timeout": 10,
    },

    # ``tampinha update`` behaviour.
    "updates": {
        ...
        #     DBs) into <TAMPINHA_HOME>/state-snapshots/ before the update.
        ...
        #     jobs) safety net.
        #   full — the quick snapshot PLUS a full ``tampinha backup``-style zip
        #     of TAMPINHA_HOME into <TAMPINHA_HOME>/backups/, restorable with
        #     ``tampinha import``. Can add minutes on large homes. This is the
        ...
        "backup_keep": 5,
        # What `tampinha update` does with uncommitted local changes to the
        ...
        # When `tampinha update` finds the source checkout parked on a feature
        ...
        #                         merge. `tampinha update --switch-branch`
    ...
    "lsp": {
        ...
        #              ``<TAMPINHA_HOME>/lsp/bin/`` on first use.
    ...
    # `tampinha tools`. These settings tune the backing Responses API call.
    "x_search": {
    ...
    # rather than storing them in ~/.tampinha/.env.
    "secrets": {
        ...
        "bitwarden": {
            ...
            # in ~/.tampinha/.env (or your shell) and never in config.yaml.
            ...
            # material under ~/.tampinha/cache/. If a later startup cannot reach
            # Bitwarden due to NETWORK/TIMEOUT, Tampinha may use this encrypted
            ...
            # ~/.tampinha/bin/ on first use.  When False you must install
            ...
            # `tampinha secrets bitwarden setup`.
            "server_url": "",
        },
        "onepassword": {
            ...
            # for headless auth.  Sourced from ~/.tampinha/.env (or the shell)
    ...
    "computer_use": {
        ...
        # by default upstream. Tampinha disables it for our users unless they
        # explicitly opt in here. When false (default), Tampinha sets
        ...
        # cua-driver ≥ 0.6.x supports --no-overlay; Tampinha also calls
        ...
        "no_overlay": None,
        # cua-driver permission mode for each Tampinha computer-use runtime.
        ...
        # manifest used when permission_mode is "bounded". Tampinha passes the
    ...
    #
    # Configure with `tampinha egress setup`.  Disabled by default — the rest of
    # Tampinha works exactly as before with `enabled: false`.
    "proxy": {
        ...
        "tunnel_port": 9090,
        # Auto-download the pinned iron-proxy binary into ~/.tampinha/bin/ on
    ...
    # Tampinha Desktop (Electron app) launch options. These only affect
    # `tampinha desktop`; they do not touch the CLI/gateway.
    "desktop": {
        ...
        # Bridged to the TAMPINHA_DESKTOP_DISABLE_GPU env var the Electron app reads.
        "disable_gpu": "auto",
        ...
        # Ignored on macOS/Windows. Bridged to the TAMPINHA_DESKTOP_PASSWORD_STORE
...
OPTIONAL_ENV_VARS = {
    ...
    "VERTEX_CREDENTIALS_PATH": {
        "description": "Path to a Google Cloud service account JSON for Vertex AI (Gemini). "
                       ...
                       "credentials Tampinha mints short-lived tokens from. Falls back to "
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'tampinha model' for endpoint-specific config)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the Tampinha auth store)",
    ...
    "PORCUPINE_ACCESS_KEY": {
        "description": "Picovoice access key for the Porcupine 'Hey Tampinha' wake word engine (optional; openWakeWord is the free default)",
    ...
    "SLACK_ALLOWED_USERS": {
        "description": "Comma-separated Slack member IDs allowed to use Tampinha, e.g. U01ABC2DEF3. Without this, Slack may connect but deny messages by default.",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @tampinha:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. TAMPINHA_BOT)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #tampinha)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: tampinha-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'tampinha-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Tampinha API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote Tampinha API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote Tampinha API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # TAMPINHA_TOOL_PROGRESS_MODE is deprecated — tool progress is configured via
    # display.tool_progress in config.yaml (off|new|all|verbose|log). The
    # gateway still falls back to TAMPINHA_TOOL_PROGRESS_MODE for backward
    ...
    # TAMPINHA_TOOL_PROGRESS is fully unsupported since the v12 config support
    # floor retired its only consumer (the v3→4 migration).
    "TAMPINHA_PREFILL_MESSAGES_FILE": {
    ...
    "TAMPINHA_EPHEMERAL_SYSTEM_PROMPT": {
