"""
Single source of truth for provider identity in Tampinha Agent.
...
2. **Tampinha overlays** — transport type, auth patterns, aggregator flags,
...
# -- Tampinha overlay ----------------------------------------------------------
# Tampinha-specific metadata that models.dev doesn't provide.

@dataclass(frozen=True)
class HermesOverlay:
    """Tampinha-specific provider metadata layered on top of models.dev."""
...
TAMPINHA_OVERLAYS: Dict[str, HermesOverlay] = {
    "moa": HermesOverlay(
    ...
    "qwen-oauth": HermesOverlay(
        ...
        base_url_env_var="TAMPINHA_QWEN_BASE_URL",
    ),
...
class ProviderDef:
    ...
    doc: str = ""
    source: str = ""                      # "models.dev", "tampinha", "user-config"
...
def get_provider(name: str, *, allow_network: bool = True) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. Tampinha overlays (for providers not in models.dev: nous, openai-codex, etc.)
      2. models.dev catalog + Tampinha overlay
    ...
    overlay = TAMPINHA_OVERLAYS.get(canonical)
    ...
    if mdev_info is not None:
        ...
        # Combine env vars: models.dev env + tampinha extra
    ...
    if overlay is not None:
        # Tampinha-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
    ...
    # tencent-tokenhub) are absent from models.dev and TAMPINHA_OVERLAYS, so
...
def determine_api_mode(provider: str, base_url: str = "", model: str = "") -> str:
    ...
    *model* is optional but required for dual-wire providers (Nous) whose
    ...
    # chat_completions. The Tampinha overlay still advertises openai_chat
    ...
    # Direct provider checks for providers not in TAMPINHA_OVERLAYS
    if provider == "bedrock":
        return "bedrock_converse"
...
def resolve_provider_full(
    ...
    # 0.5 Exact Tampinha provider IDs must win over LOSSY alias collapsing.
    ...
    if canonical != raw:
        try:
            from tampinha_cli.auth import PROVIDER_REGISTRY as _AUTH_PROVIDER_REGISTRY
