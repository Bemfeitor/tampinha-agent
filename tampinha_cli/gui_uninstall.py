"""
Tampinha Desktop (Chat GUI) uninstaller.
...
the Python agent or the user's config/data:
  ...
  1. Source-built GUI (``tampinha desktop`` / ``tampinha gui``)
     Built inside the agent checkout under ``$TAMPINHA_HOME/tampinha-agent/``:
       ...
       - ``$TAMPINHA_HOME/desktop-build-stamp.json`` (the build freshness stamp)
  ...
  2. Packaged distributable (DMG / NSIS / AppImage / deb / rpm)
     ...
     own bundled Electron + a per-user Electron ``userData`` directory:
       - macOS:   ``/Applications/Tampinha.app`` or ``~/Applications/Tampinha.app``
       - Windows: ``%LOCALAPPDATA%\\Programs\\Tampinha`` (NSIS per-user)
...
the app name ("Tampinha"), separate from ``$TAMPINHA_HOME``:
  - macOS:   ``~/Library/Application Support/Tampinha``
  - Windows: ``%APPDATA%\\Tampinha``
  - Linux:   ``$XDG_CONFIG_HOME/Tampinha`` (default ``~/.config/Tampinha``)
...
``tampinha uninstall --gui`` (and friends) without paying for the full CLI.
"""
...
def _agent_root(tampinha_home: Path) -> Path:
    """The agent checkout root — same layout install.sh / install.ps1 use."""
    return tampinha_home / "tampinha-agent"


def desktop_userdata_dir() -> Path:
    """Return the Electron ``userData`` directory for the desktop app.

    Mirrors Electron's ``app.getPath('userData')`` for an app named "Tampinha"
    ...
    if sys.platform == "darwin":
        return home / "Library" / "Application Support" / "Tampinha"
...
def source_built_gui_artifacts(tampinha_home: Path) -> "list[Path]":
    """GUI build artifacts produced by ``tampinha desktop`` inside the checkout.

    These are removable on a GUI uninstall without harming the agent: the
    Python agent runs from ``tampinha-agent/`` source + ``venv/`` and never
    needs the Electron build output or node_modules.
    """
    agent_root = _agent_root(tampinha_home)
    ...
    return [
        ...
        tampinha_home / "desktop-build-stamp.json",
    ]
...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    electron-builder output locations for the "Tampinha" product.
    """
    home = Path.home()
    paths: list[Path] = []
    if sys.platform == "darwin":
        paths += [
            Path("/Applications/Tampinha.app"),
            home / "Applications" / "Tampinha.app",
        ]
    elif sys.platform == "win32":
        local = os.environ.get("LOCALAPPDATA")
        local_base = Path(local) if local else (home / "AppData" / "Local")
        paths += [
            # NSIS per-user install (perMachine=false → Programs\Tampinha).
            local_base / "Programs" / "Tampinha",
            # Older / alternate layout some builds used.
            local_base / "tampinha-desktop",
        ]
        program_files = os.environ.get("ProgramFiles")
        if program_files:
            # NSIS per-machine fallback (needs admin to remove).
            paths.append(Path(program_files) / "Tampinha")
    else:
        ...
        # ``uninstall_gui``.
        from tampinha_cli.linux_desktop_entry import desktop_entry_path

        data = os.environ.get("XDG_DATA_HOME")
        data_base = Path(data) if data else (home / ".local" / "share")
        paths += [
            # The launcher entry `tampinha desktop` installs. Its icon lives
            # in the checkout, not in the installed app.
            desktop_entry_path(),
            # Some packaged builds emit this casing.
            data_base / "applications" / "Tampinha.desktop",
        ]
    return paths


def agent_is_installed(tampinha_home: Path) -> bool:
    """Return True when a usable Python agent install exists under TAMPINHA_HOME.
    ...
    if (agent_root / "tampinha_cli").is_dir():
        return True
...
def gui_is_installed(tampinha_home: Path) -> bool:
    """Return True when any desktop GUI artifact exists (built or packaged)."""
    for p in source_built_gui_artifacts(tampinha_home):
        if p.exists():
            return True
...
def gui_install_summary(tampinha_home: "Path | None" = None) -> dict:
...
def uninstall_gui(tampinha_home: "Path | None" = None, *, remove_userdata: bool = True) -> "list[Path]":
    ...
    Never touches ``tampinha-agent/tampinha_cli`` (agent source), ``venv/``, or any
    config / sessions / .env under ``$TAMPINHA_HOME``.
    ...
    if sys.platform.startswith("linux"):
        ...
        # Reindex so Tampinha disappears from the launcher.
        try:
            from tampinha_cli.linux_desktop_entry import (
                desktop_entry_path,
        ...
        log_info(
            ...
            "with your package manager (e.g. 'sudo apt remove tampinha' or "
            "'sudo dnf remove tampinha'). AppImage builds are a single file you "
