...
Usage:
    tampinha pairing list              # Show all pending + approved users
    tampinha pairing approve <platform> <request-id|code>  # Approve a pairing request
    tampinha pairing revoke <platform> <user_id> # Revoke user access
    tampinha pairing clear-pending     # Clear all expired/pending codes
"""

def pairing_command(args):
    """Handle tampinha pairing subcommands."""
    ...
    if action == "list":
        ...
        print("Usage: tampinha pairing {list|approve|revoke|clear-pending}")
        print("Run 'tampinha pairing --help' for details.")
...
def _cmd_list(store):
    ...
    if pending:
        ...
        print("\n  Approve with: tampinha pairing approve <platform> <request-id>")
...
def _cmd_approve(store, platform: str, code: str):
    ...
    if result:
        ...
        print(
            ...
            "~/.tampinha/platforms/pairing/_rate_limits.json\n".format(platform)
        )
    else:
        print(f"\n  Pairing request or code '{code}' not found or expired for platform '{platform}'.")
        print("  Run 'tampinha pairing list' to see pending requests.\n")
