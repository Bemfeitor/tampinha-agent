...
on Japanese locales, etc.).  Tampinha's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``tampinha_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Tampinha is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``TAMPINHA_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("TAMPINHA_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the Tampinha-managed Git install directories so
    ...
    # already-running shells don't see the change — which means tampinha
    ...
    # install without Tampinha-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Tampinha's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Tampinha's
    ``tampinha config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Tampinha (User env var in Windows Settings, or
    export in a PowerShell profile) and Tampinha picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Tampinha-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\tampinha\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``tampinha`` from right after install) retain their old PATH.

    Any subprocess Tampinha spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Tampinha-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "tampinha", "git", "cmd"),
        os.path.join(local_appdata, "tampinha", "git", "bin"),
        os.path.join(local_appdata, "tampinha", "git", "usr", "bin"),
        # Tampinha venv Scripts directory — host of the tampinha.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes tampinha, but harmless to include.
        os.path.join(local_appdata, "tampinha", "tampinha-agent", "venv", "Scripts"),
