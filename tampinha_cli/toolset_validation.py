...
the tool registry or launching Tampinha (mirrors the decoupled-helper pattern used
elsewhere in the CLI).

Motivated by #38798: a config migration silently rewrote the valid toolset name
``tampinha-cli`` to the non-existent ``tampinha``. ``resolve_toolset('tampinha')``
...
def validate_platform_toolsets(
    ...
    1. A toolset name that ``is_valid_toolset`` rejects — usually a corrupted or
       renamed entry. When ``tampinha-<platform>`` would have been valid (the exact
       #38798 shape, where ``cli`` held ``tampinha`` instead of ``tampinha-cli``),
    ...
    for platform, raw in platform_toolsets.items():
        ...
        for name in names:
            ...
            if is_valid_toolset(name):
                valid_count += 1
                continue
            suggestion = f"tampinha-{platform}"
            hint = (
    ...
    if valid_count == 0:
        warnings.append(
            ...
            "have no tools. Run `tampinha tools` to reconfigure."
        )
    return warnings
