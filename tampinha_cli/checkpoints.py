"""`tampinha checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.tampinha/checkpoints/``.  Actions:

    tampinha checkpoints               # same as `status`
    tampinha checkpoints status        # total size, project count, breakdown
    tampinha checkpoints list          # per-project checkpoint counts + workdir
    tampinha checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    tampinha checkpoints clear [-f]    # nuke the entire base (asks first)
    tampinha checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    tampinha checkpoints
    tampinha checkpoints prune --retention-days 3 --max-size-mb 200
    tampinha checkpoints clear -f
...
from tampinha_cli.sizefmt import format_bytes as _fmt_bytes
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: tampinha checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``tampinha checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `tampinha checkpoints` → status
