...
``tampinha mcp install <name>`` in the terminal.
"""
...
def setup_mcp_tool(
    ...
    if callback is None:
        return tool_error(
            "setup_mcp is only available in the Tampinha desktop app. Use the "
            "terminal instead: `tampinha mcp install <name>` for catalog entries, "
            "`tampinha mcp login <name>` for OAuth."
        )
...
SETUP_MCP_SCHEMA = {
    ...
    "description": (
        ...
        "Tampinha desktop chat. The card lets them install a catalog entry, "
        ...
        "continue without the server. Catalog names: run `tampinha mcp catalog` "
