...
def auth_json_path():
    """Return the Tampinha auth store path, respecting TAMPINHA_HOME overrides."""
    return get_tampinha_home() / "auth.json"
...
def peek_nous_access_token() -> Optional[str]:
    ...
    Availability scans (`tampinha tools`, banner/status paint, provider
...
# managed vendor's gateway URL is pinned: adding one is a Tampinha release, and
