...
def _block_message(operation: str, root: Path) -> str:
    ...
    return (
        f"Blocked: `{operation}` would rewrite Tampinha's live source checkout "
        ...
        "checkout, stop Tampinha, run the command externally, then restart "
        "Tampinha."
    )
...
def _scratch_dir_hint() -> str:
    ...
    base = Path(tampinha_home).expanduser() if tampinha_home else Path.home() / ".tampinha"
