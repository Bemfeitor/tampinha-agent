...
One ``CDPSupervisor`` runs per Tampinha ``task_id`` that has a reachable CDP
...
def _redact_cdp_error_text(exc: object) -> str:
    ...
    those credentials never reach Tampinha logs or tracebacks. Falls back to a
...
DIALOG_BRIDGE_HOST = "tampinha-dialog-bridge.invalid"
