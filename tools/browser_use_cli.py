...
_PRIVATE_BROWSER_SENTINEL = "_TAMPINHA_BU_PRIVATE_BROWSER"
...
_OWN_TAB_PREAMBLE = """\
# tampinha: pin this named session to its own tab (once per daemon process)
def _tampinha_ensure_own_tab():
    ...
    _marker = _os.path.join(
        _tf.gettempdir(), "tampinha-bu-owntab-%s-%s-%s" % (_uid, _name, _dpid)
    )
    ...
    try:
        ...
        pass
_tampinha_ensure_own_tab()
del _tampinha_ensure_own_tab
"""
...
def _base_subprocess_env() -> dict:
    ...
    # may differ from Tampinha's venv Python. PYTHONPATH/PYTHONHOME inherited
    # from the agent process point at Tampinha's venv site-packages, and a
...
def default_downgrade_notice() -> Optional[str]:
    ...
    try:
        ...
        stamp = Path(get_tampinha_home()) / "cache" / _NOTICE_STAMP_NAME
        try:
        ...
        return (
            ...
            "Run `tampinha tools` (Browser Automation → Browser Use) to install it, "
...
def _managed_bin_dir() -> Optional[str]:
    """Tampinha' own bin dir ($TAMPINHA_HOME/bin) — where install.sh puts uv/uvx
    and where install_cli() links the browser-use binary."""
    try:
        from tampinha_constants import get_tampinha_home

        return str(Path(get_tampinha_home()) / "bin")
...
def _find_cli() -> Optional[List[str]]:
    ...
    MANAGED-FIRST resolution: Tampinha' own ``$TAMPINHA_HOME/bin`` copy — the
    one every browser backend selection installs and updates via
    ``install_cli()`` — always wins, so all sessions drive one canonical,
    Tampinha-controlled binary. PATH and the user-level tool dir
...
def install_cli(timeout_s: int = 600) -> Tuple[bool, str]:
    ...
    Resolution order for uv: Tampinha' managed uv (bootstrapped on demand via
    ``tampinha_cli.managed_uv.ensure_uv``) → uv on PATH. The binary is linked
    into ``$TAMPINHA_HOME/bin`` (``UV_TOOL_BIN_DIR``) so ``_find_cli()``
    ...
    # prevent provisioning the canonical Tampinha-managed copy, or resolution
    # stays pinned to a binary we don't control (version drift, no updates
    # through tampinha tools).
...
def _workspace_dir(task_id: Optional[str]) -> Optional[str]:
    ...
    try:
        ...
        path = Path(get_tampinha_home()) / "cache" / "browser-use" / "workspace" / safe
...
def _resolve_backend_cdp(
    ...
    try:
        ...
        return (
            ...
            "switch backends via `tampinha tools` → Browser Automation."
        )
...
def _resolve_real_profile_cdp(env: dict, force_local: bool) -> Optional[str]:
    ...
    default Chromium with their logins — a browser Tampinha launches on a
    SNAPSHOT of their real profile (see tampinha_cli.browser_connect). Two ways
    in:
...
def _dynamic_schema_overrides() -> dict:
    ...
    if _real_profile_consented():
        ...
        props["local"] = {
            "type": "boolean",
            "description": (
                "Drive the user's own local browser (a Tampinha-managed copy of "
