...
Lets the LLM write a Python script that calls Tampinha tools via RPC,
collapsing multi-step tool chains into a single inference turn.

Architecture (two transports):

  **Local backend (UDS):**
  1. Parent generates a `tampinha_tools.py` stub module with UDS RPC functions
  ...
  **Remote backends (file-based RPC):**
  1. Parent generates `tampinha_tools.py` with file-based RPC stubs
...
# available on every platform Tampinha itself runs on.
...
# match a safe prefix, the operational TAMPINHA_ allowlist, or (on Windows) an
# OS-essential name.  Delegate-task child context is also an exact-name
# operational marker: without it, a sandbox script that spawns/imports Tampinha
# code can lose the DB-layer Kanban mutation guard while still inheriting
# TAMPINHA_HOME.
#
# NB: the broad "TAMPINHA_" prefix was deliberately removed (#27303) — it leaked
# TAMPINHA_*-named config that lacks a secret substring (e.g. TAMPINHA_BASE_URL,
# TAMPINHA_KANBAN_DB, TAMPINHA_*_WEBHOOK).  The child only needs the few
# location/profile vars in _TAMPINHA_CHILD_ALLOWED below; TAMPINHA_RPC_SOCKET /
# TAMPINHA_RPC_DIR / TZ / HOME are injected explicitly after scrubbing.
...
# Operational TAMPINHA_* vars the child legitimately needs by exact name — these
# are non-secret runtime-location flags (the same set tampinha_cli treats as the
# runtime location) that repo-root modules a sandbox script imports may read at
# import time.  None match _SECRET_SUBSTRINGS.
_TAMPINHA_CHILD_ALLOWED = frozenset({
    "TAMPINHA_HOME",
    "TAMPINHA_PROFILE",
    "TAMPINHA_CONFIG",
    "TAMPINHA_ENV",
...
def _scrub_child_env(source_env, is_passthrough=None, is_windows=None):
    ...
    Rules (order matters):
      ...
      4. Operational TAMPINHA_* vars (_TAMPINHA_CHILD_ALLOWED) pass by exact name.
    ...
    scrubbed = {}
    # Non-secret TAMPINHA_* vars dropped by the tightened allowlist (#27303). The
    # broad "TAMPINHA_" prefix used to pass these through; now only the
    # operational set does. The drop is intentional (those vars can carry
    # config like TAMPINHA_KANBAN_DB / TAMPINHA_BASE_URL), but a sandbox script
    ...
    for k, v in source_env.items():
        ...
        if any(k.startswith(p) for p in _SAFE_ENV_PREFIXES):
            scrubbed[k] = v
            continue
        if k in _TAMPINHA_CHILD_ALLOWED:
            scrubbed[k] = v
            continue
        if is_windows and k.upper() in _WINDOWS_ESSENTIAL_ENV_VARS:
            scrubbed[k] = v
            continue
        if k.startswith("TAMPINHA_"):
            # Non-secret (secrets were already dropped above) and not in any
            # allowlist — a deliberately-dropped TAMPINHA_* var.
            _dropped_tampinha.append(k)
    if _dropped_tampinha:
        logger.debug(
            "execute_code: dropped %d non-allowlisted TAMPINHA_* var(s) from the "
            ...
            len(_dropped_tampinha),
            ", ".join(sorted(_dropped_tampinha)),
        )
...
# ---------------------------------------------------------------------------
# tampinha_tools.py code generator
# ---------------------------------------------------------------------------
...
_TOOL_STUBS = {
    ...
    "write_file": (
        ...
        '"""Write content to a file (always overwrites). Returns dict with status. cross_profile=True opts out of the cross-Tampinha-profile soft guard."""',
    ...
    "patch": (
        ...
        '"""Targeted find-and-replace (mode="replace") or V4A multi-file patches (mode="patch"). Returns dict with status. cross_profile=True opts out of the cross-Tampinha-profile soft guard."""',
...
def _sandbox_failure_hint(stderr_text: str, enabled_tools=None) -> Optional[str]:
    ...
    tampinha_tools import misuse (importing tools that aren't in the sandbox,
    ...
    try:
        m = re.search(
            r"cannot import name '(\w+)' from 'tampinha_tools'", window
        )
        if m:
            missing = m.group(1)
...
def generate_tampinha_tools_module(enabled_tools: List[str],
                                 transport: str = "uds") -> str:
    """
    Build the source code for the tampinha_tools.py stub module.
...
"""Auto-generated Tampinha tools RPC stubs."""
...
def _connect():
    ...
    TAMPINHA_RPC_SOCKET can be either:
    ...
    global _sock
    if _sock is None:
        endpoint = os.environ["TAMPINHA_RPC_SOCKET"]
...
def _call(tool_name, args):
    ...
    request = json.dumps({
        "tool": tool_name,
        "args": args,
        "token": os.environ.get("TAMPINHA_RPC_TOKEN", ""),
    }) + "\\n"
    ...
    # re-accepts (TAMPINHA_RPC_PERSISTENT=1); retry once on a fresh socket.
    _attempts = 2 if os.environ.get("TAMPINHA_RPC_PERSISTENT") == "1" else 1
    with _call_lock:
...
"""Auto-generated Tampinha tools RPC stubs (file-based transport)."""
import json, os, shlex, tempfile, threading, time

_RPC_DIR = os.environ.get("TAMPINHA_RPC_DIR") or os.path.join(tempfile.gettempdir(), "tampinha_rpc")
_seq = 0
...
def _execute_remote(
    ...
    The script and the generated tampinha_tools.py module are shipped to
    ...
    sandbox_dir = f"{temp_dir}/tampinha_exec_{sandbox_id}"
    ...
    try:
        ...
        tools_src = generate_tampinha_tools_module(
            list(sandbox_tools), transport="file",
        )
        _ship_file_to_remote(env, f"{sandbox_dir}/tampinha_tools.py", tools_src)
        ...
        env_prefix = (
            f"TAMPINHA_RPC_DIR={shlex.quote(f'{sandbox_dir}/rpc')} "
            f"TAMPINHA_RPC_TOKEN={shlex.quote(rpc_token)} "
            f"PYTHONDONTWRITEBYTECODE=1"
        )
        tz = os.getenv("TAMPINHA_TIMEZONE", "").strip()
        if tz:
...
def _build_child_env(*, rpc_endpoint: str, rpc_token: str, tmpdir: str,
    ...
    child_env["TAMPINHA_RPC_SOCKET"] = rpc_endpoint
    child_env["TAMPINHA_RPC_TOKEN"] = rpc_token
    ...
    # TAMPINHA_TIMEZONE is an internal Tampinha setting and must not leak
    # into child processes.
    _tz_name = os.getenv("TAMPINHA_TIMEZONE", "").strip()
    if _tz_name:
        child_env["TZ"] = _tz_name
    child_env.pop("TAMPINHA_TIMEZONE", None)

    apply_subprocess_home_env(child_env)
    # ``tampinha_tools.py`` always lives in the staging directory, so that
    # directory must be importable even when project mode changes CWD.
    # Tampinha's own package root is useful too, but only when the child
    # uses the same Python environment. Project mode can select an
    # external venv; exposing Tampinha's site-packages to that interpreter
    # can mix incompatible compiled extensions (for example, Python 3.12
    # NumPy with a Python 3.9 project interpreter).
    #
    # Before re-injecting PYTHONPATH, strip Tampinha-owned entries that
    # leaked through _scrub_child_env (PYTHONPATH is in _SAFE_ENV_PREFIXES
    # so it passes the scrub).  They are redundant for same-Tampinha-
    ...
    _strip_tampinha_owned_pythonpath(child_env)
    _tampinha_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    _existing_pp = child_env.get("PYTHONPATH", "")
    _pp_parts = [tmpdir]
    if _uses_tampinha_python_environment(child_python):
        _pp_parts.append(_tampinha_root)
    elif child_python not in _external_env_logged:
        # Import behavior changes silently otherwise — surface it (once
        # per interpreter path) so "import tampinha_constants suddenly
        # fails" reports are diagnosable without log spam.
        _external_env_logged.add(child_python)
        logger.info(
            "execute_code: child interpreter %s is outside the Tampinha "
            "environment; tampinha root omitted from PYTHONPATH",
            child_python,
        )
    if _existing_pp:
...
def execute_code(
    ...
    # terminal() path refuses `launchctl bootout ai.tampinha.gateway`, but the
    ...
    # --- Set up temp directory with tampinha_tools.py and script.py ---
    tmpdir = tempfile.mkdtemp(prefix="tampinha_sandbox_")
    ...
    # selector from TAMPINHA_RPC_SOCKET (path vs. ``tcp://host:port``).
    ...
    if _use_tcp_rpc:
        ...
        sock_path = os.path.join(_sock_tmpdir, f"tampinha_rpc_{uuid.uuid4().hex}.sock")
    ...
    try:
        # Write the auto-generated tampinha_tools module.
        ...
        tools_src = generate_tampinha_tools_module(list(sandbox_tools))
        with open(os.path.join(tmpdir, "tampinha_tools.py"), "w", encoding="utf-8") as f:
            f.write(tools_src)
        ...
        #   connect.  TAMPINHA_RPC_SOCKET is set to ``tcp://127.0.0.1:<port>``
        ...
        # but scripts can still read secrets from disk (e.g. open('~/.tampinha/.env')).
...
def _load_config() -> dict:
    ...
    ``tampinha --tui`` where it is never used.  Read the lightweight raw config
...
def _get_execution_mode() -> str:
    ...
    Mode semantics:
      ...
      - ``strict``: scripts run in an isolated temp directory with
        ``sys.executable`` (tampinha-agent's python). Reproducible and the
...
# Interpreter paths already reported as outside the Tampinha environment —
...
def _python_environment_prefix(python_path: str) -> str:
    ...
    sticky empty result would silently drop the tampinha root from every
...
def _uses_tampinha_python_environment(python_path: str) -> bool:
    """Whether *python_path* belongs to Tampinha's active Python environment.
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    When tools are disabled via ``tampinha tools`` (e.g. web is turned off),
    ...
    # temp-dir staging and tampinha-agent's own python.
    if mode == "strict":
        cwd_note = (
            "Scripts run in their own temp dir, not the session's CWD — use absolute paths "
            "(os.path.expanduser('~/.tampinha/.env')) or terminal()/read_file() for user files."
        )
    else:
        cwd_note = (
    ...
    description = (
        "Run a Python script that calls Tampinha tools programmatically. "
        ...
        f"Available via `from tampinha_tools import ...`:\n\n"
        f"{tool_lines}\n\n"
            ...
            "properties": {
                "code": {
                    ...
                    "description": (
                        ...
                        f"`from tampinha_tools import {import_str}` "
