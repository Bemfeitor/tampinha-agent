...
Design contract (matches the Tampinha "no lazy-reading escape hatches / don't
...
def _check_metadata_block(frontmatter: Dict[str, Any]) -> List[LintFinding]:
    ...
    tampinha_meta = meta.get("tampinha") if isinstance(meta, dict) else None
    if not isinstance(tampinha_meta, dict):
        findings.append(
            LintFinding(
                WARNING,
                "missing-metadata",
                "frontmatter is missing metadata.tampinha.{tags, related_skills}.",
            )
        )
    else:
        if "tags" not in tampinha_meta:
            findings.append(
                LintFinding(
                    WARNING, "missing-metadata", "metadata.tampinha.tags is missing."
                )
            )
    author = str(frontmatter.get("author", ""))
    if author and author.strip().lower() in ("tampinha", "agent", "tampinha agent") and (
        author != "Tampinha Agent"
    ):
        findings.append(
            LintFinding(
                WARNING,
                "author-caps",
                f"author '{author}' should be 'Tampinha Agent' (proper caps) "
