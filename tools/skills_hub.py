#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the Tampinha Skills Hub.
...
Used by tampinha_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
def _skills_dir() -> Path:
    ...
    return Path(forced) if forced is not None else _tampinha_home() / "skills"
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            tampinha_meta = metadata.get("tampinha", {})
            if isinstance(tampinha_meta, dict):
                tags = tampinha_meta.get("tags", [])
        if not tags:
...
class UrlSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            tampinha_meta = metadata.get("tampinha", {})
            if isinstance(tampinha_meta, dict):
                raw_tags = tampinha_meta.get("tags", [])
...
class OptionalSkillSource(SkillSource):
    ...
    ~/.tampinha/skills/ during setup.  They are discoverable via the Skills Hub
    ...
    def _fetch_from_live_repo(self, rel: str) -> Optional[SkillBundle]:
        ...
        ``tampinha update``. Rather than telling them to update first, resolve
    ...
    def _scan_all(self) -> List[SkillMeta]:
        ...
        for skill_md in sorted(self._optional_dir.rglob("SKILL.md")):
            ...
            if isinstance(meta_block, dict):
                tampinha_meta = meta_block.get("tampinha", {})
                if isinstance(tampinha_meta, dict):
                    tags = tampinha_meta.get("tags", [])
...
def _category_skill_dirs(directory: Path) -> List[str]:
    ...
    ``tampinha_cli.skills_hub._existing_categories``.
    """
    skill_dirs: List[str] = []
...
# ---------------------------------------------------------------------------
# Tampinha centralized index source
# ---------------------------------------------------------------------------

TAMPINHA_INDEX_URL = "https://hermes-agent.nousresearch.com/docs/api/skills-index.json"
TAMPINHA_INDEX_TTL = 6 * 3600  # 6 hours


def _tampinha_index_cache_file() -> Path:
    return _index_cache_dir() / "tampinha-index.json"


def _load_tampinha_index() -> Optional[dict]:
    """Fetch the centralized skills index, with local cache.

    The index is a JSON file hosted on the docs site, rebuilt daily by CI.
    We cache it locally for TAMPINHA_INDEX_TTL seconds to avoid repeated
    downloads within a session.
    """
    # Check local cache
    tampinha_index_cache_file = _tampinha_index_cache_file()
    if tampinha_index_cache_file.exists():
        try:
            age = time.time() - tampinha_index_cache_file.stat().st_mtime
            if age < TAMPINHA_INDEX_TTL:
                return json.loads(tampinha_index_cache_file.read_text(encoding="utf-8"))
        except (OSError, json.JSONDecodeError):
            pass
    ...
    for accept_encoding in ("gzip, deflate", "identity"):
        try:
            ...
            if resp.status_code != 200:
                logger.debug("Tampinha index fetch returned %d", resp.status_code)
            ...
            logger.debug(
                "Tampinha index decode failed (Accept-Encoding=%s): %s",
                accept_encoding,
                e,
            )
            continue
        except (httpx.HTTPError, json.JSONDecodeError) as e:
            logger.debug("Tampinha index fetch failed: %s", e)
            return _load_stale_index_cache()
    ...
    # Cache locally
    try:
        tampinha_index_cache_file.parent.mkdir(parents=True, exist_ok=True)
        tampinha_index_cache_file.write_text(json.dumps(data), encoding="utf-8")
    except OSError:
        pass
...
class HermesIndexSource(SkillSource):
    """Skill source backed by the centralized Tampinha Skills Index.
    ...
    def _ensure_loaded(self) -> dict:
        if not self._loaded:
            self._index = _load_tampinha_index()
            self._loaded = True
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        if resolved:
            ...
            if bundle:
                bundle.source = entry.get("source", "tampinha-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "tampinha-index"),
            identifier=entry.get("identifier", ""),
...
def parallel_search_sources(
    ...
    *on_source_done* is an optional callback ``(source_id, count) -> None``
    ...
    if _effective_filter == "all":
        for src in sources:
            if (src.source_id() == "tampinha-index"
