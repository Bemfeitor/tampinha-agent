...
"""Auto-generated Tampinha session-kernel runner. One exec cell per request."""
import contextlib
import io
import json
import os
...
_SENTINEL = os.environ["TAMPINHA_KERNEL_SENTINEL"]
...
# tampinha-agent#88637 by @z80dev (stable owner id, owner-teardown disposal,
...
def _resolve_owner(task_id: str) -> str:
    ...
    try:
        ...
        if is_delegated_child_context():
            ...
            child_id = get_session_env("TAMPINHA_SESSION_ID", "") or (task_id or "")
...
def shutdown_kernels_for_owner(owner: str) -> None:
    ...
    (the /new + session-close disposal shape from tampinha-agent#88637).
    """
    if not owner:
        return
    with _KERNELS_LOCK:
...
def _rpc_forever(kernel: SessionKernel, max_tool_calls: int,
    ...
    client stub reconnects on its side (TAMPINHA_RPC_PERSISTENT).
...
def _spawn(kernel: SessionKernel, *, task_id: str, child_python: str,
    ...
    kernel.tmpdir = tempfile.mkdtemp(prefix="tampinha_kernel_")
    _sock_tmpdir = "/tmp" if sys.platform == "darwin" else tempfile.gettempdir()

    kernel.rpc_token = secrets.token_urlsafe(32)
    kernel.sentinel = "@@TAMPINHA-KERNEL-" + secrets.token_urlsafe(16) + "@@"
    ...
    if _IS_WINDOWS:
        ...
        kernel.sock_path = os.path.join(_sock_tmpdir, f"tampinha_rpc_{uuid.uuid4().hex}.sock")
    ...
    with open(os.path.join(kernel.tmpdir, "tampinha_tools.py"), "w", encoding="utf-8") as f:
        f.write(tools_src)
    runner_path = os.path.join(kernel.tmpdir, "tampinha_kernel_runner.py")
    ...
    child_env["TAMPINHA_KERNEL_SENTINEL"] = kernel.sentinel
    ...
    child_env["TAMPINHA_RPC_PERSISTENT"] = "1"
