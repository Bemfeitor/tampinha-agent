#!/usr/bin/env python3
"""Read which OS window sits directly underneath the Tampinha desktop window.
...
def read_window_below_tool(callback: Optional[Callable] = None) -> str:
    """Return the window underneath the Tampinha window as a JSON string."""
    if callback is None:
        return tool_error(
            "read_window_below is only available in the Tampinha desktop app."
        )
...
READ_WINDOW_BELOW_SCHEMA = {
    ...
    "description": (
        ...
        "Tampinha desktop window — what the user is working in behind this app. "
        ...
        "permission — never prompted for, noted in `note`). Other Tampinha "
        "windows are skipped: the nearest non-Tampinha window is reported. "
