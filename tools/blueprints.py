...
`tampinha skills publish` for sharing. No new source type, no new store, no new
...
@dataclass
class BlueprintSpec:
    """Parsed ``metadata.tampinha.blueprint`` automation spec for a skill."""
...
def parse_blueprint(skill_md_text: str) -> Optional[BlueprintSpec]:
    ...
    A skill is a blueprint iff ``metadata.tampinha.blueprint`` is a mapping containing
    ...
    tampinha = meta.get("tampinha") if isinstance(meta, dict) else None
    blueprint = tampinha.get("blueprint") if isinstance(tampinha, dict) else None
    if blueprint is None:
        return None
    if not isinstance(blueprint, dict):
        raise BlueprintError("metadata.tampinha.blueprint must be a mapping")
...
def export_blueprint(job: Dict[str, Any], body: str, *, blueprint_name: Optional[str] = None) -> str:
    ...
    and emit a SKILL.md (with a ``metadata.tampinha.blueprint`` block) they can hand
    to ``tampinha skills publish`` to share. ``body`` is the plain-language
