...
Copies bundled skills from the repo's skills/ directory into ~/.tampinha/skills/
...
The manifest lives at ~/.tampinha/skills/.bundled_manifest.
"""
...
for _stream in (sys.stdout, sys.stderr):
    if hasattr(_stream, "reconfigure"):
        try:
            ...
            pass
from tampinha_constants import get_bundled_skills_dir, get_tampinha_home, get_optional_skills_dir
...
# cron, kanban workers) import this module once under the launch TAMPINHA_HOME
# and later scope requests to a different profile via
# set_tampinha_home_override(). Frozen module constants would then resolve —
...
# re-resolve from the live profile-scoped TAMPINHA_HOME on every call.
_TAMPINHA_HOME_AT_IMPORT = TAMPINHA_HOME
_SKILLS_DIR_AT_IMPORT = SKILLS_DIR
_MANIFEST_FILE_AT_IMPORT = MANIFEST_FILE


def _tampinha_home() -> Path:
    """Return the active profile's TAMPINHA_HOME at call time."""
    configured = Path(TAMPINHA_HOME)
    if configured != _TAMPINHA_HOME_AT_IMPORT:
        return configured
...
# Marker file written by `tampinha profile create --no-skills` (named profiles)
# and by the installer's `--no-skills` flag (the default ~/.tampinha profile).
# When present in TAMPINHA_HOME, sync_skills() is a no-op so neither the
# installer, `tampinha update`, nor a direct sync re-injects bundled skills.
# Delete the file to opt back in. Mirrors
# tampinha_cli.profiles.NO_BUNDLED_SKILLS_MARKER (kept as a literal here to
...
def _essential_names() -> frozenset:
    ...
    try:
        ...
        return frozenset({"tampinha-agent"})


def _get_bundled_dir() -> Path:
    """Locate the bundled skills/ directory.

    Checks TAMPINHA_BUNDLED_SKILLS env var first (set by Nix wrapper),
...
def _read_suppressed_names() -> set:
    ...
    to reading ``~/.tampinha/skills/.curator_suppressed`` directly if that import
...
def _compute_relative_dest(skill_dir: Path, bundled_dir: Path) -> Path:
    ...
    e.g., bundled/skills/mlops/axolotl -> ~/.tampinha/skills/mlops/axolotl
    """
...
def _read_hub_install_paths() -> Set[str]:
    ...
    Hub-installed skills are owned by the hub (``tampinha skills uninstall``),
        ...
        if _dir_hash(candidate) != origin_hash:
            ...
            if not quiet:
                print(
                    ...
                    f"updates. Run `tampinha skills reset {skill_name} --restore` "
...
def sync_skills(quiet: bool = False) -> dict:
    """
    Sync bundled skills into ~/.tampinha/skills/ using the manifest.

    Returns:
        dict with keys: copied (list), updated (list), skipped (int),
                        user_modified (list), cleaned (list), total_bundled (int)
    """
    # Opt-out: a profile (named or the default ~/.tampinha) that wrote the
    ...
    # ``tampinha-agent`` skill is the agent's own operating manual and the
    ...
    essential_only = (_tampinha_home() / NO_BUNDLED_SKILLS_MARKER).exists()
    ...
    for skill_name, skill_src in bundled_skills:
        ...
        # (~/.tampinha/skills/.curator_suppressed) is written when the curator
        # archives a bundled skill with curator.prune_builtins enabled. Without
        # this skip, every `tampinha update` would resurrect a skill the user
                ...
                if dest.exists():
                    ...
                    if _dir_hash(dest) == bundled_hash:
                        ...
                        print(
                            ...
                            f"was kept. Run `tampinha skills reset {skill_name}` "
...
def _rmtree_writable(path: Path) -> None:
    ...
    # ``TAMPINHA_HOME/skills/`` to prevent the catastrophic wipe of
    # ``~/.tampinha/`` (``.env``, ``MEMORY.md``, ``kanban.db``, custom
    ...
    # ``TAMPINHA_HOME``. Require a strict-child relationship so both escape
...
def reset_bundled_skill(name: str, restore: bool = False) -> dict:
    ...
    if not in_manifest and not is_bundled:
        return {
            ...
            "message": (
                ...
                f"(Hub-installed skills use `tampinha skills uninstall`.)"
            ),
            "synced": None,
        }
    ...
    if restore and deleted_user_copy:
        ...
        message = (
            f"Cleared manifest entry for '{name}'. Future `tampinha update` runs "
...
def _is_tracked_user_modification(origin_hash: str, user_hash: str) -> bool:
    """Whether an on-disk skill counts as a user modification ``tampinha update`` keeps.
...
def list_user_modified_bundled_skills() -> List[dict]:
    """Return the bundled skills that ``tampinha update`` keeps because the user
    edited them locally.
...
def diff_bundled_skill(name: str) -> dict:
    ...
    edits or ``tampinha skills reset`` back to upstream.
    ...
    if bundled_src is None:
        return {
            ...
            "message": (
                ...
                f"diff against). Hub-installed skills use `tampinha skills inspect`."
            ),
        }
...
def set_bundled_skills_opt_out(enabled: bool) -> dict:
    ...
    When ``enabled`` is True, writes TAMPINHA_HOME/.no-bundled-skills so the
    installer, ``tampinha update``, and any direct sync stop seeding bundled
    skills. When False, removes the marker so seeding resumes on the next
    sync. This is the on-disk-state half of ``tampinha skills opt-out`` /
    ...
    Returns:
        dict with keys: ok (bool), changed (bool), marker (str path),
                        message (str).
    """
    marker = _tampinha_home() / NO_BUNDLED_SKILLS_MARKER
    existed = marker.exists()
    try:
        if enabled:
            _tampinha_home().mkdir(parents=True, exist_ok=True)
            marker.write_text(
                "This profile opted out of bundled-skill seeding "
                "(`tampinha skills opt-out`).\n"
            ...
            changed = existed
            message = (
                "Opted back in. The next `tampinha update` (or `tampinha skills "
...
def is_bundled_skills_opt_out() -> bool:
    ...
    return (_tampinha_home() / NO_BUNDLED_SKILLS_MARKER).exists()
...
if __name__ == "__main__":
    print("Syncing bundled skills into ~/.tampinha/skills/ ...")
