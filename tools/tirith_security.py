...
it is automatically downloaded from GitHub releases to $TAMPINHA_HOME/bin/tirith.
...
def _get_tampinha_home() -> str:
    """Return the Tampinha home directory, respecting TAMPINHA_HOME env var."""
    return str(get_tampinha_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_tampinha_home(), ".tirith-install-failed")
...
def _tampinha_bin_dir() -> str:
    """Return $TAMPINHA_HOME/bin, creating it if needed."""
    d = os.path.join(_get_tampinha_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $TAMPINHA_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_tampinha_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $TAMPINHA_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $TAMPINHA_HOME/bin/tirith
    ...
    tampinha_bin = os.path.join(_tampinha_bin_dir(), "tirith")
    if os.path.isfile(tampinha_bin) and os.access(tampinha_bin, os.X_OK):
        _resolved_path = tampinha_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return tampinha_bin
