"""Central registry for all tampinha-agent tools.
...
def _discovery_cache_path() -> Optional[Path]:
    ...
    try:
        ...
        # import time (tampinha_constants itself is stdlib-only, so no cycle).
        from tampinha_constants import get_tampinha_home

        return Path(get_tampinha_home()) / "cache" / "tool_discovery_cache.json"
    except Exception:
        return None
...
# timescales. Cache results for ~30 s so env-var flips via ``tampinha tools``
...
def check_fn_cache_scope() -> Optional[str]:
    ...
    cache. A multiplex gateway installs a Tampinha-home override for every
    ...
    try:
        ...
        browser_identity = (
            get_session_env("TAMPINHA_SESSION_ID", ""),
            get_session_env("TAMPINHA_BROWSER_CONTROL_PRINCIPAL", ""),
            get_session_env("TAMPINHA_BROWSER_CONTROL_TRANSPORT_FAMILY", ""),
        )
...
def invalidate_check_fn_cache() -> None:
    ...
    affect tool availability (e.g. ``tampinha tools enable``)."""
    with _check_fn_cache_lock:
...
class ToolRegistry:
    ...
    def __init__(self):
        ...
        # Plugin registrations are overlays keyed by resolved TAMPINHA_HOME. A
    ...
    def _plugin_namespace_of_module(
        ...
        if module_namespace.startswith("tampinha_plugins."):
    ...
    def deregister(self, name: str) -> None:
        ...
        with self._lock:
            ...
            if not entry.toolset.startswith("mcp-"):
                ...
                # (``tampinha_plugins.{name}``), not the exact module string.
                # A handler defined in ``tampinha_plugins.pkg.handlers`` is
                # still owned by the ``tampinha_plugins.pkg`` package — exact
    ...
    def get_definitions(self, tool_names: Set[str], quiet: bool = False) -> List[dict]:
        ...
        etc.); TTL chosen so env-var changes (``tampinha tools enable foo``)
