...
Tampinha Cloud, docker) is a persistent line. This module is the gateway-side
...
file plumbing on the gateway's own TAMPINHA root — no network. The public
...
# ``bot_mode.envelope_ttl_seconds`` default in tampinha_cli/config_defaults.py.
...
def _normalize_roster_row(row: Any) -> Optional[dict]:
    ...
    if not profile or not connection_id:
        return None
    if not handle:
        handle = "tampinha" if profile == "default" else profile
    if (
...
def _tampinha_cli() -> str:
    """Resolve the tampinha CLI beside this gateway's own interpreter.

    The deliver RPC runs on the target gateway, whose process is the venv
    python — its bin/Scripts directory holds the matching ``tampinha``
    entrypoint. A bare ``"tampinha"`` relies on PATH, which is exactly what
    ...
    sibling = exe.parent / ("tampinha.exe" if sys.platform == "win32" else "tampinha")
    if sibling.is_file():
        return str(sibling)
    found = shutil.which("tampinha")
    if found:
        return found
    return "tampinha"
...
# turns concurrently: deliveries spawn separate ``tampinha`` subprocesses, so
