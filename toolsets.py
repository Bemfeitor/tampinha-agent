...
_TAMPINHA_CORE_TOOLS = [
    ...
    # spawned as a kanban worker (TAMPINHA_KANBAN_TASK env set) or the current
...
TOOLSETS = {
    ...
    "x_search": {
        "description": (
            ...
            "or XAI_API_KEY). Off by default; enable in `tampinha tools` → "
            "X (Twitter) Search."
        ),
        "tools": ["x_search"],
        "includes": []
    ...
    "video_gen": {
        "description": (
            ...
            "``tampinha tools`` → Video Generation."
        ),
    ...
    "kanban": {
        "description": (
            ...
            "is spawned by the kanban dispatcher (TAMPINHA_KANBAN_TASK env "
    ...
    # Coding posture (base Tampinha — CLI/TUI/desktop/ACP). Auto-selected in a
    ...
    "coding": {
        ...
        # non-configurable-toolset recovery loop in tampinha_cli/tools_config.py).
        "posture": True,
    },
    
    # ==========================================================================
    # Full Tampinha toolsets (CLI + messaging platforms)
    #
    ...
    # the `tampinha send` CLI), not by the model deciding to send on its own.
    # ==========================================================================
    ...
    "tampinha-cli": {
        ...
        "tools": _TAMPINHA_CORE_TOOLS,
        "includes": []
    },

    "tampinha-cron": {
        # Mirrors tampinha-cli so cron's "default" toolset is the same set of
        # core tools users see interactively — then `tampinha tools` filters
        # them down per the platform config. _DEFAULT_OFF_TOOLSETS (moa,
        # homeassistant) are excluded by _get_platform_tools() unless
        # the user explicitly enables them.
        "description": "Default cron toolset - same core tools as tampinha-cli; gated by `tampinha tools`",
        "tools": _TAMPINHA_CORE_TOOLS,
        "includes": []
    },
    ...
    "tampinha-bluebubbles": {
        "description": "BlueBubbles iMessage bot toolset - Apple iMessage via local BlueBubbles server",
        "tools": _TAMPINHA_CORE_TOOLS,
        "includes": []
    },

    "tampinha-homeassistant": {
    ...
    "tampinha-email": {
        "description": "Email bot toolset - interact with Tampinha via email (IMAP/SMTP)",
        "tools": _TAMPINHA_CORE_TOOLS,
        "includes": []
    },
    ...
    "tampinha-sms": {
        "description": "SMS bot toolset - interact with Tampinha via SMS (Twilio)",
        "tools": _TAMPINHA_CORE_TOOLS,
        "includes": []
    },
    ...
    "tampinha-webhook": {
        ...
        "tools": _TAMPINHA_WEBHOOK_SAFE_TOOLS,
        "includes": []
    },
    ...
    "tampinha-gateway": {
        ...
        "tools": [],
        "includes": ["tampinha-telegram", "tampinha-discord", "tampinha-whatsapp", "tampinha-slack", "tampinha-signal", "tampinha-bluebubbles", "tampinha-homeassistant", "tampinha-email", "tampinha-sms", "tampinha-mattermost", "tampinha-matrix", "tampinha-dingtalk", "tampinha-feishu", "tampinha-wecom", "tampinha-wecom-callback", "tampinha-weixin", "tampinha-qqbot", "tampinha-webhook", "tampinha-yuanbao"]
    }
}
...
def bundle_non_core_tools(toolset_name: str) -> Set[str]:
    """Return a ``tampinha-*`` bundle's platform-specific tools, excluding core.

    Platform bundles are defined as ``_TAMPINHA_CORE_TOOLS + [platform extras]``.
    ...
    Bundle nesting is one level deep in practice (only ``tampinha-gateway``
...
def resolve_toolset(name: str, visited: Set[str] = None, *, include_registry: bool = True) -> List[str]:
    ...
    if not toolset:
        # Auto-generate a toolset for plugin platforms (tampinha-<name>).
        # Gives them _TAMPINHA_CORE_TOOLS plus any tools the plugin registered
        ...
        if include_registry and name.startswith("tampinha-"):
            platform_name = name[len("tampinha-"):]
            try:
                from gateway.platform_registry import platform_registry
                if platform_registry.is_registered(platform_name):
                    plugin_tools = set(_TAMPINHA_CORE_TOOLS)
                    try:
                        from tools.registry import registry
                        plugin_tools.update(
