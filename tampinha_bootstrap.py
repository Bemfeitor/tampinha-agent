"""Windows UTF-8 bootstrap for Tampinha entry points.
...
should be imported at the very top of every Tampinha entry point
(``tampinha``, ``tampinha-agent``, ``tampinha-acp``, ``python -m gateway.run``,
...
def suppress_platform_ver_console() -> None:
    ...
    Mirrors ``tampinha_cli._subprocess_compat.suppress_platform_ver_console``
    ...
    ``tampinha_bootstrap``, never ``tampinha_cli.main``.
    """
    if not _IS_WINDOWS:
        return
    try:
...
def harden_import_path(src_root: str | None = None) -> None:
    """Stop a package in the current directory from shadowing Tampinha modules.

    Tampinha ships top-level modules with common names (``utils``, ``proxy``,
    ...
    We drop the relative forms outright, then force the real Tampinha source root
    ...
    """
    root = src_root or os.environ.get("TAMPINHA_PYTHON_SRC_ROOT") or os.path.dirname(
...
def activate_durable_lazy_target() -> None:
    ...
    (``TAMPINHA_LAZY_INSTALL_TARGET``, e.g. ``/opt/data/lazy-packages``).
    ...
    """
    if not os.environ.get("TAMPINHA_LAZY_INSTALL_TARGET", "").strip():
        return
    try:
...
# Apply on import — entry points just need ``import tampinha_bootstrap``
# (or ``from tampinha_bootstrap import apply_windows_utf8_bootstrap``) at
