...
def _make_agent():
    tampinha_home = Path(tempfile.mkdtemp(prefix="tampinha-test-home-"))
    (tampinha_home / "logs").mkdir(parents=True, exist_ok=True)
    with (
        patch(
        ...
        patch("run_agent._tampinha_home", tampinha_home),
