...
binary download are exercised manually by `tampinha secrets bitwarden
...
def tampinha_home(tmp_path, monkeypatch):
    """Point Tampinha at an isolated home directory."""
    home = tmp_path / ".tampinha"
    home.mkdir()
    monkeypatch.setenv("TAMPINHA_HOME", str(home))
    # Some modules cache get_tampinha_home; clear if needed.
    import tampinha_constants
    if hasattr(tampinha_constants, "_TAMPINHA_HOME_CACHE"):
        tampinha_constants._TAMPINHA_HOME_CACHE = None  # type: ignore[attr-defined]
    return home
...
def test_install_bws_happy_path(tampinha_home, monkeypatch):
...
def test_env_loader_calls_bsm_when_enabled(tmp_path, monkeypatch):
    ...
    from tampinha_cli.env_loader import _apply_external_secret_sources
