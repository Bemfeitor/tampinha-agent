"""
Tests for timezone support (tampinha_time module + integration points).
...
import tampinha_time


def _reset_tampinha_time_cache():
    """Reset the tampinha_time module cache (replacement for removed reset_cache)."""
    tampinha_time._cached_tz = None
    tampinha_time._cached_tz_name = None
    tampinha_time._cache_resolved = False


# =========================================================================
# tampinha_time.now() — core helper
# =========================================================================
...
class TestHermesTimeNow:
    ...
    def test_valid_timezone_applies(self):
        ...
        os.environ["TAMPINHA_TIMEZONE"] = "Asia/Kolkata"
        result = tampinha_time.now()
    ...
    def test_utc_timezone(self):
        """UTC timezone works."""
        os.environ["TAMPINHA_TIMEZONE"] = "UTC"
        result = tampinha_time.now()
...
class TestGetTimezone:
    ...
    def test_returns_zoneinfo_for_valid(self):
        os.environ["TAMPINHA_TIMEZONE"] = "Europe/London"
        tz = tampinha_time.get_timezone()
...
class TestCodeExecutionTZ:
    ...
    def test_tz_injected_when_configured(self):
        """When TAMPINHA_TIMEZONE is set, child process sees TZ env var.
        ...
        #   2) TAMPINHA_TIMEZONE itself does NOT leak into the child env
        probe = (
            'import os; '
            'print("TZ=" + os.environ.get("TZ", "NOT_SET")); '
            'print("TAMPINHA_TIMEZONE=" + os.environ.get("TAMPINHA_TIMEZONE", "NOT_SET"))'
        )
        ...
        assert "TAMPINHA_TIMEZONE=NOT_SET" in result["output"], (
            "TAMPINHA_TIMEZONE should not leak into child env (only TZ)"
        )

    def test_tz_not_injected_when_empty(self):
        """When TAMPINHA_TIMEZONE is not set, child process has no TZ."""
        import json as _json
        os.environ.pop("TAMPINHA_TIMEZONE", None)
...
class TestCronTimezone:
    ...
    def test_ensure_aware_naive_preserves_absolute_time(self):
        ...
        Regression: the old code used replace(tzinfo=tampinha_tz) which shifted
        absolute time when system-local tz != Tampinha tz.  The fix interprets
    ...
    def test_get_due_jobs_naive_cross_timezone(self, tmp_path, monkeypatch):
        """Naive past timestamps must be detected as due even when Tampinha tz
        ...
        # Use a Tampinha timezone far behind UTC so that the numeric wall time
        # of the naive timestamp exceeds _tampinha_now's wall time — this would
        # have caused a false "not due" with the old replace(tzinfo=...) approach.
        os.environ["TAMPINHA_TIMEZONE"] = "Pacific/Midway"  # UTC-11
        _reset_tampinha_time_cache()
        ...
        due = get_due_jobs()
        assert len(due) == 1, (
            "Naive past timestamp should be due regardless of Tampinha timezone"
        )
    ...
    def test_create_job_stores_tz_aware_timestamps(self, tmp_path, monkeypatch):
        ...
        os.environ["TAMPINHA_TIMEZONE"] = "US/Eastern"
        _reset_tampinha_time_cache()
