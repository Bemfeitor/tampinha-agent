"""Runtime smoke test for Docker $TAMPINHA_HOME/logs/gateways seeding.

Build the real image and verify logs/ and logs/gateways/ exist and are
owned by the tampinha user after container boot.
...
registered later runs its log service as the dropped tampinha user and
...
def test_logs_gateways_seeded_and_tampinha_owned(
    built_image: str, container_name: str,
) -> None:
    """logs/ and logs/gateways/ must exist and be owned by tampinha after boot."""
    ...
    # Both must be owned by tampinha
    r = docker_exec_sh(
        container_name,
    ...
    assert "logs=tampinha" in r.stdout, (
        f"logs/ not owned by tampinha: {r.stdout}"
    )
    assert "gateways=tampinha" in r.stdout, (
        f"logs/gateways/ not owned by tampinha: {r.stdout}"
    )
