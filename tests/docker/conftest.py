...
Override the image with ``TAMPINHA_TEST_IMAGE`` env var to point at a pre-built
...
IMAGE_TAG = os.environ.get("TAMPINHA_TEST_IMAGE", "tampinha-agent-harness:latest")
...
def built_image() -> str:
    ...
    Override with ``TAMPINHA_TEST_IMAGE`` env var to point at a pre-built
    image (faster local iteration).
    """
    if os.environ.get("TAMPINHA_TEST_IMAGE"):
        return IMAGE_TAG
...
# ---------------------------------------------------------------------------
# docker_exec — default to the unprivileged tampinha user
# ---------------------------------------------------------------------------
#
# Background: every Tampinha runtime path inside the container drops to UID
# 10000 (the ``tampinha`` user) via ``s6-setuidgid tampinha``. ``docker exec``
...
# fails (PermissionError swallowed) for tampinha, so a test that ran as root
...
# helpers below run every probe under ``-u tampinha`` unless a specific
...
def docker_exec(
    ...
    *args: str,
    ...
    """Run a command inside ``container`` as ``user`` (default: tampinha).
