...
Precedence verified here:
    TAMPINHA_MAX_TOKENS env  >  model.max_tokens  >  per-provider
...
def isolated_home(tmp_path, monkeypatch):
    """Isolated TAMPINHA_HOME with a writable config.yaml and a clean module cache.

    These tests deliberately re-import ``tampinha_cli`` / ``gateway`` so each
    ...
    monkeypatch.delenv("TAMPINHA_MAX_TOKENS", raising=False)

    _saved = {
        k: v
        for k, v in sys.modules.items()
        if k.startswith(("tampinha_cli", "gateway"))
    }

    def write_cfg(body: str) -> None:
        (tampinha_home / "config.yaml").write_text(textwrap.dedent(body))

    def fresh_gateway():
        for mod in list(sys.modules.keys()):
            if mod.startswith(("tampinha_cli", "gateway")):
                del sys.modules[mod]
