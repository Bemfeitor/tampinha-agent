...
TAMPINHA_MAX_ITERATIONS, but a later per-turn load_dotenv(..., override=True)
can restore a stale .env TAMPINHA_MAX_ITERATIONS value before the next turn.
"""
...
def test_reload_runtime_env_preserves_config_terminal_backend(
    ...
    ``tampinha config get terminal.backend`` still says local.
    """
    tampinha_home = tmp_path / ".tampinha"
    tampinha_home.mkdir()
    (tampinha_home / "config.yaml").write_text(
    ...
    (tampinha_home / ".env").write_text("TERMINAL_ENV=docker\n", encoding="utf-8")
