...
class TestDefaultInteractionDispatch:
    ...
    async def test_update_prompt_click_writes_response_file(self, tmp_path, monkeypatch):
        """update_prompt:y click writes 'y' to ~/.tampinha/.update_response."""
        adapter = self._make_adapter()
        tampinha_home = tmp_path / "tampinha_home"
        tampinha_home.mkdir()
        monkeypatch.setattr(
            "tampinha_constants.get_tampinha_home",
            lambda: tampinha_home,
        )
        ...
        response = tampinha_home / ".update_response"
