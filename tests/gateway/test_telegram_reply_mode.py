...
class TestTelegramYamlConfigLoading:
    ...
    def _write_config(self, tmp_path, content: str):
        ...
        tampinha_home.mkdir()
        (tampinha_home / "config.yaml").write_text(content, encoding="utf-8")
    ...
    def test_extra_reply_to_mode_off(self, tmp_path, monkeypatch):
        ...
        tampinha_home = self._write_config(
            tmp_path, "telegram:\n  extra:\n    reply_to_mode: \"off\"\n"
        )
...
class TestDMTopicFallbackReplyToMode:
    ...
    reply_to_mode 'off' was ignored when sending via Tampinha-created DM topic
...
class TestDMTopicSyntheticSendRouting:
    ...
    response in a different chat lane than the Tampinha topic the session runs
    in. The no-anchor branch must prefer the Tampinha topic's
    ``message_thread_id`` whenever it resolves.
    """

    def test_no_anchor_prefers_tampinha_topic_thread_id(self):
