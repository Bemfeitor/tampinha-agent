...
closure the PR changed, against a real temp ``TAMPINHA_HOME``.
"""
...
def _stub_picker_dependencies(monkeypatch):
    ...
    monkeypatch.setattr(
        "tampinha_cli.model_switch.list_picker_providers",
    ...
    monkeypatch.setattr(
        "tampinha_cli.model_switch.resolve_display_context_length",
        lambda *a, **k: 272000,
    )
...
def _setup_isolated_home(tmp_path, monkeypatch, model_yaml_value):
    ...
    monkeypatch.setattr(gateway_run, "_tampinha_home", tampinha_home)
    _stub_picker_dependencies(monkeypatch)
    # save_config writes to ``get_tampinha_home() / config.yaml`` — point it here.
    monkeypatch.setattr("tampinha_constants.get_tampinha_home", lambda: tampinha_home)
    monkeypatch.setattr("tampinha_cli.config.get_tampinha_home", lambda: tampinha_home)
    return cfg_path
...
async def test_multiplex_picker_global_persists_only_named_profile(
    ...
    monkeypatch.setattr(gateway_run, "_tampinha_home", default_home)
    _stub_picker_dependencies(monkeypatch)
