...
def test_recover_closes_owned_db_when_unexpected_exception_escapes(
    ...
    db = InterruptingDB()
    monkeypatch.setattr("tampinha_state.SessionDB", lambda: db)
...
def test_get_flush_dir_uses_get_tampinha_home(tmp_path, monkeypatch):
    """Flush dir must use get_tampinha_home(), not hardcoded Path.home()."""
    import gateway.shutdown_flush as mod

    captured = {}

    def fake_get_tampinha_home():
        from pathlib import Path
        captured["called"] = True
        return tmp_path

    monkeypatch.setattr(
        "tampinha_constants.get_tampinha_home", fake_get_tampinha_home
    )
