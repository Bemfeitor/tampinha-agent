...
(/opt/tampinha/scripts/whatsapp-bridge) is read-only, so `npm install` fails
with EACCES. The resolver must detect the read-only install dir and mirror the
bridge source into a writable TAMPINHA_HOME location instead.
"""
import importlib
...
def test_readonly_install_mirrors_to_tampinha_home(tmp_path, monkeypatch):
    """A read-only install tree is mirrored into a writable TAMPINHA_HOME."""
    ...
    monkeypatch.setattr(
        "tampinha_constants.get_tampinha_home", lambda: tampinha_home
    )
    ...
    expected = tampinha_home / "scripts" / "whatsapp-bridge"
