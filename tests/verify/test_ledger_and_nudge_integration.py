...
- ``tampinha verify`` records into the evidence ledger (pass and fail),
- a passing run satisfies the verify-on-stop guard,
- the verify-on-stop nudge names ``tampinha verify --json`` when the workspace
...
def _workspace(tmp_path, *, scripts=None, manifest_recipe=None):
    ...
    if manifest_recipe is not None:
        tampinha_dir = project / ".tampinha"
        tampinha_dir.mkdir()
        (tampinha_dir / "environment.json").write_text(
...
def test_record_verify_run_marks_workspace_passed(tampinha_home):
    project = _workspace(tampinha_home)
    ...
    assert status["evidence"]["canonical_command"] == "tampinha verify"


def test_record_verify_run_records_failure(tampinha_home):
    project = _workspace(tampinha_home)
...
def test_cli_passing_run_writes_ledger_evidence(tampinha_home, capsys):
    project = _workspace(tampinha_home, manifest_recipe={"name": "Fake", "test": ["echo ok"]})
...
def test_cli_failing_run_writes_failed_evidence(tampinha_home, capsys):
    project = _workspace(tampinha_home, manifest_recipe={"name": "Fake", "test": ["false"]})
    code = run_verify_command(make_args(project))
    assert code == 1
    status = verification_status(session_id=None, cwd=project)
    assert status["status"] == "failed"
...
def test_cli_partial_run_records_targeted_scope(tampinha_home, capsys):
...
def test_cli_run_uses_tampinha_session_id_env(tampinha_home, capsys, monkeypatch):
    monkeypatch.setenv("TAMPINHA_SESSION_ID", "sess-42")
    project = _workspace(tampinha_home, manifest_recipe={"name": "Fake", "test": ["echo ok"]})
...
# ---------------------------------------------------------------------------
# closed loop: edit -> stop guard nudge -> tampinha verify -> guard satisfied
# ---------------------------------------------------------------------------


def test_passing_verify_run_satisfies_stop_guard(tampinha_home, capsys):
    project = _workspace(tampinha_home, manifest_recipe={"name": "Fake", "test": ["echo ok"]})
...
# ---------------------------------------------------------------------------
# nudge wording: recipe-aware `tampinha verify --json` suggestion
# ---------------------------------------------------------------------------


def test_nudge_mentions_tampinha_verify_when_recipe_has_start(tampinha_home):
    project = _workspace(tampinha_home, scripts={"test": "vitest", "dev": "vite"})
    changed = str(project / "src" / "app.ts")
    ...
    assert "tampinha verify --json" in nudge
    # The cheap verify commands are still listed first.
    assert "npm run test" in nudge


def test_nudge_mentions_tampinha_verify_when_manifest_exists(tampinha_home):
    # No start script, but a saved .tampinha/environment.json qualifies.
    project = _workspace(
        tampinha_home,
        scripts={"test": "vitest"},
...
def test_nudge_keeps_plain_wording_without_recipe_start(tampinha_home):
    # Verify commands but no start script and no manifest: today's wording.
    project = _workspace(tampinha_home, scripts={"test": "vitest"})
    changed = str(project / "src" / "app.ts")
    mark_workspace_edited(session_id="s1", cwd=project, paths=[changed])
    nudge = build_verify_on_stop_nudge(session_id="s1", changed_paths=[changed])
    assert nudge is not None
    assert "tampinha verify" not in nudge


def test_nudge_recipe_detection_failure_is_silent(tampinha_home, monkeypatch):
...
def test_detect_path_merges_project_facts_commands(tampinha_home, capsys):
    project = _workspace(tampinha_home)  # package.json with no scripts
...
def test_manifest_recipe_is_not_merged(tampinha_home, capsys):
...
def test_merge_skips_commands_recipe_already_has(tampinha_home, capsys):
    project = _workspace(tampinha_home, scripts={"test": "vitest"})
    code = run_verify_command(make_args(project, detect_only=True))
    assert code == 0
    payload = json.loads(capsys.readouterr().out)
