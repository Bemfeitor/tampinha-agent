...
Regression: the old implementation wrote ``tampinha_conversation_<ts>.json``
to the current working directory (CWD). Users who ran /save expected the
file to be discoverable via ``tampinha sessions browse``, but CWD-resident
snapshots are not indexed in the state DB and are generally invisible.
The fix writes snapshots under ``~/.tampinha/sessions/saved/`` and prints
...
def tampinha_home(tmp_path, monkeypatch):
    ...
    # Clear any cached tampinha_home computation
    import tampinha_constants
    if hasattr(tampinha_constants, "_tampinha_home_cache"):
        tampinha_constants._tampinha_home_cache = None
    return home
...
def test_save_conversation_writes_under_tampinha_home(tampinha_home, tmp_path, monkeypatch, capsys):
    """Snapshot must land under ~/.tampinha/sessions/saved/, not CWD."""
    # Change CWD to a different directory to prove the file does NOT go there.
    work = tmp_path / "somewhere-else"
    work.mkdir()
    monkeypatch.chdir(work)

    # Import fresh to pick up the TAMPINHA_HOME fixture
    for mod in [m for m in sys.modules if m.startswith("cli") or m == "tampinha_constants"]:
    ...
    cwd_leak = list(work.glob("tampinha_conversation_*.json"))
    assert not cwd_leak, f"snapshot leaked to CWD: {cwd_leak}"

    # File MUST be under ~/.tampinha/sessions/saved/
    saved_dir = tampinha_home / "sessions" / "saved"
    assert saved_dir.is_dir(), "expected saved/ subdirectory to be created"
    files = list(saved_dir.glob("tampinha_conversation_*.json"))
    ...
    assert "tampinha --resume 20260101_120000_abc123" in out, out


def test_save_conversation_empty_history_does_nothing(tampinha_home, capsys):
    for mod in [m for m in sys.modules if m.startswith("cli") or m == "tampinha_constants"]:
        sys.modules.pop(mod, None)
    import cli
...
def test_save_conversation_bare_shows_usage(tampinha_home, capsys):
...
def test_save_conversation_bad_format_shows_usage(tampinha_home, capsys):
    for mod in [m for m in sys.modules if m.startswith("cli") or m == "tampinha_constants"]:
        sys.modules.pop(mod, None)
    import cli
