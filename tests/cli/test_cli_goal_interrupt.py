...
def tampinha_home(tmp_path, monkeypatch):
    """Isolated TAMPINHA_HOME so SessionDB.state_meta writes stay hermetic."""
    home = tmp_path / ".tampinha"
    home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    monkeypatch.setenv("TAMPINHA_HOME", str(home))

    # Bust the goal module's DB cache so it re-resolves TAMPINHA_HOME each test.
...
class TestInterruptAutoPause:
    ...
    def test_interrupted_turn_is_resumable(self, tampinha_home):
        ...
        with patch("tampinha_cli.goals.judge_goal"):
...
class TestHealthyTurnStillRuns:
    ...
    def test_clean_response_marks_done_when_judge_says_done(self, tampinha_home):
...
class TestInterruptFlagLifecycle:
    def test_chat_resets_flag_at_entry(self, tampinha_home):
