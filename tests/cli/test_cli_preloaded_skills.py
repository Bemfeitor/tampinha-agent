...
def test_main_applies_preloaded_skills_to_system_prompt(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda skills, task_id=None: ("skill prompt", ["tampinha-agent-dev", "github-auth"], []),
    )

    with pytest.raises(SystemExit):
        cli_mod.main(skills="tampinha-agent-dev,github-auth", list_tools=True)
    ...
    assert cli_obj.preloaded_skills == ["tampinha-agent-dev", "github-auth"]
...
def test_show_banner_does_not_print_skills():
    ...
    cli_obj.preloaded_skills = ["tampinha-agent-dev", "github-auth"]
