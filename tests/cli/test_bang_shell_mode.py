...
from tampinha_cli.bang_shell import (
    USAGE_HINT,
    bang_shell_enabled,
    is_bang_command,
    parse_bang_command,
...
class TestBangContextGating:
    ...
    def test_enabled_in_plain_cli(self, monkeypatch):
        for var in ("TAMPINHA_GATEWAY_SESSION", "TAMPINHA_CRON_SESSION",
                    "TAMPINHA_SESSION_PLATFORM"):
            monkeypatch.delenv(var, raising=False)
        assert bang_shell_enabled() is True

    @pytest.mark.parametrize("var,value", [
        ("TAMPINHA_GATEWAY_SESSION", "1"),
        ("TAMPINHA_CRON_SESSION", "true"),
        ("TAMPINHA_SESSION_PLATFORM", "discord"),
    ])
    def test_disabled_in_non_cli_contexts(self, monkeypatch, var, value):
        for v in ("TAMPINHA_GATEWAY_SESSION", "TAMPINHA_CRON_SESSION",
                  "TAMPINHA_SESSION_PLATFORM"):
...
class TestBangHandlerDispatch:
    ...
    def test_bare_bang_prints_usage_and_runs_nothing(self):
        cli = _make_cli()
        with patch("tampinha_cli.bang_shell.run_bang_command") as runner:
...
class TestBangApprovalGate:
    ...
    def test_approval_gate_is_invoked_for_a_dangerous_command(self):
        ...
        with patch("tools.terminal_tool._check_all_guards", gate), \
             patch("tampinha_cli.bang_shell.run_bang_command", return_value=0):
    ...
    def test_denied_command_is_not_executed(self):
        ...
        with patch("tools.terminal_tool._check_all_guards", gate), \
             patch("tampinha_cli.bang_shell.run_bang_command") as runner:
