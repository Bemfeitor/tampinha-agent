...
``os.environ["TAMPINHA_YOLO_MODE"]``. That env var is captured once at
...
prompt. Only ``tampinha --yolo`` (process-start env), ``TAMPINHA_YOLO_MODE=1``,
and ``tampinha config set approvals.mode off`` actually bypassed.
...
def _clear_approval_state(monkeypatch):
    ...
    # Tampinha-driven test runs may inherit TAMPINHA_YOLO_MODE=1 from the parent
...
class TestIsSessionYoloActiveHelper:
    ...
    def test_helper_honors_frozen_yolo_mode(self):
        """``tampinha --yolo`` sets ``TAMPINHA_YOLO_MODE`` before tool imports, so
