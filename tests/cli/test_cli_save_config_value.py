...
class TestSaveConfigValueAtomic:
    ...
    def config_env(self, tmp_path, monkeypatch):
        ...
        # save_config_value resolves the target live via get_tampinha_home(), so
        # point TAMPINHA_HOME at the temp dir (the _tampinha_home import-time
        # constant is no longer consulted).
        monkeypatch.setenv("TAMPINHA_HOME", str(tampinha_home))
        monkeypatch.setattr("cli._tampinha_home", tampinha_home)
        return config_path
    ...
    def test_model_write_runs_shared_cron_drift_warning(self, config_env, monkeypatch):
        warning = MagicMock()
        monkeypatch.setattr(
            "tampinha_cli.config.warn_unpinned_cron_jobs_after_model_config_change",
            warning,
        )
...
class TestSaveConfigValueTargetsUserConfig:
    """Regression: persisted runtime settings must land in TAMPINHA_HOME/config.yaml
    ...
    install whose TAMPINHA_HOME/config.yaml did not exist yet, save_config_value
    fell back to the checked-in cli-config.yaml. The toggle reported success, but
    startup read TAMPINHA_HOME/config.yaml and never saw the setting."""

    def test_creates_user_config_when_absent(self, tmp_path, monkeypatch):
        # Fresh TAMPINHA_HOME with NO config.yaml (managed/desktop first launch).
        tampinha_home = tmp_path / ".tampinha"
        tampinha_home.mkdir()
        monkeypatch.setenv("TAMPINHA_HOME", str(tampinha_home))
    ...
    def test_does_not_write_repo_cli_config(self, tmp_path, monkeypatch):
        ...
        result = yaml.safe_load((tampinha_home / "config.yaml").read_text())
        assert result["wake_word"]["enabled"] is True
