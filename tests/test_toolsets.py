...
class TestToolsetConsistency:
    ...
    def test_tampinha_platforms_share_core_tools(self):
        """All tampinha-* platform toolsets share the same core tools.

        Platform-specific additions (e.g. ``discord`` / ``discord_admin``
        on tampinha-discord, gated on DISCORD_BOT_TOKEN) are allowed on top —
        the invariant is that the core set is identical across platforms.
        """
        platforms = ["tampinha-cli", "tampinha-telegram", "tampinha-discord", "tampinha-whatsapp", "tampinha-slack", "tampinha-signal", "tampinha-homeassistant"]
...
class TestDefaultPlatformWebSearchCoverage:
    def test_tampinha_whatsapp_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("tampinha-whatsapp")
...
class TestResolveToolsetMemo:
    ...
    def test_second_resolution_is_cached(self, monkeypatch):
        ...
        first = resolve_toolset("tampinha-cli")
        second = resolve_toolset("tampinha-cli")
        ...
        assert (
            "tampinha-cli", True, registry_id, generation
    ...
    def test_memo_result_matches_fresh_resolution(self):
        ...
        toolsets_mod._resolve_toolset_memo.clear()
        first = resolve_toolset("tampinha-cli", include_registry=False)
        second = resolve_toolset("tampinha-cli", include_registry=False)
        assert first == second
