...
def progress(tmp_path):
    ...
    status = tmp_path / "tampinha-update-status"
...
def test_every_state_carries_a_clock(progress):
    ...
    for state, message in [
        ...
        ("done", ""),
        ("manual", "Reopen Tampinha to finish."),
        ("error", "Update failed."),
    ]:
...
# Stands in for `tampinha update`, and reports the stage that was on screen
...
n="$(cat "$TAMPINHA_TEST_CALLS" 2>/dev/null || echo 0)"; n=$((n + 1))
printf '%s' "$n" > "$TAMPINHA_TEST_CALLS"
for f in "$TMPDIR"/tampinha-update-status.[0-9]*; do
  case "$f" in *.tmp) continue ;; esac
  cp "$f" "$TAMPINHA_TEST_CAPTURE.$n" 2>/dev/null
done
exit "$(cat "$TAMPINHA_TEST_EXITS.$n" 2>/dev/null || echo 0)"
"""
...
def _run_handoff(tmp_path, exits: dict[int, int]) -> list[dict]:
    ...
    install_root = tmp_path / "tampinha-agent"
    (install_root / "venv" / "bin").mkdir(parents=True)
    tampinha = install_root / "venv" / "bin" / "tampinha"
    tampinha.write_text(FAKE_TAMPINHA)
    tampinha.chmod(0o755)
    ...
    env = {
        **os.environ,
        "TMPDIR": str(tmp_path),
        "TAMPINHA_TEST_CAPTURE": str(capture),
        "TAMPINHA_TEST_CALLS": str(calls),
        "TAMPINHA_TEST_EXITS": str(tmp_path / "exits"),
    }
    ...
    result = tmp_path / ".tampinha-update-result.json"
...
def test_update_gate_publishes_its_stage_before_running(tmp_path):
    """`tampinha update` is the longest wait in the hand-off and the one the
