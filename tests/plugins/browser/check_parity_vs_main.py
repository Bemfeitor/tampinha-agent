...
Run from the PR worktree:
    ...
    cd ~/.tampinha/tampinha-agent/.worktrees/browser-providers-plugin
...
# checkout lives two levels up at ``~/.tampinha/tampinha-agent``.
MAIN_DIR = REPO_ROOT.parent.parent  # ~/.tampinha/tampinha-agent
PR_DIR = REPO_ROOT  # the worktree we're in
assert (MAIN_DIR / "tools" / "browser_tool.py").exists(), (
    f"MAIN_DIR={MAIN_DIR} doesn't look like a tampinha-agent checkout"
)
...
# Isolated TAMPINHA_HOME for the config write.
