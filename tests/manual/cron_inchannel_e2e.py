...
Uses a throwaway TAMPINHA_HOME so it never touches ~/.tampinha. No real names.
"""
...
def _fresh_home():
    """Point TAMPINHA_HOME at a throwaway dir BEFORE importing gateway modules
    (mirror.py binds _SESSIONS_INDEX from get_tampinha_home() at import time)."""
    d = tempfile.mkdtemp(prefix="cron_inchannel_e2e_")
    os.environ["TAMPINHA_HOME"] = d
    return Path(d)


HOME = _fresh_home()

# Import AFTER TAMPINHA_HOME is set.
...
# a different get_tampinha_home() at import if something imported it earlier).
...
def main():
    ...
    print(f"TAMPINHA_HOME (throwaway): {HOME}")
