...
class TestWriteDenyExactPaths:
    ...
    def test_tampinha_root_env_when_running_under_profile(self, tmp_path, monkeypatch):
        ...
        ``~/.tampinha/.env`` (whose credentials are inherited by every profile)
        could be silently overwritten by ``write_file`` while a profile was
        active.
        """
        root = tmp_path / "tampinha_root"
        ...
        # Sanity check: TAMPINHA_HOME does point to the profile dir, not the root.
        from tampinha_constants import get_tampinha_home, get_default_tampinha_root
...
class TestWriteAllowed:
    ...
    def test_tampinha_control_files_requested_writable(self):
