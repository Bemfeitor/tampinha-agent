...
was gated on ``[ -f "$TAMPINHA_HOME/.env" ]`` while the first-boot seed that
...
def _run_keygen(
    ...
    script = (
        ...
        "set -eu\n"
        f"{env_setup}"
        f'TAMPINHA_HOME="{home}"\n'
        # In tests we run unprivileged; as_tampinha is a passthrough then.
        'as_tampinha() { "$@"; }\n'
...
def test_keygen_skips_when_container_env_provides_key(
    ...
    Tampinha loads $TAMPINHA_HOME/.env with override=True, so a key generated
...
def test_dockerignore_keeps_env_example_template() -> None:
    """The first-boot seed copies /opt/tampinha/.env.example -> $TAMPINHA_HOME/.env.
    ...
    assert verdict == "included", (
        ...
        "seeds $TAMPINHA_HOME/.env from it on first boot"
    )
