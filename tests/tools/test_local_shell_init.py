...
class TestSnapshotEndToEnd:
    ...
    def test_exported_env_changes_persist_between_commands(self, tmp_path):
        ...
        try:
            first = env.execute(
                'export TAMPINHA_STICKY_ENV_PROBE="sticky"; '
                'export PATH="/tmp/tampinha-session-bin:$PATH"; '
                'echo "first=$TAMPINHA_STICKY_ENV_PROBE"'
            )
            second = env.execute(
                'echo "second=$TAMPINHA_STICKY_ENV_PROBE"; echo "PATH=$PATH"'
            )
        finally:
            env.cleanup()
    ...
    def test_snapshot_picks_up_init_file_exports(self, tmp_path, monkeypatch):
        ...
        with patch(
            ...
            try:
                result = env.execute(
                    'echo "PROBE=$TAMPINHA_SHELL_INIT_PROBE"; echo "PATH=$PATH"'
                )
            finally:
                env.cleanup()
