...
def worker_home(tmp_path, monkeypatch):
    ...
    for var in ("TAMPINHA_KANBAN_DB", "TAMPINHA_KANBAN_WORKSPACES_ROOT", "TAMPINHA_KANBAN_HOME", "TAMPINHA_KANBAN_BOARD"):
...
def test_seed_then_inject_new_comment(worker_home, monkeypatch):
    ...
    monkeypatch.setenv("TAMPINHA_PROFILE", "worker-bot")
    agent = FakeAgent()
