...
class TestRegisterCredentialFiles:
    def test_dict_with_path_key(self, tmp_path):
        ...
        tampinha_home.mkdir()
        (tampinha_home / "token.json").write_text("{}")

        with patch.dict(os.environ, {"TAMPINHA_HOME": str(tampinha_home)}):
            missing = register_credential_files([{"path": "token.json"}])

        assert missing == []
        mounts = get_credential_file_mounts()
        assert len(mounts) == 1
        assert mounts[0]["host_path"] == str(tampinha_home / "token.json")
        assert mounts[0]["container_path"] == "/root/.tampinha/token.json"
    ...
    def test_path_takes_precedence_over_name(self, tmp_path):
        ...
        tampinha_home = tmp_path / ".tampinha"
        tampinha_home.mkdir()
        (tampinha_home / "real.json").write_text("{}")
...
class TestSkillsDirectoryMount:
    def test_returns_mount_when_skills_dir_exists(self, tmp_path):
        tampinha_home = tmp_path / ".tampinha"
        skills_dir = tampinha_home / "skills"
        ...
        assert mounts[0]["container_path"] == "/root/.tampinha/skills"


    def test_custom_container_base(self, tmp_path):
        tampinha_home = tmp_path / ".tampinha"
        (tampinha_home / "skills").mkdir(parents=True)

        with patch.dict(os.environ, {"TAMPINHA_HOME": str(tampinha_home)}):
            mounts = get_skills_directory_mount(container_base="/home/user/.tampinha")

        assert mounts[0]["container_path"] == "/home/user/.tampinha/skills"
...
class TestIterSkillsFiles:
    def test_returns_files_skipping_symlinks(self, tmp_path):
        ...
        assert "/root/.tampinha/skills/cat/myskill/SKILL.md" in paths
        assert "/root/.tampinha/skills/cat/myskill/scripts/run.sh" in paths
...
class TestPathTraversalSecurity:
    ...
    def test_dotdot_traversal_rejected(self, tmp_path, monkeypatch):
        """'../sensitive' must not escape TAMPINHA_HOME."""
        monkeypatch.setenv("TAMPINHA_HOME", str(tmp_path / ".tampinha"))
        (tmp_path / ".tampinha").mkdir()

        # Create a sensitive file one level above tampinha_home
    ...
    def test_deep_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        # Create a fake sensitive file outside tampinha_home
    ...
    def test_nested_subdir_inside_tampinha_home_allowed(self, tmp_path, monkeypatch):
        """Files in subdirectories of TAMPINHA_HOME must be allowed."""
        tampinha_home = tmp_path / ".tampinha"
        tampinha_home.mkdir()
        subdir = tampinha_home / "creds"
        subdir.mkdir()
    ...
    def test_symlink_traversal_rejected(self, tmp_path, monkeypatch):
        """A symlink inside TAMPINHA_HOME pointing outside must be rejected."""
        tampinha_home = tmp_path / ".tampinha"
        tampinha_home.mkdir()
        monkeypatch.setenv("TAMPINHA_HOME", str(tampinha_home))

        # Create a sensitive file outside tampinha_home
        sensitive = tmp_path / "sensitive.json"
        sensitive.write_text('{"secret": "value"}')

        # Create a symlink inside tampinha_home pointing outside
        symlink = tampinha_home / "evil_link.json"
        try:
        ...
        # The resolved path escapes TAMPINHA_HOME — must be rejected
        assert result is False
        assert get_credential_file_mounts() == []
...
class TestConfigPathTraversal:
    ...
    def _write_config(self, tampinha_home: Path, cred_files: list):
        import yaml
    ...
    def test_config_traversal_rejected(self, tmp_path, monkeypatch):
        """'../secret' in config.yaml must not escape TAMPINHA_HOME."""
        tampinha_home = tmp_path / ".tampinha"
        tampinha_home.mkdir()
        monkeypatch.setenv("TAMPINHA_HOME", str(tampinha_home))
        ...
        sensitive.write_text("{}")
        self._write_config(tampinha_home, ["../secret.json"])
    ...
    def test_config_absolute_path_rejected(self, tmp_path, monkeypatch):
        ...
        sensitive.write_text("{}")
        self._write_config(tampinha_home, [str(sensitive)])

        mounts = get_credential_file_mounts()
        assert mounts == []

    def test_config_legitimate_file_works(self, tmp_path, monkeypatch):
        """Normal files inside TAMPINHA_HOME via config must still mount."""
        tampinha_home = tmp_path / ".tampinha"
        tampinha_home.mkdir()
        monkeypatch.setenv("TAMPINHA_HOME", str(tampinha_home))

        (tampinha_home / "oauth.json").write_text("{}")
        self._write_config(tampinha_home, ["oauth.json"])
...
class TestCacheDirectoryMounts:
    ...
    def test_returns_existing_cache_dirs(self, tmp_path, monkeypatch):
        ...
        tampinha_home = tmp_path / ".tampinha"
        tampinha_home.mkdir()
        (tampinha_home / "cache" / "documents").mkdir(parents=True)
        (tampinha_home / "cache" / "audio").mkdir(parents=True)
        (tampinha_home / "cache" / "videos").mkdir(parents=True)
        monkeypatch.setenv("TAMPINHA_HOME", str(tampinha_home))

        mounts = get_cache_directory_mounts()
        paths = {m["container_path"] for m in mounts}
        assert "/root/.tampinha/cache/documents" in paths
        assert "/root/.tampinha/cache/audio" in paths
        assert "/root/.tampinha/cache/videos" in paths
    ...
    def test_legacy_dir_names_resolved(self, tmp_path, monkeypatch):
        ...
        ``has content`` for ``get_tampinha_dir``'s populated-legacy check
        (see #27602 — empty legacy stubs are no longer honoured).
        """
        tampinha_home = tmp_path / ".tampinha"
        tampinha_home.mkdir()
        # Use legacy dir name with content — get_tampinha_dir prefers
        # populated old over new.
        legacy_doc = tampinha_home / "document_cache"
        legacy_img = tampinha_home / "image_cache"
        legacy_doc.mkdir()
        legacy_img.mkdir()
        ...
        mounts = get_cache_directory_mounts()
        host_paths = {m["host_path"] for m in mounts}
        assert str(tampinha_home / "document_cache") in host_paths
        assert str(tampinha_home / "image_cache") in host_paths
        # Container paths always use the new layout
        container_paths = {m["container_path"] for m in mounts}
        assert "/root/.tampinha/cache/documents" in container_paths
        assert "/root/.tampinha/cache/images" in container_paths
    ...
    def test_empty_tampinha_home(self, tmp_path, monkeypatch):
        ...
        mounts = get_cache_directory_mounts()
        container_paths = {m["container_path"] for m in mounts}
        assert "/root/.tampinha/attachments" in container_paths
        assert "/root/.tampinha/images" in container_paths
        assert "/root/.tampinha/cache/images" in container_paths
        for mount in mounts:
    ...
    def test_images_upload_dir_is_mounted(self, tmp_path, monkeypatch):
        ...
        Desktop / clipboard / PDF uploads land in ``TAMPINHA_HOME/images``, not
        ...
        """
        tampinha_home = tmp_path / ".tampinha"
        (tampinha_home / "images").mkdir(parents=True)
        monkeypatch.setenv("TAMPINHA_HOME", str(tampinha_home))

        mounts = get_cache_directory_mounts()
        by_container = {m["container_path"]: m["host_path"] for m in mounts}
        assert "/root/.tampinha/images" in by_container
        assert by_container["/root/.tampinha/images"] == str(tampinha_home / "images")
    ...
    def test_images_upload_file_maps_into_container(self, tmp_path, monkeypatch):
        ...
        tampinha_home = tmp_path / ".tampinha"
        (tampinha_home / "images").mkdir(parents=True)
        upload = tampinha_home / "images" / "upload_20260722_181019_1.png"
        upload.write_bytes(bytes.fromhex("89504e470d0a1a0a"))
        monkeypatch.setenv("TAMPINHA_HOME", str(tampinha_home))

        assert (
            map_cache_path_to_container(str(upload))
            == "/root/.tampinha/images/upload_20260722_181019_1.png"
        )
...
class TestMapCachePathToContainer:
    ...
    def test_maps_path_under_cache_dir(self, tmp_path, monkeypatch):
        tampinha_home = tmp_path / ".tampinha"
        img_dir = tampinha_home / "cache" / "images"
        ...
        assert (
            ...
            == "/root/.tampinha/cache/images/generated.png"
        )
    ...
    def test_maps_path_even_when_cache_dir_missing(self, tmp_path, monkeypatch):
        ...
        mapped = map_cache_path_to_container(str(tampinha_home / "cache" / "images" / "x.png"))
        assert mapped == "/root/.tampinha/cache/images/x.png"
        assert (tampinha_home / "cache" / "images").is_dir()


class TestToAgentVisiblePathPerBackend:
    """#76577 follow-up: translation covers every backend that relocates the
    Tampinha cache — not just docker — and skips the ones where the host path
    stays correct (local; singularity auto-binds the host home)."""

    def _staged(self, tmp_path, monkeypatch):
        tampinha_home = tmp_path / ".tampinha"
        (tampinha_home / "attachments").mkdir(parents=True)
        monkeypatch.setenv("TAMPINHA_HOME", str(tampinha_home))
        return str(tampinha_home / "attachments" / "drop.zip")

    def test_docker_maps_to_root_tampinha(self, tmp_path, monkeypatch):
        staged = self._staged(tmp_path, monkeypatch)
        monkeypatch.setenv("TERMINAL_ENV", "docker")
        from tools.credential_files import to_agent_visible_cache_path
        assert to_agent_visible_cache_path(staged) == "/root/.tampinha/attachments/drop.zip"

    def test_ssh_maps_to_tilde_tampinha(self, tmp_path, monkeypatch):
        staged = self._staged(tmp_path, monkeypatch)
        monkeypatch.setenv("TERMINAL_ENV", "ssh")
        from tools.credential_files import to_agent_visible_cache_path
        assert to_agent_visible_cache_path(staged) == "~/.tampinha/attachments/drop.zip"
...
class TestIterCacheFiles:
    ...
    def test_enumerates_files(self, tmp_path, monkeypatch):
        ...
        tampinha_home = tmp_path / ".tampinha"
        doc_dir = tampinha_home / "cache" / "documents"
...
class TestMasterCredentialStoresAreNeverMountable:
    """Containment is not enough — TAMPINHA_HOME *is* where the keys live.
    ...
    ``../../.ssh/id_rsa`` from escaping TAMPINHA_HOME, but every master
    ...
    def test_master_credential_store_is_refused(self, tmp_path, rel_path):
        ...
        with patch.dict(os.environ, {"TAMPINHA_HOME": str(home)}):
    ...
    def test_per_service_token_still_mounts(self, tmp_path):
        ...
        assert [m["container_path"] for m in mounts] == [
            "/root/.tampinha/google_token.json"
        ]
    ...
    def test_refused_entry_does_not_block_the_rest_of_the_batch(self, tmp_path):
        ...
        paths = [m["container_path"] for m in mounts]
        assert "/root/.tampinha/google_token.json" in paths
        assert "/root/.tampinha/.env" not in paths
