"""Tests for tools.wake_word — the "Hey Tampinha" hotword detector.
...
def test_config_defaults_and_clamping():
    ...
    assert ww.wake_phrase({"phrase": "hey tampinha"}) == "hey tampinha"
    assert ww.wake_phrase({}) == "hey tampinha"
...
def test_looks_like_path():
    assert ww._looks_like_path("models/hey_tampinha.onnx")
...
def test_load_wake_word_config_guards_non_dict(monkeypatch):
    monkeypatch.setattr(
        "tampinha_cli.config.load_config", lambda: {"wake_word": "oops"}
    )
...
def test_requirements_openwakeword_available(monkeypatch):
    ...
    r = ww.check_wake_word_requirements(
        {"provider": "openwakeword", "phrase": "hey tampinha"}
    )
    assert r["available"] is True
    assert r["provider"] == "openwakeword"
    assert r["phrase"] == "hey tampinha"
...
def _install_fake_openwakeword(monkeypatch):
    ...
    class _FakeModel:
        def __init__(self, wakeword_models, inference_framework="onnx"):
            ...
            self.models = {"hey_tampinha": object()}

        def predict(self, frame):
            return {"hey_tampinha": 0.0}
...
def test_openwakeword_ensures_base_models_for_custom_path(monkeypatch):
    ...
    eng = ww._OpenWakeWordEngine(
        {"provider": "openwakeword", "openwakeword": {"model": "/models/hey_tampinha.onnx"}}
    )
    assert calls["download"] == [["/models/hey_tampinha.onnx"]]
    assert eng._labels == ["hey_tampinha"]


def test_bundled_hey_tampinha_model_ships_on_disk():
    # The "hey tampinha" wake word works out of the box only if the model is
