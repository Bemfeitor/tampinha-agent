...
Older images made /opt/tampinha/ui-tui and /opt/tampinha/node_modules writable so a
...
def test_dockerfile_uses_prebuilt_tui_instead_of_writable_node_modules() -> None:
    ...
    assert "ENV TAMPINHA_TUI_DIR=/opt/tampinha/ui-tui" in text
    assert "cd ../ui-tui && npm run build" in text
    assert "chown -R tampinha:tampinha /opt/tampinha/ui-tui" not in text
    assert "chown -R tampinha:tampinha /opt/tampinha/node_modules" not in text
