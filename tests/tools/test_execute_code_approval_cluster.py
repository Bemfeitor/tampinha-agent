...
Covers the canonical fix for issues #4146, #27303, #30882, #33057:
  ...
  4. tools.code_execution_tool._scrub_child_env — broad TAMPINHA_ prefix dropped,
...
def gw_session(monkeypatch):
    """A clean gateway session: TAMPINHA_GATEWAY_SESSION set, a bound session
...
def test_env_scrub_tampinha_allowlist_and_secret_blocks():
    from tools.code_execution_tool import _scrub_child_env

    env = {
        # operational allowlist → kept
        "TAMPINHA_HOME": "/h", "TAMPINHA_PROFILE": "p",
        "TAMPINHA_CONFIG": "/c.yaml", "TAMPINHA_ENV": "/e",
        "TAMPINHA_DELEGATED_CHILD_CONTEXT": "1",
        # other TAMPINHA_* → dropped (broad prefix removed)
        "TAMPINHA_BASE_URL": "https://x", "TAMPINHA_INTERACTIVE": "1",
        "TAMPINHA_KANBAN_DB": "postgres://u:p@h/db",
    ...
    for kept in (
        ...
        "TAMPINHA_DELEGATED_CHILD_CONTEXT", "PATH",
    ):
        assert kept in out, f"{kept} should be kept"
    for dropped in (
        "TAMPINHA_BASE_URL", "TAMPINHA_INTERACTIVE", "TAMPINHA_KANBAN_DB",
...
def test_env_scrub_no_log_when_nothing_dropped(caplog):
    """No diagnostic noise when there are no dropped TAMPINHA_* vars."""
    import logging
