...
def _restore_tool_modules():
    original_tampinha_home = os.environ.get("TAMPINHA_HOME")
    original_modules = {
        name: module
        for name, module in sys.modules.items()
        if name == "tools"
        or name.startswith("tools.")
        or name == "tampinha_cli"
        or name.startswith("tampinha_cli.")
        or name == "modal"
    ...
    try:
        ...
        if original_tampinha_home is None:
            os.environ.pop("TAMPINHA_HOME", None)
        else:
            os.environ["TAMPINHA_HOME"] = original_tampinha_home
        _reset_modules(("tools", "tampinha_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    tampinha_cli = types.ModuleType("tampinha_cli")
    tampinha_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["tampinha_cli"] = tampinha_cli
    tampinha_home = tmp_path / "tampinha-home"
    os.environ["TAMPINHA_HOME"] = str(tampinha_home)
    sys.modules["tampinha_cli.config"] = types.SimpleNamespace(
        get_tampinha_home=lambda: tampinha_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="tampinha-agent", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": tampinha_home / "modal_snapshots.json",
