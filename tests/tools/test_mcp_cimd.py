...
Under CIMD the ``client_id`` is the HTTPS URL of a document Tampinha publishes,
...
the conditions under which Tampinha is allowed to present it.
...
def test_document_client_id_is_the_url_tampinha_sends():
    """A CIMD document is only valid when its client_id is its own URL."""
    assert _document()["client_id"] == _CIMD_CLIENT_METADATA_URL


def test_document_declares_every_callback_tampinha_can_build(tmp_path, monkeypatch):
...
def test_cached_metadata_decides_whether_to_pin(
    ...
    long after Tampinha fixes the redirect URI. Metadata cached by an earlier
...
def test_sdk_chooses_cimd_only_when_the_server_advertises_it(
    ...
    """Closes the loop on the handoff: feed what Tampinha configured into the
...
def test_reauthorizing_clears_the_rejection(tmp_path, monkeypatch, private_ports):
    """`tampinha mcp login` wipes stored state, so a fixed document is retried."""
    monkeypatch.setenv("TAMPINHA_HOME", str(tmp_path))
    storage = HermesTokenStorage("srv")
...
def test_timeout_on_a_cimd_flow_names_the_document_and_the_escape_hatch(monkeypatch):
    ...
    symptom Tampinha sees is the callback timing out."""
