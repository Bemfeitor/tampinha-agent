...
def test_output_path_rejects_tampinha_oauth_store(tmp_path, monkeypatch):
    """TTS output_path must not bypass the shared protected-file write guard."""
    import agent.file_safety as file_safety

    tampinha_home = tmp_path / "tampinha-home"
    tampinha_home.mkdir()
    monkeypatch.setattr(file_safety, "_tampinha_home_path", lambda: tampinha_home)
    monkeypatch.setattr(file_safety, "_tampinha_root_path", lambda: tampinha_home)

    target = tampinha_home / ".anthropic_oauth.json"
    result = json.loads(text_to_speech_tool(
        text="hello",
...
def test_output_path_rejects_mcp_token_directory(tmp_path, monkeypatch):
    ...
    tampinha_home = tmp_path / "tampinha-home"
    token_dir = tampinha_home / "mcp-tokens"
