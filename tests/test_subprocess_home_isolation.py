...
Tampinha state stays profile-scoped through TAMPINHA_HOME. Host subprocesses should
...
class TestGetSubprocessHome:
    """Unit tests for tampinha_constants.get_subprocess_home()."""

    def _host_mode(self, monkeypatch):
        monkeypatch.setattr(tampinha_constants, "is_container", lambda: False)
        monkeypatch.delenv("TERMINAL_HOME_MODE", raising=False)
        monkeypatch.delenv("TAMPINHA_REAL_HOME", raising=False)
    ...
    def test_host_auto_keeps_real_home_when_profile_home_exists(self, tmp_path, monkeypatch):
        ...
        tampinha_home = real_home / ".tampinha" / "profiles" / "coder"
        profile_home = tampinha_home / "home"
    ...
    def test_real_mode_repairs_parent_home_already_pointing_at_profile(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("TAMPINHA_REAL_HOME", str(real_home))

        from tampinha_constants import get_subprocess_home, get_real_home
    ...
    def test_two_profiles_get_different_homes(self, tmp_path, monkeypatch):
        self._container_mode(monkeypatch)
        base = tmp_path / ".tampinha" / "profiles"
        ...
        monkeypatch.setenv("TAMPINHA_HOME", str(base / "alpha"))
        home_a = get_subprocess_home()

        monkeypatch.setenv("TAMPINHA_HOME", str(base / "beta"))
...
class TestMakeRunEnvHomeInjection:
    ...
    def test_host_auto_preserves_real_home_when_profile_home_exists(self, tmp_path, monkeypatch):
        ...
        assert result["TAMPINHA_REAL_HOME"] == str(real_home)
    ...
    def test_profile_mode_injects_profile_home_when_profile_home_exists(self, tmp_path, monkeypatch):
        ...
        assert result["HOME"] == str(tampinha_home / "home")
        assert result["TAMPINHA_REAL_HOME"] == str(real_home)
    ...
    def test_no_injection_when_tampinha_home_unset(self, monkeypatch):
...
class TestProfileBootstrap:
    ...
    def test_profile_dirs_includes_home(self):
        from tampinha_cli.profiles import _PROFILE_DIRS
    ...
    def test_create_profile_bootstraps_home_dir(self, tmp_path, monkeypatch):
        ...
        from tampinha_cli.profiles import create_profile
