...
def test_frontmatter_required_fields():
    ...
    tampinha = fm["metadata"]["tampinha"]
    assert tampinha["tags"]
    assert "related_skills" in tampinha
...
def test_author_credits_human_first():
    fm, _ = _frontmatter_and_body()
    assert not fm["author"].startswith("Tampinha Agent"), "human contributor must be credited first"
...
def test_related_skills_resolve_in_repo():
    ...
    for name in fm["metadata"]["tampinha"]["related_skills"]:
