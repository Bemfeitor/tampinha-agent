...
def test_required_frontmatter_fields(p):
    ...
    tampinha = (fm.get("metadata") or {}).get("tampinha") or {}
    if not (tampinha.get("tags") or fm.get("tags")) and not _grandfathered(p, "tags"):
        pytest.fail(f"{_rel(p)}: no tags (metadata.tampinha.tags or top-level tags)")
...
def test_related_skills_resolve(p):
    ...
    tampinha = (fm.get("metadata") or {}).get("tampinha") or {}
    dangling = [
        rs for rs in (tampinha.get("related_skills") or []) if rs not in _all_names()
    ]
