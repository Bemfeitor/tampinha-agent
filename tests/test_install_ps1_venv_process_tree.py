"""Windows installer regression for Tampinha children outside the venv.

The venv sweep deliberately selects process roots by executable path so it
does not kill unrelated Python processes. A selected Tampinha process can spawn
...
def test_venv_sweep_stops_managed_runtime_children_but_not_unrelated_processes(
    ...
    install_dir = tampinha_home / "tampinha-agent"
    venv_scripts = install_dir / "venv" / "Scripts"
    runtime_dir = tampinha_home / ".tampinha-runtime" / "python" / "generation-test"
    ...
    # Keep the test away from real gateway tasks and real Tampinha launchers
