...
from tampinha_cli.plugins import PluginManager, VALID_HOOKS


def _make_enabled_plugin(tampinha_home: Path, name: str, register_body: str) -> Path:
    """Create a plugin under <tampinha_home>/plugins/<name> and opt it in."""
    plugin_dir = tampinha_home / "plugins" / name
...
def test_no_plugins_returns_empty_results(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("TAMPINHA_HOME", str(tmp_path / "tampinha_empty"))
