...
def test_registered_observable_metric_names_cover_snapshot_metrics(monkeypatch):
    ...
    gateway_snapshot = type("S", (), {"metrics": [
        _M("tampinha.gateway.up"), _M("tampinha.gateway.active_agents"),
        _M("tampinha.gateway.busy"), _M("tampinha.gateway.drainable"),
        _M("tampinha.gateway.restart_requested"),
        _M("tampinha.platform.up"), _M("tampinha.platform.degraded"),
    ]})()
    cron_snapshot = type("S", (), {"metrics": [
        _M("tampinha.cron.scheduler.heartbeat_age_seconds"),
        _M("tampinha.cron.scheduler.last_success_age_seconds"),
        _M("tampinha.cron.scheduler.catch_up_occurrences"),
        _M("tampinha.cron.jobs.enabled"), _M("tampinha.cron.jobs.running"),
        _M("tampinha.cron.jobs.overdue"),
    ]})()
...
def test_monitoring_docs_distinguish_relay_health_scope_and_terminal_flush():
    ...
    assert "Tampinha Agent-owned Relay transport health" in text
