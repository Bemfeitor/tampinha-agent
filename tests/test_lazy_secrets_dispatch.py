...
1. `tampinha secrets bitwarden setup --help` works (dispatch path)
2. `tampinha update --check` works (update path)
3. `tampinha secrets bitwarden disable` works (handler execution)
4. `tampinha secrets onepassword status` works (lazy backend loads on demand)
...
def _run_tampinha(args: list[str], timeout: int = 30) -> subprocess.CompletedProcess[str]:
    """Run tampinha CLI as a subprocess from repo root.
    ...
    connectivity — but ``tampinha update --check`` really does ``git fetch``
    ...
    env.update(
        {
            ...
            "GIT_CONFIG_KEY_0": "url.file:///nonexistent-tampinha-test-remote/.insteadOf",
            "GIT_CONFIG_VALUE_0": "https://",
            "GIT_CONFIG_KEY_1": "url.file:///nonexistent-tampinha-test-remote/.insteadOf",
...
class TestSecretsDispatchE2E:
    ...
    def test_bitwarden_status(self) -> None:
        """`tampinha secrets bitwarden status` must exit 0 (runs lazy backend)."""
        result = _run_tampinha(["secrets", "bitwarden", "status"])
    ...
    def test_bitwarden_disable(self) -> None:
        """`tampinha secrets bitwarden disable` must exit 0."""
        result = _run_tampinha(["secrets", "bitwarden", "disable"])
        assert result.returncode == 0, (
    ...
    def test_onepassword_status(self) -> None:
        """`tampinha secrets onepassword status` must exit 0 (1Password lazy backend)."""
        result = _run_tampinha(["secrets", "onepassword", "status"])
        assert result.returncode in (0, 1), (
    ...
    def test_onepassword_setup_help(self) -> None:
        """`tampinha secrets onepassword setup --help` must exit 0."""
        result = _run_tampinha(["secrets", "onepassword", "setup", "--help"])
...
class TestUpdatePathE2E:
    ...
    These tests invoke the real `tampinha update --check` path as a subprocess.
    ...
    def test_update_check_clean(self) -> None:
        """`tampinha update --check` must not load cryptography._rust."""
        result = _run_tampinha(["update", "--check"])
    ...
    def test_main_update_check_crypto_absent_in_sys_modules(self) -> None:
        """Decisive invariant: invoking main() with argv=['tampinha','update','--check']
...
sys.argv = ['tampinha', 'update', '--check']

import tampinha_cli.main as m
...
# resolves the tampinha_cli.main module's lazily re-exported attribute —
# so the patch must land on tampinha_cli.main._cmd_update_check.
with patch('tampinha_cli.main._cmd_update_check', capture_update_check):
    try:
        m.main()
