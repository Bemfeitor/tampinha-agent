...
def test_compression_split_continuation(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "new", previous_tampinha_session_id="old"
    )
...
def test_meta_wrapper_shape(db):
    ...
    assert set(meta.keys()) == {"tampinha"}
    assert "sessionProvenance" in meta["tampinha"]
    assert meta["tampinha"]["sessionProvenance"]["currentHermesSessionId"] == "root1"
