...
def test_dry_run_prints_plan_without_mutating(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(uninstall, "get_tampinha_home", lambda: tampinha_home)
    monkeypatch.setattr(uninstall, "_is_default_tampinha_home", lambda home: False)
    ...
    assert str(tampinha_home) in output
    assert project_root.exists()
    assert tampinha_home.exists()
