...
def test_config_get_unset_subcommands_parse():
    """`tampinha config get/unset` parse key args (and --json for get)."""
    parser = argparse.ArgumentParser(prog="tampinha")
...
# ── deprecated `tampinha login` fails gracefully, not with argparse error ────
#
# `tampinha login` is a removed command; its handler (`login_command` in
# `tampinha_cli/auth.py`) prints a deprecation notice pointing at `tampinha auth` /
# `tampinha model` and exits 0.  Two behavior contracts guard the UX:
...
def test_login_subparser_help_is_suppressed():
    """The deprecated `login` row must not appear in `tampinha --help`.
