...
tampinha_cli.config._load_config_impl, so the managed-scope merge has to be
applied in BOTH places or the interactive CLI/TUI surface (skin, display prefs)
silently ignores administrator-pinned values while `tampinha config`/`doctor`
...
def _load_cli_config(home):
    ...
    cli.py binds ``_tampinha_home = get_tampinha_home()`` at import time (module
    singleton), so monkeypatching TAMPINHA_HOME after import doesn't move it.
    ...
    this only matters for tests that swap TAMPINHA_HOME.)
    """
    import cli

    cli._tampinha_home = home
