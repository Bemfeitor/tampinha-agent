"""Regression tests for #34107 — Docker UID/GID handling in ensure_tampinha_home.

When Tampinha runs in Docker with ``TAMPINHA_UID=1000`` / ``TAMPINHA_GID=911``,
the entrypoint chowns the top-level ``TAMPINHA_HOME`` once at startup. But
...
The fix is a ``_chown_to_tampinha_uid`` helper that reads the env vars and
...
# ---------------------------------------------------------------------------
# _resolve_tampinha_uid_gid
# ---------------------------------------------------------------------------


class TestResolveHermesUidGid:
    def test_returns_parsed_values_when_both_set(self, monkeypatch):
        monkeypatch.setenv("TAMPINHA_UID", "1000")
        monkeypatch.setenv("TAMPINHA_GID", "911")
        from tampinha_cli.config import _resolve_tampinha_uid_gid
...
class TestChownToHermesUid:
    def test_calls_os_chown_when_both_set(self, tmp_path, monkeypatch):
        monkeypatch.setenv("TAMPINHA_UID", "1000")
        monkeypatch.setenv("TAMPINHA_GID", "911")
        from tampinha_cli import config as cfg

        d = tmp_path / "subdir"
        d.mkdir()

        with patch.object(cfg.os, "chown") as mock_chown:
            cfg._chown_to_tampinha_uid(d)
    ...
    def test_attributeerror_swallowed_for_windows_compat(self, tmp_path, monkeypatch):
        ...
        with patch.object(cfg.os, "chown", side_effect=AttributeError("no chown on this platform")):
            cfg._chown_to_tampinha_uid(d)  # must not raise
...
class TestSecureDirChown:
    ...
    def test_secure_dir_no_chown_when_env_unset(self, tmp_path, monkeypatch):
        monkeypatch.delenv("TAMPINHA_UID", raising=False)
        monkeypatch.delenv("TAMPINHA_GID", raising=False)
        from tampinha_cli import config as cfg
