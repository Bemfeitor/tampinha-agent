"""Tests for the `tampinha memory reset` CLI command.
...
- Profile-scoped reset (uses TAMPINHA_HOME)
"""
...
def memory_env(tmp_path, monkeypatch):
    """Set up a fake TAMPINHA_HOME with memory files."""
    tampinha_home = tmp_path / ".tampinha"
    memories = tampinha_home / "memories"
    memories.mkdir(parents=True)
    monkeypatch.setenv("TAMPINHA_HOME", str(tampinha_home))

    # Create sample memory files
    (memories / "MEMORY.md").write_text(
        "§\nHermes repo is at ~/.tampinha/tampinha-agent\n§\nUser prefers dark themes",
        encoding="utf-8",
    )
...
def _run_memory_reset(target="all", yes=False, monkeypatch=None, confirm_input="no"):
    ...
    Simulates what happens when `tampinha memory reset` is run.
    """
...
class TestMemoryReset:
    ...
    def test_reset_no_files_exist(self, tmp_path, monkeypatch):
        ...
        tampinha_home = tmp_path / ".tampinha"
        (tampinha_home / "memories").mkdir(parents=True)
        monkeypatch.setenv("TAMPINHA_HOME", str(tampinha_home))
