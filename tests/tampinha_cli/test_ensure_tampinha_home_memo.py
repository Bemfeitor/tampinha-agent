"""ensure_tampinha_home is memoized per home path (perf: it runs on every
...
def test_distinct_home_paths_each_get_the_skeleton(tmp_path, monkeypatch):
    first = tmp_path / "a" / ".tampinha"
    second = tmp_path / "b" / ".tampinha"

    monkeypatch.setenv("TAMPINHA_HOME", str(first))
    cfg.ensure_tampinha_home()

    # Profile switch: TAMPINHA_HOME moves → the new path is ensured too.
    monkeypatch.setenv("TAMPINHA_HOME", str(second))
    cfg.ensure_tampinha_home()
