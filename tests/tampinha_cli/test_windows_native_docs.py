...
def test_windows_native_install_path_docs_match_installer() -> None:
    ...
    # (TAMPINHA_HOME\bin, next to the managed uv) — NOT the whole venv\Scripts
    # (which would shadow the user's python, #83797) and NOT a dir inside
    # the checkout (which `tampinha update`'s autostash swept off disk).
    assert "%LOCALAPPDATA%\\tampinha\\bin" in doc
    assert (
        "Get-Command tampinha        # should print "
        "C:\\Users\\<you>\\AppData\\Local\\tampinha\\bin\\tampinha.exe"
    ) in doc
    # Installer exposes $HermesHome\bin, and must copy the launchers into it.
    assert '$hermesBin = "$HermesHome\\bin"' in install
    assert "tampinha.exe" in install and "tampinha-acp.exe" in install
