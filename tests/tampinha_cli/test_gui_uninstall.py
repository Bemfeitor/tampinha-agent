"""Tests for tampinha_cli.gui_uninstall — GUI-only uninstall + install discovery.
...
import tampinha_cli.gui_uninstall as gu


def _make_agent(tampinha_home: Path) -> Path:
    """Create a fake agent install: source package + venv."""
    agent_root = tampinha_home / "tampinha-agent"
    (agent_root / "tampinha_cli").mkdir(parents=True)
    (agent_root / "tampinha_cli" / "__init__.py").write_text("")
    (agent_root / "venv" / "bin").mkdir(parents=True)
    return agent_root


def _make_gui_build(tampinha_home: Path) -> None:
    """Create the source-built GUI artifacts a `tampinha desktop` run produces."""
    desktop = tampinha_home / "tampinha-agent" / "apps" / "desktop"
    ...
    (tampinha_home / "tampinha-agent" / "node_modules").mkdir(parents=True)
    (tampinha_home / "desktop-build-stamp.json").write_text("{}")


def _make_user_data(tampinha_home: Path) -> None:
    (tampinha_home / "config.yaml").write_text("x: 1\n")
    (tampinha_home / ".env").write_text("KEY=secret\n")
    (tampinha_home / "sessions").mkdir()
...
def test_gui_install_summary_shape(tmp_path, monkeypatch):
    ...
    _make_agent(tampinha_home)
    _make_gui_build(tampinha_home)
    monkeypatch.setattr(gu, "packaged_gui_app_paths", lambda: [])
    monkeypatch.setattr(gu, "desktop_userdata_dir", lambda: tmp_path / "none")

    summary = gu.gui_install_summary(tampinha_home)
    ...
    assert summary["tampinha_home"] == str(tampinha_home)
...
def test_linux_discovery_includes_launcher_entry(tmp_path, monkeypatch):
    """The launcher entry that `tampinha desktop` installs is removable."""
...
def test_uninstall_removes_launcher_entry_and_refreshes_cache(tmp_path, monkeypatch):
    ...
    tampinha_home = tmp_path / ".tampinha"
    _make_agent(tampinha_home)
    icon = lde.icon_path(tampinha_home / "tampinha-agent")
    icon.parent.mkdir(parents=True, exist_ok=True)
    icon.write_bytes(b"\x89PNG")
    monkeypatch.setattr(gu, "desktop_userdata_dir", lambda: tmp_path / "none")

    removed = gu.uninstall_gui(tampinha_home)
    ...
    assert lde.icon_path(tampinha_home / "tampinha-agent").exists()
    # The agent itself survives a GUI uninstall.
    assert (tampinha_home / "tampinha-agent" / "tampinha_cli").is_dir()
...
def test_uninstall_skips_cache_refresh_when_no_launcher_entry(tmp_path, monkeypatch):
    ...
    gu.uninstall_gui(tmp_path / ".tampinha")
...
def test_uninstall_args_namespace_mode_mapping():
    ...
    import tampinha_cli.uninstall as uninstall
