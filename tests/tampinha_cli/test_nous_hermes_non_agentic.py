...
``"tampinha"`` anywhere (case-insensitive). That false-positived on unrelated
local Modelfiles such as ``tampinha-brain:qwen3-14b-ctx16k`` — a tool-capable
Qwen3 wrapper that happens to live under the "tampinha" tag namespace.
...
def test_matches_real_nous_tampinha_chat_models(model_name: str) -> None:
