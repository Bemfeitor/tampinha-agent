...
class TestUnifiedDashboardRouting:
    ...
    def test_profile_launch_reexecs_machine_dashboard(self, main_mod, monkeypatch):
        ...
        monkeypatch.setattr(
            "tampinha_cli.profiles.get_active_profile_name", lambda: "worker_x"
        )
        ...
        # TAMPINHA_HOME.  For a standard install (TAMPINHA_HOME unset) that root is
        # the platform-native default (~/.tampinha), NOT dropped — see the Docker
        # test below for why we resolve explicitly instead of popping.
        from tampinha_constants import get_default_tampinha_root
        assert env.get("TAMPINHA_HOME") == str(get_default_tampinha_root())


    def test_desktop_profile_backend_skips_machine_dashboard_reroute(self, main_mod, monkeypatch):
        """A desktop-spawned named-profile backend (TAMPINHA_DESKTOP=1) must NOT
