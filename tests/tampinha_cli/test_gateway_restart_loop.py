...
Covers:
- Defense 1: gateway stop/restart refuse when _TAMPINHA_GATEWAY=1
...
class TestGatewayLifecyclePattern:
    ...
    @pytest.mark.parametrize("text", [
        "tampinha gateway restart",
        "tampinha gateway stop",
        "tampinha gateway uninstall",
        "tampinha  gateway  restart",         # double spaces
        "Hermez Gateway Restart".lower().replace("z", "s"),  # case handled
        "TAMPINHA GATEWAY RESTART",           # uppercase
    ])
    def test_tampinha_gateway_commands(self, text):
    ...
    @pytest.mark.parametrize("text", [
        ...
        "launchctl submit -l ai.tampinha.gateway-hard-restart-no-photon-notice -- /bin/sh ~/.tampinha/scripts/hard_restart_gateway_no_photon_notice.sh",
        "launchctl submit -l tampinha-gateway-restart-helper -- /bin/sh helper.sh",
        # bootstrap loads an arbitrary plist — same laundering shape.
        "launchctl bootstrap gui/501 ~/Library/LaunchAgents/ai.tampinha.gateway.restart-once.plist",
        "launchctl bootout gui/501/ai.tampinha.gateway",
        ...
        (
            "launchctl submit \\\n"
            "  -l ai.tampinha.gateway-hard-restart-no-photon-notice \\\n"
            "  -- /bin/sh ~/.tampinha/scripts/hard_restart_gateway_no_photon_notice.sh"
        ),
    ])
    ...
    def test_label_defined_before_verb_is_caught(self):
        ...
        cmd = (
            "uid=$(id -u); for item in 'ai.tampinha.gateway-apollo:/a.plist' "
            "'ai.tampinha.gateway-cronus:/c.plist' 'ai.tampinha.gateway-plutus:/p.plist' "
            "'ai.tampinha.gateway:/Users/botuser/Library/LaunchAgents/ai.tampinha.gateway.plist'; "
    ...
    def test_line_continuation_does_not_bridge_unrelated_lines(self):
        ...
        text = (
            ...
            "unrelated tampinha note on the next line"
        )
    ...
    @pytest.mark.parametrize("text", [
        ...
        # `launchctl kick"start" ... ai.tampinha.gateway` executes exactly as
        ...
        'launchctl kick"start" -k gui/501/ai.tampinha.gateway',
        "launchctl kick'start' -k gui/501/ai.tampinha.gateway",
        "launchctl kick\\start -k gui/501/ai.tampinha.gateway",
        'launchctl "kickstart" -k gui/501/ai.tampinha.gateway',
        # Splices on the newer/legacy unload spellings this PR added.
        'launchctl boot"out" gui/501/ai.tampinha.gateway',
        "launchctl dis\\able gui/501/ai.tampinha.gateway",
        # The gateway identifier itself can be spliced just as easily.
        'launchctl bootout gui/501/ai.tampinha."gateway"',
        # Same class on the systemctl and tampinha-CLI branches.
        'systemctl re"start" tampinha-gateway',
        'tampinha gateway re"start"',
    ])
    ...
    def test_spliced_verb_inside_shell_c_payload_is_blocked(self):
        ...
        command = 'sh -c \'launchctl kick"start" -k gui/501/ai.tampinha.gateway\''
    ...
    @pytest.mark.parametrize("text", [
        ...
        'launchctl kick"start" -k gui/501/ai.tampinha.update-checker',
        'systemctl re"start" tampinha-meta.service',
    ...
    @pytest.mark.parametrize("text", [
        ...
        "tampinha cron list",
        "tampinha update",
        "tampinha config set model claude",
        "echo 'just a normal cron job'",
        "run the backup script",
        "gateway is running fine",
        # `tampinha gateway start` is benign — starting a gateway from inside a
        ...
        "tampinha gateway start",
        "tampinha gateway start --all",
        # Tightened launchctl/systemctl branches: ops on NON-gateway tampinha
        # services must not be falsely blocked (the old `.*tampinha` matched any
        # tampinha token).
        "launchctl unload ai.tampinha.update-checker.plist",
        "launchctl restart ai.tampinha.daemon",
        ...
        "systemctl restart tampinha-meta.service",
        "systemctl restart tampinha-cron-helper",
        ...
        "echo after the tampinha gateway restarted cleanly",
        "the tampinha gateway stopped responding, please investigate",
        # #92372 Branch D: `p?kill` without a leading \b matched the "kill"
        # tail of "skill".
        "tampinha skill view gateway-notes && echo tampinha gateway docs",
        # #77173/#77536: a file path with embedded spaces containing the
        # lifecycle words must not match — `tampinha` is a path component
        # there, not a command.
        "cat '/docs/tampinha gateway restart-notes.md'",
        "less /home/user/notes/tampinha gateway restart runbook.txt",
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        "tampinha gateway restart",
        "tampinha gateway restart; echo done",
        "tampinha gateway stop && echo stopped",
        # #77173 command-position anchor must not weaken separator/subshell
        # forms either.
        "true;tampinha gateway restart",
        "true && tampinha gateway stop",
        "echo $(tampinha gateway restart)",
        "echo `tampinha gateway restart`",
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        'import subprocess\nsubprocess.run(["launchctl", "bootout", "gui/501/ai.tampinha.gateway"])',
        'subprocess.run(["tampinha", "gateway", "restart"])',
        'os.system("launchctl kickstart -k gui/501/ai.tampinha.gateway")',
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        'data = ["tampinha", "notes"]  # unrelated list',
    ])
    ...
    def test_inert_heredoc_body_prose_not_blocked(self):
        ...
        text = (
            ...
            "If the box is wedged, a human can run: tampinha gateway restart\n"
            "EOF"
        )
        assert not _contains_gateway_lifecycle_command(text), f"Should NOT match: {text!r}"
...
class TestProfileFlagGatewayLifecycle:
    """#78028: `tampinha -p <profile> gateway restart|stop` bypasses Branch A's
    ...
    def _pin_profile_identity(self, monkeypatch):
        # The ambient test env may carry TAMPINHA_HOME/TAMPINHA_PROFILE; pin the
        # profile identity explicitly so every assertion is deterministic.
        monkeypatch.setenv("TAMPINHA_PROFILE", "zeus")
        monkeypatch.delenv("TAMPINHA_PROFILE_NAME", raising=False)

    @pytest.mark.parametrize("text", [
        "tampinha -p zeus gateway stop",
        "tampinha -p zeus gateway restart",
        "tampinha --profile zeus gateway restart",
        "tampinha --profile zeus gateway stop",
        "tampinha --profile=zeus gateway restart",
        # Global flags before/after the selector must not hide the shape.
        "tampinha -v -p zeus gateway restart",
        "tampinha -p zeus -v gateway restart",
        "tampinha --debug --profile zeus gateway stop",
    ...
    @pytest.mark.parametrize("text", [
        "tampinha -p venus gateway stop",
        "tampinha -p venus gateway restart",
        "tampinha --profile venus gateway restart",
        "tampinha --profile=venus gateway stop",
        "tampinha -p venus -v gateway restart",
    ])
    def test_sibling_allowed(self, text):
        assert not _contains_gateway_lifecycle_command(text), f"Should allow: {text!r}"

    @pytest.mark.parametrize("text", [
        "tampinha -p zeus gateway start",
        "tampinha -p zeus gateway start --all",
    ])
    ...
    def test_adjacent_form_still_blocked(self):
        ...
        assert _contains_gateway_lifecycle_command("tampinha gateway restart")
        assert _contains_gateway_lifecycle_command("tampinha gateway stop")

    def test_tampinha_home_derived_profile(self, monkeypatch):
        # Without TAMPINHA_PROFILE the guard falls back to the TAMPINHA_HOME-
        # derived profile identity (get_active_profile_name) — the signal the
        # gateway process itself carries.
        monkeypatch.delenv("TAMPINHA_PROFILE", raising=False)
        monkeypatch.delenv("TAMPINHA_PROFILE_NAME", raising=False)
        import tampinha_cli.profiles as profiles_mod

        monkeypatch.setattr(profiles_mod, "get_active_profile_name", lambda: "zeus")
        assert _contains_gateway_lifecycle_command("tampinha -p zeus gateway restart")
        assert not _contains_gateway_lifecycle_command("tampinha -p venus gateway restart")
    ...
    def test_no_profile_context_conservative_allow(self, monkeypatch):
        ...
        assert not _contains_gateway_lifecycle_command("tampinha -p zeus gateway restart")
        assert _contains_gateway_lifecycle_command("tampinha gateway restart")
...
class TestCronCreateLifecycleBlock:
    ...
    def test_block_tampinha_gateway_restart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="30m",
            prompt="Upgrade tampinha then run tampinha gateway restart",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
    ...
    def test_block_script_with_lifecycle_command(self, tmp_path, capsys, monkeypatch):
        ...
        # restart_tampinha_gateway_once.sh). The script must live under
        # TAMPINHA_HOME/scripts so the scheduler — and the guard — resolve it.
        monkeypatch.setenv("TAMPINHA_HOME", str(tmp_path / ".tampinha"))
        scripts_dir = tmp_path / ".tampinha" / "scripts"
...
class TestGatewaySelfTargetingGuard:
    ...
    def test_stop_allows_outside_gateway(self, monkeypatch):
        ...
        monkeypatch.delenv("_TAMPINHA_GATEWAY", raising=False)
...
class TestTerminalToolGatewayLifecycleGuard:
    """terminal_tool must refuse gateway lifecycle commands when _TAMPINHA_GATEWAY=1.

    Issue #37453: systemctl --user restart tampinha-gateway runs as a child of the
    ...
    @pytest.mark.parametrize("cmd", [
        "systemctl restart tampinha-gateway",
        "systemctl --user restart tampinha-gateway",
        "systemctl stop tampinha-gateway.service",
        "tampinha gateway restart",
        "tampinha gateway uninstall",
        "launchctl kickstart gui/501/ai.tampinha.gateway",
        "launchctl bootout gui/501/ai.tampinha.gateway",
        # #62891 exact reported shape and its bootstrap sibling.
        "launchctl submit -l ai.tampinha.gateway-hard-restart-no-photon-notice -- /bin/sh ~/.tampinha/scripts/hard_restart_gateway_no_photon_notice.sh",
        "launchctl submit -l com.foo -- /path/gateway",
        "launchctl bootstrap gui/501 ~/Library/LaunchAgents/ai.tampinha.gateway.restart-once.plist",
        "pkill -f tampinha.*gateway",
    ])
    ...
    def test_force_true_cannot_bypass_block(self, monkeypatch):
        ...
        result = json.loads(tt.terminal_tool(
            command="systemctl restart tampinha-gateway", force=True
        ))
    ...
    def test_blocks_launchctl_submit_inside_gateway(self, monkeypatch, tmp_path):
        ...
        result = json.loads(tt.terminal_tool(
            command=(
                "launchctl submit -l ai.tampinha.delayed-ops -- "
                f"/bin/bash {script}"
            )
        ))
    ...
    @pytest.mark.parametrize("command", [
        # Neutral, non-tampinha label: label-independent detection is the point
        # (#62891 second reproduction used `ai.tampinha.svc-reload-tmp`).
        "launchctl submit -l com.foo -- /path/gateway",
        "launchctl submit -l ai.tampinha.svc-reload-tmp -- /bin/sh /tmp/h-svc-reload.sh",
    ...
    def test_cli_agent_session_not_blocked_by_inherited_env(
        ...
        """#92560: CLI/TUI agent sessions inherit _TAMPINHA_GATEWAY=1 from the
        ...
        # Simulate a CLI agent session: _TAMPINHA_GATEWAY=1 is in the
        ...
        self._patch_env(monkeypatch, _FakeEnv(), inside_gateway=False)
        monkeypatch.setenv("_TAMPINHA_GATEWAY", "1")
        monkeypatch.setattr(
            tt, "_check_all_guards", lambda cmd, env, **kwargs: {"approved": True}
        )

        result = json.loads(tt.terminal_tool(command="tampinha gateway restart"))

        assert result["exit_code"] == 0
        assert calls == ["tampinha gateway restart"]
    ...
    def test_blocks_launchctl_submit_hidden_in_referenced_script(
        ...
        script.write_text(
            "#!/bin/bash\nlaunchctl submit -l ai.tampinha.loop -- /bin/true\n"
        )
        self._patch_env(monkeypatch, self._make_fake_env(), inside_gateway=True)
    ...
    def test_launchctl_submit_parser_handles_shell_quoting(self, monkeypatch):
        ...
        self._patch_env(monkeypatch, self._make_fake_env(), inside_gateway=True)
        result = json.loads(tt.terminal_tool(
            command="launchctl sub\"\"mit -l ai.tampinha.loop -- /bin/true"
        ))
    ...
    def test_safe_systemctl_commands_pass_through(self, monkeypatch):
        """Non-tampinha systemctl commands must not be blocked by this guard."""
        import tools.terminal_tool as tt
...
class TestLifecycleGuardModule:
    ...
    def test_prompt_with_command_raises(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked) as exc:
            check_gateway_lifecycle("please run tampinha gateway restart", None)
    ...
    def test_split_across_prompt_and_script_still_blocks(self, tmp_path):
        ...
        script.write_text("tampinha gateway stop\n", encoding="utf-8")
        with pytest.raises(GatewayLifecycleBlocked):
    ...
    def test_relative_script_resolved_under_scripts_dir(self, tmp_path, monkeypatch):
        """A bare/relative script name resolves under TAMPINHA_HOME/scripts (the
    ...
    def test_python_script_with_literal_lifecycle_command_still_blocked(
        ...
        script.write_text('import os\nos.system("tampinha gateway restart")\n', encoding="utf-8")
        with pytest.raises(GatewayLifecycleBlocked):
            check_gateway_lifecycle("clean prompt", str(script))
    ...
    def test_read_referenced_script_tolerates_nul_in_path(self):
        ...
        text, unsafe = _read_referenced_script(Path("/tmp/tampinha\x00binary"))
        assert text is None
    ...
    def test_binary_from_remote_callback_never_false_positives(self):
        ...
        def _remote_read(_path: str):
            return "MZ\x00\x00\x90\x00 tampinha gateway restart \x00\x00junk"
    ...
    def test_cron_guard_total_when_home_unresolvable(self, monkeypatch):
        """`get_tampinha_home()` falls back to Path.home(), which raises
        RuntimeError when neither TAMPINHA_HOME nor HOME resolves
...
class TestRelativePathDoesNotDisableDataExemption:
    ...
    @pytest.mark.parametrize("command", [
        "grep -r 'systemctl restart tampinha-gateway' .",
        "grep -rn 'tampinha gateway restart' ./logs",
        "rg 'tampinha gateway restart' ../archive",
        "grep -c 'systemctl stop tampinha-gateway' ./var/log/syslog",
        "sqlite3 ./stats.db \"SELECT restart_reason FROM tampinha_gateway_restarts\"",
    ])
    ...
    @pytest.mark.parametrize("command", [
        ...
        'sqlite3 ./db ".shell tampinha gateway restart"',
        'sqlite3 ./db ".system systemctl restart tampinha-gateway"',
        'psql ./x -c "\\! systemctl restart tampinha-gateway"',
        "grep -r 'tampinha gateway restart' . | sh",
        "grep -r 'tampinha gateway restart' ./logs | bash",
        "grep -r 'tampinha gateway restart' . | sudo sh",
        "grep -r 'x' . ; tampinha gateway restart",
        "grep -r 'x' . && systemctl restart tampinha-gateway",
        'grep -r "$(tampinha gateway restart)" .',
        "rg 'x' ./logs | xargs systemctl restart tampinha-gateway",
    ])
    ...
    @pytest.mark.parametrize("command", [
        ...
        'sqlite3 db ".shell tampinha gateway restart"',
        'sqlite3 db ".system systemctl restart tampinha-gateway"',
        'psql -c "\\! systemctl restart tampinha-gateway"',
    ])
...
class TestCreateJobBlocksLifecycleCommands:
    ...
    tampinha_cli.cron.cron_create. Enforcing at create_job covers both."""
    ...
    def test_create_job_blocks_prompt_command(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked):
            create_job(prompt="then run tampinha gateway restart", schedule="30m")
    ...
    def test_cronjob_tool_surfaces_block_as_error(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("TAMPINHA_HOME", str(tmp_path / ".tampinha"))
        (tmp_path / ".tampinha").mkdir(parents=True)
        from tools.cronjob_tools import cronjob
        result = json.loads(cronjob(
            action="create", schedule="0 9 * * *",
            prompt="please run tampinha gateway restart nightly",
        ))
...
class TestLifecycleGuardDataArgumentExemption:
    ...
    @pytest.mark.parametrize("command", [
        ...
        "'systemctl stop tampinha-gateway'\"",
        # grep/rg pattern arguments hunting for the lifecycle string.
        "grep -c 'systemctl restart tampinha-gateway' /var/log/syslog",
        "rg 'tampinha gateway restart' /home/user/.tampinha/logs/",
        "journalctl -u tampinha-gateway --grep 'systemctl restart tampinha-gateway'",
        # SQL with stop/restart column/value words but no command shape.
        'sqlite3 stats.db "SELECT stop_time, restart_reason FROM '
        'tampinha_gateway_restarts"',
    ...
    @pytest.mark.parametrize("command", [
        ...
        'sqlite3 db ".shell tampinha gateway restart"',
        'psql -c "\\! systemctl restart tampinha-gateway"',
        "grep 'systemctl restart tampinha-gateway' cmds.txt | sh",
        "grep gateway f | xargs systemctl restart tampinha-gateway",
        'grep "$(systemctl restart tampinha-gateway)" f',
        "grep 'restart' log; systemctl restart tampinha-gateway",
        'sqlite3 db "SELECT 1"; tampinha gateway stop',
        # Plain lifecycle commands are unaffected by the exemption.
        "tampinha gateway restart",
        "sudo systemctl stop tampinha-gateway",
    ])
    ...
    def test_python_script_branch_gets_the_same_exemption(self, tmp_path):
        ...
        prompt = (
            ...
            "WHERE msg LIKE '%systemctl restart tampinha-gateway%'\""
        )
