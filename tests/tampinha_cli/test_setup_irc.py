"""Tests for IRC gateway configuration via `tampinha setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh Tampinha install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new Tampinha install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#tampinha")
            monkeypatch.setenv("IRC_NICKNAME", "tampinha-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `tampinha setup gateway` experience with IRC present."""
