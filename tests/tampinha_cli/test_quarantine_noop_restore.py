...
``tampinha*.exe`` shims aside (``tampinha.exe.old.<ms>``) before invoking the
...
quarantined shims stayed renamed aside and ``tampinha`` vanished from PATH
...
def _make_scripts_dir(tmp_path: Path) -> Path:
    ...
    for name in ("tampinha", "tampinha-agent", "tampinha-acp", "tampinha-gateway"):
...
# ---------------------------------------------------------------------------
# tampinha_cli.main._run_quarantined_install
# ---------------------------------------------------------------------------
...
def test_main_noop_success_restores_shims(tmp_path):
    ...
    assert "tampinha.exe" in names, "tampinha.exe must be restored after a no-op install"
    assert "tampinha-acp.exe" in names
    assert "tampinha-gateway.exe" in names
    assert (scripts / "tampinha.exe").read_bytes() == b"MZ-old-tampinha"
...
def test_main_rewriting_success_keeps_fresh_shims(tmp_path):
    ...
    assert (scripts / "tampinha.exe").read_bytes() == b"MZ-new-tampinha"
...
# ---------------------------------------------------------------------------
# tampinha_cli._install_repair._run_install_cmd (the deferred-recovery path)
# ---------------------------------------------------------------------------
...
def test_repair_noop_success_restores_shims(tmp_path):
    ...
    names = _shim_names(scripts)
    assert "tampinha.exe" in names, "tampinha.exe must be restored after a no-op recovery install"
    assert (scripts / "tampinha.exe").read_bytes() == b"MZ-old-tampinha"
...
def test_repair_rewriting_success_keeps_fresh_shims(tmp_path):
    ...
    def fake_run(cmd, cwd=None, check=None, env=None):
        (scripts / "tampinha.exe").write_bytes(b"MZ-new-tampinha")
