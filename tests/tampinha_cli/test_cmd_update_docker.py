"""Tests for ``tampinha update`` / ``--check`` inside the Docker container.
...
fix, ``tampinha update`` would fall through to ``"✗ Not a git repository.
Please reinstall: curl ... install.sh"`` — that script installs a *new*
host-side Tampinha, not an update to the running container, so the message
...
from tampinha_cli.main import _cmd_update_check, cmd_update
...
@patch("tampinha_cli.config.detect_install_method", return_value="docker")
@patch("subprocess.run")
def test_cmd_update_in_docker_prints_guidance_and_exits(
    mock_run, _mock_method, _mock_managed, capsys
):
    """``tampinha update`` inside Docker → friendly message + exit 2, no git calls.
...
def test_format_docker_update_message_contents():
    ...
    """
    from tampinha_cli.config import format_docker_update_message
    ...
    assert "TAMPINHA_HOME" in msg or "/opt/data" in msg, (
