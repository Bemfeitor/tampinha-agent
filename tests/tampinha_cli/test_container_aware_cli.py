...
def container_env(tmp_path, monkeypatch):
    """Set up a fake TAMPINHA_HOME with .container-mode file."""
    tampinha_home = tmp_path / ".tampinha"
    tampinha_home.mkdir()
    monkeypatch.setenv("TAMPINHA_HOME", str(tampinha_home))
    monkeypatch.delenv("TAMPINHA_DEV", raising=False)

    container_mode = tampinha_home / ".container-mode"
    container_mode.write_text(
        "# Written by NixOS activation script. Do not edit manually.\n"
        "backend=podman\n"
        "container_name=tampinha-agent\n"
        "exec_user=tampinha\n"
        "tampinha_bin=/data/current-package/bin/tampinha\n"
    )
...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    with patch("tampinha_constants.is_container", return_value=False):
        info = get_container_exec_info()

    assert info is not None
    assert info["backend"] == "podman"
    assert info["container_name"] == "tampinha-agent"
    assert info["exec_user"] == "tampinha"
    assert info["tampinha_bin"] == "/data/current-package/bin/tampinha"
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from tampinha_cli.main import _exec_in_container
    ...
    assert "-it" in cmd
    idx_u = cmd.index("-u")
    assert cmd[idx_u + 1] == "tampinha"
    e_indices = [i for i, v in enumerate(cmd) if v == "-e"]
    e_values = [cmd[i + 1] for i in e_indices]
    assert "TERM=xterm-256color" in e_values
    assert "LANG=en_US.UTF-8" in e_values
    assert "tampinha-agent" in cmd
    assert "/data/current-package/bin/tampinha" in cmd
    assert "chat" in cmd
