...
Regression for #86794: a background/quiet `tampinha chat -c "<title>" -q "..."`
...
def isolated_home(tmp_path, monkeypatch):
    """Point TAMPINHA_HOME at a temp dir so session creation stays isolated."""
...
def _create_titled_session(title):
    from tampinha_cli.main import _create_titled_session as fn
...
class TestChatCFailLoudlyOnStderr:
    ...
    def test_missing_session_fails_on_stderr(self, isolated_home, monkeypatch):
        ...
        assert not stderr_lines[0].startswith("Use 'tampinha sessions list'")
