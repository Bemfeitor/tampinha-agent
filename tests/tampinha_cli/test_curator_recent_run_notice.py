...
The notice prints the most recent curator run summary on `tampinha update`,
...
session start) so users normally never see the rename map. `tampinha update`
...
def curator_env(tmp_path, monkeypatch, capsys):
    ...
    importlib.reload(tampinha_main)
