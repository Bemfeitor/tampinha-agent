"""Tests for the top-level `./tampinha` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./tampinha` should use `tampinha_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "tampinha"
    called = []

    fake_main_module = types.ModuleType("tampinha_cli.main")

    def fake_main():
        called.append("tampinha_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "tampinha_cli.main", fake_main_module)
    ...
    assert called == ["tampinha_cli.main"]
