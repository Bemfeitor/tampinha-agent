...
from tampinha_cli.codex_runtime_plugin_migration import (
    MIGRATION_MARKER,
    MIGRATION_END_MARKER,
    _build_tampinha_tools_mcp_entry,
    _format_toml_value,
...
# ---- end-to-end migrate(, expose_tampinha_tools=False) ----
...
class TestMigrate:
    ...
    def test_plugin_discovery_writes_plugin_blocks(self, tmp_path, monkeypatch):
        ...
        from tampinha_cli import codex_runtime_plugin_migration as crpm
    ...
    def test_plugin_discovery_failure_non_fatal(self, tmp_path, monkeypatch):
        ...
        report = migrate({"mcp_servers": {"x": {"command": "y"}}},
                         codex_home=tmp_path, discover_plugins=True, expose_tampinha_tools=False)
    ...
    def test_full_migration_round_trip(self, tmp_path):
        ...
        report = migrate(tampinha_cfg, codex_home=tmp_path, expose_tampinha_tools=False)
        assert report.written
        text = (tmp_path / "config.toml").read_text()
    ...
    def test_preserves_user_mcp_server_outside_managed_block(self, tmp_path):
        ...
        to ~/.codex/config.toml outside Tampinha' managed block, re-running
        ...
        migrate({"mcp_servers": {"tampinha-mcp": {"command": "npx"}}},
        ...
        assert "[mcp_servers.tampinha-mcp]" in final
    ...
    def test_summary_reports_migration_count(self, tmp_path):
        ...
        }, codex_home=tmp_path, expose_tampinha_tools=False)
...
class TestStripUnmanagedPluginTables:
    ...
    ``tampinha codex-runtime migrate`` would re-emit them inside the managed
    ...
    def test_migrate_dedups_codex_owned_plugin_tables(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            "tampinha_cli.codex_runtime_plugin_migration._query_codex_plugins",
            fake_query,
        )
        migrate({}, codex_home=tmp_path, discover_plugins=True, expose_tampinha_tools=False)
...
# ---- Bug C: TAMPINHA_HOME tempdir leak into ~/.codex/config.toml ----


class TestHermesHomeLeakGuard:
    """Regression tests for issue #26250 Bug C.

    Previously ``_build_tampinha_tools_mcp_entry()`` read ``TAMPINHA_HOME``
    ...
    once codex spawned the tampinha-tools MCP subprocess.
    """




    def test_real_tampinha_home_propagates(self, monkeypatch, tmp_path):
        """A legitimate TAMPINHA_HOME (not a tempdir path) DOES propagate so the
        ...
        real_path = "/Users/alice/.tampinha"
        monkeypatch.setenv("TAMPINHA_HOME", real_path)
        entry = _build_tampinha_tools_mcp_entry()
        env = entry.get("env", {})
        assert env.get("TAMPINHA_HOME") == real_path

    def test_unset_tampinha_home_omits_env_key(self, monkeypatch):
        """When TAMPINHA_HOME is unset in the environment, the MCP entry MUST
        NOT bake in a resolved-default path. The codex subprocess should
        inherit whatever TAMPINHA_HOME its launcher (systemd, gateway, shell)
        ...
        entry = _build_tampinha_tools_mcp_entry()
        env = entry.get("env", {})
        assert "TAMPINHA_HOME" not in env, (
            f"TAMPINHA_HOME should not be set when env var is unset, got: "
            f"{env.get('TAMPINHA_HOME')!r}"
        )
