"""Tests for ``tampinha doctor --live`` — opt-in bounded real-call tool-backend probes.
...
from tampinha_cli import doctor_live
from tampinha_cli.doctor_live import (
    ProbeResult,
...
class TestBrowserAvailableNpxRung:
    ...
    with. It must fall through to the same cascade `tampinha doctor` uses."""

    def _block_path_and_node_modules_checks(self, monkeypatch, tmp_path):
        monkeypatch.setattr("shutil.which", lambda *a, **k: None)
        monkeypatch.setattr("tampinha_cli.doctor.TAMPINHA_HOME", tmp_path / "home")
        monkeypatch.setattr("tampinha_cli.doctor.PROJECT_ROOT", tmp_path / "root")
