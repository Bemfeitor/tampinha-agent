"""Tests for the unified provider catalog (tampinha_cli.provider_catalog).

These are invariant tests, not snapshots: they assert the parity *contract*
between what ``tampinha model`` shows (``CANONICAL_PROVIDERS``) and what the
...
from tampinha_cli.provider_catalog import (
    ProviderDescriptor,
    provider_catalog,
