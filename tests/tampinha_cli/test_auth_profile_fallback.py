...
When ``TAMPINHA_HOME`` points to a named profile, ``read_credential_pool()``
...
def profile_env(tmp_path, monkeypatch):
    """Set up a global root + an active profile under Path.home()/.tampinha/profiles/coder.

    * Path.home() -> tmp_path
    * Global root -> tmp_path/.tampinha            (has its own auth.json fixture)
    * Profile     -> tmp_path/.tampinha/profiles/coder   (active, TAMPINHA_HOME points here)
...
# ``_load_provider_state`` itself, those helpers raise ``"Tampinha is not
...
def classic_env(tmp_path, monkeypatch):
    """Classic single-root layout (TAMPINHA_HOME != ~/.tampinha, no profiles)."""
    fake_home = tmp_path / "home"
    fake_home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: fake_home)
    tampinha_home = tmp_path / "classic"
    tampinha_home.mkdir()
