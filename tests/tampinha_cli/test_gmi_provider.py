...
class TestGmiAliases:
    ...
    def test_providers_normalize_provider(self):
        from tampinha_cli.providers import normalize_provider as normalize_provider_in_providers
...
class TestGmiConfigRegistry:
    def test_optional_env_vars_include_gmi(self):
        ...
        # tampinha model, not via upgrade prompts.
...
class TestGmiProvidersModule:
    def test_overlay_exists(self):
        ...
        assert "gmi" in TAMPINHA_OVERLAYS
        overlay = TAMPINHA_OVERLAYS["gmi"]
...
class TestGmiDoctor:
    def test_provider_env_hints_include_gmi(self):
        from tampinha_cli.doctor import _PROVIDER_ENV_HINTS
...
class TestGmiAuxiliary:
    ...
    def test_gmi_profile_declares_tampinha_user_agent(self):
...
class TestGmiMainFlow:
    def test_chat_parser_accepts_gmi_provider(self, monkeypatch):
        ...
        monkeypatch.setattr("tampinha_cli.config.get_container_exec_info", lambda: None)
        monkeypatch.setattr(
            "tampinha_cli.main.cmd_chat",
            lambda args: recorded.setdefault("provider", args.provider),
        )
        monkeypatch.setattr(sys, "argv", ["tampinha", "chat", "--provider", "gmi"])
    ...
    def test_select_provider_and_model_routes_gmi_to_generic_flow(self, monkeypatch):
        ...
        monkeypatch.setattr("tampinha_cli.auth.resolve_provider", lambda *args, **kwargs: None)
        ...
        monkeypatch.setattr("tampinha_cli.main._prompt_provider_choice", fake_prompt_provider_choice)
        monkeypatch.setattr("tampinha_cli.main._model_flow_api_key_provider", fake_model_flow_api_key_provider)
    ...
    def test_model_flow_api_key_provider_persists_gmi_selection(self, monkeypatch):
        ...
        with patch(
            ...
            from tampinha_cli.main import _model_flow_api_key_provider
        ...
        config = yaml.safe_load((get_tampinha_home() / "config.yaml").read_text()) or {}
