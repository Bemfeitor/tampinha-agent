...
which broke ``tampinha --tui`` because ``ui-tui/`` had vanished.
...
from tampinha_cli.main import _atomic_replace_dir
...
def test_atomic_replace_swaps_content_on_success(tmp_path: Path) -> None:
    ...
    assert not (dst.parent / "ui-tui.tampinha-update-staging").exists()
    assert not (dst.parent / "ui-tui.tampinha-update-old").exists()
