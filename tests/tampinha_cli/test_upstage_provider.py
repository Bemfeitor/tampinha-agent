...
Regression guard for the bug where `tampinha model` saved `provider: upstage`
correctly but, on re-entry, showed a different provider as active. Root cause:
`tampinha_cli/providers.py` (the resolver behind `resolve_provider_full`) had no
...
class TestUpstageOverlay:
    def test_overlay_exists(self):
        ...
        assert "upstage" in TAMPINHA_OVERLAYS
        overlay = TAMPINHA_OVERLAYS["upstage"]
...
class TestUpstageConfigProviderWins:
    ...
    Mirrors the display logic in `tampinha_cli/main.py` (cmd_model): read
