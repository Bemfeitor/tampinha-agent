"""Tests for --ignore-user-config and --ignore-rules flags on `tampinha chat`.
...
files. In Tampinha the equivalent isolation is:

* ``--ignore-user-config`` → skip ``~/.tampinha/config.yaml`` in ``load_cli_config()``
...
def _clean_env(monkeypatch):
    ...
    """
    for var in ("TAMPINHA_IGNORE_USER_CONFIG", "TAMPINHA_IGNORE_RULES"):
...
class TestIgnoreUserConfigEnvGate:
    """``load_cli_config()`` must honour ``TAMPINHA_IGNORE_USER_CONFIG=1``.

    When the env var is set, user config at ``<tampinha_home>/config.yaml`` is
    ...
    def _write_user_config(self, tmp_path, model_default):
        ...
        # config. With TAMPINHA_IGNORE_USER_CONFIG=1, load_cli_config() falls
    ...
    def _reload_cli(self, monkeypatch, tmp_path):
        """Point cli._tampinha_home at tmp_path and return a fresh load_cli_config."""
        import cli
    ...
    def test_flag_ignored_when_set_to_other_value(self, tmp_path, monkeypatch):
        ...
        self._write_user_config(tmp_path, "test-vendor/ignore-user-config-sentinel")
        monkeypatch.setenv("TAMPINHA_IGNORE_USER_CONFIG", "true")  # not "1"
...
class TestIgnoreRulesEnvGate:
    ...
    def test_env_var_enables_ignore_rules(self, monkeypatch):
        """Setting TAMPINHA_IGNORE_RULES=1 flips HermesCLI.ignore_rules True."""
        monkeypatch.setenv("TAMPINHA_IGNORE_RULES", "1")
        ...
        obj.ignore_rules = ignore_rules or os.environ.get("TAMPINHA_IGNORE_RULES") == "1"

        assert obj.ignore_rules is True


class TestCmdChatWiring:
    """The wiring inside ``cmd_chat()`` in ``tampinha_cli/main.py`` must set
    ...
    def test_both_flags_set_both_env_vars(self, monkeypatch):
        monkeypatch.delenv("TAMPINHA_IGNORE_USER_CONFIG", raising=False)
        monkeypatch.delenv("TAMPINHA_IGNORE_RULES", raising=False)
        ...
        assert os.environ.get("TAMPINHA_IGNORE_USER_CONFIG") == "1"
        assert os.environ.get("TAMPINHA_IGNORE_RULES") == "1"
    ...
    def test_flags_absent_sets_nothing(self, monkeypatch):
        ...
        assert "TAMPINHA_IGNORE_USER_CONFIG" not in os.environ
        assert "TAMPINHA_IGNORE_RULES" not in os.environ
