...
The bug class: on Windows, when `tampinha.exe`/sibling shims could not be
...
import tampinha_cli._install_repair as ir
import tampinha_cli.update_cmd as update_cmd


def _make_shims(scripts_dir: Path, names=("tampinha", "tampinha-gateway")) -> list[Path]:
...
def test_strict_quarantine_refuses_before_install(windows, tmp_path, monkeypatch):
    ...
    # tampinha.exe cannot be renamed; tampinha-gateway.exe can
    real_rename = Path.rename

    def deny_tampinha(self, target):
        if self.name == "tampinha.exe":
            raise PermissionError(13, "held open")
        return real_rename(self, target)

    monkeypatch.setattr(Path, "rename", deny_tampinha)
    monkeypatch.setattr(cli_main, "_tampinha_exe_shims", lambda d: shims)
    ...
    assert install_ran == []
    assert "tampinha.exe" in exc_info.value.failed_shims
    # The successful rename (tampinha-gateway.exe) was rolled back.
    assert (scripts / "tampinha-gateway.exe").exists()
    assert not list(scripts.glob("tampinha-gateway.exe.old.*"))


def test_non_strict_keeps_warn_and_try(windows, tmp_path, monkeypatch):
    scripts = tmp_path / "venv" / "Scripts"
    shims = _make_shims(scripts, names=("tampinha",))
    monkeypatch.setattr(
        Path, "rename",
...
def test_recovery_install_cmd_fail_closed(windows, tmp_path, monkeypatch):
    ...
    _make_shims(scripts, names=("tampinha",))
...
def test_refusal_writes_marker_and_exits_2(monkeypatch, capsys):
    ...
    exc = cli_main.ShimQuarantineError(["tampinha.exe"])
    with pytest.raises(SystemExit) as exit_info:
        update_cmd._refuse_update_for_contended_shims(exc)
    assert exit_info.value.code == 2
    assert wrote == [1]
    out = capsys.readouterr().out
    assert "tampinha.exe" in out
