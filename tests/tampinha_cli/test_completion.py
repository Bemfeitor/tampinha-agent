"""Tests for tampinha_cli/completion.py — shell completion script generation."""
...
from tampinha_cli.completion import _walk, generate_bash, generate_zsh, generate_fish
...
def _make_parser() -> argparse.ArgumentParser:
    """Build a minimal parser that mirrors the real tampinha structure."""
    p = argparse.ArgumentParser(prog="tampinha")
...
class TestGenerateBash:
    def test_contains_completion_function_and_register(self):
        ...
        assert "_tampinha_completion()" in out
        assert "complete -F _tampinha_completion tampinha" in out
...
class TestGenerateZsh:
    ...
    def test_preserves_valid_zsh_arguments_alias_syntax(self):
        ...
        assert "'(-)'{-p,--profile}'[Profile name]:profile:_tampinha_profiles'" in out
...
class TestProfileCompletion:
    ...
    def test_bash_profile_actions_complete_profile_names(self):
        """After 'tampinha profile use', complete with profile names."""
        out = generate_bash(_make_parser())
        # The profile case should have _tampinha_profiles for name-taking actions
        lines = out.split("\n")
        ...
        for line in lines:
            ...
            if in_profile_case and "_tampinha_profiles" in line:
                has_profiles_in_action = True
                break
        assert has_profiles_in_action, "profile actions should complete with _tampinha_profiles"
    ...
    def test_fish_profile_actions_complete_names(self):
        ...
        assert "__tampinha_profiles" in out
        count = out.count("(__tampinha_profiles)")
