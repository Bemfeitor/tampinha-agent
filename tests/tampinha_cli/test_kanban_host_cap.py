...
1. The standalone daemon path (``tampinha kanban daemon --force`` /
   :func:`tampinha_cli.kanban_db.run_daemon`) never resolved
...
def test_run_daemon_resolves_and_passes_max_in_progress(
    ...
    ``tampinha kanban dispatch`` paths both resolved the memory-derived
    default.
    """
    captured: dict = {}
...
def test_configured_max_in_progress_parsing(monkeypatch):
    import tampinha_cli.config as cfgmod
...
def test_nonspawnable_review_does_not_tax_ready_budget(
    ...
    import tampinha_cli.config as cfgmod
    import tampinha_cli.profiles as profmod
