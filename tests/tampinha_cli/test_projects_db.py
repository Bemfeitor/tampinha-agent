"""Tests for the per-profile Projects store (tampinha_cli/projects_db)."""
...
def test_create_get_list(conn):
    pid = pdb.create_project(conn, name="Tampinha Agent", folders=["/tmp/tampinha"])
    proj = pdb.get_project(conn, pid)

    assert proj is not None
    assert proj.slug == "tampinha-agent"
    assert proj.name == "Tampinha Agent"
    # First folder becomes primary.
    assert proj.primary_path == "/tmp/tampinha"
    assert [f.path for f in proj.folders] == ["/tmp/tampinha"]
    assert proj.folders[0].is_primary is True

    # Lookup by slug too.
    assert pdb.get_project(conn, "tampinha-agent").id == pid
...
def test_per_profile_isolation(tmp_path):
    # Two distinct DB paths stand in for two profiles' TAMPINHA_HOME.
