...
Covers the three paths changed by fix/bedrock-provider-model-ids-live-discovery:
  ...
  2. list_authenticated_providers() Section 2 (TAMPINHA_OVERLAYS) — bedrock
...
# ---------------------------------------------------------------------------
# 2. list_authenticated_providers() — bedrock via TAMPINHA_OVERLAYS (Section 2)
# ---------------------------------------------------------------------------
...
class TestBedrockOverlayRegistration:
    """bedrock entry in TAMPINHA_OVERLAYS is correctly configured."""

    def test_bedrock_overlay_exists(self):
        from tampinha_cli.providers import TAMPINHA_OVERLAYS
        assert "bedrock" in TAMPINHA_OVERLAYS
