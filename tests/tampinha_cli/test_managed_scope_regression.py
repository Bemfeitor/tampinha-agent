...
def tampinha_home(tmp_path, monkeypatch):
    ...
    # /etc/tampinha on the dev/CI box can't influence the test.
    monkeypatch.setenv("TAMPINHA_MANAGED_DIR", str(tmp_path / "no_such_managed_dir"))
...
def test_user_config_overrides_default(tampinha_home, monkeypatch):
...
def test_env_expansion_in_user_config(tampinha_home, monkeypatch):
    from tampinha_cli.config import load_config, cfg_get
...
def test_user_env_overrides_shell(tmp_path, monkeypatch):
    ...
    load_tampinha_dotenv(tampinha_home=str(home))
