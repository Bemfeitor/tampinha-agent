...
TAMPINHA_HOME (tests/conftest.py isolation) and assert every store agrees
afterwards.
...
@pytest.fixture
def tampinha_home(monkeypatch, tmp_path):
    """Fresh TAMPINHA_HOME with .env + auth.json + config.yaml fixtures."""
...
def test_delete_clears_provider_models_cache(tampinha_home):
    _write_env(tampinha_home, ZAI_API_KEY=FAKE_ZAI_KEY)
    _write_auth(tampinha_home, {"zai": [_zai_pool_fixture()["zai"][0]]})
    cache_path = tampinha_home / "provider_models_cache.json"
...
def test_update_rotates_config_yaml_model_mirror(tampinha_home):
    old = "sk-oe-" + "f" * 24
    new = "sk-oe-" + "g" * 24
    _write_env(tampinha_home, OPENAI_API_KEY=old)
    _write_config(
        tampinha_home,
        "model:\n"
        "  provider: custom\n"
    ...
    cfg_text = tampinha_home.joinpath("config.yaml").read_text(encoding="utf-8")
