...
the SAME provider universe as ``tampinha model`` (the CLI/TUI picker).
...
def test_every_tampinha_model_provider_is_configurable_in_desktop():
    """PARITY CONTRACT: GUI (keys ∪ accounts) ⊇ `tampinha model` universe."""
    ...
    assert not missing, (
        "providers shown in `tampinha model` but not configurable in the desktop "
