"""Tests for tampinha_cli.uninstall.remove_node_symlinks.

Regression for #34536: the POSIX installer drops node/npm/npx symlinks in
~/.local/bin pointing into $TAMPINHA_HOME/node and prepends ~/.local/bin to
PATH, shadowing an existing nvm. Uninstall must remove those symlinks, but
only when they still resolve into the Tampinha-managed node dir.
"""
...
def _make_tampinha_node(tampinha_home: Path) -> Path:
    """Create a fake $TAMPINHA_HOME/node/bin/{node,npm,npx} tree."""
    node_bin = tampinha_home / "node" / "bin"
...
def test_leaves_unrelated_symlinks_untouched(fake_home):
    ...
    tampinha_home = fake_home / ".tampinha"
    _make_tampinha_node(tampinha_home)
    ...
    removed = uninstall.remove_node_symlinks(tampinha_home)
...
def test_removes_fhs_symlinks_in_usr_local_bin(fake_home, tmp_path, monkeypatch):
    ...
    """
    tampinha_home = fake_home / ".tampinha"
    node_bin = _make_tampinha_node(tampinha_home)
