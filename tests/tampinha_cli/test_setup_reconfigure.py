...
- Bare `tampinha setup` drops straight into the full reconfigure wizard
  (every prompt shows the current value as its default).
- `tampinha setup --quick` runs the narrower "fill in missing items" flow.
- `tampinha setup --reconfigure` is a backwards-compat alias for the
  bare-setup default.
...
def _enter_existing_install_patches(stack, **extra):
    ...
    for target, kwargs in [
        ("tampinha_cli.setup.ensure_tampinha_home", {}),
        ("tampinha_cli.setup.is_interactive_stdin", {"return_value": True}),
        ("tampinha_cli.config.is_managed", {"return_value": False}),
        ("tampinha_cli.setup.load_config", {"return_value": {}}),
        ("tampinha_cli.setup.save_config", {}),
        ("tampinha_cli.setup.get_env_value", {"return_value": None}),
        ("tampinha_cli.auth.get_active_provider", {"return_value": "openrouter"}),
        ("tampinha_cli.setup._print_setup_summary", {}),
        ("tampinha_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
...
def _enter_fresh_install_patches(stack, **extra):
    for target, kwargs in [
        ...
        ("tampinha_cli.setup.load_config", {"return_value": {}}),
        ("tampinha_cli.setup.save_config", {}),
        ("tampinha_cli.auth.get_active_provider", {"return_value": None}),
        ("tampinha_cli.setup.get_env_value", {"return_value": None}),
        ("tampinha_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
        stack.enter_context(patch(target, **kwargs))
...
class TestExistingInstallDefault:
    """Bare `tampinha setup` on an existing install = full reconfigure wizard."""
    ...
    def test_bare_setup_runs_full_reconfigure_without_menu(self, existing_install):
        ...
        with ExitStack() as stack:
            m = _enter_existing_install_patches(
                stack,
                prompt_choice="tampinha_cli.setup.prompt_choice",
                quick="tampinha_cli.setup._run_quick_setup",
                model="tampinha_cli.setup.setup_model_provider",
                terminal="tampinha_cli.setup.setup_terminal_backend",
                agent="tampinha_cli.setup.setup_agent_settings",
                gateway="tampinha_cli.setup.setup_gateway",
                tools="tampinha_cli.setup.setup_tools",
            )
...
class TestQuickFlag:
    ...
    def test_quick_flag_runs_quick_setup_only(self, existing_install):
        ...
        with ExitStack() as stack:
            ...
            from tampinha_cli.setup import run_setup_wizard
            from tampinha_cli import setup as setup_mod
...
class TestFreshInstall:
    ...
    def test_reconfigure_on_fresh_install_falls_through(self, fresh_install):
        ...
        with ExitStack() as stack:
            m = _enter_fresh_install_patches(
                stack,
                prompt=("tampinha_cli.setup.prompt_choice", {"return_value": 0}),
                first="tampinha_cli.setup._run_first_time_quick_setup",
            )
            from tampinha_cli.setup import run_setup_wizard
            from tampinha_cli import setup as setup_mod
    ...
    def test_blank_slate_runs_inside_navigation_step(self, fresh_install):
        ...
        with ExitStack() as stack:
            m = _enter_fresh_install_patches(
                stack,
                prompt=("tampinha_cli.setup.prompt_choice", {"return_value": 2}),
                blank="tampinha_cli.setup._run_blank_slate_setup",
            )
            from tampinha_cli import setup as setup_mod
...
class TestArgparse:
    ...
    def test_reconfigure_flag_reaches_cmd_setup(self, monkeypatch):
        import sys
        from tampinha_cli.main import main

        captured = {}
        monkeypatch.setattr(
            "tampinha_cli.setup.run_setup_wizard",
            lambda args: captured.setdefault("args", args),
        )
        monkeypatch.setattr(sys, "argv", ["tampinha", "setup", "--reconfigure"])
        try:
            main()
        except SystemExit:
            pass
