"""Tests for issue #26670 — concurrent tampinha.exe detection and improved
quarantine retry / reboot-deferred fallback during `tampinha update` on Windows.
...
# Tests in this module either exercise the REAL _detect_concurrent_tampinha_instances
# helper (and need the autouse stub in tests/tampinha_cli/conftest.py disabled),
...
def _make_proc(pid: int, exe: str, name: str = "tampinha.exe"):
...
def test_detect_concurrent_parents_call_robust_to_one_bad_hop(_winp, tmp_path):
    ...
    rows = [
        ...
        _make_proc(launcher_pid, str(shim), "tampinha.exe"),
    ]
    ...
    with patch.dict(sys.modules, {"psutil": fake_psutil}):
        result = cli_main._detect_concurrent_tampinha_instances(scripts_dir)

    # No crash; helper completes. (Degenerate stub: launcher exe unreadable.)
    assert result == [(launcher_pid, "tampinha.exe")]
...
# ---------------------------------------------------------------------------
# _quarantine_running_tampinha_exe — retry, then report
# ---------------------------------------------------------------------------
...
def test_quarantine_succeeds_first_attempt(_winp, tmp_path):
    ...
    shim = tmp_path / "tampinha.exe"
    shim.write_bytes(b"old")

    pairs = cli_main._quarantine_running_tampinha_exe(tmp_path)

    assert len(pairs) == 1
    orig, quarantine = pairs[0]
    assert orig == shim
    assert quarantine.name.startswith("tampinha.exe.old.")
...
def test_quarantine_reports_a_lock_it_cannot_break(_winp, tmp_path, capsys, monkeypatch):
    ...
    monkeypatch.setattr(cli_main, "_tampinha_exe_shims", lambda d: [shim])
...
def test_pause_windows_gateways_for_update_stops_profile_and_unmapped_pids(
    ...
    monkeypatch.setattr(
        ...
        lambda pid: ["pythonw.exe", "-m", "tampinha_cli.main", "gateway", "run"]
        if pid == 202
        else None,
    )
    ...
    assert token == {
        ...
        "unmapped": [
            {
                "pid": 202,
                "argv": ["pythonw.exe", "-m", "tampinha_cli.main", "gateway", "run"],
            }
        ],
    }
...
def test_unreadable_argv_falls_back_to_the_captured_prefix(monkeypatch):
    ...
    gateway_prefix = r"venv\Scripts\python.exe -m tampinha_cli.main gateway run"
...
# when every concurrent tampinha.exe is a gateway runtime — the pause
...
def test_classify_concurrent_instance_recognises_gateway_runtimes(monkeypatch):
    ...
    launcher shape (python -m, tampinha.exe shim, tampinha-gateway.exe,
    gateway/run.py, bare `tampinha gateway` which defaults to run)."""
    cases = [
        [r"C:\venv\Scripts\python.exe", "-m", "tampinha_cli.main", "gateway", "run"],
        [r"C:\venv\Scripts\tampinha.exe", "gateway", "run"],
        [r"C:\venv\Scripts\tampinha-gateway.exe"],
        [r"C:\venv\Scripts\python.exe", "gateway/run.py"],
        ["tampinha.exe", "GATEWAY", "RUN"],  # matcher is case-insensitive
        ["tampinha.exe", "gateway"],  # bare `tampinha gateway` defaults to run
        # profile selector before the subcommand — canonical matcher strips it
        ["tampinha.exe", "--profile", "work", "gateway", "run"],
    ]
    for argv in cases:
...
def test_classify_concurrent_instance_recognises_non_gateways(monkeypatch):
    ...
    cases = [
        [r"C:\venv\Scripts\tampinha.exe"],  # interactive REPL
        [r"C:\venv\Scripts\tampinha.exe", "dashboard"],
        ["tampinha.exe", "gateway", "status"],  # management, not runtime
        ["tampinha.exe", "gateway", "stop"],
...
def test_filter_non_gateway_concurrent_instances_splits(monkeypatch):
    ...
    monkeypatch.setitem(
        ...
        _fake_psutil_classify(
            {
                100: ["tampinha.exe", "gateway", "run"],
                200: ["tampinha.exe"],  # REPL — keep
                300: ["tampinha.exe", "dashboard"],  # keep
    ...
    assert kept == [(200, "tampinha.exe"), (300, "tampinha.exe"), (400, "tampinha.exe")]
...
def test_filter_non_gateway_concurrent_instances_gateway_only(monkeypatch):
    ...
    monkeypatch.setitem(
        ...
        _fake_psutil_classify(
            {
                111: ["tampinha.exe", "gateway", "run"],
                222: [r"C:\venv\Scripts\tampinha-gateway.exe"],
            }
        ),
    )
    matches = [(111, "tampinha.exe"), (222, "tampinha-gateway.exe")]
...
def test_update_gate_skips_abort_when_only_concurrent_is_gateway(
    ...
    with patch.object(
        ...
        cli_main,
        "_detect_concurrent_tampinha_instances",
        return_value=[(1000, "tampinha.exe"), (2000, "tampinha-gateway.exe")],
    ), patch.object(
    ...
    captured = capsys.readouterr().out
    assert "Another tampinha.exe is running" not in captured
...
def test_update_gate_still_aborts_on_non_gateway_concurrent(
    ...
    with patch.object(
        ...
        cli_main,
        "_detect_concurrent_tampinha_instances",
        return_value=[(1000, "tampinha.exe"), (3000, "tampinha.exe")],
    ), patch.object(
        cli_main,
        "_filter_non_gateway_concurrent_instances",
        return_value=[(3000, "tampinha.exe")],
    ), patch.object(
        cli_main, "_run_pre_update_backup"
    ) as mock_backup:
