...
from tampinha_cli import config as tampinha_config
...
def _setup_update_mocks(monkeypatch, tmp_path):
    ...
    (tmp_path / ".git").mkdir()
    monkeypatch.setattr(tampinha_main, "PROJECT_ROOT", tmp_path)
    monkeypatch.setattr(tampinha_main, "_stash_local_changes_if_needed", lambda *a, **kw: None)
    monkeypatch.setattr(tampinha_main, "_restore_stashed_changes", lambda *a, **kw: True)
    monkeypatch.setattr(tampinha_config, "get_missing_env_vars", lambda required_only=True: [])
    monkeypatch.setattr(tampinha_config, "get_missing_config_fields", lambda: [])
    monkeypatch.setattr(tampinha_config, "check_config_version", lambda: (5, 5))
    monkeypatch.setattr(tampinha_config, "migrate_config", lambda **kw: {"env_added": [], "config_added": []})
    monkeypatch.setattr(tampinha_main, "_upgrade_pip_before_lazy_refresh", lambda *a, **kw: None)
    monkeypatch.setattr(tampinha_main, "_refresh_active_lazy_features", lambda *a, **kw: True)
...
def test_refresh_active_memory_provider_dependencies_reinstalls_active_provider(monkeypatch):
    ...
    monkeypatch.setattr(
        "tampinha_cli.memory_setup._install_dependencies",
        lambda provider_name, force=False: recorded.append((provider_name, force)),
    )

    tampinha_main._refresh_active_memory_provider_dependencies()
...
def test_reload_updated_runtime_modules_restores_new_tampinha_constants_symbol(monkeypatch):
    """A pre-pull module object missing a new helper is repaired by reload."""
    import tampinha_constants

    monkeypatch.delattr(tampinha_constants, "apply_subprocess_home_env", raising=False)
    assert not hasattr(tampinha_constants, "apply_subprocess_home_env")

    tampinha_main._reload_updated_runtime_modules()

    assert callable(tampinha_constants.apply_subprocess_home_env)
...
def test_cmd_update_skips_stash_restore_when_reset_fails(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(
        tampinha_main, "_stash_local_changes_if_needed",
        lambda *a, **kw: "abc123deadbeef",
    )
    restore_calls = []
    monkeypatch.setattr(
        tampinha_main, "_restore_stashed_changes",
        lambda *a, **kw: restore_calls.append(1) or True,
    )

    side_effect, _ = _make_update_side_effect(ff_only_fails=True, reset_fails=True)
    monkeypatch.setattr(tampinha_main.subprocess, "run", side_effect)

    with pytest.raises(SystemExit, match="1"):
        tampinha_main.cmd_update(SimpleNamespace())
...
def _setup_setting_test(monkeypatch, tmp_path, mode):
    ...
    monkeypatch.setattr(
        tampinha_main, "_discard_stashed_changes",
...
def _setup_keep_stash_test(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        tampinha_main, "_park_stashed_changes",
    ...
    monkeypatch.setattr(
        "tampinha_cli.gateway.find_gateway_pids", lambda **kw: [], raising=False
    )
...
def test_update_keep_stash_parks_instead_of_restoring(monkeypatch, tmp_path):
    ...
    tampinha_main.cmd_update(SimpleNamespace(yes=True, keep_stash=True))
...
def test_update_without_keep_stash_still_restores(monkeypatch, tmp_path):
    ...
    tampinha_main.cmd_update(SimpleNamespace(yes=True, keep_stash=False))
...
def test_bootstrap_marker_not_autostashed_by_update(tmp_path):
    ...
    ``tampinha update``'s ``git stash push --include-untracked`` does not sweep it
    ...
    repo_gitignore = Path(tampinha_main.__file__).resolve().parents[1] / ".gitignore"
    ...
    marker = tmp_path / ".tampinha-bootstrap-complete"
    marker.write_text("")

    # Exact flags used by tampinha update (tampinha_cli/main.py).
    git("stash", "push", "--include-untracked", "-m", "tampinha-update-autostash")

    assert marker.exists(), (
        ".tampinha-bootstrap-complete was swept into the update autostash — it must "
    ...
    assert ".tampinha-bootstrap-complete" not in status
