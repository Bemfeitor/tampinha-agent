...
#88251: ``tampinha update`` treated a failed desktop pack as non-fatal, printed
...
def desktop_env(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "tampinha_constants.with_tampinha_node_path", lambda: {}, raising=False
    )
    monkeypatch.setattr(
        "tampinha_constants.display_tampinha_home", lambda: str(tmp_path), raising=False
    )
...
def test_summary_omits_success_banner_when_desktop_rebuild_failed(capsys):
    ...
    assert "tampinha desktop" in out
...
def test_gateway_exit_code_file_tracks_desktop_rebuild(tmp_path, monkeypatch):
    monkeypatch.setattr(update_cmd, "get_tampinha_home", lambda: tmp_path)
