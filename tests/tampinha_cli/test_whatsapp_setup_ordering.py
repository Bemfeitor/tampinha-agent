...
Before the fix, ``tampinha whatsapp`` wrote ``WHATSAPP_ENABLED=true`` at
...
``tampinha gateway`` then paid a 30s bridge-bootstrap timeout and queued
...
def _env_value(tampinha_home: Path, key: str) -> str | None:
    env_file = tampinha_home / ".env"
...
def test_aborted_setup_does_not_enable_whatsapp(isolated_home, monkeypatch):
    ...
    """
    from tampinha_cli.main import cmd_whatsapp
    ...
    monkeypatch.setattr("tampinha_cli.main._require_tty", lambda *_a, **_kw: None)
...
def test_existing_pairing_skip_branch_enables_whatsapp(isolated_home, monkeypatch):
    """User runs ``tampinha whatsapp`` with an existing paired session and
