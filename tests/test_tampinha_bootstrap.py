"""Tests for tampinha_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every Tampinha entry point
(tampinha, tampinha-agent, tampinha-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every Tampinha entry point imports tampinha_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported tampinha_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("tampinha_bootstrap", None)
    import tampinha_bootstrap  # noqa: WPS433
    return tampinha_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the Tampinha CLI
...
class TestEntryPointsImportBootstrap:
    """Every Tampinha entry point must import tampinha_bootstrap as its
    ...
    # Entry points that invoke Tampinha as a process.  Each one must
    # import tampinha_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "tampinha_cli/main.py",   # tampinha CLI (console_script)
        "run_agent.py",          # tampinha-agent (console_script)
        "acp_adapter/entry.py",  # tampinha-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import tampinha_bootstrap' and that
        ...
        half-finished ``tampinha update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``tampinha_bootstrap`` as a top-level module) leaves tampinha
        ...
        """
        # Resolve relative to the tampinha-agent repo root.  Tests live
        # at tests/test_tampinha_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # tampinha start even when tampinha_bootstrap hasn't been
        ...
        assert first_import_name == "tampinha_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'tampinha_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import tampinha_bootstrap' line to be the first import."
        )


class TestHardenImportPath:
    """harden_import_path() must keep a same-named package in the launch
    directory from shadowing Tampinha's own top-level modules — covering both
    the relative ('' / '.') and absolute-path forms the cwd can take on
    sys.path (issue #51286)."""

    def _run(self, hb, path_seed, env=None):
        original = sys.path[:]
        original_env = os.environ.get("TAMPINHA_PYTHON_SRC_ROOT")
        try:
            sys.path[:] = path_seed
            if env is not None:
                os.environ["TAMPINHA_PYTHON_SRC_ROOT"] = env
            elif "TAMPINHA_PYTHON_SRC_ROOT" in os.environ:
                del os.environ["TAMPINHA_PYTHON_SRC_ROOT"]
            hb.harden_import_path(src_root="/opt/tampinha")
            return sys.path[:]
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("TAMPINHA_PYTHON_SRC_ROOT", None)
            else:
                os.environ["TAMPINHA_PYTHON_SRC_ROOT"] = original_env

    def test_relative_cwd_forms_removed(self):
        hb = _fresh_import()
        result = self._run(hb, ["", ".", "/opt/tampinha", "/usr/lib/python"])
    ...
    def test_absolute_cwd_path_loses_to_src_root(self):
        ...
        # Tampinha root.  The guard must relocate Tampinha to the front.
        hb = _fresh_import()
        result = self._run(hb, ["/home/user/tg-ws-proxy", "/opt/tampinha"])
        assert result[0] == "/opt/tampinha"
        # The cwd absolute path may still appear (it can hold legit deps),
        # but only AFTER the Tampinha root.
        assert result.index("/opt/tampinha") < result.index("/home/user/tg-ws-proxy")
    ...
    def test_env_var_used_when_no_arg(self):
        ...
        try:
            ...
            os.environ["TAMPINHA_PYTHON_SRC_ROOT"] = "/env/tampinha"
            hb.harden_import_path()
            assert sys.path[0] == "/env/tampinha"
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("TAMPINHA_PYTHON_SRC_ROOT", None)
