    ...
    with patch.dict(
        "sys.modules",
        {
            "tampinha_constants": MagicMock(
                get_tampinha_home=MagicMock(return_value="/tmp/tampinha_test_review_summary")
            ),
...
def test_init_session_attaches_background_review_callback(server, monkeypatch):
    ...
    cb("💾 Self-improvement review: Skill 'tampinha-release' patched")
    ...
    assert sid == "sid-abc"
    assert payload == {
        "text": "💾 Self-improvement review: Skill 'tampinha-release' patched"
    }
