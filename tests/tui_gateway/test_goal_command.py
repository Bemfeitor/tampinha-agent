...
def tampinha_home(tmp_path, monkeypatch):
    ...
    # Bust the goal-module DB cache so it re-resolves TAMPINHA_HOME.
...
@pytest.fixture()
def server(tampinha_home, monkeypatch):
    ...
    # Pin config resolution to the isolated TAMPINHA_HOME. Sibling test
    ...
    # module-level ``_tampinha_home = get_tampinha_home()`` snapshot freezes
    ...
    monkeypatch.setattr(mod, "_tampinha_home", tampinha_home)
...
def test_moa_bare_returns_usage(server, session, tampinha_home):
    _write_moa_config(tampinha_home, """
moa:
