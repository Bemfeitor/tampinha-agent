...
guard is covered in ``tests/tampinha_cli/test_picker_prewarm.py``):

- ``main()`` invokes ``tampinha_cli.model_switch.prewarm_picker_cache_async``
...
def _run_main(monkeypatch, events, *, prewarm=None):
    ...
    # entry.main() imports the helper lazily from tampinha_cli.model_switch,
