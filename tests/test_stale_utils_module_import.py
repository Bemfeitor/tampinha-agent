...
Real incident (gateway session 1518671026962174144)::
    ...
    cannot import name 'env_float' from 'utils' (~/.tampinha/tampinha-agent/utils.py)
...
2. ``tampinha update`` ran ``git pull``, updating ``utils.py`` (now defining
...
``tampinha_cli/main.py`` (the ``tampinha update`` flow, ~line 9326) already
acknowledges this exact hazard -- "source files on disk are newer than cached
Python modules in this process" -- and reloads ``tampinha_constants`` after the
