...
def http_server(tmp_path, monkeypatch):
    """Spin up a localhost HTTP server and isolate TAMPINHA_HOME under tmp_path."""
    monkeypatch.setenv("TAMPINHA_HOME", str(tmp_path / ".tampinha"))
    (tmp_path / ".tampinha").mkdir()

    # Force the constants/image cache helpers to re-read TAMPINHA_HOME.
    import sys
    for mod in list(sys.modules):
        if mod.startswith("tampinha_constants") or mod.startswith("agent.image_gen_provider"):
...
class TestSaveUrlImage:
    def test_writes_real_bytes_to_tampinha_home_cache(self, http_server):
        base, _ = http_server
        ...
        # The cache directory must be under TAMPINHA_HOME — gateway cleanup
