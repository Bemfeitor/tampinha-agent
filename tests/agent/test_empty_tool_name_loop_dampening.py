...
@pytest.fixture()
def agent_env():
    """Spin up the mock provider + an isolated TAMPINHA_HOME, yield (agent, helpers)."""
    ...
    test_home = tempfile.mkdtemp(prefix="tampinha_e2e_47967_")
    os.makedirs(os.path.join(test_home, ".tampinha"))
    prev_home = os.environ.get("TAMPINHA_HOME")
    os.environ["TAMPINHA_HOME"] = os.path.join(test_home, ".tampinha")
    ...
    for mod in list(sys.modules):
        if mod == "run_agent" or mod.startswith("agent.") or mod.startswith("tools.") or mod.startswith("tampinha_"):
            del sys.modules[mod]
    ...
    try:
        ...
        if prev_home is None:
            ...
            os.environ["TAMPINHA_HOME"] = prev_home
