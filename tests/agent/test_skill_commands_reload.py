...
def tampinha_home(monkeypatch):
    """Isolate TAMPINHA_HOME for ``reload_skills`` tests.

    Rather than popping cache-bearing modules from ``sys.modules``,
    we monkeypatch the module-level ``TAMPINHA_HOME`` / ``SKILLS_DIR``
    constants in place so the isolation is local to this fixture's scope.
    """
    td = tempfile.mkdtemp(prefix="tampinha-reload-skills-")
    monkeypatch.setenv("TAMPINHA_HOME", td)
    home = Path(td)
    ...
    monkeypatch.setattr(_st, "TAMPINHA_HOME", home, raising=False)
...
class TestReloadSkillsHelper:
    ...
    def test_detects_removed_skill_carries_description(self, tampinha_home):
        from agent.skill_commands import reload_skills

        skill_dir = _write_skill(tampinha_home / "skills", "demo", "soon to be gone")
    ...
    def test_does_not_invalidate_prompt_cache_snapshot(self, tampinha_home):
