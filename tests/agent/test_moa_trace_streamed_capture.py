...
path via ``tampinha chat --query``) couldn't see what the aggregator actually
produced without joining to the session DB by hand.

These exercise the real ``consume_and_save_trace`` → ``save_moa_turn`` path
with real file I/O against a temp TAMPINHA_HOME — no mocks on the write path.
"""
...
def _enable_traces(tmp_path, monkeypatch):
    """Point TAMPINHA_HOME at a temp dir and turn moa.save_traces on."""
    tampinha_home = tmp_path / ".tampinha"
    tampinha_home.mkdir()
    monkeypatch.setenv("TAMPINHA_HOME", str(tampinha_home))

    # save_moa_turn reads config via tampinha_cli.config.load_config; stub it to
    ...
    return tampinha_home / "moa-traces"
