...
manipulation. Tampinha keeps canonical conversation history in state.db and may
...
def fake_homes(tmp_path, monkeypatch):
    """Point TAMPINHA_HOME at a temp profile dir.

    Uses the real env-var resolution chain (get_tampinha_home /
    get_default_tampinha_root) instead of monkeypatching private helpers —
    ...
    TAMPINHA_HOME=<root>/profiles/<name> makes get_default_tampinha_root()
