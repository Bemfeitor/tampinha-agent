...
def test_reconcile_absorbed_into_beats_everything_else(curator_env):
    ...
    out = curator_env._reconcile_classification(
        ...
        destinations={"tampinha-agent-dev"},
        absorbed_declarations={
            "pr-review-format": {"into": "tampinha-agent-dev", "declared": True},
        },
    )
    ...
    assert e["into"] == "tampinha-agent-dev"
...
# `tampinha curator status`). The full data has always been in REPORT.md on
...
# ---------------------------------------------------------------------------
# Pin hint — surfaces `tampinha curator pin <umbrella>` in the rename block so
