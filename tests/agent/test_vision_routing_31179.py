...
def isolated_home(monkeypatch):
    """Temp TAMPINHA_HOME with config + clean credential env vars."""
    test_home = tempfile.mkdtemp(prefix="tampinha_test_31179_")
    tampinha_home = os.path.join(test_home, ".tampinha")
    os.makedirs(tampinha_home)
...
def _fresh_modules():
    """Drop cached tampinha modules so each test reloads against current env."""
    _drop_reload_targets()
