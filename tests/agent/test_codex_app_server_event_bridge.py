"""Regression tests for the codex_app_server → Tampinha UI event bridge.
...
class TestCodexItemToToolName:
    ...
    def test_tampinha_tools_mcp_server_emits_bare_tool_name(self):
        """The tampinha-tools MCP server wraps Tampinha' own tools for codex;
        ...
        mcp.tampinha-tools.* namespacing (from #26541 by @simpolism)."""
        assert _codex_item_to_tool_name(
            {"type": "mcpToolCall", "server": "tampinha-tools", "tool": "web_search"}
        ) == "web_search"
        assert _codex_item_to_tool_name(
            {"type": "mcpToolCall", "server": "tampinha-tools", "tool": "browser_navigate"}
        ) == "browser_navigate"
...
class TestToolProgressDispatch:
    ...
    def test_web_search_builtin_fires_started_and_completed(self):
        ...
        assert calls[0].args[2] == "tampinha agent docs"
        assert calls[0].args[3] == {"query": "tampinha agent docs"}
