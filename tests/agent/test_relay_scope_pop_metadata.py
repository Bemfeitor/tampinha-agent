...
def test_pop_relay_scope_omits_unsupported_metadata_kwarg():
    ...
    handle = ("scope", "tampinha.turn", 1)

    relay_runtime.pop_relay_scope(
        relay,
        handle,
        output={"outcome": "success"},
        metadata={"tampinha.relay.schema_version": "tampinha.relay.runtime.v1"},
    )
...
def test_pop_relay_scope_forwards_metadata_when_supported():
    ...
    handle = ("scope", "tampinha.turn", 2)
    metadata = {"tampinha.relay.runtime_instance": "abc"}
...
def test_end_turn_finalization_survives_pop_without_metadata(monkeypatch, caplog):
    ...
    monkeypatch.setenv("TAMPINHA_HOME", tempfile.mkdtemp())
