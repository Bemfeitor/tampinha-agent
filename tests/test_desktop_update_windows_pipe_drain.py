...
last of them to close it. ``tampinha update`` deliberately runs build steps with
stdout inherited (the tee-stderr runner in ``tampinha_cli/main.py``), so the
...
``.tampinha-update-result.json`` is never written, ``.tampinha-update-in-progress``
is never cleared, and the Desktop is never relaunched -- so the app sits on
"Updating Tampinha" until the user kills the gateway by hand, and the stale marker
...
step writing to both pipes took 18.3s vs 0.29s. ``tampinha update`` is exactly this
...
class TestIdleWatchdogCountsUpdateLogGrowth:
    ...
    Real updates are stdout-silent for 40+ minutes: ``tampinha update`` captures
    the (very loud) Electron/vite build into ``logs/update.log`` — NOT the
    child's stdout (``tampinha_cli/update_cmd.py``, the update-log tee) — so the
    ...
    the ``TAMPINHA_UPDATE_STEP_IDLE_SECONDS`` override each fails a test here.
    """
    ...
    def test_progress_log_overridable_for_self_test(self):
        assert "TAMPINHA_UPDATE_PROGRESS_LOG" in self._src()

    def test_idle_override_env_retained(self):
        # The user/test-facing idle override must survive the amendment.
        assert "TAMPINHA_UPDATE_STEP_IDLE_SECONDS" in self._src()
...
def test_update_step_survives_pipe_leak_flood_and_live_child_stall(
    ...
    *logstall* -- a step that is silent on its pipes but appends to the
    ...
    This is the shape of every real ``tampinha update`` build: output streams to
    ...
    env = {
        **os.environ,
        ...
        "TAMPINHA_UPDATE_PIPE_DRAIN_SECONDS": "3",
        "TAMPINHA_UPDATE_STEP_IDLE_SECONDS": "3",
        "TAMPINHA_SELFTEST_HOLD_SECONDS": "45",
    }
    ...
    assert "PIPE-DRAIN SELF-TEST: PASS" in result.stdout, (
        ...
        "Tampinha' forever) or metered a chatty step (backpressure on the running "
