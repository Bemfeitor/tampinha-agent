"""Guard: Tampinha-owned subprocesses must not resolve managed runtimes by bare PATH.

Tampinha installs runtimes for itself — ``uv`` at ``$TAMPINHA_HOME/bin/uv``, Node at
``$TAMPINHA_HOME/node``. Neither directory is on the ambient PATH of an arbitrary
process, so ``shutil.which("uv")`` / ``shutil.which("node")`` in Tampinha's own
...
* when a system copy also exists, the one Tampinha does not own wins — which is
...
``iter_tampinha_node_dirs()``, ``resolve_uv()``, or ``ensure_uv()``. This test is
...
# Runtimes Tampinha provisions into TAMPINHA_HOME and must therefore resolve
# through a managed-aware helper rather than PATH.
_MANAGED_COMMANDS = frozenset({"uv", "node", "npm", "npx"})

# Directories that are not Tampinha-owned subprocess code: plugins ship their own
...
_ALLOWED: dict[tuple[str, str], str] = {
    ...
    ("tampinha_cli/update_cmd.py", "npm"): (
...
def _source_files() -> list[Path]:
    ...
    for path in REPO_ROOT.rglob("*.py"):
        ...
        # tampinha_agent-0.20.5/, build/ and *.egg-info dirs). CI jobs that
...
def test_no_unreviewed_bare_managed_runtime_lookups():
    ...
    assert not unexpected, (
        "Bare PATH lookup for a Tampinha-managed runtime.\n\n"
        + "\n".join(f"  {rel}:{lineno}  which({cmd!r})" for rel, cmd, lineno in unexpected)
        + "\n\n$TAMPINHA_HOME/bin (uv) and $TAMPINHA_HOME/node are not on an "
        ...
        "  node/npm -> tampinha_constants.find_node_executable()\n"
        "  PATH env -> tampinha_constants.iter_tampinha_node_dirs()\n"
...
def test_managed_node_helpers_exist(helper):
    ...
    assert callable(getattr(tampinha_constants, helper))


def test_managed_uv_helpers_exist():
    from tampinha_cli.managed_uv import ensure_uv, managed_uv_path, resolve_uv
