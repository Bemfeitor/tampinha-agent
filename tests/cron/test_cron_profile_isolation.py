...
profile's TAMPINHA_HOME and EXECUTE under it.

- Storage: a job created under profile ``coder`` writes to
  ``~/.tampinha/profiles/coder/cron/jobs.json`` — NOT the shared default root.
- Execution: the profile-scoped gateway's in-process ticker resolves the
  active TAMPINHA_HOME (profile home) at call time, so jobs run with that
...
one store and runs them under whatever TAMPINHA_HOME the ticker happens to have —
...
def _set_profile_env(monkeypatch, root: Path, profile_home: Path) -> None:
    ...
    TAMPINHA_HOME is a profile under it (``<root>/profiles/<name>``)."""
    import tampinha_constants

    monkeypatch.setattr(
        tampinha_constants, "_get_platform_default_tampinha_home", lambda: root
    )
    monkeypatch.setenv("TAMPINHA_HOME", str(profile_home))


def test_cron_storage_anchors_at_profile_home(tmp_path, monkeypatch):
    """Under a profile TAMPINHA_HOME (<root>/profiles/<name>), the cron store
    ...
    assert tampinha_constants.get_tampinha_home().resolve() == profile_home.resolve()
    assert tampinha_constants.get_default_tampinha_root().resolve() == root.resolve()

    # cron/jobs.py computes TAMPINHA_DIR from get_tampinha_home() at import, so a
    # fresh import under this env anchors the store at <profile>/cron.
    import cron.jobs as jobs

    importlib.reload(jobs)
    try:
        assert jobs.TAMPINHA_DIR.resolve() == profile_home.resolve()
        assert (
