...
def _run_booked_job(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(tampinha_state, "SessionDB", _RecordingSessionDB)
    monkeypatch.setattr(run_agent, "AIAgent", _FakeCronAgent)
    monkeypatch.setattr(
        "tampinha_constants.resolve_reasoning_config", lambda *_a, **_k: None
    )
    ...
    monkeypatch.setenv("TAMPINHA_TEST_RUNTIME_KEY", "unused-placeholder")

    def _fake_runtime(**_kwargs):
        return {
            "api_key": os.environ.get("TAMPINHA_TEST_RUNTIME_KEY", ""),
            "base_url": None,
    ...
    monkeypatch.setattr(
        "tampinha_cli.runtime_provider.resolve_runtime_provider", _fake_runtime
    )
