...
def cron_store(tmp_path, monkeypatch):
    ...
    (tampinha_home / "cron").mkdir(parents=True)
    monkeypatch.setenv("TAMPINHA_HOME", str(tampinha_home))
    monkeypatch.setattr(J, "TAMPINHA_DIR", tampinha_home)
    monkeypatch.setattr(J, "CRON_DIR", tampinha_home / "cron")
    monkeypatch.setattr(J, "JOBS_FILE", tampinha_home / "cron" / "jobs.json")
    monkeypatch.setattr(J, "OUTPUT_DIR", tampinha_home / "cron" / "output")
...
class TestCronRearmRespectsScheduleLegality:
    def test_weekday_job_errored_friday_does_not_fire_on_saturday(self, cron_store):
        ...
        with mock.patch.object(J, "_tampinha_now", lambda: now):
            due = J.get_due_jobs()
