...
the shared /blueprint command handler (tampinha_cli/blueprint_cmd.py), and
the docs generator. Uses an isolated TAMPINHA_HOME for anything that touches the
cron job store.
"""
...
class TestRenderers:
    ...
    def test_deeplink_shape(self):
        ...
        assert url.startswith("tampinha://blueprint/morning-brief?")
