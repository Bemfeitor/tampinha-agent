"""Shared fixtures for the tampinha-agent test suite.
...
2. **Isolated TAMPINHA_HOME.** TAMPINHA_HOME points to a per-test tempdir so
   code reading ``~/.tampinha/*`` via ``get_tampinha_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".tampinha"`` instead of the canonical
   ``get_tampinha_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No TAMPINHA_SESSION_* inheritance** — the agent's current gateway
...
# ── Sandbox TAMPINHA_HOME before ANY test module is imported ──────────────────
# `tampinha_cli/main.py` calls `setup_logging()` at MODULE level, which resolves
# `get_tampinha_home()` and attaches rotating file handlers to the ROOT logger.
...
# real `~/.tampinha/logs/agent.log` and `errors.log`.
#
# The `_isolate_env` fixture below also sandboxes TAMPINHA_HOME, but fixtures run
...
# the REAL Tampinha root — capture it BEFORE the sandbox rewires TAMPINHA_HOME,
# otherwise the deny-list would point at the throwaway tempdir and the guard
# would silently stop protecting the operator's actual ~/.tampinha (#69385).
_PRE_SANDBOX_KANBAN_OVERRIDE = os.environ.get("TAMPINHA_KANBAN_HOME", "").strip()
_PRE_SANDBOX_TAMPINHA_HOME = os.environ.get("TAMPINHA_HOME", "")


def _tampinha_home_points_at_production(value: str) -> bool:
    """True when a pre-set TAMPINHA_HOME resolves to the real production root.

    Gateway-launched shells (and developer shells that ``export
    TAMPINHA_HOME=~/.tampinha``) hand pytest the PRODUCTION home. Historically
    the session sandbox below honored any pre-set value, so collection-time
    imports (logging handlers, ``tampinha_state.DEFAULT_DB_PATH``) froze paths
    inside the real ``~/.tampinha`` — the escape vector that landed pytest
    ...
    TAMPINHA_HOME is honored now.
    """
    if not value:
        return True
    try:
        resolved = Path(value).expanduser().resolve()
        real_root = (Path.home() / ".tampinha").resolve()
    except Exception:
        return True
...
if _tampinha_home_points_at_production(os.environ.get("TAMPINHA_HOME", "")):
    _SESSION_TAMPINHA_HOME = tempfile.mkdtemp(prefix="tampinha-test-home-")
    os.environ["TAMPINHA_HOME"] = _SESSION_TAMPINHA_HOME
    atexit.register(shutil.rmtree, _SESSION_TAMPINHA_HOME, True)
...
# like a real CLI") — which used to disarm tampinha_state's live-DB guard in
# the child at the same moment the child lost the TAMPINHA_HOME redirect.
# TAMPINHA_TEST_ISOLATION is OUR marker: exported here (before any test module
# imports), inherited by every child by default, and honored by
# tampinha_state._running_under_pytest() as a test-context signal. A child
...
# open a real DB must export TAMPINHA_STATE_DB_GUARD_BYPASS=1 in that child's
# env instead of stripping markers.
os.environ["TAMPINHA_TEST_ISOLATION"] = os.environ.get("TAMPINHA_HOME", "") or "1"

#: TAMPINHA_HOME as it stood when conftest was imported - i.e. before any test
...
TAMPINHA_HOME_AT_CONFTEST_IMPORT = os.environ.get("TAMPINHA_HOME", "")
...
# TAMPINHA_* vars that change test behavior by being set. Unset all of these
# unconditionally — individual tests that need them set do so explicitly.
_TAMPINHA_BEHAVIORAL_VARS = frozenset({
    ...
    # final response text to ``tampinha_cli.voice.speak_text`` — real synthesis,
    ...
    # any test run launched FROM a Tampinha agent session inherits them and
    # tampinha_constants home-resolution helpers prefer them over monkeypatched
    # HOME (test_subprocess_home_isolation red locally, green on CI).
    "TAMPINHA_REAL_HOME",
    "TERMINAL_HOME_MODE",
    "TAMPINHA_INTERACTIVE",
    "TAMPINHA_QUIET",
    "TAMPINHA_TOOL_PROGRESS",
    "TAMPINHA_TOOL_PROGRESS_MODE",
    "TAMPINHA_MAX_ITERATIONS",
    "TAMPINHA_SESSION_PLATFORM",
    "TAMPINHA_SESSION_CHAT_ID",
    "TAMPINHA_SESSION_CHAT_NAME",
    "TAMPINHA_SESSION_CHAT_TYPE",
    "TAMPINHA_SESSION_THREAD_ID",
    "TAMPINHA_SESSION_SOURCE",
    "TAMPINHA_SESSION_KEY",
    "TAMPINHA_GATEWAY_SESSION",
    "TAMPINHA_CRON_SESSION",
    "_TAMPINHA_GATEWAY",
    "TAMPINHA_PLATFORM",
    "TAMPINHA_MODEL",
    "TAMPINHA_INFERENCE_MODEL",
    "TAMPINHA_INFERENCE_PROVIDER",
    "TAMPINHA_TUI_PROVIDER",
    "TAMPINHA_MANAGED",
    "TAMPINHA_MANAGED_DIR",
    "TAMPINHA_DEV",
    "TAMPINHA_CONTAINER",
    "TAMPINHA_EPHEMERAL_SYSTEM_PROMPT",
    "TAMPINHA_TIMEZONE",
    "TAMPINHA_REDACT_SECRETS",
    "TAMPINHA_BACKGROUND_NOTIFICATIONS",
    "TAMPINHA_EXEC_ASK",
    "TAMPINHA_HOME_MODE",
    "TAMPINHA_AGENT_USE_LEGACY_SESSION_KEYS",
    # Kanban path/board pins must never leak from a developer shell or
    # dispatched worker into tests; otherwise tests can write fake tasks to
    # the real ~/.tampinha/kanban.db instead of the per-test TAMPINHA_HOME.
    "TAMPINHA_KANBAN_DB",
    "TAMPINHA_KANBAN_BOARD",
    "TAMPINHA_KANBAN_HOME",
    "TAMPINHA_KANBAN_WORKSPACES_ROOT",
    "TAMPINHA_KANBAN_LOGS_ROOT",
    "TAMPINHA_KANBAN_TASK",
    "TAMPINHA_KANBAN_WORKSPACE",
    "TAMPINHA_KANBAN_RUN_ID",
    "TAMPINHA_KANBAN_CLAIM_LOCK",
    "TAMPINHA_KANBAN_DISPATCH_IN_GATEWAY",
    ...
    "TAMPINHA_DELEGATED_CHILD_CONTEXT",
    "TAMPINHA_TENANT",
    ...
    # otherwise-unrelated config tests away from the default "tampinha" host.
    "TAMPINHA_HONCHO_HOST",
    ...
    "TAMPINHA_DASHBOARD_OAUTH_CLIENT_ID",
    "TAMPINHA_DASHBOARD_PORTAL_URL",
    "TERMINAL_CWD",
    "TERMINAL_ENV",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and TAMPINHA_HOME to per-test tempdirs so code that
    reads ``~/.tampinha/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 2. Blank behavioral TAMPINHA_* vars that could change test semantics.
    for name in _TAMPINHA_BEHAVIORAL_VARS:
        monkeypatch.delenv(name, raising=False)
    ...
    monkeypatch.setenv("TAMPINHA_HONCHO_HOST", "tampinha")

    # 3. Redirect TAMPINHA_HOME to a per-test tempdir. Code that reads
    #    ``~/.tampinha/*`` via ``get_tampinha_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.tampinha/*`` via
    #    ``Path.home() / ".tampinha"`` instead of ``get_tampinha_home()``
    #    is a bug to fix at the callsite.
    fake_tampinha_home = tmp_path / "tampinha_test"
    fake_tampinha_home.mkdir()
    (fake_tampinha_home / "sessions").mkdir()
    (fake_tampinha_home / "cron").mkdir()
    (fake_tampinha_home / "memories").mkdir()
    (fake_tampinha_home / "skills").mkdir()
    monkeypatch.setenv("TAMPINHA_HOME", str(fake_tampinha_home))
    ...
    # tampinha_state's live-DB guard stays armed in them even when the test
    # strips pytest's own PYTEST_* vars from the child env.
    monkeypatch.setenv("TAMPINHA_TEST_ISOLATION", str(fake_tampinha_home))
    # And never let a developer-shell (or leaked child) bypass disarm the
    # guard for in-process code under test.
    monkeypatch.delenv("TAMPINHA_STATE_DB_GUARD_BYPASS", raising=False)

    # 3b. tampinha_state computes ``DEFAULT_DB_PATH = get_tampinha_home() / "state.db"``
    #     at import time. When the module is first imported at collection (any
    #     test file with a top-level ``from tampinha_state import ...``) that
    ...
    tampinha_state_mod = sys.modules.get("tampinha_state")
    if tampinha_state_mod is not None and hasattr(tampinha_state_mod, "DEFAULT_DB_PATH"):
        monkeypatch.setattr(
            tampinha_state_mod, "DEFAULT_DB_PATH", fake_tampinha_home / "state.db"
        )
    ...
    monkeypatch.setenv("TAMPINHA_DISABLE_LAZY_INSTALLS", "1")

    # 5. Reset plugin singleton so tests don't leak plugins from
    #    ~/.tampinha/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import tampinha_cli.plugins as _plugins_mod
        ...
        # previous test's tmp_path TAMPINHA_HOME can't leak forward. Paths
...
@pytest.fixture(autouse=True)
def _isolate_tampinha_home(_hermetic_environment):
...
def _neutralize_kanban_memory_guard(request, monkeypatch):
    ...
    if request.node.get_closest_marker("real_memory_guard"):
        return
    try:
        from tampinha_cli import kanban_db as _kb_mod
    except Exception:
        return
...
# invocation), kanban writes silently pollute the real ~/.tampinha. This autouse
...
# allow-list because test-level fixtures legitimately move TAMPINHA_HOME to
...
def _capture_real_kanban_root() -> Path:
    ...
    file (before the session TAMPINHA_HOME sandbox rewired the env), so the
    deny-list keeps pointing at the operator's actual root. Mirrors
    ``kanban_db.kanban_home()`` resolution order:
    1. ``TAMPINHA_KANBAN_HOME`` env var when set and non-empty
    2. the real (pre-sandbox) Tampinha root otherwise
    """
    if _PRE_SANDBOX_KANBAN_OVERRIDE:
        return Path(_PRE_SANDBOX_KANBAN_OVERRIDE).expanduser().resolve()
    if _PRE_SANDBOX_TAMPINHA_HOME and not _tampinha_home_points_at_production(
        _PRE_SANDBOX_TAMPINHA_HOME
    ):
        # TAMPINHA_HOME was genuinely set to a CUSTOM root before the sandbox
        ...
        return get_default_tampinha_root().resolve()
    # No pre-existing TAMPINHA_HOME: the real root is the platform default,
    # NOT the sandbox tempdir now sitting in the env.
    return (Path.home() / ".tampinha").resolve()
...
def _kanban_write_guard(_hermetic_environment, monkeypatch):
    ...
    ``~/.tampinha`` captured at import time. Hermetic tests that legitimately
    move TAMPINHA_HOME to sibling tempdirs are unaffected.

    Only patches when ``tampinha_cli.kanban_db`` is *already imported* — a
    ...
    """
    _kdb = sys.modules.get("tampinha_cli.kanban_db")
    if _kdb is None:
        return

    # The sys.modules probe can observe the module MID-IMPORT: a fixture
    # boundary firing while another test's lazy `import tampinha_cli.kanban_db`
    ...
    def _guarded_connect(db_path=None, *args, **kwargs):
        ...
        raise RuntimeError(
            ...
            f"to the real ~/.tampinha. See #69283."
        )
...
# ``tampinha_state._ensure_test_isolation`` (the single choke point every
# ``SessionDB()`` construction goes through) refuses, under pytest, any DB
# path that resolves inside the REAL Tampinha root. This fixture wires the
# test-side knobs:
...
#     installs where TAMPINHA_HOME is not ~/.tampinha) into the guard's
...
# so subprocess children that import tampinha_state directly are covered even
# without this fixture.


@pytest.fixture(autouse=True)
def _state_db_write_guard(request, monkeypatch):
    _hs = sys.modules.get("tampinha_state")
    ...
    if _PRE_SANDBOX_TAMPINHA_HOME and not _tampinha_home_points_at_production(
        ...
        extra_roots.append(
            Path(_PRE_SANDBOX_TAMPINHA_HOME).expanduser().resolve()
        )
    monkeypatch.setattr(
...
def _reset_tui_gateway_server_state():
    ...
    # A leaked context-local Tampinha home override redirects every later
    # ``get_tampinha_home()`` call (active-session registry, config paths)
    # to a stale per-test tmpdir. Force the main-thread ContextVar back
    # to its default.
    try:
        from tampinha_constants import get_tampinha_home_override, set_tampinha_home_override

        if get_tampinha_home_override() is not None:
            set_tampinha_home_override(None)
    except Exception:
        pass
...
@pytest.fixture()
def mock_config():
    """Return a minimal tampinha config dict suitable for unit tests."""
    return {
...
# environment and finds the developer's live ``tampinha-gateway`` process
...
# correlated with a ``tests/tampinha_cli/`` pytest run starting up.
#
...
#    ``check_output`` reject any ``systemctl ... <verb> tampinha-gateway``
...
def _wal_is_usable() -> bool:
    """True when Tampinha will actually put a database into WAL mode here.

    Tampinha refuses journal_mode=WAL on SQLite builds carrying the upstream
    ...
    running Tampinha can link DIFFERENT SQLite versions: a repo ``.venv`` on
    3.50.4 (vulnerable → DELETE) alongside a Tampinha managed runtime on 3.53.1
    (fixed → WAL). The same test then passes in one and fails in the other.

    IMPORTANT: this must NOT import ``tampinha_state``. That module computes
    ``DEFAULT_DB_PATH`` from ``get_tampinha_home()`` at import time, so importing
    it during collection — before the per-test ``_isolate_tampinha_home`` fixture
    redirects ``TAMPINHA_HOME`` — permanently caches the DEVELOPER'S REAL
    ``~/.tampinha/state.db`` for the whole session. Tests then read live
    production sessions instead of a tempdir. The version predicate is
    duplicated from ``tampinha_state._is_sqlite_wal_reset_vulnerable`` (upstream
...
#      process environment: ``os.environ["TAMPINHA_VOICE_TTS"] = "1"``. The
...
#      ``tampinha_cli.voice.speak_text(final_response)`` on a daemon thread.
...
# to mock it:
#
#  • ``tampinha_cli.voice.speak_text`` — the synth+playback entry point both
#    gateway call sites late-import, so patching the module attribute catches
#    them wherever they import it from.
#  • ``tampinha_cli.voice.play_audio_file`` — the module-level binding
#    ``speak_text`` actually plays through. Patching the binding inside
#    ``tampinha_cli.voice`` (not ``tools.voice_mode``) keeps the real function
...
# OS gating
#
# Tampinha runs on Linux, macOS and native Windows, and a lot of its behaviour
# genuinely differs per host: PTY vs pywinpty, taskkill vs SIGTERM, launchd
# vs systemd, Keychain vs libsecret, ``%LOCALAPPDATA%`` vs ``~/.tampinha``.
#
...
def pytest_configure(config):  # noqa: D401 — pytest hook
    ...
    config.addinivalue_line(
        ...
        "SQLite WAL mode; skipped on builds where Tampinha falls back to "
...
def pytest_collection_modifyitems(config, items):  # noqa: D401 — pytest hook
    ...
    reason = (
        f"SQLite {sqlite3.sqlite_version} has the WAL-reset bug — Tampinha uses "
...
def _live_system_guard(request, monkeypatch):
    ...
    tokens[0]), so ``bash -c "systemctl restart tampinha-gateway"``,
    ``sudo systemctl ...``, ``env systemctl ...``, ``setsid systemctl ...``
    are all caught. ``pkill``/``killall``/``taskkill`` invocations
    targeting tampinha/python patterns are also blocked.
    """
    ...
    def _matches_tampinha_gateway(cmd_str: str) -> bool:
        low = cmd_str.lower()
        return any(tok in low for tok in _TAMPINHA_TOKENS)

    def _is_blocked_systemctl(cmd) -> bool:
        cmd_str = _cmd_to_string(cmd)
        if "systemctl" not in cmd_str:
            return False
        if not _matches_tampinha_gateway(cmd_str):
            return False
        try:
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in killer_tokens:
            ...
            if head in _PROCESS_KILLERS:
                low = cmd_str.lower()
                # pkill -f pattern: catch tampinha-themed patterns + a
                # plain "python" -f which would catch the live gateway
                # whose cmdline contains "python -m tampinha_cli.main".
                if (
                    "tampinha" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live tampinha-gateway systemd unit. Mock "
        ...
        if _is_process_killer(cmd):
            raise RuntimeError(
                ...
                "targeting tampinha/python could hit the live gateway. "
                "Mark with @pytest.mark.live_system_guard_bypass if "
                "intentional."
            )
        # Block any subprocess that would run `tampinha update` (or the
        # equivalent `python -m tampinha_cli.main update`).  These commands
        ...
        if "update" in low and (
            # tampinha update / tampinha update --gateway / setsid bash -c ... tampinha update
            ("tampinha" in low and "update" in low.split())
            or
            # python -m tampinha_cli.main update --gateway
            ("tampinha_cli" in low and "update" in low.split())
            or
            # venv/bin/tampinha update  (absolute path variant used in tests)
            (".venv/bin/tampinha" in low and "update" in low)
        ):
            raise RuntimeError(
                f"tests/conftest.py live-system guard: blocked "
                f"subprocess.{name}({cmd!r}) — this command would run "
                "`tampinha update` against the real checkout, fetching "
...
def _audio_playback_guard(request, monkeypatch):
    ...
    depth behind ``_TAMPINHA_BEHAVIORAL_VARS``: the env blanking stops the flag
    ...
    try:
        import tampinha_cli.voice as _voice
    except Exception:
