...
The ``tampinha update`` Python path already clears the conflict with ``git reset``
...
def test_install_ps1_stops_venv_resident_processes_before_parking_venv() -> None:
    ...
    ``venv\\Scripts\\pythonw.exe -m tampinha_cli.main gateway run`` — image name
    ``pythonw``, not ``tampinha.exe`` — so the ``taskkill /IM tampinha.exe`` guard
    ...
    # The tampinha.exe tree-kill is preserved (kills spawned child processes too).
    assert 'taskkill /F /T /IM tampinha.exe' in text
